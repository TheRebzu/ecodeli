...
if (missingInDeps.length > 0) {
  console.log(`❌ Seeds manquants dans dependencies.ts: ${missingInDeps.join(', ')}`);
} else {
  console.log('✅ Tous les seeds sont référencés dans dependencies.ts');
}

if (missingFiles.length > 0) {
  console.log(`❌ Fichiers manquants pour les seeds: ${missingFiles.join(', ')}`);
} else {
...
console.log(`   Cohérence: ${missingInDeps.length === 0 && missingFiles.length === 0 ? '✅ Parfaite' : '❌ Problèmes détectés'}`);
