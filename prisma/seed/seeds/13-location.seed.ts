...
import { generateAccessCode } from '../utils/generators/code-generator'
...
export async function seedLocations(ctx: SeedContext) {
  const { prisma } = ctx
  ...
  for (const data of warehouseData) {
    const coords = CONSTANTS.warehouses[data.city.toUpperCase()]
    ...
    const warehouse = await prisma.warehouse.create({
      data: {
        ...
        currentOccupancy: Math.floor(data.capacity * 0.3 + Math.random() * data.capacity * 0.4), // 30-70% occupé
    ...
    const boxSizes = Object.keys(CONSTANTS.boxSizes)
    for (let i = 1; i <= data.boxCount; i++) {
      ...
      const box = await prisma.storageBox.create({
        data: {
          ...
          size,
          pricePerDay: CONSTANTS.pricing.storage.daily[size],
          isAvailable
        }
      })
  ...
  const relayPoints = [
    { name: 'Relay Paris Gare du Nord', city: 'Paris', lat: 48.8809, lng: 2.3553 },
    { name: 'Relay Lyon Part-Dieu', city: 'Lyon', lat: 45.7605, lng: 4.8595 },
    { name: 'Relay Marseille Saint-Charles', city: 'Marseille', lat: 43.3028, lng: 5.3806 }
  ]
