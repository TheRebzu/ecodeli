...
export async function seedCertifications(ctx: SeedContext) {
  const { prisma } = ctx;
  ...
  console.log(
    `Found ${providers.length} providers and ${deliverers.length} deliverers`,
  );
  ...
  for (const provider of providers) {
    ...
    for (let i = 0; i < numCerts; i++) {
      ...
      const expiryDate = new Date(
        issueDate.getTime() +
          (2 + Math.random() * 3) * 365 * 24 * 60 * 60 * 1000,
      );
      ...
      const cert = await prisma.certification.create({
        data: {
          ...
          category: "TECHNICAL" as any, // Using TECHNICAL as default category
    ...
    console.log(
      `Created ${numCerts} certifications for provider ${provider.user.email}`,
    );
  }
    ...
    for (const certType of requiredCerts) {
      ...
      const cert = await prisma.certification.create({
        data: {
          ...
          category: "COMPLIANCE" as any, // Using COMPLIANCE for deliverer certs
          level: "BASIC" as any,
