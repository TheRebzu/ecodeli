...
export async function seedAnnouncements(ctx: SeedContext) {
  ...
  for (const client of clients) {
    ...
    for (let i = 0; i < numAnnouncements; i++) {
      ...
      const fromAddress = parisAddresses[Math.floor(Math.random() * parisAddresses.length)]
      const toAddress = parisAddresses[Math.floor(Math.random() * parisAddresses.length)]
