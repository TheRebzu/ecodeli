...
import { UserRole, ServiceType, PaymentStatus, PaymentType } from '@prisma/client';
...
export async function seedProviderBookings() {
  ...
  try {
    ...
    for (const [index, bookingData] of bookingsData.entries()) {
      ...
      await prisma.intervention.create({
        data: {
          ...
          endTime: new Date(bookingData.scheduledDate.getTime() + bookingData.duration * 60000),
          actualDuration: bookingData.duration,
          report: `Intervention réalisée avec succès. Client satisfait du service rendu.`,
          isCompleted: true,
          completedAt: new Date(bookingData.scheduledDate.getTime() + bookingData.duration * 60000),
        },
      });
      ...
      if (clientUser) {
        await prisma.payment.create({
          data: {
            ...
            paidAt: new Date(bookingData.scheduledDate.getTime() + bookingData.duration * 60000 + 3600000), // 1h après la fin
            metadata: {
              bookingId: booking.id,
              providerId: provider.id,
    ...
    console.log(`✅ Created ${bookingsData.length} bookings with payments for provider billing archives`);
