...
export async function seedReviews(ctx: SeedContext) {
  ...
  for (const booking of completedBookings) {
    ...
    if (Math.random() > 0.15) {
      ...
      const comments =
        reviewTemplates.SERVICE[rating as keyof typeof reviewTemplates.SERVICE];
      ...
      if (Math.random() < 0.4) {
        const responseText =
          reviewTemplates.PROVIDER_RESPONSES[
            Math.floor(
              Math.random() * reviewTemplates.PROVIDER_RESPONSES.length,
            )
          ];
        ...
        await prisma.review.update({
          ...
          data: {
            ...
            respondedAt: new Date(
              review.createdAt.getTime() + Math.random() * 48 * 60 * 60 * 1000,
            ), // 0-48h après l'avis
          },
        });
      }
