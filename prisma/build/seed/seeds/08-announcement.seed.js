...
async function seedAnnouncements(ctx) {
    ...
    const clients = users.filter(u => u.role === constants_1.CONSTANTS.roles.CLIENT);
    const merchants = users.filter(u => u.role === constants_1.CONSTANTS.roles.MERCHANT);
    const announcements = [];
    ...
    for (const client of clients) {
        ...
        for (let i = 0; i < numAnnouncements; i++) {
            const type = Object.keys(announcementTemplates)[Math.floor(Math.random() * Object.keys(announcementTemplates).length)];
            ...
            const fromAddress = paris_1.parisAddresses[Math.floor(Math.random() * paris_1.parisAddresses.length)];
            const toAddress = paris_1.parisAddresses[Math.floor(Math.random() * paris_1.parisAddresses.length)];
            // Calculer la date prévue
            const scheduledDays = Math.floor(1 + Math.random() * 14); // 1 à 14 jours dans le futur
            const scheduledAt = new Date(Date.now() + scheduledDays * 24 * 60 * 60 * 1000);
            let description = template.descriptions[descIndex];
            if (type === 'PACKAGE_DELIVERY') {
                description = description.replace('{weight}', template.weights[Math.floor(Math.random() * template.weights.length)].toString());
            }
            else if (type === 'AIRPORT_TRANSFER') {
                description = description.replace('{nbPersons}', Math.floor(1 + Math.random() * 4).toString());
            }
            else if (type === 'SHOPPING') {
                description = description.replace('{budget}', template.budgets[Math.floor(Math.random() * template.budgets.length)].toString());
            }
            else if (type === 'CART_DROP') {
                description = description.replace('{store}', template.stores[Math.floor(Math.random() * template.stores.length)]);
            }
