...
async function seedPayments(ctx) {
    ...
    for (const delivery of deliveries) {
        try {
            ...
            console.log(`   Error creating payment for delivery ${delivery.id}:`, error.message);
        }
    }
    ...
    for (const booking of bookings) {
        try {
            ...
            console.log(`   Error creating payment for booking ${booking.id}:`, error.message);
        }
    }
    ...
    for (const wallet of walletsWithBalance.slice(0, Math.min(5, walletsWithBalance.length))) {
        const withdrawalAmount = Math.min(50 + Math.random() * 100, wallet.balance * 0.5);
        await prisma.walletOperation.create({
            data: {
                walletId: wallet.id,
    ...
    console.log(`   Created withdrawal operations for ${Math.min(5, walletsWithBalance.length)} wallets`);
