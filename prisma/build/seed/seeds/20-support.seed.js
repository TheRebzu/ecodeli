...
exports.seedSupport = seedSupport;
...
async function seedSupport(ctx) {
    ...
    const clients = users.filter((u) => u.role === 'CLIENT').slice(0, 3);
    ...
    const categories = ['DELIVERY_ISSUE', 'PAYMENT_PROBLEM', 'ACCOUNT_ACCESS', 'TECHNICAL_SUPPORT', 'GENERAL_INQUIRY'];
    ...
    for (let i = 0; i < 5; i++) {
        ...
        const ticket = await prisma.supportTicket.create({
            data: {
                ticketNumber: (0, code_generator_1.generateTicketNumber)(),
                authorId: client.id,
    ...
    const templates = [
        { name: 'Problème de livraison', category: 'DELIVERY_ISSUE', content: 'Nous allons vérifier avec le livreur.' },
        { name: 'Problème de paiement', category: 'PAYMENT_PROBLEM', content: 'Nous vérifions votre paiement.' },
        { name: 'Problème de compte', category: 'ACCOUNT_ACCESS', content: 'Nous allons vérifier votre compte.' }
    ];
