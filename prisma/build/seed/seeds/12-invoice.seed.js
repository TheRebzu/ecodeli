...
async function seedInvoices(ctx) {
  const { prisma } = ctx;
  ...
  for (const delivery of completedDeliveries) {
    const invoice = await prisma.invoice.create({
      data: {
        invoiceNumber: (0, code_generator_1.generateInvoiceNumber)("LIV"),
        type: "DELIVERY",
        status: "PAID",
        ...
        paidAt: new Date(),
        pdfUrl: `https://storage.ecodeli.fr/invoices/${(0, code_generator_1.generateInvoiceNumber)("LIV")}.pdf`,
      },
    });
  ...
  for (const booking of completedBookings) {
    const invoice = await prisma.invoice.create({
      data: {
        ...
        invoiceNumber: (0, code_generator_1.generateInvoiceNumber)("SRV"),
        type: "SERVICE",
        status: "PAID",
        ...
        dueDate: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000),
        paidAt: new Date(),
        pdfUrl: `https://storage.ecodeli.fr/invoices/${(0, code_generator_1.generateInvoiceNumber)("SRV")}.pdf`,
      },
    });
    invoices.push(invoice);
  }
      ...
      if (totalAmount > 0) {
        const monthlyInvoice = await prisma.providerMonthlyInvoice.create({
          data: {
            ...
            invoiceNumber: (0, code_generator_1.generateInvoiceNumber)(
              `PRO-${month.toString().padStart(2, "0")}`,
            ),
            ...
            paidAt:
              monthOffset === 0
                ? null
                : new Date(invoiceDate.getTime() + 5 * 24 * 60 * 60 * 1000),
  ...
  for (const client of subscribedClients) {
    ...
    for (let monthOffset = 2; monthOffset >= 0; monthOffset--) {
      ...
      const amount = client.subscriptionPlan === "PREMIUM" ? 19.99 : 9.9;
      const invoice = await prisma.invoice.create({
        data: {
          clientId: client.id,
          invoiceNumber: (0, code_generator_1.generateInvoiceNumber)("SUB"),
          type: "SUBSCRIPTION",
          status: "PAID",
          subtotal: amount,
          tax: amount * 0.2,
          ...
          paidAt: invoiceDate,
          pdfUrl: `https://storage.ecodeli.fr/invoices/subscriptions/${(0, code_generator_1.generateInvoiceNumber)("SUB")}.pdf`,
        },
      });
      invoices.push(invoice);
    }
