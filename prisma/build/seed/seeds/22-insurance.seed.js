...
exports.seedInsurance = seedInsurance;
async function seedInsurance(ctx) {
  const { prisma } = ctx;
  ...
  const transportPolicy = policies.find(
    (p) => p.category === "GOODS_TRANSPORT",
  );
  const liabilityPolicy = policies.find(
    (p) => p.category === "PROFESSIONAL_LIABILITY",
  );
  if (transportPolicy && deliveries.length > 0) {
    // Couvrir quelques livraisons
    const coveredDeliveries = deliveries.slice(
      0,
      Math.min(3, deliveries.length),
    );
  ...
  const warranties = [
    {
      ...
      conditions: {
        ...
        applicableServices: ["cleaning", "gardening"],
      },
      exclusions: [
        { type: "weather", description: "Weather-related cancellations" },
      ],
    },
    {
      ...
      exclusions: [
        { type: "force_majeure", description: "Force majeure events" },
      ],
    },
  ];
