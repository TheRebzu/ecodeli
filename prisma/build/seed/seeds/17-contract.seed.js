...
exports.seedContracts = seedContracts;
async function seedContracts(context) {
  ...
  for (const merchant of merchants) {
    ...
    const contractType =
      index === 0 ? "ENTERPRISE" : index === 1 ? "PREMIUM" : "STANDARD";
    ...
    const validUntil = new Date(
      signedDate.getTime() + 12 * 30 * 24 * 60 * 60 * 1000,
    ); // 12 mois
    const contract = await prisma.contract.create({
      data: {
        ...
        commissionRate: commissionRates[contractType],
        setupFee: contractType === "ENTERPRISE" ? 500 : 0,
        monthlyFee: monthlyFees[contractType],
        ...
        deliveryZones: [
          {
            zone: "PARIS_CENTER",
            radius: contractType === "ENTERPRISE" ? 50 : 25,
          },
        ...
        allowedServices: [
          "DELIVERY",
          "EXPRESS_DELIVERY",
          ...(contractType === "ENTERPRISE" ? ["PRIORITY_DELIVERY"] : []),
        ],
        merchantSignedAt: Math.random() > 0.1 ? signedDate : null,
        merchantSignature:
          Math.random() > 0.1 ? "merchant_signature_hash" : null,
        ...
        signedDocumentPath:
          Math.random() > 0.1
            ? `/contracts/signed/${merchant.id}_contract.pdf`
            : null,
    ...
    if (index < 3 && Math.random() > 0.5) {
      await prisma.contractAmendment.create({
        data: {
          ...
          description:
            "Réduction de commission suite à l'augmentation du volume",
          changes: {
            oldCommissionRate: commissionRates[contractType],
            newCommissionRate: commissionRates[contractType] - 0.5,
            reason: "Volume mensuel dépassé",
          },
          effectiveDate: new Date(
            signedDate.getTime() + 6 * 30 * 24 * 60 * 60 * 1000,
          ), // 6 mois après
    ...
    console.log(
      `Created contract for merchant ${merchant.user.email} (${contractType})`,
    );
  }
  console.log(
    `Contracts seeding completed - ${merchants.length} contracts processed`,
  );
}
