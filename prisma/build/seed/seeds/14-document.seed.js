...
async function seedDocuments(ctx) {
  ...
  const deliverers = users.filter(
    (u) => u.role === constants_1.CONSTANTS.roles.DELIVERER,
  );
        ...
        data: {
          ...
          validatedAt:
            status !== "PENDING"
              ...
              : null,
          rejectionReason:
            status === "REJECTED" ? "Document illisible ou expiré" : null,
          expirationDate:
            docTemplate.type === "DRIVING_LICENSE" ||
            docTemplate.type === "INSURANCE"
  ...
  for (const provider of providers) {
    ...
    for (const docTemplate of documentTemplates.PROVIDER) {
      const status =
        provider.validationStatus === "VALIDATED" ? "APPROVED" : "PENDING";
      const document = await prisma.document.create({
        data: {
          userId: provider.id,
          type: docTemplate.type,
  ...
  const merchants = users.filter(
    (u) => u.role === constants_1.CONSTANTS.roles.MERCHANT,
  );
