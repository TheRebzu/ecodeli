...
export async function seedAuditLogs(
  ...
  if ((existingAuditLogs + existingActivityLogs) > 100 && !options.force) {
    logger.warning('AUDIT_LOGS', `${existingAuditLogs + existingActivityLogs} logs déjà présents - utiliser force:true pour recréer`);
  ...
  const USER_ACTIVITIES: UserActivity[] = [
    {
      ...
      details: ['Connexion réussie', 'Connexion via mobile', 'Connexion via web'],
      frequency: 2000
    },
    {
      type: ActivityType.LOGOUT,
      targetRoles: [UserRole.CLIENT, UserRole.DELIVERER, UserRole.MERCHANT, UserRole.PROVIDER, UserRole.ADMIN],
      details: ['Déconnexion manuelle', 'Session expirée', 'Déconnexion automatique'],
      frequency: 1800
    },
    {
      type: ActivityType.PASSWORD_CHANGE,
      targetRoles: [UserRole.CLIENT, UserRole.DELIVERER, UserRole.MERCHANT, UserRole.PROVIDER, UserRole.ADMIN],
      details: ['Changement volontaire', 'Réinitialisation forcée', 'Mise à jour sécurisée'],
      frequency: 100
    },
    {
      type: ActivityType.PROFILE_UPDATE,
      targetRoles: [UserRole.CLIENT, UserRole.DELIVERER, UserRole.MERCHANT, UserRole.PROVIDER],
      details: ['Mise à jour informations personnelles', 'Modification photo profil', 'Changement coordonnées'],
      frequency: 300
    },
    {
      type: ActivityType.EMAIL_VERIFICATION,
      targetRoles: [UserRole.CLIENT, UserRole.DELIVERER, UserRole.MERCHANT, UserRole.PROVIDER],
      details: ['Vérification email initiale', 'Nouvelle vérification', 'Changement adresse email'],
      frequency: 80
    },
    {
    ...
    },
    {
      ...
      targetRoles: [UserRole.CLIENT],
      details: ['Paiement carte bancaire', 'Paiement PayPal', 'Paiement Apple Pay'],
      frequency: 750
    },
    {
      type: ActivityType.DELIVERY_ACCEPTED,
      targetRoles: [UserRole.DELIVERER],
      details: ['Acceptation livraison urgente', 'Acceptation livraison standard', 'Acceptation livraison express'],
      frequency: 400
    },
    {
      type: ActivityType.SERVICE_BOOKED,
      targetRoles: [UserRole.CLIENT],
      details: ['Réservation plomberie', 'Réservation ménage', 'Réservation jardinage'],
      frequency: 200
    },
    {
      type: ActivityType.NOTIFICATION_READ,
      targetRoles: [UserRole.CLIENT, UserRole.DELIVERER, UserRole.MERCHANT, UserRole.PROVIDER],
      details: ['Lecture notification mobile', 'Lecture notification web', 'Lecture email'],
      frequency: 1500
    },
    {
      type: ActivityType.SEARCH_PERFORMED,
      targetRoles: [UserRole.CLIENT],
      details: ['Recherche service', 'Recherche entrepôt', 'Recherche livreur'],
      frequency: 600
    }
  ];
    ...
    for (let i = 0; i < eventCount; i++) {
      try {
        ...
        if (options.verbose && totalAuditLogs % 50 === 0) {
          logger.progress('AUDIT_LOGS', totalAuditLogs, 'calculé dynamiquement', 
            `Logs audit créés: ${totalAuditLogs}`);
        }

      } catch (error: any) {
        logger.error('AUDIT_LOGS', `❌ Erreur création audit log ${auditEvent.action}: ${error.message}`);
        result.errors++;
      }
    }
  }
    ...
    for (let i = 0; i < activityCount; i++) {
      try {
        ...
        if (options.verbose && totalActivityLogs % 100 === 0) {
          logger.progress('AUDIT_LOGS', totalActivityLogs, 'calculé dynamiquement', 
            `Logs activité créés: ${totalActivityLogs}`);
        }

      } catch (error: any) {
        logger.error('AUDIT_LOGS', `❌ Erreur création activity log ${activity.type}: ${error.message}`);
        result.errors++;
      }
    }
  }
  ...
  logger.info('AUDIT_LOGS', `📅 7 derniers jours: ${recentAuditLogs} audit + ${recentActivityLogs} activité`);
  logger.info('AUDIT_LOGS', `🔢 Total: ${finalAuditLogs.length} audit logs + ${finalActivityLogs.length} activity logs`);
...
function generateAuditChanges(entityType: string, action: string): any {
  ...
  switch (`${entityType}_${action}`) {
    ...
    case 'Document_APPROVE':
      return {
        ...baseChanges,
        documentType: faker.helpers.arrayElement(['IDENTITY_CARD', 'DRIVING_LICENSE', 'PROOF_OF_ADDRESS']),
    ...
    case 'System_MAINTENANCE_START':
      return {
        ...baseChanges,
        maintenanceType: faker.helpers.arrayElement(['Mise à jour sécurité', 'Optimisation base', 'Nouveau déploiement']),
        estimatedDuration: `${faker.number.int({ min: 30, max: 240 })} minutes`,
        affectedServices: faker.helpers.arrayElements(['API', 'Frontend', 'Paiements', 'Notifications'])
      };
...
export async function validateAuditLogs(
  ...
  if (auditLogsWithValidChanges.length === auditLogs.length) {
    ...
    logger.warning('VALIDATION', `⚠️ ${auditLogs.length - auditLogsWithValidChanges.length} logs avec changements invalides`);
  }
  ...
  if (auditLogsWithUser.length === auditLogs.length) {
    ...
    logger.warning('VALIDATION', `⚠️ ${auditLogs.length - auditLogsWithUser.length} logs sans utilisateur`);
  }
  ...
  if (activityLogsWithUser.length === activityLogs.length) {
    ...
    logger.warning('VALIDATION', `⚠️ ${activityLogs.length - activityLogsWithUser.length} logs sans utilisateur`);
  }
  ...
  if (recentAuditLogs > 0 && recentActivityLogs > 0) {
    logger.success('VALIDATION', `✅ Activité récente détectée: ${recentAuditLogs} audit + ${recentActivityLogs} activité`);
  } else {
