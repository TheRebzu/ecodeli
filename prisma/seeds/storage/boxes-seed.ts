...
export async function seedBoxes(
  ...
  const BOX_CONFIGURATIONS: Record<BoxType, BoxConfiguration> = {
    ...
    [BoxType.CLIMATE_CONTROLLED]: {
      ...
      features: ['Accès 24h/24', 'Sécurisé', 'Climatisé', 'Contrôle température', 'Hygrométrie'],
      maxWeight: 150,
      dimensions: [
    ...
    [BoxType.SECURE]: {
      ...
      features: ['Accès 24h/24', 'Haute sécurité', 'Double authentification', 'Vidéosurveillance'],
      maxWeight: 100,
      dimensions: [
      ...
      for (const [boxType, count] of Object.entries(boxDistribution)) {
        ...
        for (let i = 0; i < count; i++) {
          try {
            ...
            const client = isOccupied && clients.length > 0 ? getRandomElement(clients) : null;
  ...
  const boxesByWarehouse = finalBoxes.reduce((acc: Record<string, number>, box) => {
  ...
  // Historique d'usage
  const boxesWithHistory = finalBoxes.filter(box => box.usageHistory.length > 0).length;
  const boxesWithSubscriptions = finalBoxes.filter(box => box.subscriptions.length > 0).length;
  ...
  if (finalBoxes.length >= totalBoxes - result.errors) {
    logger.validation('BOXES', 'PASSED', `${finalBoxes.length} boxes créées avec succès`);
  } else {
    logger.validation('BOXES', 'FAILED', `Attendu: ${totalBoxes}, Créé: ${finalBoxes.length}`);
  }
...
export async function validateBoxes(
  ...
  const boxesWithInconsistentSize = boxes.filter(box => {
    try {
      const dimensions = box.dimensions as { width: number; height: number; depth: number };
  ...
  const totalUsageHistory = boxes.reduce((sum, box) => sum + box.usageHistory.length, 0);
  const totalSubscriptions = boxes.reduce((sum, box) => sum + box.subscriptions.length, 0);

  logger.info('VALIDATION', `📊 Historique d'usage: ${totalUsageHistory} entrées`);
