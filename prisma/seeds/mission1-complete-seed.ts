...
async function executeMission1(config: MissionConfig): Promise<MissionResult> {
  ...
  try {
    ...
    if (!result.environment.isValid && !config.force) {
      throw new Error(
        "Environnement non valide - utiliser --force pour continuer",
      );
    }
    ...
    // Seeds de base
    result.seedResults.permissions = await seedPermissions(
      prisma,
      logger,
      baseOptions,
    );
    result.seedResults.serviceCategories = await seedServiceCategories(
      prisma,
      logger,
      baseOptions,
    );
    ...
    result.seedResults.serviceTypes = await seedServiceTypes(
      prisma,
      logger,
      baseOptions,
    );
    ...
    result.seedResults.serviceRatings = await seedServiceRatings(
      prisma,
      logger,
      baseOptions,
    );
    ...
    logger.info(
      "MISSION1",
      "📡 PHASE 5: Exécution des seeds d'infrastructure...",
    );
    ...
    result.seedResults.auditLogs = await seedAuditLogs(
      prisma,
      logger,
      baseOptions,
    );
    ...
    logger.info(
      "MISSION1",
      "⚙️ PHASE 6: Exécution des seeds de configuration...",
    );

    result.seedResults.systemSettings = await seedSystemSettings(
      prisma,
      logger,
      baseOptions,
    );
    result.seedResults.pricingRules = await seedPricingRules(
      prisma,
      logger,
      baseOptions,
    );
...
async function checkEnvironment(
  ...
  if (environment.isValid) {
    ...
    logger.error(
      "ENV_CHECK",
      `❌ Environnement non valide: ${environment.errors.join(", ")}`,
    );
  }
  ...
  if (environment.warnings.length > 0) {
    logger.warning(
      "ENV_CHECK",
      `⚠️ Avertissements: ${environment.warnings.join(", ")}`,
    );
  }
...
async function generateFinalReport(
  ...
  logger.info(
    "REPORT",
    `${successIcon} MISSION 1 TERMINÉE - STATUT: ${statusText}`,
  );
  logger.info("REPORT", "");
  ...
  logger.info(
    "REPORT",
    `   • Entités créées: ${result.totalCreated.toLocaleString("fr-FR")}`,
  );
  logger.info(
    "REPORT",
    `   • Entités ignorées: ${result.totalSkipped.toLocaleString("fr-FR")}`,
  );
  logger.info("REPORT", `   • Erreurs: ${result.totalErrors}`);
  logger.info(
    "REPORT",
    `   • Temps d'exécution: ${(result.executionTime / 1000).toFixed(2)}s`,
  );
  logger.info("REPORT", "");
  ...
  for (const [seedName, seedResult] of Object.entries(result.seedResults)) {
    ...
    const successRate =
      seedResult.created + seedResult.skipped > 0
        ? (
            (seedResult.created / (seedResult.created + seedResult.skipped)) *
            100
          ).toFixed(1)
        : "0";
  ...
  if (result.success) {
    ...
    logger.info(
      "REPORT",
      "   📝 Vous pouvez maintenant utiliser l'application EcoDeli",
    );
    logger.info(
      "REPORT",
      "   🚀 Prochaine étape: Démarrer le serveur de développement",
    );
  } else {
    logger.info("REPORT", "   ❌ Mission échouée - résolution requise");
    logger.info(
      "REPORT",
      "   🔧 Corriger les erreurs et relancer avec --force",
    );
    logger.info(
      "REPORT",
