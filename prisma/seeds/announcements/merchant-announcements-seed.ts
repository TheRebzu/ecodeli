...
export async function seedMerchantAnnouncements(
  ...
  for (let i = 0; i < totalAnnouncements; i++) {
    try {
      ...
      const businessInfo =
        businessSpecifics[businessType as keyof typeof businessSpecifics] ||
        businessSpecifics.Épicerie;
...
function generateMerchantPrice(
  ...
  if (type === "PACKAGE_DELIVERY") {
    ...
    const groceryAmount = faker.number.float({
      min: avgPriceRange[0] * 2,
      max: avgPriceRange[1] * 1.5,
    });
