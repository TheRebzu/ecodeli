...
export async function seedClientAnnouncements(
  ...
  const principalClient = clients.find(
    (c) => c.email === "jean.dupont@orange.fr",
  );
  if (!principalClient) {
    logger.warning(
      "CLIENT_ANNOUNCEMENTS",
      "Client principal jean.dupont@orange.fr non trouvé",
    );
  }
  ...
  if (principalClient) {
    try {
      logger.progress(
        ...
        1,
        1,
        "Création annonce spécifique Jean Dupont",
      );
      ...
      logger.success(
        "CLIENT_ANNOUNCEMENTS",
        "✅ Annonce spécifique de Jean Dupont créée",
      );
    } catch (error: any) {
      logger.error(
        "CLIENT_ANNOUNCEMENTS",
  ...
  for (let i = 0; i < totalAnnouncements; i++) {
    try {
      ...
      const client =
        principalClient && Math.random() > 0.2
          ? principalClient
      ...
      const deliveryCity =
        Math.random() > 0.7 ? getRandomElement(cities) : pickupCity; // 70% même ville
      ...
      logger.error(
        "CLIENT_ANNOUNCEMENTS",
        `❌ Erreur création annonce ${i + 1}: ${error.message}`,
      );
      result.errors++;
    }
  }
...
export async function validateClientAnnouncements(
  ...
  if (invalidDates.length > 0) {
    logger.error(
      "VALIDATION",
      `❌ ${invalidDates.length} annonces avec dates incohérentes`,
    );
    isValid = false;
  }
