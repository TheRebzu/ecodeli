...
export async function seedClientAnnouncements(
  ...
  if (clients.length === 0) {
    logger.warning('CLIENT_ANNOUNCEMENTS', 'Aucun client trouvé - exécuter d\'abord les seeds utilisateurs');
    return result;
  }
  ...
  if (existingAnnouncements > 0 && !options.force) {
    logger.warning('CLIENT_ANNOUNCEMENTS', `${existingAnnouncements} annonces déjà présentes - utiliser force:true pour recréer`);
  ...
  // Types d'annonce
  const announcementTypes = ['PACKAGE_DELIVERY', 'GROCERY_SHOPPING', 'PERSON_TRANSPORT', 'AIRPORT_TRANSFER', 'FOREIGN_PURCHASE', 'PET_CARE', 'HOME_SERVICES'];
  ...
  const totalAnnouncements = Math.min(activeClients.length * faker.number.int({ min: 2, max: 5 }), 500);
  ...
  for (let i = 0; i < totalAnnouncements; i++) {
    try {
      ...
      if (isActiveClient) {
        ...
        status = getRandomElement(['PUBLISHED', 'PUBLISHED', 'IN_APPLICATION', 'ASSIGNED', 'COMPLETED', 'DRAFT']);
      } else {
      ...
      const { title, description } = generateAnnouncementContent(type, pickupCity.name, deliveryCity.name);
  ...
  if (finalAnnouncements.length >= totalAnnouncements - result.errors) {
    logger.validation('CLIENT_ANNOUNCEMENTS', 'PASSED', `${finalAnnouncements.length} annonces créées avec succès`);
  } else {
    logger.validation('CLIENT_ANNOUNCEMENTS', 'FAILED', `Attendu: ${totalAnnouncements}, Créé: ${finalAnnouncements.length}`);
  }
  ...
  logger.info('CLIENT_ANNOUNCEMENTS', `✅ Annonces actives: ${activeAnnouncements.length} (${Math.round(activeAnnouncements.length / finalAnnouncements.length * 100)}%)`);

  // Prix moyen
  const avgPrice = finalAnnouncements
    .filter(a => a.suggestedPrice)
    .reduce((sum, a) => sum + (Number(a.suggestedPrice) || 0), 0) / finalAnnouncements.filter(a => a.suggestedPrice).length;
...
function generateRealisticAddress(city: any): string {
  ...
  const streetNames = [
    'de la République', 'Victor Hugo', 'Jean Jaurès', 'de la Paix', 'du Commerce',
    'des Roses', 'de la Liberté', 'du Marché', 'de l\'Église', 'des Écoles'
  ];
 ...
 */
function generateAnnouncementContent(type: string, pickupCity: string, deliveryCity: string): { title: string; description: string } {
  switch (type) {
...
export async function validateClientAnnouncements(
  ...
  if (announcementsCount === 0) {
    ...
    logger.success('VALIDATION', `✅ ${announcementsCount} annonces trouvées pour ${clientsCount} clients`);
  }

  // Vérifier la cohérence des dates
  const announcements = await prisma.announcement.findMany();
  
  const invalidDates = announcements.filter(a => 
  ...
  logger.info('VALIDATION', `✅ Annonces actives: ${activeAnnouncements.length}/${announcementsCount}`);
