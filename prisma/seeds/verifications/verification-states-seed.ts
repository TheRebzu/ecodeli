...
export async function seedVerificationStates(
  ...
  if (admins.length === 0) {
    logger.warning('VERIFICATION_STATES', 'Aucun admin trouvé pour les vérifications');
  }
  ...
  const byStatus = finalVerifications.reduce((acc: Record<string, number>, verification) => {
    acc[verification.status] = (acc[verification.status] || 0) + 1;
    return acc;
  }, {});

  logger.info('VERIFICATION_STATES', `📊 Vérifications par statut: ${JSON.stringify(byStatus)}`);

  // Statistiques par rôle
  const byRole = finalVerifications.reduce((acc: Record<string, number>, verification) => {
    const role = verification.submitter.role;
    acc[role] = (acc[role] || 0) + 1;
    return acc;
  }, {});

  logger.info('VERIFICATION_STATES', `👥 Vérifications par rôle: ${JSON.stringify(byRole)}`);
  ...
  if (treatedVerifications.length > 0) {
    const totalDelay = treatedVerifications.reduce((sum, v) => {
      const delay = (v.verifiedAt!.getTime() - v.requestedAt.getTime()) / (24 * 60 * 60 * 1000); // jours
      return sum + delay;
    }, 0);
    const avgDelay = Math.round(totalDelay / treatedVerifications.length);
    logger.info('VERIFICATION_STATES', `⏱️ Délai moyen de traitement: ${avgDelay} jours`);
  }
...
export async function validateVerificationStates(
  ...
  if (rejectedWithoutReason.length > 0) {
    logger.warning('VALIDATION', `⚠️ ${rejectedWithoutReason.length} documents rejetés sans motif`);
  }

  logger.success('VALIDATION', '✅ Validation des états de vérification terminée');
  return isValid;
}
