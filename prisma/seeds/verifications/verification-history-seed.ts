...
export async function seedVerificationHistory(
  ...
  if (verifications.length === 0) {
    logger.warning('VERIFICATION_HISTORY', 'Aucune vérification trouvée - exécuter d\'abord les seeds de vérifications');
    return result;
  }
  ...
  if (existingHistory > 0 && !options.force) {
    logger.warning('VERIFICATION_HISTORY', `${existingHistory} entrées d'historique déjà présentes - utiliser force:true pour recréer`);
  ...
  for (const verification of verifications) {
    try {
      logger.progress('VERIFICATION_HISTORY', totalHistoryEntries + 1, verifications.length * 3, 
      ...
      if (verification.status !== VerificationStatus.PENDING) {
        ...
        if (hasResubmission && verification.status === VerificationStatus.APPROVED) {
          ...
          const firstRejectionDate = faker.date.between({
            ...
            to: verification.verifiedAt || new Date()
                     ...
                     const verifierId = verification.verifierId || getRandomElement(admins)?.id;
           if (verifierId) {
             await createHistoryEntry(
               prisma,
               {
         ...
         if (verification.verifiedAt && verification.verifierId) {
           await createHistoryEntry(
             prisma,
             {
               ...
               comment: verification.status === VerificationStatus.APPROVED ? 
      ...
      logger.error('VERIFICATION_HISTORY', `❌ Erreur création historique pour ${verification.document.filename}: ${error.message}`);
      result.errors++;
    }
  }
  ...
  if (finalHistory.length >= totalHistoryEntries - result.errors) {
    logger.validation('VERIFICATION_HISTORY', 'PASSED', `${finalHistory.length} entrées d'historique créées avec succès`);
  } else {
    logger.validation('VERIFICATION_HISTORY', 'FAILED', `Attendu: ${totalHistoryEntries}, Créé: ${finalHistory.length}`);
  }
  ...
  logger.info('VERIFICATION_HISTORY', `📊 Historique par statut: ${JSON.stringify(byStatus)}`);
  ...
  logger.info('VERIFICATION_HISTORY', `👥 Historique par rôle: ${JSON.stringify(byRole)}`);

  // Calculer le nombre de re-soumissions
  const resubmissions = finalHistory.filter(entry => 
    entry.comment?.includes('re-soumission') || entry.comment?.includes('nouvelle version')
  );
  
  if (resubmissions.length > 0) {
    logger.info('VERIFICATION_HISTORY', `🔄 Re-soumissions: ${resubmissions.length} documents corrigés`);
  }
  ...
  logger.info('VERIFICATION_HISTORY', `👨‍💼 Admins actifs: ${activeAdmins} administrateurs ont traité des vérifications`);
 ...
 */
function generateSubmissionComment(docType: string, userRole: UserRole): string {
  ...
  const roleComments = submissionComments[userRole] || submissionComments[UserRole.CLIENT];
 ...
 */
function generateRejectionComment(docType: string, userRole: UserRole, isInitialReject: boolean): string {
  const baseReasons = [
  ...
  const advice = isInitialReject ? ` - ${getRandomElement(resubmissionAdvice)}` : "";
 ...
 */
function generateResubmissionComment(docType: string, userRole: UserRole): string {
...
export async function validateVerificationHistory(
  ...
  if (historyEntries.length === 0) {
    ...
    logger.success('VALIDATION', `✅ ${historyEntries.length} entrées d'historique trouvées pour ${verificationsCount} vérifications`);
  }

  // Vérifier que chaque document a au moins une entrée de soumission
  const documents = await prisma.document.findMany();
  const documentsWithHistory = new Set(historyEntries.map(entry => entry.documentId));
  
  const documentsWithoutHistory = documents.filter(doc => !documentsWithHistory.has(doc.id));
  
  if (documentsWithoutHistory.length > 0) {
    logger.warning('VALIDATION', `⚠️ ${documentsWithoutHistory.length} documents sans historique de vérification`);
  }
  ...
  if (invalidTimelines.length > 0) {
    logger.error('VALIDATION', `❌ ${invalidTimelines.length} entrées d'historique avec chronologie incohérente`);
    isValid = false;
  }

  // Vérifier que les actions de vérification ont un admin assigné
  const verificationActionsWithoutAdmin = historyEntries.filter(entry => 
    entry.status !== VerificationStatus.PENDING && !entry.verifiedById
  );

  if (verificationActionsWithoutAdmin.length > 0) {
    logger.warning('VALIDATION', `⚠️ ${verificationActionsWithoutAdmin.length} actions de vérification sans admin assigné`);
  }

  // Statistiques de l'historique
  const submissionEntries = historyEntries.filter(entry => entry.status === VerificationStatus.PENDING);
  const approvalEntries = historyEntries.filter(entry => entry.status === VerificationStatus.APPROVED);
  const rejectionEntries = historyEntries.filter(entry => entry.status === VerificationStatus.REJECTED);
  
  logger.info('VALIDATION', `📋 Soumissions: ${submissionEntries.length}, Approbations: ${approvalEntries.length}, Rejets: ${rejectionEntries.length}`);

  logger.success('VALIDATION', '✅ Validation de l\'historique de vérification terminée');
  return isValid;
}
...
export async function generateVerificationHistoryStats(
  ...
  historyEntries.forEach(entry => {
    if ((entry.status === VerificationStatus.APPROVED || entry.status === VerificationStatus.REJECTED) && entry.document) {
      ...
      const processingDays = (verificationTime - submissionTime) / (24 * 60 * 60 * 1000);
  ...
  const resubmissions = historyEntries.filter(entry => 
    entry.comment?.toLowerCase().includes('re-soumission') || 
    entry.comment?.toLowerCase().includes('nouvelle version')
  );
  
  const resubmissionRate = Math.round((resubmissions.length / historyEntries.length) * 100);
  logger.info('STATS', `🔄 Taux de re-soumission: ${resubmissionRate}% (${resubmissions.length} corrections)`);
