...
export async function seedDelivererDocuments(
  ...
  try {
    ...
    logger.progress('DELIVERER_DOCUMENTS', 4, 4, 'Création document historique');
  ...
  logger.info('DELIVERER_DOCUMENTS', `✅ Documents vérifiés: ${verifiedDocuments.length}`);
  logger.info('DELIVERER_DOCUMENTS', `⏳ Documents non vérifiés: ${unverifiedDocuments.length}`);
  ...
  if (soonToExpire.length > 0) {
    logger.info('DELIVERER_DOCUMENTS', `⚠️ Documents expirant bientôt: ${soonToExpire.length}`);
  } else {
    logger.info('DELIVERER_DOCUMENTS', `🔒 Tous les documents valides pour 90+ jours`);
  }
...
export async function validateDelivererDocuments(
  ...
  if (documents.length === 0) {
    ...
    logger.success('VALIDATION', `✅ ${documents.length} documents Marie Laurent trouvés`);
  }
  ...
  if (invalidUrls.length === 0) {
    ...
    logger.warning('VALIDATION', `⚠️ ${invalidUrls.length} documents avec URL invalide`);
  }
