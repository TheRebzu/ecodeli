...
export async function seedMerchantDocuments(
  ...
  if (merchants.length === 0) {
    logger.warning('MERCHANT_DOCUMENTS', 'Aucun commerçant trouvé - exécuter d\'abord les seeds utilisateurs');
    return result;
  }
  ...
  if (existingDocuments > 0 && !options.force) {
    logger.warning('MERCHANT_DOCUMENTS', `${existingDocuments} documents commerçants déjà présents - utiliser force:true pour recréer`);
  ...
  for (const merchant of merchants) {
    try {
      logger.progress('MERCHANT_DOCUMENTS', totalDocuments + 1, merchants.length * requiredDocuments.length, 
      ...
      const documentsToCreate = isVerified ? 
        requiredDocuments : 
        faker.helpers.arrayElements(requiredDocuments, faker.number.int({ min: 4, max: requiredDocuments.length }));

      for (const docType of documentsToCreate) {
        try {
          // Vérifier que docType est défini
          if (!docType) {
            logger.warning('MERCHANT_DOCUMENTS', `Type de document undefined pour ${merchant.name}`);
            continue;
          }
          ...
          const { filename, fileUrl, mimeType, fileSize, expiryDate } = generateMerchantDocumentMetadata(docType);
          ...
          const rejectionReason = status === VerificationStatus.REJECTED ? 
            getMerchantRejectionReason(docType) : null;
          ...
          logger.error('MERCHANT_DOCUMENTS', `❌ Erreur création document ${docType} pour ${merchant.name}: ${error.message}`);
          result.errors++;
        }
      }

    } catch (error: any) {
      logger.error('MERCHANT_DOCUMENTS', `❌ Erreur traitement commerçant ${merchant.name}: ${error.message}`);
      result.errors++;
    }
  }
  ...
  if (finalDocuments.length >= totalDocuments - result.errors) {
    logger.validation('MERCHANT_DOCUMENTS', 'PASSED', `${finalDocuments.length} documents commerçants créés avec succès`);
  } else {
    logger.validation('MERCHANT_DOCUMENTS', 'FAILED', `Attendu: ${totalDocuments}, Créé: ${finalDocuments.length}`);
  }
  ...
  logger.info('MERCHANT_DOCUMENTS', `✅ Taux de vérification: ${verificationRate}% (${approvedDocs.length}/${finalDocuments.length})`);
...
function generateMerchantDocumentNotes(docType: DocumentType): string {
  const notesMap: { [key: string]: string } = {
    ...
    [DocumentType.BUSINESS_REGISTRATION]: "Extrait Kbis de moins de 3 mois attestant l'existence légale",
    ...
    [DocumentType.INSURANCE_CERTIFICATE]: "Assurance responsabilité civile professionnelle commerciale",
...
export async function validateMerchantDocuments(
  ...
  if (merchantDocuments.length === 0) {
    ...
    logger.success('VALIDATION', `✅ ${merchantDocuments.length} documents commerçants trouvés pour ${merchantsCount} commerçants`);
  }
  ...
  const verifiedWithoutKbis = verifiedMerchants.filter(merchant => 
    !merchant.documents.some(doc => doc.type === DocumentType.BUSINESS_REGISTRATION && doc.verificationStatus === VerificationStatus.APPROVED)
  );

  if (verifiedWithoutKbis.length > 0) {
    logger.warning('VALIDATION', `⚠️ ${verifiedWithoutKbis.length} commerçants vérifiés sans Kbis approuvé`);
  }
