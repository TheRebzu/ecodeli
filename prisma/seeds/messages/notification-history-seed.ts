...
export async function seedNotificationHistory(
  ...
  logger.info(
    ...
    "📊 Initialisation de l'historique des notifications...",
  );
  ...
  for (const user of users) {
    try {
      ...
      for (let i = 0; i < notificationCount; i++) {
        ...
        // Statistiques
        historyByChannel[notification.channel] =
          (historyByChannel[notification.channel] || 0) + 1;
        historyByStatus[notification.status] =
          (historyByStatus[notification.status] || 0) + 1;
        historyByType[notification.type] =
          (historyByType[notification.type] || 0) + 1;
      }
  ...
  logger.info(
    "NOTIFICATION_HISTORY",
    `📊 Par canal: ${JSON.stringify(historyByChannel)}`,
  );
  logger.info(
    "NOTIFICATION_HISTORY",
    `📈 Par statut: ${JSON.stringify(historyByStatus)}`,
  );
  logger.info(
    "NOTIFICATION_HISTORY",
    `📋 Par type: ${JSON.stringify(historyByType)}`,
  );
  ...
  logger.info(
    "NOTIFICATION_HISTORY",
    `💯 Taux de livraison: ${engagementMetrics.deliveryRate}%`,
  );
  logger.info(
    "NOTIFICATION_HISTORY",
    `👁️ Taux d'ouverture: ${engagementMetrics.openRate}%`,
  );
  logger.info(
    "NOTIFICATION_HISTORY",
    `🖱️ Taux de clic: ${engagementMetrics.clickRate}%`,
  );
  logger.info(
    "NOTIFICATION_HISTORY",
    `❌ Taux d'erreur: ${engagementMetrics.errorRate}%`,
  );
 ...
 */
function generateNotificationHistory(
  user: any,
  startDate: Date,
...
function getStatusByChannel(
  channel: string,
): NotificationHistoryData["status"] {
  ...
  const probabilities =
    statusProbabilities[channel as keyof typeof statusProbabilities] ||
    statusProbabilities.email;
...
function generateNotificationContent(type: string, userName: string): any {
  const contents = {
    ...
    SYSTEM_ALERT: {
      ...
      message:
        "Maintenance programmée demain de 2h à 4h. Services temporairement indisponibles.",
...
function generateEventTimeline(
  sentAt: Date,
  status: NotificationHistoryData["status"],
): any {
  const timeline: any = {};
  ...
  if (["OPENED", "CLICKED"].includes(status)) {
    ...
    timeline.openedAt = new Date(
      minTime + faker.number.int({ min: 300, max: 86400 }) * 1000,
    );
  }
  ...
  if (status === "CLICKED") {
    ...
    timeline.clickedAt = new Date(
      minTime + faker.number.int({ min: 10, max: 7200 }) * 1000,
    );
  }
...
async function createSpecialCampaigns(
  ...
  logger.info(
    "NOTIFICATION_HISTORY",
    "🎯 Création d'historique de campagnes spéciales...",
  );
  ...
  for (const campaign of campaigns) {
    try {
      ...
      logger.error(
        "NOTIFICATION_HISTORY",
        `❌ Erreur campagne ${campaign.name}: ${error.message}`,
      );
      result.errors++;
    }
  }
  ...
  for (const analytic of analytics) {
    try {
      ...
      if (options.verbose) {
        ...
        const openRate = (
          (analytic.metrics.opened / analytic.metrics.delivered) *
          100
        ).toFixed(1);
...
function analyzeChannelPerformance(historyByChannel: Record<string, number>) {
  const total = Object.values(historyByChannel).reduce(
    (sum, count) => sum + count,
    0,
  );
  ...
  const bestChannel =
    Object.entries(historyByChannel).sort(([, a], [, b]) => b - a)[0]?.[0] ||
    "email";
