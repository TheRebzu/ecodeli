...
export async function seedNotificationHistory(
  ...
  if (users.length === 0) {
    logger.warning('NOTIFICATION_HISTORY', 'Aucun utilisateur trouvé - créer d\'abord les seeds d\'utilisateurs');
    return result;
  }
  ...
  for (const user of users) {
    try {
      ...
      for (let i = 0; i < notificationCount; i++) {
        ...
        logger.database('NOTIFICATION_HISTORY', 
          `${notification.type}_${notification.channel}_${user.id}`, 1);
      ...
      if (options.verbose && totalNotifications % 500 === 0) {
        logger.progress('NOTIFICATION_HISTORY', totalNotifications, users.length * 50, 
          `Historique créé: ${totalNotifications}`);
      }

    } catch (error: any) {
      logger.error('NOTIFICATION_HISTORY', `❌ Erreur historique utilisateur ${user.id}: ${error.message}`);
      result.errors++;
    }
  }
  ...
  logger.info('NOTIFICATION_HISTORY', `🎯 Canal le plus performant: ${channelAnalysis.bestChannel}`);
  logger.info('NOTIFICATION_HISTORY', `📱 Distribution canaux: ${JSON.stringify(channelAnalysis.distribution)}`);

  // Validation
  if (totalNotifications >= users.length * 10 - result.errors) {
    logger.validation('NOTIFICATION_HISTORY', 'PASSED', `${totalNotifications} entrées d'historique créées avec succès`);
  } else {
    logger.validation('NOTIFICATION_HISTORY', 'FAILED', `Attendu minimum: ${users.length * 10}, Créé: ${totalNotifications}`);
  }
...
function generateNotificationHistory(user: any, startDate: Date): NotificationHistoryData {
  ...
  return {
    ...
    errorMessage: status === 'FAILED' || status === 'BOUNCED' ? 
      getRandomElement([
...
function getNotificationTypeByRole(role: UserRole): string {
  const typesByRole = {
    [UserRole.CLIENT]: [
      'WELCOME', 'DELIVERY_CONFIRMATION', 'DELIVERY_STATUS', 
      'SERVICE_REMINDER', 'PAYMENT_CONFIRMATION', 'PROMOTION'
    ],
    [UserRole.DELIVERER]: [
      'DELIVERY_STATUS', 'SCHEDULE_CHANGE', 'PAYMENT_CONFIRMATION',
      'MESSAGE_RECEIVED', 'SYSTEM_ALERT'
    ],
    [UserRole.MERCHANT]: [
      'DELIVERY_CONFIRMATION', 'PAYMENT_CONFIRMATION', 'DOCUMENT_UPDATE',
      'PROMOTION', 'SYSTEM_ALERT'
    ],
    [UserRole.PROVIDER]: [
      'SERVICE_REMINDER', 'PAYMENT_CONFIRMATION', 'DOCUMENT_UPDATE',
      'MESSAGE_RECEIVED', 'SCHEDULE_CHANGE'
    ],
    [UserRole.ADMIN]: [
      'SYSTEM_ALERT', 'DOCUMENT_UPDATE', 'MESSAGE_RECEIVED',
      'DELIVERY_STATUS', 'PAYMENT_CONFIRMATION'
    ]
  };
...
function getStatusByChannel(channel: string): NotificationHistoryData['status'] {
  ...
  const probabilities = statusProbabilities[channel as keyof typeof statusProbabilities] || statusProbabilities.email;
...
function generateEventTimeline(sentAt: Date, status: NotificationHistoryData['status']): any {
  ...
  if (['DELIVERED', 'OPENED', 'CLICKED'].includes(status)) {
    ...
    timeline.deliveredAt = new Date(sentAt.getTime() + faker.number.int({ min: 30, max: 300 }) * 1000);
  }
...
async function createSpecialCampaigns(
  ...
  for (const campaign of campaigns) {
    try {
      logger.database('CAMPAIGN_HISTORY', 
        campaign.name.replace(/\s+/g, '_'), campaign.notifications);
      
      result.created += campaign.notifications;
      
      if (options.verbose) {
        logger.success('NOTIFICATION_HISTORY', 
...
async function createPerformanceAnalytics(
  ...
  for (const analytic of analytics) {
    try {
      logger.database('PERFORMANCE_ANALYTICS', 
        analytic.period.replace(/\s+/g, '_'), analytic.metrics.totalSent);
      
      result.created += 1;
      
      if (options.verbose) {
        const deliveryRate = ((analytic.metrics.delivered / analytic.metrics.totalSent) * 100).toFixed(1);
      ...
      logger.error('NOTIFICATION_HISTORY', `❌ Erreur analytics ${analytic.period}: ${error.message}`);
      result.errors++;
    }
  }
}
...
function calculateEngagementMetrics(historyByStatus: Record<string, number>, total: number) {
  const delivered = (historyByStatus.DELIVERED || 0) + (historyByStatus.OPENED || 0) + (historyByStatus.CLICKED || 0);
...
function analyzeChannelPerformance(historyByChannel: Record<string, number>) {
  ...
  const distribution = Object.entries(historyByChannel).reduce((acc, [channel, count]) => {
    acc[channel] = `${((count / total) * 100).toFixed(1)}%`;
    return acc;
  }, {} as Record<string, string>);

  const bestChannel = Object.entries(historyByChannel)
...
export async function validateNotificationHistory(
  ...
  logger.info('VALIDATION', '📝 Note: L\'historique est simulé car aucun modèle correspondant n\'existe dans le schéma Prisma');
