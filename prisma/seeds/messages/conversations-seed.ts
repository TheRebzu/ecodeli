...
export async function seedConversations(
  ...
  try {
    ...
    logger.progress(
      ...
      1,
      4,
      "Création conversation Jean ↔ Marie",
    );
    ...
    if (techShop) {
      logger.progress(
        ...
        2,
        4,
        "Création conversation Jean ↔ TechShop",
      );
    ...
    if (pierreMartin) {
      logger.progress(
        ...
        3,
        4,
        "Création conversation client ↔ Pierre Martin",
      );
      ...
      if (clientForPierre) {
        ...
        logger.success(
          "CONVERSATIONS",
          "✅ Conversation client ↔ Pierre Martin créée",
        );
      }
    }
    ...
    if (adminUser) {
      logger.progress(
        ...
        4,
        4,
        "Création conversation support admin",
      );
    ...
    logger.error(
      "CONVERSATIONS",
      `❌ Erreur création conversations: ${error.message}`,
    );
    result.errors++;
  }
  ...
  const byStatus = finalConversations.reduce(
    (acc: Record<string, number>, conversation) => {
  ...
  logger.info(
    "CONVERSATIONS",
    `📊 Conversations par statut: ${JSON.stringify(byStatus)}`,
  );
  ...
  logger.info(
    "CONVERSATIONS",
    `💬 Conversations actives: ${activeConversations.length}`,
  );
  logger.info(
    "CONVERSATIONS",
  ...
  const avgParticipants = (
    totalParticipants / finalConversations.length
  ).toFixed(1);

  logger.info(
    "CONVERSATIONS",
    `👥 Participants moyen par conversation: ${avgParticipants}`,
  );
...
export async function validateConversations(
  ...
  if (invalidConversations.length === 0) {
    logger.success(
      "VALIDATION",
      "✅ Toutes les conversations ont au moins 2 participants",
    );
  } else {
    logger.warning(
      "VALIDATION",
  ...
  if (jeanMarieConv) {
    logger.success(
      "VALIDATION",
      "✅ Conversation principale Jean-Marie trouvée",
    );
  } else {
    logger.warning(
      "VALIDATION",
      "⚠️ Conversation principale Jean-Marie manquante",
    );
  }
