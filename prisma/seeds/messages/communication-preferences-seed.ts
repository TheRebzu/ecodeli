...
export async function seedCommunicationPreferences(
  ...
  if (users.length === 0) {
    logger.warning('COMMUNICATION_PREFERENCES', 'Aucun utilisateur trouvé - créer d\'abord les seeds d\'utilisateurs');
    return result;
  }
  ...
  for (const user of users) {
    try {
      ...
      preferencesByLanguage[preferences.language] = (preferencesByLanguage[preferences.language] || 0) + 1;
      preferencesByTimezone[preferences.timezone] = (preferencesByTimezone[preferences.timezone] || 0) + 1;

      if (options.verbose && totalPreferences % 50 === 0) {
        logger.progress('COMMUNICATION_PREFERENCES', totalPreferences, users.length, 
          `Préférences créées: ${totalPreferences}/${users.length}`);
      }

    } catch (error: any) {
      logger.error('COMMUNICATION_PREFERENCES', `❌ Erreur préférences utilisateur ${user.id}: ${error.message}`);
      result.errors++;
    }
  }
  ...
  logger.info('COMMUNICATION_PREFERENCES', `🌍 Par langue: ${JSON.stringify(preferencesByLanguage)}`);
  logger.info('COMMUNICATION_PREFERENCES', `⏰ Par timezone: ${JSON.stringify(preferencesByTimezone)}`);

  // Analyser les patterns de préférences
  const analysisResults = analyzePreferencePatterns();
  logger.info('COMMUNICATION_PREFERENCES', `📈 Canaux populaires: ${JSON.stringify(analysisResults.popularChannels)}`);
  logger.info('COMMUNICATION_PREFERENCES', `🔇 Taux opt-out: ${analysisResults.optOutRate}%`);
  logger.info('COMMUNICATION_PREFERENCES', `🌙 Utilisateurs avec heures silencieuses: ${analysisResults.quietHoursUsers}%`);

  // Validation
  if (totalPreferences >= users.length - result.errors) {
    logger.validation('COMMUNICATION_PREFERENCES', 'PASSED', `${totalPreferences} préférences créées avec succès`);
  } else {
    logger.validation('COMMUNICATION_PREFERENCES', 'FAILED', `Attendu: ${users.length}, Créé: ${totalPreferences}`);
  }
...
function generateUserPreferences(user: any): CommunicationPreferenceData {
  ...
  const frequency = getRandomElement(roleBasedPreferences.preferredFrequencies) as 'IMMEDIATE' | 'HOURLY' | 'DAILY' | 'WEEKLY';
...
async function createAdvancedPreferences(
  ...
  for (const config of advancedConfigs) {
    try {
      ...
      if (options.verbose) {
        logger.success('COMMUNICATION_PREFERENCES', 
          `✅ Config avancée: ${config.name} (${config.users} utilisateurs)`
        );
      }

    } catch (error: any) {
      logger.error('COMMUNICATION_PREFERENCES', `❌ Erreur config ${config.name}: ${error.message}`);
      result.errors++;
    }
  }
}
...
export async function validateCommunicationPreferences(
  ...
  logger.info('VALIDATION', '📝 Note: Les préférences sont simulées car aucun modèle correspondant n\'existe dans le schéma Prisma');
