...
export async function seedCommunicationPreferences(
  ...
  logger.info(
    "COMMUNICATION_PREFERENCES",
    `📊 Par rôle: ${JSON.stringify(preferencesByRole)}`,
  );
  logger.info(
    "COMMUNICATION_PREFERENCES",
  ...
  logger.info(
    "COMMUNICATION_PREFERENCES",
    `🔇 Taux opt-out: ${analysisResults.optOutRate}%`,
  );
  logger.info(
    "COMMUNICATION_PREFERENCES",
...
function generateUserPreferences(user: any): CommunicationPreferenceData {
  ...
  const frequency = getRandomElement(
    roleBasedPreferences.preferredFrequencies,
  ...
  const channels = generateChannelPreferences(
    emailEnabled,
    smsEnabled,
    pushEnabled,
    user.role,
...
function generateChannelPreferences(
  ...
  return {
    deliveryUpdates: preferences.deliveryUpdates.filter((ch) =>
      availableChannels.includes(ch),
    ),
    serviceReminders: preferences.serviceReminders.filter((ch) =>
      availableChannels.includes(ch),
    ),
    promotions: preferences.promotions.filter((ch) =>
      availableChannels.includes(ch),
    ),
    systemAlerts: preferences.systemAlerts.filter((ch) =>
      availableChannels.includes(ch),
    ),
  };
}
...
async function createAdvancedPreferences(
  ...
  logger.info(
    "COMMUNICATION_PREFERENCES",
    "⚙️ Création de configurations avancées...",
  );
  ...
  const advancedConfigs = [
    {
      ...
      description:
        "Notifications urgentes uniquement pendant heures de travail",
      users: 50,
    },
    {
...
async function createDefaultPreferenceGroups(
  ...
  logger.info(
    "COMMUNICATION_PREFERENCES",
    "👥 Création de groupes de préférences par défaut...",
  );
  ...
  for (const group of defaultGroups) {
    try {
      ...
      logger.error(
        "COMMUNICATION_PREFERENCES",
        `❌ Erreur groupe ${group.name}: ${error.message}`,
      );
      result.errors++;
    }
  }
...
export async function validateCommunicationPreferences(
  ...
  logger.info(
    "VALIDATION",
    "🔍 Validation des préférences de communication...",
  );

  // Cette validation est simulée car il n'y a pas de modèle CommunicationPreference
  logger.success(
    "VALIDATION",
    "✅ Préférences de communication validées (simulation)",
  );
  logger.info(
    "VALIDATION",
