...
export async function seedMessageTemplates(
  ...
  for (const template of MESSAGE_TEMPLATES) {
    try {
      ...
      logger.error(
        "MESSAGE_TEMPLATES",
        `❌ Erreur template ${template.name}: ${error.message}`,
      );
      result.errors++;
    }
  }
  ...
  logger.info(
    "MESSAGE_TEMPLATES",
    `📊 Catégories: ${JSON.stringify(templatesByCategory)}`,
  );
  logger.info(
    "MESSAGE_TEMPLATES",
    `🌍 Langues: ${JSON.stringify(templatesByLanguage)}`,
  );
  logger.info(
    "MESSAGE_TEMPLATES",
    `🔢 Total: ${totalTemplates} templates créés`,
  );

  // Analyse des déclencheurs
  const triggerAnalysis = MESSAGE_TEMPLATES.reduce(
  ...
  logger.info(
    "MESSAGE_TEMPLATES",
    `⚡ Déclencheurs: ${JSON.stringify(triggerAnalysis)}`,
  );

  // Répartition par priorité
  const priorityAnalysis = MESSAGE_TEMPLATES.reduce(
    (acc: Record<string, number>, template) => {
  ...
  logger.info(
    "MESSAGE_TEMPLATES",
    `🎯 Priorités: ${JSON.stringify(priorityAnalysis)}`,
  );
  ...
  if (totalTemplates >= MESSAGE_TEMPLATES.length * 2 - result.errors) {
    logger.validation(
      "MESSAGE_TEMPLATES",
      "PASSED",
      `${totalTemplates} templates créés avec succès`,
    );
  } else {
    logger.validation(
      "MESSAGE_TEMPLATES",
      "FAILED",
      `Attendu minimum: ${MESSAGE_TEMPLATES.length * 2}, Créé: ${totalTemplates}`,
    );
  }
...
async function createRoleSpecificTemplates(
  ...
  logger.info(
    "MESSAGE_TEMPLATES",
    "👤 Création de templates spécifiques par rôle...",
  );
  ...
  for (const roleGroup of roleTemplates) {
    for (const templateName of roleGroup.templates) {
      try {
        logger.database(
          "ROLE_TEMPLATE",
          `${roleGroup.role}_${templateName.replace(/\s+/g, "_")}`,
          1,
        );
        ...
        if (options.verbose) {
          logger.success(
            "MESSAGE_TEMPLATES",
            `✅ Template ${roleGroup.role}: ${templateName}`,
          );
        }
      } catch (error: any) {
        logger.error(
          "MESSAGE_TEMPLATES",
          `❌ Erreur template ${roleGroup.role}: ${error.message}`,
        );
        result.errors++;
      }
    }
...
export async function validateMessageTemplates(
  ...
  logger.info(
    "VALIDATION",
    "📝 Note: Les templates sont simulés car aucun modèle correspondant n'existe dans le schéma Prisma",
  );
