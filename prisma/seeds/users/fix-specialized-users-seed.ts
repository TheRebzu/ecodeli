...
export async function seedFixSpecializedUsers(
  ...
  try {
    ...
    logger.info('FIX_SPECIALIZED_USERS', `📊 Utilisateurs par rôle: ${JSON.stringify(usersByRole)}`);
    ...
    for (const user of adminUsers) {
      try {
        ...
        if (!error.message.includes('Unique constraint')) {
          totalErrors++;
          logger.error('FIX_SPECIALIZED_USERS', `❌ Erreur admin ${user.name}: ${error.message}`);
        }
      }
    }
    ...
    for (const user of clientUsers) {
      try {
        ...
        if (!error.message.includes('Unique constraint')) {
          totalErrors++;
          logger.error('FIX_SPECIALIZED_USERS', `❌ Erreur client ${user.name}: ${error.message}`);
        }
      }
    }
      ...
      try {
        await prisma.deliverer.create({
          data: {
            userId: user.id,
            phone: user.phoneNumber || '0100000000',
            vehicleType: ['BIKE', 'SCOOTER', 'CAR', 'VAN'][Math.floor(Math.random() * 4)],
        ...
        if (!error.message.includes('Unique constraint')) {
          totalErrors++;
          logger.error('FIX_SPECIALIZED_USERS', `❌ Erreur livreur ${user.name}: ${error.message}`);
        }
      }
    }
      ...
      try {
        await prisma.merchant.create({
          data: {
            ...
            phone: user.phoneNumber || '0100000000',
            businessType: ['RESTAURANT', 'RETAIL', 'PHARMACY', 'SUPERMARKET'][Math.floor(Math.random() * 4)],
            businessCity: 'Paris',
        ...
        totalCreated++;
        logger.info('FIX_SPECIALIZED_USERS', `✅ Commerçant créé: ${user.name}`);
      } catch (error: any) {
        if (!error.message.includes('Unique constraint')) {
          totalErrors++;
          logger.error('FIX_SPECIALIZED_USERS', `❌ Erreur commerçant ${user.name}: ${error.message}`);
        }
      }
    }
      ...
      try {
        await prisma.provider.create({
          data: {
            ...
            phone: user.phoneNumber || '0100000000',
            services: ['Nettoyage', 'Jardinage', 'Bricolage'].slice(0, Math.floor(Math.random() * 3) + 1),
            isVerified: Math.random() > 0.4, // 60% des prestataires vérifiés
            rating: parseFloat((3.5 + Math.random() * 1.5).toFixed(1)), // 3.5-5.0
            serviceType: ['PLUMBING', 'ELECTRICITY', 'CLEANING', 'GARDENING'][Math.floor(Math.random() * 4)],
        ...
        totalCreated++;
        logger.info('FIX_SPECIALIZED_USERS', `✅ Prestataire créé: ${user.name}`);
      } catch (error: any) {
        if (!error.message.includes('Unique constraint')) {
          totalErrors++;
          logger.error('FIX_SPECIALIZED_USERS', `❌ Erreur prestataire ${user.name}: ${error.message}`);
        }
      }
    }
    ...
    logger.info('FIX_SPECIALIZED_USERS', `📊 Utilisateurs spécialisés créés: ${JSON.stringify(finalCounts)}`);
    logger.success('FIX_SPECIALIZED_USERS', `✅ Total créé: ${totalCreated}, Erreurs: ${totalErrors}`);
    ...
    logger.error('FIX_SPECIALIZED_USERS', `❌ Erreur dans seedFixSpecializedUsers: ${error.message}`);
    throw error;
  }
} 