...
export async function seedAdminUsers(
    ...
    try {
      ...
      if (existingUser) {
        ...
        if (!existingUser.admin) {
          ...
          logger.success(
            "ADMIN_USERS",
            `✅ Profil admin créé pour: ${existingUser.name}`,
          );
          result.created++;
        } else {
          logger.info(
            "ADMIN_USERS",
            `👤 Admin ${adminData.name} existe déjà avec profil - ignoré`,
          );
          result.skipped++;
        }
        continue;
      ...
      logger.success(
        "ADMIN_USERS",
        `✅ Admin créé: ${user.name} (${adminData.department})`,
      );
      result.created++;
    } catch (error: any) {
      logger.error(
        "ADMIN_USERS",
        `❌ Erreur création admin ${adminData.name}: ${error.message}`,
      );
      result.errors++;
    }
  }
  ...
  logger.info(
    "ADMIN_USERS",
    `📊 Répartition par département: ${JSON.stringify(byDepartment)}`,
  );
...
export async function validateAdminUsers(
  ...
  if (activeSuperAdmins.length === 0) {
    ...
    logger.success(
      "VALIDATION",
      `✅ ${activeSuperAdmins.length} super-administrateur(s) actif(s)`,
    );
  }
  ...
  if (adminsWithoutProfile.length > 0) {
    logger.error(
      "VALIDATION",
      `❌ ${adminsWithoutProfile.length} administrateurs sans profil`,
    );
    isValid = false;
  } else {
    logger.success(
      "VALIDATION",
      "✅ Tous les administrateurs ont un profil associé",
    );
  }
