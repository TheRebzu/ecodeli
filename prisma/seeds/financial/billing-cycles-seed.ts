...
export async function seedBillingCycles(
  ...
  const invoicesByRole = recentInvoices.reduce((acc: Record<string, number>, invoice) => {
    const role = invoice.user.role;
    acc[role] = (acc[role] || 0) + 1;
    return acc;
  }, {});

  logger.info('BILLING_CYCLES', `📊 Factures récentes par rôle: ${JSON.stringify(invoicesByRole)}`);
...
async function createBillingCycle(
  ...
  const invoice = await prisma.invoice.create({
    data: {
      ...
      locale: 'fr',
      paymentTerms: config.frequency === 'WEEKLY' ? 'Paiement à 7 jours' : 'Paiement à 15 jours',
  ...
  await createCycleInvoiceItems(prisma, invoice.id, user.role, amount, config.frequency);
}
...
async function createOverdueReminders(
  ...
  if (overdueInvoices.length > 0) {
    logger.info('BILLING_CYCLES', `📧 ${overdueInvoices.length} rappels d'impayés envoyés`);
  }
}
...
async function createAccountingExports(
  ...
  if (exportCount > 0) {
    logger.info('BILLING_CYCLES', `📊 ${exportCount} factures marquées pour export comptable`);
 ...
 */
async function calculateMonthlyRecurringRevenue(prisma: PrismaClient): Promise<number> {
...
export async function validateBillingCycles(
  ...
  if (recurringInvoices.length === 0) {
    ...
    logger.success('VALIDATION', `✅ ${recurringInvoices.length} factures récurrentes trouvées`);
  }
  ...
  if (invalidPeriods.length === 0) {
    logger.success('VALIDATION', '✅ Toutes les périodes de facturation sont cohérentes');
  } else {
    logger.warning('VALIDATION', `⚠️ ${invalidPeriods.length} factures avec périodes incohérentes`);
  }
