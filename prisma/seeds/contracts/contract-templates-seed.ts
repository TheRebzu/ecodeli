...
export async function seedContractTemplates(
  ...
  if (existingTemplates > 0 && !options.force) {
    logger.warning('CONTRACT_TEMPLATES', `${existingTemplates} templates déjà présents - utiliser force:true pour recréer`);
  ...
  if (!admin) {
    logger.error('CONTRACT_TEMPLATES', 'Aucun admin trouvé - exécuter d\'abord les seeds utilisateurs');
    return result;
  }
  ...
  const templates = [
    ...
    // Templates pour prestataires de services
    {
      ...
      isActive: true,
      defaultType: "STANDARD",
      defaultMonthlyFee: 0.00, // Pas de frais fixe pour prestataires
    ...
    },
    {
      ...
      content: generateProviderTemplateContent("CUSTOM", "Artisanat spécialisé")
    }
  ];
  ...
  for (const template of templates) {
    try {
      logger.progress('CONTRACT_TEMPLATES', result.created + 1, templates.length, 
      ...
      logger.error('CONTRACT_TEMPLATES', `❌ Erreur création template ${template.name}: ${error.message}`);
      result.errors++;
    }
  }
  ...
  if (finalTemplates.length >= templates.length - result.errors) {
    logger.validation('CONTRACT_TEMPLATES', 'PASSED', 
      `${finalTemplates.length} templates créés avec succès`);
  } else {
    logger.validation('CONTRACT_TEMPLATES', 'FAILED', 
      `Attendu: ${templates.length}, Créé: ${finalTemplates.length}`);
  }

  // Statistiques par type
  const byType = finalTemplates.reduce((acc: Record<string, number>, template) => {
    acc[template.defaultType] = (acc[template.defaultType] || 0) + 1;
    return acc;
  }, {});

  logger.info('CONTRACT_TEMPLATES', `📋 Templates par type: ${JSON.stringify(byType)}`);

  // Templates actifs
  const activeTemplates = finalTemplates.filter(t => t.isActive);
  logger.info('CONTRACT_TEMPLATES', `✅ Templates actifs: ${activeTemplates.length}/${finalTemplates.length}`);
 ...
 */
function generateMerchantTemplateContent(type: string, category: string): string {
  const baseContent = {
    template: {
    ...
    header: {
      ...
      parties: {
        ecodeli: "EcoDeli SAS, société par actions simplifiée au capital de 100 000€, RCS Paris 123 456 789",
 ...
 */
function generateProviderTemplateContent(type: string, category: string): string {
  const baseContent = {
    template: {
...
function getSpecificClausesByCategory(category: string, type: string): any {
  const baseClause = {
    exclusivity: type === "PARTNER" ? "Clause d'exclusivité territoriale" : "Pas d'exclusivité",
...
export async function validateContractTemplates(
  ...
  if (invalidTemplates.length > 0) {
    logger.error('VALIDATION', `❌ ${invalidTemplates.length} templates avec contenu JSON invalide`);
    isValid = false;
  }

  // Vérifier les templates actifs
  const activeTemplates = templates.filter(t => t.isActive);
  logger.info('VALIDATION', `✅ Templates actifs: ${activeTemplates.length}/${templates.length}`);
