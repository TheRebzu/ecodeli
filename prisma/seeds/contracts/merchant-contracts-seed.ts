...
export async function seedMerchantContracts(
  ...
  if (merchants.length === 0) {
    logger.warning('MERCHANT_CONTRACTS', 'Aucun commerçant trouvé - exécuter d\'abord les seeds utilisateurs');
    return result;
  }
  ...
  if (existingContracts > 0 && !options.force) {
    logger.warning('MERCHANT_CONTRACTS', `${existingContracts} contrats déjà présents - utiliser force:true pour recréer`);
  ...
  const businessTypes = {
    ...
    'Fast-food': { monthlyFee: [100, 200], commission: [0.10, 0.15] },
    'Boulangerie-pâtisserie': { monthlyFee: [80, 150], commission: [0.08, 0.12] },
    'Pharmacie générale': { monthlyFee: [200, 400], commission: [0.05, 0.10] },
    'Épicerie fine': { monthlyFee: [120, 250], commission: [0.10, 0.16] },
  ...
  for (const merchant of merchants) {
    try {
      logger.progress('MERCHANT_CONTRACTS', totalContracts + 1, merchants.length, 
      ...
      if (isVerified && isActive) {
        ...
        contractStatus = getRandomElement(['ACTIVE', 'ACTIVE', 'ACTIVE', 'PENDING_SIGNATURE', 'DRAFT']);
      } else if (!isActive) {
      ...
      logger.error('MERCHANT_CONTRACTS', `❌ Erreur création contrat pour ${merchant.name}: ${error.message}`);
      result.errors++;
    }
  }
  ...
  if (finalContracts.length >= totalContracts - result.errors) {
    logger.validation('MERCHANT_CONTRACTS', 'PASSED', `${finalContracts.length} contrats commerçants créés avec succès`);
  } else {
    logger.validation('MERCHANT_CONTRACTS', 'FAILED', `Attendu: ${totalContracts}, Créé: ${finalContracts.length}`);
  }
  ...
  logger.info('MERCHANT_CONTRACTS', `✅ Contrats actifs: ${activeContracts.length} (${Math.round(activeContracts.length / finalContracts.length * 100)}%)`);
...
export async function validateMerchantContracts(
  ...
  if (contractsCount === 0) {
    ...
    logger.success('VALIDATION', `✅ ${contractsCount} contrats trouvés pour ${merchantsCount} commerçants`);
  }
  ...
  if (expiredActiveContracts.length > 0) {
    logger.warning('VALIDATION', `⚠️ ${expiredActiveContracts.length} contrats actifs expirés à traiter`);
  }
