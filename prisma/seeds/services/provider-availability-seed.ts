...
export async function seedProviderAvailability(
  ...
  if (providers.length === 0) {
    logger.warning('PROVIDER_AVAILABILITY', 'Aucun prestataire trouvé - exécuter d\'abord les seeds utilisateurs');
    return result;
  }
  ...
  if (existingAvailabilities > 0 && !options.force) {
    logger.warning('PROVIDER_AVAILABILITY', `${existingAvailabilities} disponibilités déjà présentes - utiliser force:true pour recréer`);
  ...
  const AVAILABILITY_PROFILES = {
    FULL_TIME: {
      name: 'Temps plein',
      weekDays: [
        { day: 1, slots: [{ start: '08:00', end: '12:00' }, { start: '14:00', end: '18:00' }] }, // Lundi
        { day: 2, slots: [{ start: '08:00', end: '12:00' }, { start: '14:00', end: '18:00' }] }, // Mardi
        { day: 3, slots: [{ start: '08:00', end: '12:00' }, { start: '14:00', end: '18:00' }] }, // Mercredi
        { day: 4, slots: [{ start: '08:00', end: '12:00' }, { start: '14:00', end: '18:00' }] }, // Jeudi
        { day: 5, slots: [{ start: '08:00', end: '12:00' }, { start: '14:00', end: '18:00' }] }, // Vendredi
    ...
    PART_TIME: {
      ...
      weekDays: [
        ...
        { day: 6, slots: [{ start: '09:00', end: '12:00' }, { start: '14:00', end: '17:00' }] } // Samedi
      ]
    },
    FLEXIBLE: {
      name: 'Flexible',
  ...
  for (const provider of providers) {
    try {
      logger.progress('PROVIDER_AVAILABILITY', totalAvailabilities + 1, providers.length, 
      ...
      const selectedProfile = AVAILABILITY_PROFILES[getRandomElement(profileNames) as keyof typeof AVAILABILITY_PROFILES];
      ...
      for (const dayConfig of selectedProfile.weekDays) {
        for (const slot of dayConfig.slots) {
          try {
            ...
            logger.error('PROVIDER_AVAILABILITY', `❌ Erreur création disponibilité ${slot.start}-${slot.end}: ${error.message}`);
            result.errors++;
          }
        }
      }
      ...
      logger.error('PROVIDER_AVAILABILITY', `❌ Erreur traitement prestataire ${provider.name}: ${error.message}`);
      result.errors++;
    }
  }
  ...
  if (finalAvailabilities.length >= totalAvailabilities - result.errors) {
    logger.validation('PROVIDER_AVAILABILITY', 'PASSED', `${finalAvailabilities.length} disponibilités créées avec succès`);
  } else {
    logger.validation('PROVIDER_AVAILABILITY', 'FAILED', `Attendu: ${totalAvailabilities}, Créé: ${finalAvailabilities.length}`);
  }

  // Statistiques par prestataire
  const availabilityByProvider = finalAvailabilities.reduce((acc: Record<string, number>, availability) => {
    const providerName = availability.provider.name.split(' ')[0];
    acc[providerName] = (acc[providerName] || 0) + 1;
    return acc;
  }, {});

  const avgAvailabilityPerProvider = Object.values(availabilityByProvider).reduce((sum, count) => sum + count, 0) / Object.keys(availabilityByProvider).length;
  logger.info('PROVIDER_AVAILABILITY', `📅 Moyenne créneaux/prestataire: ${avgAvailabilityPerProvider.toFixed(1)}`);

  // Statistiques par jour de la semaine
  const availabilityByDay = finalAvailabilities.reduce((acc: Record<string, number>, availability) => {
  ...
  logger.info('PROVIDER_AVAILABILITY', `📊 Créneaux par jour: ${JSON.stringify(availabilityByDay)}`);
...
export async function validateProviderAvailability(
  ...
  if (availabilitiesWithInvalidTime.length === 0) {
    ...
    logger.warning('VALIDATION', `⚠️ ${availabilitiesWithInvalidTime.length} créneaux avec horaires incohérents`);
  }
  ...
  if (providersWithoutAvailability.length === 0) {
    ...
    logger.warning('VALIDATION', `⚠️ ${providersWithoutAvailability.length} prestataires sans disponibilités`);
  }
  ...
  if (workDaysTotal > weekendTotal) {
    logger.success('VALIDATION', `✅ Distribution cohérente: ${workDaysTotal} créneaux semaine vs ${weekendTotal} weekend`);
  } else {
    logger.warning('VALIDATION', `⚠️ Plus de créneaux weekend que semaine: ${workDaysTotal} vs ${weekendTotal}`);
  }
