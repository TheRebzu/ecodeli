...
export async function seedFixProviderServices(
  ...
  try {
    ...
    logger.info(
      ...
      `📊 Total prestataires: ${providerCount}`,
    );
    ...
    for (const provider of providersToVerify) {
      try {
        ...
        logger.info(
          "FIX_PROVIDER_SERVICES",
          `✅ Prestataire vérifié: ${provider.id}`,
        );
      } catch (error: any) {
        totalErrors++;
        logger.error(
    ...
    logger.info(
      "FIX_PROVIDER_SERVICES",
      `📊 Prestataires vérifiés: ${verifiedProviders.length}`,
    );
    ...
    logger.info(
      "FIX_PROVIDER_SERVICES",
      `📊 Catégories disponibles: ${serviceCategories.length}`,
    );

    if (serviceCategories.length === 0) {
      logger.error(
        "FIX_PROVIDER_SERVICES",
        "❌ Aucune catégorie de service trouvée",
      );
      return {
        entity: "fix_provider_services",
        created: 0,
        ...
        try {
          ...
          const service = await prisma.service.create({
            data: {
              ...
              maxParticipants:
                Math.random() > 0.7 ? Math.floor(Math.random() * 8) + 1 : null, // Parfois groupe
