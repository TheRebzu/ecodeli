...
export async function seedProviderServices(
  ...
  if (verifiedProviders.length === 0) {
    logger.warning('PROVIDER_SERVICES', 'Aucun prestataire vérifié trouvé - exécuter d\'abord les seeds prestataires');
    return result;
  }
  ...
  if (serviceCategories.length === 0) {
    logger.warning('PROVIDER_SERVICES', 'Aucune catégorie de service trouvée - exécuter d\'abord les seeds catégories');
    return result;
  }
  ...
  if (existingServices.length > 0 && !options.force) {
    logger.warning('PROVIDER_SERVICES', `${existingServices.length} services déjà présents - utiliser force:true pour recréer`);
  ...
  const serviceTemplates: Record<string, any[]> = {
    'Nettoyage': [
      { name: 'Nettoyage domicile standard', price: 25, duration: 120, description: 'Nettoyage complet de votre domicile' },
      { name: 'Nettoyage bureaux', price: 35, duration: 180, description: 'Nettoyage professionnel d\'espaces de travail' },
      { name: 'Nettoyage après travaux', price: 45, duration: 240, description: 'Remise en état après rénovation' },
      { name: 'Nettoyage vitres', price: 15, duration: 60, description: 'Nettoyage des vitres intérieures et extérieures' }
    ],
    'Jardinage': [
      { name: 'Tonte de pelouse', price: 20, duration: 90, description: 'Tonte et ramassage de l\'herbe' },
      { name: 'Taille de haies', price: 30, duration: 120, description: 'Taille et mise en forme des haies' },
      { name: 'Plantation saisonnière', price: 40, duration: 180, description: 'Plantation de fleurs et arbustes' },
      { name: 'Entretien jardin complet', price: 60, duration: 240, description: 'Entretien complet de votre jardin' }
    ],
    'Bricolage': [
      { name: 'Montage meubles', price: 25, duration: 120, description: 'Montage et installation de mobilier' },
      { name: 'Petites réparations', price: 35, duration: 90, description: 'Réparations diverses du quotidien' },
      { name: 'Installation électrique', price: 50, duration: 180, description: 'Installation d\'équipements électriques' },
      { name: 'Peinture intérieure', price: 40, duration: 300, description: 'Peinture de pièces et surfaces' }
    ],
    'Livraisons': [
      { name: 'Livraison express', price: 12, duration: 30, description: 'Livraison rapide en moins d\'1h' },
      { name: 'Livraison standard', price: 8, duration: 60, description: 'Livraison dans la journée' },
      { name: 'Livraison volumineux', price: 25, duration: 90, description: 'Transport d\'objets encombrants' },
      { name: 'Livraison fragile', price: 18, duration: 45, description: 'Transport sécurisé d\'objets fragiles' }
    ]
  };
  ...
  for (const provider of verifiedProviders) {
    try {
      logger.progress('PROVIDER_SERVICES', totalServices + 1, verifiedProviders.length, 
      ...
      logger.error('PROVIDER_SERVICES', `❌ Erreur création services pour ${provider.user.name}: ${error.message}`);
      result.errors++;
    }
  }
  ...
  if (finalServices.length > 0) {
    ...
    logger.info('PROVIDER_SERVICES', `📊 Services par catégorie: ${JSON.stringify(servicesByCategory)}`);
  } else {
  ...
  logger.info('PROVIDER_SERVICES', `💰 Prix moyen: ${avgPrice.toFixed(2)}€ (min: ${minPrice}€, max: ${maxPrice}€)`);
  ...
  logger.info('PROVIDER_SERVICES', `📈 Services actifs: ${activeServices}, inactifs: ${inactiveServices}`);
