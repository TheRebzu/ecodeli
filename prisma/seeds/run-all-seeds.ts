...
class SeedOrchestrator {
  ...
  async run(): Promise<void> {
    try {
      this.logger.info(
        "ORCHESTRATOR",
        "🚀 Démarrage du système de seeds EcoDeli",
      );
      this.logger.info(
        "ORCHESTRATOR",
  ...
  private async dryRun(): Promise<void> {
    ...
    this.logger.info(
      "DRY_RUN",
      `📋 ${seedsToRun.length} seeds seraient exécutés dans cet ordre:`,
    );
    ...
    seedsToRun.forEach((seed, index) => {
      ...
      if (seed.dependencies.length > 0) {
        this.logger.info(
          "DRY_RUN",
          `   Dépendances: ${seed.dependencies.join(", ")}`,
        );
      }
    });
  ...
  private async validateOnly(): Promise<void> {
    ...
    if (stats.failedRules > 0) {
      throw new Error(
        `Validation échouée: ${stats.failedRules} règles non respectées`,
      );
    }
  ...
  private resolveDependencies(): SeedDefinition[] {
    ...
    const visit = (seedName: string) => {
      ...
      if (visiting.has(seedName)) {
        throw new Error(
          `Dépendance circulaire détectée impliquant: ${seedName}`,
        );
      }
  ...
  private async executeSeedsInOrder(seeds: SeedDefinition[]): Promise<void> {
    ...
    for (let i = 0; i < seeds.length; i++) {
      ...
      this.logger.info(
        "EXECUTION",
        `[${i + 1}/${seeds.length}] Exécution: ${seed.name}`,
      );
      ...
      try {
        ...
        this.logger.error(
          "EXECUTION",
          `❌ Échec du seed ${seed.name}: ${error.message}`,
        );
        totalErrors++;
