...
export async function GET(
  ...
  try {
    ...
    const tracking = await warehouseManagementService.trackPackageInWarehouse(announcementId)
...
export async function POST(
  ...
  try {
    ...
    // Valider le statut
    const validStatuses = ['INCOMING', 'STORED', 'PREPARING', 'READY_FOR_PICKUP', 'DISPATCHED']
    ...
    if (updated.announcement) {
      ...
      console.log(`Notification à envoyer pour annonce ${updated.announcement.id}`)
    }
