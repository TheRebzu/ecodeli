...
export async function GET(
  ...
  try {
    ...
    if (invoice.providerId !== session.user.id && session.user.role !== 'ADMIN') {
      return NextResponse.json(
    ...
    doc.setFontSize(10)
    doc.text(`${invoice.provider.profile?.firstName} ${invoice.provider.profile?.lastName}`, 20, 62)
    ...
    invoice.items.forEach(item => {
      const commission = item.totalPrice * 0.20 // 20% de commission
    ...
    if (invoice.paidAt) {
      ...
      doc.text(`Échéance: ${format(invoice.dueDate || new Date(), 'dd/MM/yyyy')}`, 20, y)
      doc.text("Mode: Virement bancaire automatique sous 5 jours ouvrés", 20, y + 5)
    }
    ...
    doc.text("Document à conserver pour votre comptabilité d'autoentrepreneur", 20, y + 5)
