...
export async function GET(request: NextRequest) {
  try {
    ...
    const formattedInvoices = invoices.map(invoice => ({
      id: invoice.id,
      month: invoice.period?.split('-')[1] || new Date(invoice.createdAt).getMonth() + 1,
      year: invoice.period?.split('-')[0] || new Date(invoice.createdAt).getFullYear(),
      totalServices: invoice.invoiceItems.length,
      totalHours: invoice.invoiceItems.reduce((sum, item) => sum + (item.quantity || 0), 0),
      ...
      services: invoice.invoiceItems.map(item => ({
        ...
        clientName: item.booking 
          ...
          : 'Client inconnu',
        date: item.booking?.scheduledAt?.toISOString() || item.createdAt.toISOString(),
...
export async function POST(request: NextRequest) {
  try {
    ...
    const invoice = await generateProviderInvoice(targetProviderId, month, year)
