...
export async function GET(request: NextRequest) {
  try {
    ...
    doc.text(`Date de génération prévue: 30 ${format(targetDate, "MMMM yyyy", { locale: fr })} à 23h00`, 20, 55);
    ...
    doc.text(`Commission EcoDeli (20%): -${totalCommission.toFixed(2)}€`, 20, 110);
    doc.setFontSize(12);
    doc.text(`MONTANT NET À PERCEVOIR: ${totalNet.toFixed(2)}€`, 20, 120);
    
    // Note
    doc.setFontSize(8);
    doc.text("Cet aperçu est fourni à titre indicatif. La facture définitive sera générée automatiquement", 20, 140);
    doc.text("le 30 du mois à 23h00 et pourra présenter des différences selon les prestations effectuées.", 20, 145);
