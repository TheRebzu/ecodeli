...
const createApplicationSchema = z.object({
  serviceRequestId: z.string().min(1, 'L\'ID de la demande de service est requis'),
  message: z.string().min(10, 'Le message doit contenir au moins 10 caractères'),
  proposedPrice: z.number().min(0, 'Le prix proposé doit être positif'),
  estimatedDuration: z.number().min(0.5, 'La durée estimée doit être d\'au moins 30 minutes'),
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`✅ Trouvé ${formattedApplications.length} candidatures sur ${total} total`)
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!provider.isValidated) {
      return NextResponse.json({ 
        error: 'Votre compte prestataire doit être validé avant de pouvoir candidater' 
      }, { status: 403 })
    }
    ...
    if (serviceRequest.assignedProviderId && serviceRequest.assignedProviderId !== provider.id) {
      return NextResponse.json({ 
