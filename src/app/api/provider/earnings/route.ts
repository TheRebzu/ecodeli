...
export async function GET(request: NextRequest) {
  try {
    ...
    const previousPeriod = ProviderBillingService.getBillingPeriod(previousMonth, previousYear)
    ...
    const comparison = {
      earningsChange: previousEarnings ? 
        ((earnings.netAmount - previousEarnings.total) / previousEarnings.total * 100) : 0,
      interventionsChange: previousEarnings?.metadata?.totalInterventions ? 
        ((earnings.totalInterventions - previousEarnings.metadata.totalInterventions) / previousEarnings.metadata.totalInterventions * 100) : 0,
      hoursChange: previousEarnings?.metadata?.totalHours ? 
        ((earnings.totalHours - previousEarnings.metadata.totalHours) / previousEarnings.metadata.totalHours * 100) : 0
    }
