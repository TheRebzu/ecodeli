...
export async function GET(request: NextRequest) {
  try {
    ...
    const formattedInterventions = upcomingInterventions.map(booking => {
      ...
      const addressData = typeof booking.address === 'object' && booking.address ? booking.address as any : {};
      const fullAddress = [
        addressData.address || '',
        addressData.city || '',
      ...
      return {
        ...
        clientName: `${booking.client.user.profile?.firstName || ''} ${booking.client.user.profile?.lastName || ''}`.trim() || booking.client.user.email,
