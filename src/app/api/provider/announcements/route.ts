...
export async function GET(request: NextRequest) {
  try {
    ...
    const where: any = {
      type: {
        in: ['HOME_SERVICE', 'PET_SITTING', 'PERSON_TRANSPORT', 'AIRPORT_TRANSFER']
      },
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { announcementId, proposedPrice, message, availableSlots, estimatedDuration } = bodySchema.parse(body)
    ...
    if (announcement.status !== 'ACTIVE') {
      return NextResponse.json({ error: 'Cette annonce n\'est plus disponible' }, { status: 409 })
    }
