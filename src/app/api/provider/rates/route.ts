...
export async function POST(request: NextRequest) {
  try {
    ...
    const { providerId, serviceType, proposedRate, minimumCharge, providerNotes } = createRateSchema.parse(body);
    ...
    const serviceRatesConfig = {
      ...
      'TUTORING': { baseRate: 30, commission: 0.10, unit: 'HOUR' },
      'HEALTHCARE': { baseRate: 45, commission: 0.08, unit: 'HOUR' },
      'BEAUTY': { baseRate: 40, commission: 0.15, unit: 'HOUR' },
      'PET_SITTING': { baseRate: 15, commission: 0.20, unit: 'HOUR' },
    };

    const config = serviceRatesConfig[serviceType as keyof typeof serviceRatesConfig];
    if (!config) {
      return NextResponse.json(
