...
export async function POST(request: NextRequest) {
  try {
    ...
    const { merchantId, timeRange, format, data } =
...
async function generateCSV(data: any): Promise<Buffer> {
  ...
  if (data.overview?.revenue) {
    ...
    lines.push(
      `Chiffre d'affaires total,${revenue.totalRevenue},${formatPeriod(data.lastUpdated)}`,
    );
    lines.push(
      `Nombre de commandes,${revenue.totalOrders},${formatPeriod(data.lastUpdated)}`,
    );
    lines.push(
      `Panier moyen,${revenue.averageOrderValue.toFixed(2)},${formatPeriod(data.lastUpdated)}`,
    );
    lines.push(
      `Taux de croissance,${revenue.growthRate.toFixed(2)}%,${formatPeriod(data.lastUpdated)}`,
    );
  }
  ...
  if (data.overview?.customers) {
    ...
    lines.push(
      `Nombre total de clients,${customers.totalCustomers},${formatPeriod(data.lastUpdated)}`,
    );
    lines.push(
      `Nouveaux clients,${customers.newCustomers},${formatPeriod(data.lastUpdated)}`,
    );
    lines.push(
      `Clients récurrents,${customers.returningCustomers},${formatPeriod(data.lastUpdated)}`,
    );
    lines.push(
      `Taux de rétention,${customers.customerRetentionRate.toFixed(2)}%,${formatPeriod(data.lastUpdated)}`,
    );
  }
  ...
  if (data.overview?.deliveries) {
    ...
    lines.push(
      `Total livraisons,${deliveries.totalDeliveries},${formatPeriod(data.lastUpdated)}`,
    );
    lines.push(
      `Livraisons réussies,${deliveries.successfulDeliveries},${formatPeriod(data.lastUpdated)}`,
    );
    lines.push(
      `Temps de livraison moyen,${deliveries.averageDeliveryTime.toFixed(2)}h,${formatPeriod(data.lastUpdated)}`,
    );
    lines.push(
      `Taux de ponctualité,${deliveries.onTimeDeliveryRate.toFixed(2)}%,${formatPeriod(data.lastUpdated)}`,
    );
  }
