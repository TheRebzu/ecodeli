...
const csvRowSchema = z.object({
  ...
  type: z.enum(['PACKAGE_DELIVERY', 'PERSON_TRANSPORT', 'AIRPORT_TRANSFER', 'SHOPPING', 'INTERNATIONAL_PURCHASE', 'CART_DROP']),
...
export async function POST(request: NextRequest) {
  try {
    ...
    const expectedHeaders = [
      'title', 'description', 'type', 'basePrice', 'pickupAddress', 
      'deliveryAddress', 'weight', 'dimensions', 'isFragile', 
      'requiresInsurance', 'pickupDate', 'isUrgent'
    ]
    ...
    if (missingHeaders.length > 0) {
      results.warnings.push(`En-têtes manquants : ${missingHeaders.join(', ')}`)
    }
    if (extraHeaders.length > 0) {
      results.warnings.push(`En-têtes supplémentaires ignorés : ${extraHeaders.join(', ')}`)
    }
    ...
    for (let i = 0; i < maxRows; i++) {
      ...
      try {
        ...
        headers.forEach((header, index) => {
          if (expectedHeaders.includes(header) && rowData[index] !== undefined) {
            let value = rowData[index]
            
            // Conversions de type
            if (['basePrice', 'weight'].includes(header)) {
              value = parseFloat(value) || 0
            } else if (['isFragile', 'requiresInsurance', 'isUrgent'].includes(header)) {
        ...
        if (error instanceof z.ZodError) {
          ...
          errorMessages = [error instanceof Error ? error.message : 'Erreur inconnue']
        }
    ...
    if (results.invalidRows > results.validRows) {
      recommendations.push('Plus de 50% des lignes sont invalides. Vérifiez le format de votre fichier.')
    }
    
    if (results.validRows === 0) {
      recommendations.push('Aucune ligne valide détectée. Vérifiez le format et les données.')
    }
    
    if (results.validRows > 100) {
      recommendations.push(`${results.validRows} lignes valides détectées. L'import sera effectué par batches de 50.`)
    }
