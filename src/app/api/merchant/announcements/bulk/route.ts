      ...
      try {
        const announcement: any = {
          ...
          basePrice: parseFloat(
            row[columnMapping.basePrice]?.toString().replace(",", ".") || "0",
          ),
          pickupAddress: row[columnMapping.pickupAddress]?.toString().trim(),
          deliveryAddress: row[columnMapping.deliveryAddress]
            ?.toString()
            .trim(),
        };
        ...
        if (columnMapping.category) {
          announcement.category =
            row[columnMapping.category]?.toString().trim() || "Divers";
        }

        if (columnMapping.weight) {
          const weight = parseFloat(
            row[columnMapping.weight]?.toString().replace(",", ".") || "0",
          );
          if (weight > 0) announcement.weight = weight;
        }

        if (columnMapping.dimensions) {
          announcement.dimensions = row[columnMapping.dimensions]
            ?.toString()
            .trim();
        }

        if (columnMapping.isFragile) {
          const fragile = row[columnMapping.isFragile]
            ?.toString()
            .toLowerCase();
          announcement.isFragile = ["oui", "yes", "true", "1"].includes(
            fragile,
          );
        }

        if (columnMapping.requiresInsurance) {
          const insurance = row[columnMapping.requiresInsurance]
            ?.toString()
            .toLowerCase();
          announcement.requiresInsurance = ["oui", "yes", "true", "1"].includes(
            insurance,
          );
        }
        ...
        if (columnMapping.maxInsuranceValue) {
          const value = parseFloat(
            row[columnMapping.maxInsuranceValue]
              ?.toString()
              .replace(",", ".") || "0",
          );
        ...
        if (!announcement.availableFrom)
          announcement.availableFrom = new Date().toISOString();
    ...
    if (preview) {
      return NextResponse.json({
        ...
        statistics: {
          ...
          priceRange: {
            min: Math.min(
              ...processedAnnouncements.map((ann) => ann.basePrice),
            ),
            max: Math.max(
              ...processedAnnouncements.map((ann) => ann.basePrice),
            ),
    ...
    if (format === "excel") {
      ...
      return new NextResponse(buffer, {
        headers: {
          "Content-Type":
            "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
          "Content-Disposition":
            'attachment; filename="template_annonces_ecodeli.xlsx"',
        },
      });
    } else {
      ...
      return new NextResponse(csvContent, {
        headers: {
          ...
          "Content-Disposition":
            'attachment; filename="template_annonces_ecodeli.csv"',
        },
      });
    }
  } catch (error) {
