  ...
  try {
    ...
    const schedule = merchant.cartDropConfig.timeSlots.map((slot: any) => {
      const slotOrders = orders.filter(order => {
        ...
        return orderDate.getDay() === slot.day && 
      ...
      return {
        ...slot,
        dayName: ['Dimanche', 'Lundi', 'Mardi', 'Mercredi', 'Jeudi', 'Vendredi', 'Samedi'][slot.day],
        ordersCount: slotOrders.length,
        availableSlots: merchant.cartDropConfig.maxOrdersPerSlot - slotOrders.length,
