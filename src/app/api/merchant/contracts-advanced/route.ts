    ...
    return NextResponse.json({
      contracts: contracts.map((contract) => ({
        ...
        bonusThresholds: contract.bonusThresholds
          ? JSON.parse(contract.bonusThresholds as string)
          : null,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const defaultConditions = {
      ...
      CUSTOM: {
        paymentTerms: 7,
        minimumVolume: validatedData.expectedVolume || 10000,
      },
    };
    ...
    console.log(
      `Nouvelle demande de contrat ${validatedData.type} pour le commerçant ${session.user.id}`,
    );

    // Si c'est un contrat custom, créer une demande de négociation automatique
    if (
      validatedData.type === "CUSTOM" &&
      validatedData.requestedCommissionRate
    ) {
      await prisma.contractNegotiation.create({
        data: {
          ...
          reason:
            ...
            "Demande de contrat personnalisé",
          status: "pending",
        },
      });
    }
