...
export async function GET(request: NextRequest) {
  try {
    ...
    const inventory = announcements.map((announcement) => {
      ...
      const totalRevenue = announcement.deliveries.reduce(
        (sum, delivery) =>
          sum + (delivery.payment ? Number(delivery.payment.amount) : 0),
        0,
      );
      ...
      const lastDelivery = announcement.deliveries.sort(
        (a, b) =>
      ...
      const lastRestocked = lastDelivery
        ? new Date(
            lastDelivery.createdAt.getTime() + 7 * 24 * 60 * 60 * 1000,
          ).toISOString() // 7 days after last delivery
