...
export async function GET(request: NextRequest) {
  try {
    ...
    const settings = {
      ...
      deliveryEnabled: merchantProfile.merchantSettings?.deliveryEnabled ?? true,
      deliveryZones: merchantProfile.merchantSettings?.deliveryZones ?? [],
      deliveryHours: merchantProfile.merchantSettings?.deliveryHours ?? { start: '09:00', end: '18:00' },
      minOrderAmount: merchantProfile.merchantSettings?.minOrderAmount ?? 0,
      deliveryFee: merchantProfile.merchantSettings?.deliveryFee ?? 5.00,
      
      emailNotifications: merchantProfile.merchantSettings?.emailNotifications ?? true,
      smsNotifications: merchantProfile.merchantSettings?.smsNotifications ?? false,
      pushNotifications: merchantProfile.merchantSettings?.pushNotifications ?? true,
      orderNotifications: merchantProfile.merchantSettings?.orderNotifications ?? true,
      deliveryNotifications: merchantProfile.merchantSettings?.deliveryNotifications ?? true,
      marketingNotifications: merchantProfile.merchantSettings?.marketingNotifications ?? false,
      
      paymentMethods: merchantProfile.merchantSettings?.paymentMethods ?? ['stripe'],
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (validatedData.posIntegration || validatedData.inventorySync || validatedData.accountingSync) {
