...
export async function POST(request: NextRequest) {
  try {
    ...
    ecoLogger.billing.info('Début de la facturation automatique mensuelle des prestataires')

    // Récupérer le mois et l'année cibles depuis les paramètres (pour les tests)
    const { searchParams } = new URL(request.url)
    const targetMonth = searchParams.get('month') ? parseInt(searchParams.get('month')!) : undefined
    const targetYear = searchParams.get('year') ? parseInt(searchParams.get('year')!) : undefined

    // Générer les factures mensuelles avec le service dédié
    const results = await ProviderBillingService.generateMonthlyInvoices(targetMonth, targetYear)
