...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const provider of activeProviders) {
      try {
        console.log(
          `💼 Traitement prestataire: ${provider.businessName || provider.user.profile?.firstName}`,
        );
        ...
        if (completedBookings === 0) {
          console.log(
            `⏭️  Aucune intervention à facturer pour ${provider.businessName}`,
          );
          results.errors.push({
            providerId: provider.id,
            providerName:
              provider.businessName ||
              provider.user.profile?.firstName ||
              "Inconnu",
        ...
        const invoiceId = await BillingService.generateMonthlyInvoice(
          provider.id,
          {
            start: startOfMonth,
            end: endOfMonth,
        ...
        console.log(
          `✅ Facture générée pour ${provider.businessName}: ${invoiceId}`,
        );
...
async function generateInvoicePDF(
  ...
  doc.text(
    "À conserver pour votre comptabilité d'autoentrepreneur",
    20,
    y + 10,
  );
