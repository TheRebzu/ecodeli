...
export async function POST(request: NextRequest) {
  try {
    console.log('🔍 [POST /api/client/applications/verify-payment] Vérification paiement candidature')
    ...
    try {
      ...
      const session = await stripe.checkout.sessions.retrieve(validatedData.sessionId)
      
      if (!session) {
        console.log('❌ Session Stripe non trouvée')
        return NextResponse.json({ error: 'Session de paiement non trouvée' }, { status: 404 })
      }

      console.log('✅ Session Stripe récupérée:', session.id, 'Status:', session.payment_status)
      ...
      const intervention = await db.serviceIntervention.findFirst({
        where: {
      ...
      if (intervention) {
        await db.serviceIntervention.update({
      ...
      await db.notification.create({
        data: {
          ...
          message: `Le client a confirmé le paiement pour le service "${application.serviceRequest.title}".`,
      ...
      return NextResponse.json({
        success: true,
        amount: session.amount_total ? session.amount_total / 100 : 0,
        providerName: `${application.provider.user.profile?.firstName} ${application.provider.user.profile?.lastName}`,
