...
export async function POST(
  ...
  try {
    console.log('🔍 [POST /api/client/applications/[id]/respond] Réponse client à candidature')
    ...
    try {
      ...
      console.log(`✅ Candidature ${validatedData.action === 'ACCEPT' ? 'acceptée' : 'refusée'}`)
      ...
      const notificationTitle = validatedData.action === 'ACCEPT' 
        ? 'Candidature acceptée' 
        : 'Candidature refusée'
      
      const notificationMessage = validatedData.action === 'ACCEPT'
      ...
      await db.notification.create({
        data: {
          ...
          type: validatedData.action === 'ACCEPT' ? 'APPLICATION_ACCEPTED' : 'APPLICATION_REJECTED',
          data: {
