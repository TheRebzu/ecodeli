...
export async function GET(request: NextRequest) {
  try {
    ...
    let totalSavings = 0
    if (subscription) {
      const discountRate = subscription.plan === 'STARTER' ? 0.05 : subscription.plan === 'PREMIUM' ? 0.09 : 0
    ...
    thisMonth.setDate(1)
    const deliveriesThisMonth = deliveries.filter(d => d.createdAt >= thisMonth).length
    ...
    // Top villes
    const cityStats = deliveries.reduce((acc, delivery) => {
      const city = delivery.deliveryAddress.split(',').pop()?.trim() || 'Inconnu'
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const monthDeliveries = deliveries.filter(d => 
        d.createdAt >= month && d.createdAt < nextMonth
      )
