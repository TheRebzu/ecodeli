...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    console.log('👤 [POST /api/client/deliveries/[id]/validate] Début de la requête')
    ...
    if (delivery.status !== 'IN_TRANSIT') {
      console.log('❌ Livraison ne peut pas être validée, statut:', delivery.status)
    ...
    if (validatedData.validated) {
      ...
      await db.notification.create({
        data: {
          ...
          message: 'Le client a rejeté la livraison. Veuillez contacter le support.',
          data: JSON.stringify({
            deliveryId: delivery.id,
