...
export async function GET(
  ...
  try {
    ...
    if (['ACCEPTED', 'PICKED_UP', 'IN_TRANSIT', 'DELIVERED'].includes(delivery.status)) {
      history.push({
        status: 'ACCEPTED',
    ...
    if (['IN_TRANSIT', 'DELIVERED'].includes(delivery.status)) {
      history.push({
        status: 'IN_TRANSIT',
        timestamp: new Date(delivery.createdAt.getTime() + 90 * 60 * 1000).toISOString(), // +1h30
    ...
    if (delivery.status === 'DELIVERED') {
      history.push({
        status: 'DELIVERED',
        timestamp: delivery.actualDeliveryDate?.toISOString() || new Date().toISOString(),
    ...
    const deliveryDetails = {
      ...
      scheduledDate: delivery.deliveryDate?.toISOString() || delivery.pickupDate?.toISOString() || null,
      ...
      packageDetails: {
        ...
        weight: delivery.announcement.weight ? Number(delivery.announcement.weight) : null,
      ...
      proofOfDelivery: delivery.ProofOfDelivery ? {
        ...
        uploadedAt: delivery.ProofOfDelivery.createdAt?.toISOString() || null
      } : null,
