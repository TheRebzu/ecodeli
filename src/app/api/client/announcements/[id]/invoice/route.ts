...
export async function GET(
  ...
  try {
    ...
    logger.info(`Récupération facture pour annonce ${announcementId} par client ${user.id}`)
    ...
    if (announcement.status !== 'COMPLETED' || announcement.delivery?.status !== 'DELIVERED') {
      return NextResponse.json({
        error: 'La facture n\'est disponible qu\'après validation de la livraison',
    ...
    if (existingInvoice && existingInvoice.pdfUrl) {
      ...
      try {
        invoiceUrl = await InvoiceGeneratorService.generateAnnouncementInvoice(announcementId)
...
export async function POST(
  ...
  try {
    ...
    logger.info(`Régénération facture pour annonce ${announcementId} par ${user.role} ${user.id}`)
    ...
    if (announcement.status !== 'COMPLETED') {
      return NextResponse.json({
        error: 'Impossible de générer une facture pour une livraison non terminée'
      }, { status: 400 })
    }
