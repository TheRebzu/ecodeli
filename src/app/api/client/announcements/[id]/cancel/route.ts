...
const cancelAnnouncementSchema = z.object({
  reason: z.string().min(10, 'Raison d\'annulation requise (minimum 10 caractères)').max(500, 'Raison trop longue (maximum 500 caractères)'),
  confirmCancel: z.boolean().refine(val => val === true, 'Confirmation d\'annulation requise')
})
...
export async function POST(
  ...
  try {
    ...
    logger.info(`Demande d'annulation pour annonce ${announcementId} par client ${user.id}`)
    ...
    if (!cancellableStatuses.includes(announcement.status)) {
      const statusMessages = {
        'IN_PROGRESS': 'Cette annonce ne peut plus être annulée car elle est en cours de livraison',
        'COMPLETED': 'Cette annonce est déjà terminée',
        'CANCELLED': 'Cette annonce est déjà annulée'
      }
      
      return NextResponse.json({ 
        error: statusMessages[announcement.status as keyof typeof statusMessages] || 'Annulation impossible dans ce statut',
...
export async function GET(
  ...
  try {
    ...
    const response = {
      ...
      cancellation: {
        canCancel,
        reason: canCancel ? null : getCancellationBlockReason(announcement.status),
        fees: cancellationInfo.fees,
        refund: cancellationInfo.refund,
        conditions: [
...
function calculateCancellationFees(announcement: any) {
  ...
  if (timeElapsed <= gracePeriodMs) {
    return {
      ...
      refund: {
        ...
        reason: announcement.status !== 'DRAFT' 
  ...
  let processingFee = 2.50 // Frais fixes de traitement
...
function getCancellationBlockReason(status: string): string {
  ...
  return reasons[status as keyof typeof reasons] || 'Statut incompatible avec l\'annulation'
}