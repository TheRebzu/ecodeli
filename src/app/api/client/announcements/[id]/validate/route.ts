...
const validateDeliverySchema = z.object({
  validationCode: z
    ...
    .regex(
      /^\d{6}$/,
      "Le code de validation ne doit contenir que des chiffres",
    ),
  location: z
    .object({
...
export async function POST(
  ...
  try {
    ...
    logger.info(
      `Validation de livraison pour annonce ${announcementId} par client ${user.id}`,
    );
