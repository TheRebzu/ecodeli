...
export async function POST(
  ...
  try {
    ...
    logger.info(`Création PaymentIntent pour annonce ${announcementId} par client ${user.id}`)
    ...
    const existingPayment = await announcementPaymentService.getClientPayments(user.id, {
      status: 'PENDING'
    })
...
export async function PUT(
  ...
  try {
    ...
    const clientPayments = await announcementPaymentService.getClientPayments(user.id, {
      status: 'PENDING'
    })

    const payment = clientPayments.payments.find(p => p.announcementId === announcementId)
...
export async function GET(
  ...
  try {
    ...
    if (payment.stripePaymentId && ['PENDING', 'REQUIRES_ACTION'].includes(payment.status)) {
      try {
        currentStatus = await announcementPaymentService.checkPaymentStatus(payment.stripePaymentId)
      } catch (error) {
