...
export async function GET(
  ...
  try {
    ...
    logger.info(`Récupération code validation pour annonce ${announcementId} par client ${user.id}`)
    ...
    if (!validStatuses.includes(announcement.delivery.status)) {
      return NextResponse.json({
        ...
        reason: getValidationCodeUnavailableReason(announcement.delivery.status)
      }, { status: 400 })
    }

    // Récupérer ou générer le code de validation
    let validationInfo = await ValidationCodeService.getValidationInfo(announcement.delivery.id)

    if (!validationInfo) {
      // Générer un nouveau code si aucun n'existe
      const newCode = await ValidationCodeService.generateValidationCode(announcement.delivery.id)
      validationInfo = await ValidationCodeService.getValidationInfo(announcement.delivery.id)
      
      if (!validationInfo) {
        throw new Error('Impossible de générer le code de validation')
      }

      logger.info(`Nouveau code de validation généré pour livraison ${announcement.delivery.id}`)
    }
    ...
    if (isExpired) {
      ...
      logger.info(`Code de validation régénéré pour livraison ${announcement.delivery.id}`)
    }
    ...
    const response = {
      ...
      validation: {
        code: validationInfo.code,
        expiresAt: validationInfo.expiresAt.toISOString(),
        timeRemaining: Math.max(0, validationInfo.expiresAt.getTime() - Date.now()),
        timeRemainingFormatted: formatTimeRemaining(Math.max(0, validationInfo.expiresAt.getTime() - Date.now())),
...
export async function POST(
  ...
  try {
    ...
    logger.info(`Régénération code validation pour annonce ${announcementId} par client ${user.id}`)
    ...
    if (!announcement || !announcement.delivery) {
      return NextResponse.json({ error: 'Annonce ou livraison introuvable' }, { status: 404 })
    }
    ...
    // Régénérer le code
    const newCode = await ValidationCodeService.generateValidationCode(announcement.delivery.id)
    const validationInfo = await ValidationCodeService.getValidationInfo(announcement.delivery.id)
...
function getValidationCodeUnavailableReason(status: string): string {
  ...
  return reasons[status as keyof typeof reasons] || 'Statut de livraison incompatible'
}
