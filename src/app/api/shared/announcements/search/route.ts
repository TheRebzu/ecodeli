...
const searchSchema = z.object({
  ...
  status: z.enum(['ACTIVE', 'MATCHED', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED']).optional(),
  ...
  try {
    ...
    if (validatedParams.query) {
      filters.OR = [
        ...
        { pickupAddress: { contains: validatedParams.query, mode: 'insensitive' } },
        { deliveryAddress: { contains: validatedParams.query, mode: 'insensitive' } }
      ]
    }
    ...
    if (validatedParams.minPrice || validatedParams.maxPrice) {
      filters.price = {}
      if (validatedParams.minPrice) filters.price.gte = validatedParams.minPrice
      if (validatedParams.maxPrice) filters.price.lte = validatedParams.maxPrice
    }

    if (validatedParams.dateFrom || validatedParams.dateTo) {
      filters.pickupDate = {}
      if (validatedParams.dateFrom) filters.pickupDate.gte = new Date(validatedParams.dateFrom)
      if (validatedParams.dateTo) filters.pickupDate.lte = new Date(validatedParams.dateTo)
    }
