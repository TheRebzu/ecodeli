...
export async function POST(
  ...
  try {
    ...
    console.log(`[VALIDATION TENTATIVE] Livraison: ${deliveryId}, Code: ${validationCode}`)

    // Valider la livraison via le service critique
    const result = await deliveryValidationService.validateDeliveryWithCode(validatedData)
...
export async function GET(
  ...
  try {
    ...
    const isValid = await deliveryValidationService.isValidationCodeValid(deliveryId, code)
...
export async function DELETE(
  ...
  try {
    ...
    console.log(`[CODE INVALIDÉ] Livraison: ${deliveryId}, Admin: ${session.user.id}`)
