...
export async function POST(request: NextRequest) {
  try {
    ...
    const { title, message, target, data, sendPush } =
      broadcastSchema.parse(body);
    ...
    switch (target) {
      ...
      case "deliverers":
        result = await NotificationService.notifyAllDeliverers(
          title,
          message,
          data,
        );
      ...
      case "providers":
        result = await NotificationService.notifyAllProviders(
          title,
          message,
          data,
        );
