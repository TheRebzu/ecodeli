...
export async function GET(request: NextRequest) {
  try {
    console.log('📥 [API RECRUITMENT] Début GET avec cookies:', request.headers.get('cookie')?.substring(0, 50) + '...');
    
    const user = await getUserFromSession(request);
    if (!user) {
      console.error('❌ [API RECRUITMENT] Utilisateur non authentifié');
      return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    }

    console.log('✅ [API RECRUITMENT] Utilisateur authentifié:', { id: user.id, role: user.role });
    ...
    try {
      ...
      const optionalDocuments = ["OTHER", "CERTIFICATION"]; // Documents optionnels
      ...
      console.error("❌ [API RECRUITMENT] Erreur traitement GET:", error);
    ...
    console.error("❌ [API RECRUITMENT] Erreur critique GET:", error);
    return NextResponse.json(
...
export async function POST(request: NextRequest) {
  try {
    console.log('📥 [API RECRUITMENT] Début POST avec cookies:', request.headers.get('cookie')?.substring(0, 50) + '...');
    
    const user = await getUserFromSession(request);
    if (!user) {
      console.error('❌ [API RECRUITMENT] Utilisateur non authentifié (POST)');
      return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
    }

    console.log('✅ [API RECRUITMENT] Utilisateur authentifié (POST):', { id: user.id, role: user.role });
    ...
    try {
      ...
      console.error("❌ [API RECRUITMENT] Erreur traitement POST:", error);
      return NextResponse.json(
        { error: "Internal server error" },
        { status: 500 },
      );
    ...
    console.error("❌ [API RECRUITMENT] Erreur critique POST:", error);
    return NextResponse.json(
      { error: "Critical error" },
      { status: 500 },
    );
