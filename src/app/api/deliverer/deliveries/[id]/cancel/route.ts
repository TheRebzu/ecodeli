...
export async function POST(
  ...
  try {
    console.log(
      "🚚 [POST /api/deliverer/deliveries/[id]/cancel] Début de la requête",
    );
    ...
    if (!cancellableStatuses.includes(existingDelivery.status)) {
      console.log(
        "❌ Livraison ne peut pas être annulée, statut:",
    ...
    if (
      existingDelivery.payment &&
      existingDelivery.payment.status === "COMPLETED"
    ) {
