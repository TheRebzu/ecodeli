...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    console.log('🚚 [POST /api/deliverer/deliveries/[id]/generate-code] Début de la requête')
    ...
    if (existingDelivery.status !== 'in_transit') {
      console.log('❌ Livraison ne peut pas être validée, statut:', existingDelivery.status)
