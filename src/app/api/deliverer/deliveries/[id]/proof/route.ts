...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    console.log('📸 [POST /api/deliverer/deliveries/[id]/proof] Début de la requête')
    ...
    if (!['IN_PROGRESS', 'DELIVERED'].includes(existingDelivery.status)) {
      console.log('❌ Livraison ne peut pas recevoir de preuve, statut:', existingDelivery.status)
    ...
    const proofData = {
      ...
      gpsCoordinates: validatedData.gpsCoordinates ? JSON.stringify(validatedData.gpsCoordinates) : null,
    ...
    await db.notification.create({
      data: {
        ...
        message: 'Le livreur a uploadé une preuve de livraison. Vous pouvez maintenant valider la livraison.',
        data: JSON.stringify({
  ...
  try {
    ...
    return NextResponse.json({
      proof: delivery.ProofOfDelivery ? {
        ...
        gpsCoordinates: delivery.ProofOfDelivery.gpsCoordinates ? JSON.parse(delivery.ProofOfDelivery.gpsCoordinates) : null,
