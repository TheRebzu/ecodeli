...
const deliveriesFiltersSchema = z.object({
  ...
  sortBy: z
    .enum(["createdAt", "pickupDate", "deliveryDate"])
    .default("createdAt"),
...
export async function GET(request: NextRequest) {
  try {
    ...
    const deliveries = await db.delivery.findMany({
      ...
      orderBy:
        (await params).sortBy === "createdAt"
          ...
          : (await params).sortBy === "pickupDate"
            ? { pickupDate: (await params).sortOrder }
    ...
    console.log(
      `✅ Trouvé ${formattedDeliveries.length} livraisons sur ${total} total`,
    );
  ...
  try {
    ...
    const delivery = await db.delivery.create({
      data: {
        ...
        status: "ACCEPTED",
        price:
          validatedData.proposedPrice ||
          announcement.finalPrice ||
          announcement.basePrice,
        delivererFee:
          (validatedData.proposedPrice ||
            announcement.finalPrice ||
            announcement.basePrice) * 0.8, // 80% pour le livreur
        platformFee:
          (validatedData.proposedPrice ||
            announcement.finalPrice ||
            announcement.basePrice) * 0.15, // 15% pour la plateforme
        insuranceFee:
          (validatedData.proposedPrice ||
            announcement.finalPrice ||
            announcement.basePrice) * 0.05, // 5% pour l'assurance
        validationCode: validationCode,
        pickupDate: validatedData.estimatedPickupTime
          ? new Date(validatedData.estimatedPickupTime)
          : null,
        deliveryDate: validatedData.estimatedPickupTime
          ? new Date(
              new Date(validatedData.estimatedPickupTime).getTime() +
                2 * 60 * 60 * 1000,
            ) // +2h par défaut
          : null,
      },
