...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('[API] /api/deliverer/profile - User:', user ? `${user.email} (${user.id})` : 'Aucun utilisateur')
    if (!user) {
    ...
    console.log('[API] /api/deliverer/profile - delivererProfile:', delivererProfile)
    ...
    const pendingDocuments = delivererProfile.user.documents.filter(d => d.validationStatus === 'PENDING').length
    const approvedDocuments = delivererProfile.user.documents.filter(d => d.validationStatus === 'APPROVED').length
    ...
    const profile = {
      ...
      stats: {
        ...
        pendingDocuments,
        approvedDocuments,
        totalReviews: deliveries.reduce((sum, d) => sum + (d.announcement?.reviews?.length || 0), 0)
      },
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (updates.licensePlate) delivererUpdates.licensePlate = updates.licensePlate
