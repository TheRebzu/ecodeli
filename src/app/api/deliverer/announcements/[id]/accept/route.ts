...
export async function POST(
  ...
  try {
    ...
    console.log("🚚 Acceptation d'annonce:", {
      announcementId,
      delivererId,
      userId: user.id,
    });
    ...
    const result = await db.$transaction(async (tx) => {
      ...
      const delivery = await tx.delivery.create({
        data: {
          ...
          delivererFee:
            (announcement.finalPrice || announcement.basePrice) * 0.8,
          platformFee:
            (announcement.finalPrice || announcement.basePrice) * 0.15,
          insuranceFee:
            (announcement.finalPrice || announcement.basePrice) * 0.05,
        },
      });
