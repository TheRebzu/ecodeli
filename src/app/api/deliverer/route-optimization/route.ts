...
export async function GET(request: NextRequest) {
  try {
    ...
    const suggestions = await RouteOptimizationService.getOptimizationSuggestions(deliverer.id)
    ...
    const stats = {
      ...
      averageEfficiency: recentRoutes.length > 0 ? 
        recentRoutes.reduce((sum, route) => sum + (route.metadata?.efficiency || 0), 0) / recentRoutes.length : 0,
      totalDistance: recentRoutes.reduce((sum, route) => sum + route.totalDistance, 0),
      totalEarnings: recentRoutes.reduce((sum, route) => sum + route.estimatedEarnings, 0),
      co2Saved: recentRoutes.reduce((sum, route) => sum + (route.metadata?.co2Savings || 0), 0)
    }
