...
export async function GET(request: NextRequest) {
  try {
    ...
    const totalBoxes = locations.reduce(
      (total, location) => total + location.storageBoxes.length,
      0,
    );
    const availableBoxes = locations.reduce(
      (total, location) =>
        total + location.storageBoxes.filter((box) => box.isAvailable).length,
      0,
    );
    const occupiedBoxes = locations.reduce(
      (total, location) =>
        total + location.storageBoxes.filter((box) => !box.isAvailable).length,
      0,
    );
