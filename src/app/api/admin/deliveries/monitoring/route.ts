...
export async function GET(request: NextRequest) {
  try {
    ...
    const successRateValue = totalCompleted > 0 ? (totalDelivered / totalCompleted) * 100 : 0;
    ...
    const formattedActiveDeliveries = activeDeliveriesData.map(delivery => ({
      ...
      estimatedDeliveryTime: delivery.deliveryDate?.toISOString() || delivery.pickupDate?.toISOString() || new Date().toISOString(),
