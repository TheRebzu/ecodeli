...
async function calculateFinancialStats(dateRange: {
  startDate: Date;
  endDate: Date;
}) {
  ...
  const providerCommissions = new Map<
    string,
  ...
  completedInterventions.forEach((intervention) => {
    ...
    const commission = revenue * 0.1; // 10% de commission
  ...
  const revenueGrowth =
    lastMonthInterventions > 0
      ? ((currentMonthInterventions - lastMonthInterventions) /
          lastMonthInterventions) *
        100
      : 0;
  ...
  const userStats = {
    ...
    activeProviders: allUsers.filter((u) => u.provider && u.provider.isActive)
      .length,
