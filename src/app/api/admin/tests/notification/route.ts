...
import { TestsService, NotificationTestRequest } from '@/features/admin/services/tests.service'
import { z } from 'zod'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const result = await TestsService.sendTestNotification(validatedData as NotificationTestRequest)
...
export async function GET() {
  try {
    ...
    const notificationLogs = logs.filter(log => log.category === 'TEST_NOTIFICATION')
    ...
    console.error('Erreur lors de la récupération des logs notification:', error)
