...
export async function GET(request: NextRequest) {
  try {
    ...
    const [referralStats, referralRewards, referralActivities] =
      await Promise.all([
    ...
    const codeGenerations = referralActivities.filter(
      (a) => a.type === "CODE_GENERATED",
    ).length;
    const successfulReferrals = referralActivities.filter(
      (a) => a.type === "REFERRAL_REGISTERED",
    ).length;
    const conversionRate =
      codeGenerations > 0 ? (successfulReferrals / codeGenerations) * 100 : 0;
