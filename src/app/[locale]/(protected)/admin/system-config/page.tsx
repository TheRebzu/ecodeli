...
export default function AdminSystemConfigPage() {
  ...
  const testService = async (serviceName: string) => {
    try {
      const response = await fetch(`/api/admin/system-config/test/${serviceName}`, {
        method: 'POST'
      })
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <ServiceConfigCard
                  ...
                  onToggle={(enabled) => setConfig(prev => ({
                    ...prev,
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">{t('oneSignal.description')}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <Switch
                    ...
                    />
                    <Badge variant={config.oneSignal.status === 'connected' ? 'default' : 'secondary'}>
                      {config.oneSignal.status === 'connected' ? <CheckCircle className="h-3 w-3 mr-1" /> : <XCircle className="h-3 w-3 mr-1" />}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="onesignal-app-id">{t('oneSignal.appId')}</Label>
                    <Input
                      id="onesignal-app-id"
                      ...
                      onChange={(e) => setConfig(prev => ({
                        ...prev,
                        oneSignal: { ...prev.oneSignal, appId: e.target.value }
                      }))}
                      placeholder="Your OneSignal App ID"
                    />
                  </div>
                  <div>
                    <Label htmlFor="onesignal-api-key">{t('oneSignal.apiKey')}</Label>
                    <Input
                      ...
                      onChange={(e) => setConfig(prev => ({
                        ...prev,
                        oneSignal: { ...prev.oneSignal, apiKey: e.target.value }
                      }))}
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <Badge variant={config.stripe.status === 'connected' ? 'default' : 'secondary'}>
                      {config.stripe.status === 'connected' ? <CheckCircle className="h-3 w-3 mr-1" /> : <XCircle className="h-3 w-3 mr-1" />}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="stripe-publishable-key">{t('stripe.publishableKey')}</Label>
                    <Input
                  ...
                  <div>
                    <Label htmlFor="stripe-secret-key">{t('stripe.secretKey')}</Label>
                    <Input
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                <div>
                  <Label htmlFor="api-key-permissions">{t('apiKeys.permissions')}</Label>
                  <Textarea
