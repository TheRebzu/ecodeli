...
export default function BookingDetailPage() {
  ...
  const handleBookingAction = async (action: 'accept' | 'refuse') => {
    ...
    try {
      ...
      const response = await fetch(`/api/provider/bookings/${bookingId}/${endpoint}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({
          reason: action === 'refuse' ? 'Not available at the requested time' : undefined
        })
      })
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center gap-3">
              <Avatar>
                <AvatarFallback>
                  {booking.client.user.profile.firstName?.[0]}{booking.client.user.profile.lastName?.[0]}
                </AvatarFallback>
              </Avatar>
              <div>
                <h3 className="font-semibold">
                  {booking.client.user.profile.firstName} {booking.client.user.profile.lastName}
                </h3>
              </div>
            </div>
        ...
        <Card className="mt-6">
          ...
          <CardContent>
            ...
            <p className="text-sm text-muted-foreground mt-4">
              If you accept this booking, the client will be notified and prompted to complete payment. 
              Once payment is confirmed, the booking will be finalized.
            </p>
          </CardContent>
        </Card>
      )}
            ...
            <div className="text-center">
              {booking.status === 'CONFIRMED' && (
                <div className="text-green-600">
                  ...
                  <p>This booking has been accepted. The client will proceed with payment.</p>
                </div>
              )}
