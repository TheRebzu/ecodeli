...
export default function MerchantContractsAdvancedPage() {
  ...
  const fetchContractsData = async () => {
    try {
      ...
      const [contractsRes, templatesRes] = await Promise.all([
        ...
        fetch("/api/merchant/contracts-advanced/templates", {
          credentials: "include",
        }),
      ]);
  ...
  const handleStartNegotiation = async (contractId: string, data: any) => {
    try {
      const response = await fetch(
        `/api/merchant/contracts-advanced/${contractId}/negotiate`,
        {
          method: "POST",
      ...
      if (response.ok) {
        toast({
          ...
          description:
            "Votre demande de négociation a été envoyée à votre account manager",
        });
  ...
  const handleSignContract = async (contractId: string) => {
    try {
      setSigning(true);
      const response = await fetch(
        `/api/merchant/contracts-advanced/${contractId}/sign`,
        {
          method: "POST",
          credentials: "include",
        },
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Gérez vos contrats EcoDeli, négociez vos conditions et signez
            électroniquement
          </p>
        </div>
        ...
        <Card>
          <CardContent className="p-4 text-center">
            <div className="text-2xl font-bold text-purple-600">
              {contracts.find((c) => c.status === "active")?.commissionRate ||
                0}
              %
            </div>
          ...
          <div className="space-y-3">
            <div className="flex justify-between">
              ...
              <span className="font-semibold text-lg">
                {activeContract.commissionRate}%
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm">Délai de paiement</span>
              <span className="font-medium">
                {activeContract.paymentTerms} jours
              </span>
            </div>
            {activeContract.minimumVolume && (
              <div className="flex justify-between">
                ...
                <span className="font-medium">
                  {activeContract.minimumVolume.toLocaleString()}€
                </span>
              </div>
            )}
          </div>
                ...
                <div className="space-y-2">
                  {activeContract.bonusThresholds.map((threshold, index) => (
                    <div key={index} className="flex justify-between text-sm">
                      <span>
                        +{threshold.bonusRate}% si{" "}
                        {threshold.volume.toLocaleString()}€
                      </span>
...
function NegotiationsSection({ contracts }: { contracts: Contract[] }) {
  const negotiationContracts = contracts.filter(
    (c) => c.status === "under_negotiation" || c.negotiations.length > 0,
  );
                  ...
                  {negotiation.response && (
                    <div className="bg-muted p-3 rounded text-sm">
                      ...
                      {negotiation.counterOffer && (
                        <div className="mt-2">
                          <span className="font-medium">
                            Contre-proposition: {negotiation.counterOffer}%
                          </span>
                        </div>
                      )}
                    </div>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {templates.map((template) => (
          <Card
            key={template.id}
            className={`relative ${template.recommended ? "ring-2 ring-primary" : ""}`}
          >
              ...
              <input
                ...
                onChange={(e) => {
                  if (e.target.checked) {
                    ...
                    setFormData((prev) => ({
                      ...prev,
                      requestedChanges: prev.requestedChanges.filter(
                        (c) => c !== option,
                      ),
                    }));
                  }
