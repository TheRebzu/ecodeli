...
export default function BulkImportPage() {
  ...
  const analyzeFile = async (file: File) => {
    try {
      ...
      const response = await fetch(
        "/api/merchant/announcements/bulk-import/analyze",
        {
          method: "POST",
          body: formData,
        },
  ...
  const validateMappings = (): boolean => {
    ...
    const mappedFields = Object.values(preview.mappings).filter(
      (field) => field !== "unmapped",
    );
  ...
  const startImport = async () => {
    ...
    try {
      ...
      const validMappings = Object.fromEntries(
        Object.entries(preview.mappings).filter(
          ([_, field]) => field !== "unmapped",
        ),
      );
      ...
      const response = await fetch(
        "/api/merchant/announcements/bulk-import/process",
        {
          method: "POST",
          body: formData,
        },
  ...
  const startProgressTracking = (importId: string) => {
    const interval = setInterval(async () => {
      try {
        const response = await fetch(
          `/api/merchant/announcements/bulk-import/${importId}/status`,
        );
        ...
        if (status.status === "COMPLETED" || status.status === "FAILED") {
          ...
          if (status.status === "COMPLETED") {
            alert(
              `Import terminé avec succès! ${status.successfulRows} annonces créées.`,
            );
          } else {
            alert(
              "Import échoué. Consultez l'historique pour plus de détails.",
            );
          }
        }
      } catch (error) {
  ...
  const loadImportHistory = async () => {
    try {
      const response = await fetch(
        "/api/merchant/announcements/bulk-import/history",
      );
      if (response.ok) {
                  ...
                  <div className="flex gap-4 pt-4">
                    ...
                    {!validateMappings() && (
                      <Alert className="flex-1">
                        <AlertCircleIcon className="h-4 w-4" />
                        <AlertDescription>
                          Les champs obligatoires (titre, description, type,
                          prix) doivent être mappés.
                        </AlertDescription>
                      </Alert>
                    )}
                  </div>
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                            ...
                            <div>
                              ...
                              <div className="font-medium">
                                {Math.round(
                                  (item.processedRows / item.totalRows) * 100,
                                )}
                                %
                              </div>
                            </div>
                            ...
                            <details className="mt-2">
                              ...
                              <div className="mt-2 space-y-1 text-sm">
                                ...
                                {item.errors.length > 5 && (
                                  <div className="text-muted-foreground">
                                    ... et {item.errors.length - 5} autres
                                    erreurs
                                  </div>
                                )}
                              </div>
