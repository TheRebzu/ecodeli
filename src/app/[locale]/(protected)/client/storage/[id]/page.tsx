...
import { PackageCheck, Clock, Calendar, MapPin, Building2, ChevronLeft } from 'lucide-react';
import {
  Card,
  CardContent,
  CardDescription,
...
export default async function ReservationDetailPage({
  ...
  const t = await getTranslations({ locale: params.locale, namespace: 'storage' });
  ...
  return (
    <Container>
      <div className="mb-6">
        <Button variant="ghost" className="flex items-center -ml-2 mb-4" asChild>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="flex flex-col">
                  <span className="text-sm text-muted-foreground">{t('detailPage.startDate')}</span>
                ...
                <div className="flex flex-col">
                  <span className="text-sm text-muted-foreground">{t('detailPage.endDate')}</span>
                  <div className="flex items-center gap-2 mt-1">
                    <Calendar className="h-4 w-4" />
              ...
              <div className="space-y-3">
                <span className="font-medium">{t('detailPage.paymentInfo')}</span>
              ...
              {isActive(reservation.status) && (
                <div className="flex justify-end w-full space-x-3">
                  <Button variant="outline" asChild>
                    <Link href={`/client/storage/${params.id}/extend`}>
                      {t('detailPage.extendReservation')}
                    </Link>
                  </Button>
                  <Button asChild>
                    <Link href={`/client/storage/${params.id}/access`}>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-3">
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <span className="font-medium">{reservation.box.warehouse.name}</span>
                    <p className="text-sm text-muted-foreground">{warehouseAddress}</p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <span className="font-medium">{t('detailPage.openingHours')}</span>
            ...
            <CardFooter>
              <Button
                ...
                className="w-full"
                onClick={() =>
                  window.open(`https://maps.google.com/?q=${warehouseAddress}`, '_blank')
                }
              >
