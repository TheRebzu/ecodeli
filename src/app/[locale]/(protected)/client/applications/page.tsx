...
export default function ClientApplicationsPage() {
  ...
  const handleRespond = async () => {
    ...
    try {
      setResponding(true)
      const response = await fetch(`/api/client/applications/${selectedApplication.id}/respond`, {
        method: 'POST',
        headers: {
  ...
  const handlePayService = async (application: Application) => {
    try {
      setPaying(true)
      const response = await fetch(`/api/client/applications/${application.id}/pay`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (data.stripe && data.stripe.paymentIntentId) {
        ...
        const checkoutResponse = await fetch('/api/client/applications/create-checkout-session', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
      ...
      toast.success('Paiement effectué avec succès ! Le prestataire a été notifié.')
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'ACCEPTED':
        return <Badge variant="default" className="bg-green-500"><CheckCircle className="w-3 h-3 mr-1" /> Acceptée</Badge>
      case 'REJECTED':
    ...
    <div className="container mx-auto py-8">
        ...
        <Card>
          <CardContent className="py-12">
            <div className="text-center">
              ...
              <p className="text-muted-foreground">
                Vous n'avez pas encore reçu de candidatures pour vos demandes de services.
              </p>
            </div>
          </CardContent>
        </Card>
            ...
            <Card key={application.id} className="overflow-hidden">
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-center space-x-4">
                    <Avatar className="w-12 h-12">
                      <AvatarImage src={application.provider.user.profile.avatar} />
                      <AvatarFallback>
                        {application.provider.user.profile.firstName?.[0]}{application.provider.user.profile.lastName?.[0]}
                      </AvatarFallback>
                    </Avatar>
                    <div>
                      <div className="flex items-center space-x-2">
                        ...
                        {application.provider.averageRating > 0 && (
                          <div className="flex items-center space-x-1">
                            ...
                            <span className="text-sm">{application.provider.averageRating.toFixed(1)}</span>
                          </div>
                        )}
                      </div>
                      <div className="flex items-center space-x-4 text-sm text-muted-foreground">
                        ...
                        <span>{application.provider.totalBookings} interventions</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center space-x-2">
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">{application.serviceRequest.title}</p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div>
                    <p className="text-sm font-medium text-muted-foreground">Prix proposé</p>
                    <p className="text-lg font-semibold">{formatPrice(application.proposedPrice)}</p>
                  </div>
                  <div>
                    <p className="text-sm font-medium text-muted-foreground">Durée estimée</p>
                    <p className="text-lg font-semibold">{application.estimatedDuration} min</p>
                  </div>
                  <div>
                ...
                {application.message && (
                  <>
                    <Separator />
                    <div>
                      <p className="text-sm font-medium text-muted-foreground mb-2">Message du prestataire</p>
      ...
      <Dialog open={responseDialog} onOpenChange={setResponseDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {responseAction === 'ACCEPT' ? 'Accepter la candidature' : 'Refuser la candidature'}
            </DialogTitle>
            <DialogDescription>
              {responseAction === 'ACCEPT' 
          ...
          <DialogFooter>
            ...
            >
              {responding ? 'Traitement...' : (responseAction === 'ACCEPT' ? 'Accepter' : 'Refuser')}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
