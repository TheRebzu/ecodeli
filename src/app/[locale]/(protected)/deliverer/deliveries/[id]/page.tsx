    ...
    const fetchDeliveryDetails = async () => {
      try {
        ...
        if (response.ok) {
          ...
          setError(
            errorData.error || "Erreur lors du chargement de la livraison",
          );
        }
      } catch (err) {
    ...
    try {
      ...
      if (response.ok) {
        setValidationResult({
          success: true,
          message:
            "Livraison validée avec succès ! Le paiement a été débloqué.",
        });
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="max-w-4xl mx-auto">
          <Alert variant="destructive">
            <AlertCircle className="h-4 w-4" />
            <AlertDescription>
              Erreur: Les détails de l'annonce ne sont pas disponibles pour
              cette livraison.
            </AlertDescription>
          </Alert>
          <Button asChild className="mt-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            ...
            <div className="text-sm text-muted-foreground">
              Votre part:{" "}
              {delivery.delivererFee
                ? delivery.delivererFee.toFixed(2)
                : "0.00"}{" "}
              €
            </div>
