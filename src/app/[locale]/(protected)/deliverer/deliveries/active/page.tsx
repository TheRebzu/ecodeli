...
export default function ActiveDeliveriesPage() {
  ...
  const updateDeliveryStatus = async (deliveryId: string, status: DeliveryStatus) => {
    try {
      ...
      setTimeout(() => {
        setActiveDeliveries(prevDeliveries => prevDeliveries.map(delivery => 
  ...
  const filteredDeliveries = activeDeliveries.filter(delivery => {
    ...
    if (activeTab === 'pickup' && ['ASSIGNED', 'EN_ROUTE_TO_PICKUP'].includes(delivery.status)) return true;
    if (activeTab === 'inProgress' && ['PICKED_UP', 'IN_TRANSIT'].includes(delivery.status)) return true;
    if (activeTab === 'delivery' && ['AT_DROPOFF', 'DELIVERED'].includes(delivery.status)) return true;
    return false;
  });
  ...
  return (
    <div className="container py-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">{t('activeDeliveries')}</h1>
          <p className="text-muted-foreground mt-1">{t('activeDeliveriesDescription')}</p>
        </div>
        
        <Button variant="outline" onClick={fetchActiveDeliveries} disabled={isLoading}>
          ...
          {deliveryToUpdate && (
            <div className="space-y-4 py-4">
              ...
              <Alert>
                ...
                <AlertDescription>{t('locationSharingDescription')}</AlertDescription>
              </Alert>
            </div>
          )}
          
          <DialogFooter className="sm:justify-end">
            <Button variant="outline" onClick={() => setDeliveryToUpdate(null)} disabled={isUpdating}>
              {t('cancel')}
            </Button>
            <Button 
              onClick={() => {
                if (deliveryToUpdate) {
                  const delivery = activeDeliveries.find(d => d.id === deliveryToUpdate);
                  if (delivery) {
                    updateDeliveryStatus(deliveryToUpdate, getNextStatus(delivery.status));
                  }
                }
              }}
              disabled={isUpdating}
    ...
    if (filteredDeliveries.length === 0) {
      return (
        <div className="text-center py-12 bg-muted/50 rounded-lg">
          ...
          <p className="text-muted-foreground mt-2">{t('noDeliveriesMessage')}</p>
        ...
        {filteredDeliveries.map((delivery) => (
          <Card key={delivery.id} className="mb-6">
                ...
                <div className="space-y-4">
                  <div className="flex items-start">
                    ...
                    <div>
                      ...
                      {delivery.clientPhone && <p className="text-sm text-muted-foreground">{delivery.clientPhone}</p>}
                    </div>
                  </div>
                  ...
                  <div className="flex items-start">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">{formatDate(delivery.pickupDate)} {formatTime(delivery.pickupDate)}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">{formatDate(delivery.deliveryDate)} {formatTime(delivery.deliveryDate)}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  {delivery.requiresSignature && (
                    <div className="flex items-start">
                      ...
                      <div>
                        <p className="text-sm font-medium">{t('requiresSignature')}</p>
                        <p className="text-sm text-muted-foreground">{t('requiresSignatureDescription')}</p>
                      </div>
                    </div>
                  )}
                </div>
            ...
            <CardFooter className="flex flex-col-reverse sm:flex-row gap-3 justify-between pt-4">
              ...
              <div className="flex gap-3 w-full sm:w-auto">
                <Button variant="default" className="flex-1 sm:flex-none" onClick={() => setDeliveryToUpdate(delivery.id)}>
                ...
                {['EN_ROUTE_TO_PICKUP', 'PICKED_UP', 'EN_ROUTE_TO_DROPOFF'].includes(delivery.status) && (
                ...
                {['AT_DROPOFF', 'DELIVERED'].includes(delivery.status) && delivery.requiresSignature && (
                  <Button variant="secondary" className="flex-1 sm:flex-none" asChild>
                    <Link href={`/deliverer/deliveries/${delivery.id}/confirm`}>
