...
export default function ActiveDeliveriesPage() {
      ...
      setTimeout(() => {
        setActiveDeliveries(prevDeliveries =>
          prevDeliveries.map(delivery =>
            delivery.id === deliveryId ? { ...delivery, status, updatedAt: new Date() } : delivery
          )
        );
  ...
  const filteredDeliveries = activeDeliveries.filter(delivery => {
    ...
    if (activeTab === 'pickup' && ['ASSIGNED', 'EN_ROUTE_TO_PICKUP'].includes(delivery.status))
      return true;
    if (activeTab === 'inProgress' && ['PICKED_UP', 'IN_TRANSIT'].includes(delivery.status))
      return true;
    if (activeTab === 'delivery' && ['AT_DROPOFF', 'DELIVERED'].includes(delivery.status))
      return true;
    return false;
  });
              ...
              <div className="space-y-2">
                ...
                <Badge
                  variant={getStatusBadgeVariant(
                    activeDeliveries.find(d => d.id === deliveryToUpdate)?.status || 'PENDING'
                  )}
                >
                  {getStatusLabel(
                    activeDeliveries.find(d => d.id === deliveryToUpdate)?.status || 'PENDING'
            ...
            <CardHeader className="pb-2">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <CardDescription>
                    {formatDistance(delivery.distance)} km • {formatDate(delivery.deliveryDate)}
                  </CardDescription>
                </div>
                  ...
                  <div className="flex items-start">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">
                        {formatDate(delivery.pickupDate)} {formatTime(delivery.pickupDate)}
                      </p>
                    </div>
                  </div>
