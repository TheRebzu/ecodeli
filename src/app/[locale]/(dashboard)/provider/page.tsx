...
import { Briefcase, Calendar, Star, AlertTriangle, FileText, DollarSign } from "lucide-react";
...
export default function ProviderDashboardPage() {
  ...
  const { data, isLoading, error } = api.dashboard.getProviderDashboard.useQuery();
    ...
    <DashboardLayout sidebar={<ProviderSidebar />}>
        ...
        <DashboardCard title={t("services.title")}>
          {isLoading ? (
            ...
            <div className="text-center py-8">
              ...
              <h3 className="mt-2 text-lg font-medium">{t("services.empty.title")}</h3>
              <p className="text-muted-foreground">{t("services.empty.description")}</p>
              <Button className="mt-4" asChild>
                <a href="/provider/services/new">{t("services.empty.action")}</a>
              </Button>
            </div>
          )}
        </DashboardCard>
        ...
        <DashboardCard title={t("currentBookings.title")}>
          {isLoading ? (
            ...
            <div className="text-center py-8">
              ...
              <h3 className="mt-2 text-lg font-medium">{t("currentBookings.empty.title")}</h3>
              <p className="text-muted-foreground">{t("currentBookings.empty.description")}</p>
            </div>
          )}
        </DashboardCard>
      </DashboardSection>
      ...
      <Tabs defaultValue="history" className="mb-8">
        ...
        <TabsContent value="history">
          <DashboardCard title={t("bookingHistory.title")} description={t("bookingHistory.description")}>
            {isLoading ? (
              <div className="space-y-2">
                <Skeleton className="h-12" />
                <Skeleton className="h-12" />
              ...
              <div className="text-center py-8">
                <Calendar className="mx-auto h-12 w-12 text-muted-foreground opacity-50" />
                <h3 className="mt-2 text-lg font-medium">{t("bookingHistory.empty.title")}</h3>
                <p className="text-muted-foreground">{t("bookingHistory.empty.description")}</p>
              </div>
            )}
          </DashboardCard>
        </TabsContent>
