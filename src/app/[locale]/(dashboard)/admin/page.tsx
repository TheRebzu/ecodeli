...
import { Users, ShoppingBag, Store, Briefcase, FileText, DollarSign, Bell } from "lucide-react";
    ...
    <DashboardLayout sidebar={<AdminSidebar />}>
        ...
        <DashboardCard title={t("pendingDocuments.title")}>
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-6">
              ...
              <h3 className="mt-2 text-xl font-medium">{data.pendingDocuments}</h3>
              <p className="text-muted-foreground">{t("pendingDocuments.waiting")}</p>
              <Button className="mt-4" asChild>
                <a href="/admin/documents/verification">{t("pendingDocuments.action")}</a>
              </Button>
            </div>
          ) : (
            <div className="text-center py-8">
              <FileText className="mx-auto h-12 w-12 text-muted-foreground opacity-50" />
              <h3 className="mt-2 text-lg font-medium">{t("pendingDocuments.empty.title")}</h3>
              <p className="text-muted-foreground">{t("pendingDocuments.empty.description")}</p>
            </div>
          )}
        </DashboardCard>
      </DashboardSection>
      ...
      <Tabs defaultValue="users" className="mb-8">
        ...
        <TabsContent value="users">
          <DashboardCard title={t("recentUsers.title")} description={t("recentUsers.description")}>
            {isLoading ? (
              <div className="space-y-2">
                <Skeleton className="h-12" />
                <Skeleton className="h-12" />
              ...
              <div className="text-center py-8">
                ...
                <h3 className="mt-2 text-lg font-medium">{t("recentUsers.empty.title")}</h3>
                <p className="text-muted-foreground">{t("recentUsers.empty.description")}</p>
              </div>
            )}
          </DashboardCard>
        </TabsContent>
        <TabsContent value="orders">
          <DashboardCard title={t("recentOrders.title")} description={t("recentOrders.description")}>
            {isLoading ? (
              <div className="space-y-2">
                <Skeleton className="h-12" />
                <Skeleton className="h-12" />
