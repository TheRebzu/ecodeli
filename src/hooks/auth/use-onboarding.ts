...
export function useOnboardingStatus() {
  const { data, isLoading, error, refetch } =
    api.userPreferences.getOnboardingStatus.useQuery(undefined, {
...
export function useOnboardingNavigation() {
  ...
  const setStepsConfiguration = useCallback(
    (currentStepIndex: number, totalStepsCount: number) => {
...
export function useMission1Onboarding() {
  ...
  const isMission1Required =
    !status?.hasCompletedOnboarding && !status?.tutorialSkipped;
