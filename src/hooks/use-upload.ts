...
export function useUpload(options: UseUploadOptions) {
  ...
  const validateFile = useCallback(
    (file: File): string | null => {
      ...
      if (file.size > maxSizeBytes) {
        return (
          t('fileSizeError') ||
          `La taille du fichier dépasse la limite autorisée (${options.maxFileSize || 5}MB max)`
        );
      }
  ...
  const uploadMultipleFiles = useCallback(
    async (files: File[], announcementId?: string) => {
      if (files.length > (options.maxFiles || 5)) {
        const error =
          t('maxPhotosError', { count: options.maxFiles || 5 }) ||
  ...
  return {
    ...
    isUploading:
      ...
      uploadAnnouncementPhotosMutation.isPending,
