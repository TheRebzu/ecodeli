...
export function useContractPerformance(
  ...
  const [filters, setFilters] = useState<PerformanceFilters>({ period: "MONTHLY",
  ...
  const {
    ...
    refetch: refetchPerformance} = api.admin.contracts.getPerformance.useQuery(filters);

  const { data: performanceStats, isLoading: isLoadingStats } =
    api.admin.contracts.getPerformanceStats.useQuery({ period: filters.period,
      year: filters.year,
      month: filters.month,
      quarter: filters.quarter });

  const { data: performanceTrends, isLoading: isLoadingTrends } =
    api.admin.contracts.getPerformanceTrends.useQuery({ contractId: filters.contractId,
      period: filters.period,
      year: filters.year });

  const { data: topPerformers, isLoading: isLoadingTopPerformers } =
    api.admin.contracts.getTopPerformers.useQuery({ period: filters.period,
      year: filters.year,
      month: filters.month,
      quarter: filters.quarter,
      limit: 10 });
  ...
  // Mutations
  const updatePerformanceMutation =
    api.admin.contracts.updatePerformance.useMutation({ onSuccess: () => {
        toast({
          title: "Performance mise à jour",
          description:
            "Les données de performance ont été mises à jour avec succès." });
        refetchPerformance();
      },
      onError: (error) => {
        toast({
  ...
  const generatePerformanceReportMutation =
    api.admin.contracts.generatePerformanceReport.useMutation({ onSuccess: (data) => {
        toast({
  ...
  const recalculatePerformanceMutation =
    api.admin.contracts.recalculatePerformance.useMutation({ onSuccess: () => {
        toast({
  ...
  const updatePerformance = async (contractId: string, data: any) => {
    await updatePerformanceMutation.mutateAsync({ contractId,
      ...data });
  };
  ...
  const recalculatePerformance = async (contractId?: string) => {
    await recalculatePerformanceMutation.mutateAsync({ contractId,
      period: filters.period,
      year: filters.year,
      month: filters.month,
      quarter: filters.quarter });
  ...
  const calculateAverageMetrics = () => {
    ...
    const totals = performance.reduce(
      (acc, item) => ({ revenue: acc.revenue + (item.totalRevenue || 0),
