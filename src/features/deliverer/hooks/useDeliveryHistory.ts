...
export function useDeliveryHistory(params: UseDeliveryHistoryParams = {}): UseDeliveryHistoryReturn {
  const {
    initialPage = 1,
    limit = 10,
  ...
  const fetchDeliveries = useCallback(async (page?: number, status?: string) => {
    try {
      setLoading(true)
      setError(null)
      ...
      const response = await fetch(`/api/deliverer/deliveries/history?${params}`)
