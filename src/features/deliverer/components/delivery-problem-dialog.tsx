...
export default function DeliveryProblemDialog({
  ...
  const handleSubmit = async () => {
    if (!selectedProblem || !problemDescription.trim()) {
      toast.error(
        "Veuillez sélectionner un type de problème et donner une description",
      );
      return;
    }

    const isCancel = PROBLEM_TYPES.find(
    ...
    try {
      ...
      if (response.ok) {
        toast.success(
          isCancel
            ? "Livraison annulée"
            : "Problème signalé, notre équipe va vous contacter",
        );
        setSelectedProblem("");
  ...
  const selectedProblemData = PROBLEM_TYPES.find(
    (p) => p.id === selectedProblem,
  );
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Décrivez le problème rencontré avec cette livraison. Notre équipe
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        >
                          <Icon
                            className={`w-4 h-4 ${problem.isCancel ? "text-red-500" : "text-gray-500"}`}
                          />
                          <span className="font-medium">{problem.title}</span>
                        </Label>
                        <p
                          className={`text-sm mt-1 ${problem.isCancel ? "text-red-600" : "text-gray-600"}`}
                        >
                          {problem.description}
                        </p>
                      </div>
          ...
          {selectedProblemData?.isCancel && (
            <div className="bg-red-50 p-3 rounded-lg border border-red-200">
              ...
              <p className="text-sm text-red-700">
                L'annulation d'une livraison peut impacter votre note.
                Assurez-vous d'avoir une raison valable.
              </p>
            </div>
          )}
        </div>
