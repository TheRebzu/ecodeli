...
interface RecruitmentDocument {
  id: string;
  type: 'IDENTITY_CARD' | 'DRIVING_LICENSE' | 'VEHICLE_REGISTRATION' | 'INSURANCE' | 'CRIMINAL_RECORD' | 'BANK_RIB';
  name: string;
  fileName: string;
...
export default function DelivererRecruitmentSystem({ userId }: DelivererRecruitmentSystemProps) {
  ...
  const fetchApplication = async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/deliverer/recruitment?userId=${userId}`);
      if (response.ok) {
  ...
  const saveApplication = async (submit = false) => {
    try {
      ...
      if (response.ok) {
        toast({
          title: submit ? t("success.application_submitted") : t("success.application_saved"),
          description: submit ? t("success.application_submitted_desc") : t("success.application_saved_desc")
        });
        fetchApplication();
      } else {
  ...
  const downloadDocument = async (documentId: string, fileName: string) => {
    try {
      const response = await fetch(`/api/deliverer/recruitment/documents/${documentId}/download`);
      if (response.ok) {
  ...
  const canSubmit = () => {
    return formData.firstName && formData.lastName && formData.email && 
           formData.phone && formData.vehicleType && 
    ...
    <div className="space-y-6">
      ...
      {application && (
        <Card className={`border-2 ${
          ...
          application.status === 'UNDER_REVIEW' ? 'border-blue-200 bg-blue-50' :
          ...
          <CardContent>
            <div className="space-y-4">
              <div>
                ...
                <Progress value={application.validationProgress} className="h-2" />
              </div>
              ...
              {application.status === 'APPROVED' && (
                <div className="p-4 bg-green-100 border border-green-200 rounded-lg">
                  <div className="flex items-center space-x-2 text-green-800">
                    ...
                    <div>
                      <p className="font-medium">{t("status.congratulations")}</p>
              ...
              {application.status === 'REJECTED' && application.rejectionReason && (
                <div className="p-4 bg-red-100 border border-red-200 rounded-lg">
                  <div className="text-red-800">
                    <p className="font-medium">{t("status.rejection_reason")}:</p>
      ...
      <Tabs defaultValue="personal" className="space-y-4">
        <TabsList className="grid w-full grid-cols-3">
          ...
          <TabsTrigger value="professional">{t("tabs.professional_info")}</TabsTrigger>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <Label htmlFor="nationality">{t("personal.nationality")}</Label>
                  <Input
                    id="nationality"
              ...
              {application?.status !== 'APPROVED' && (
                <div className="flex gap-2">
                  <Button onClick={() => saveApplication(false)} variant="outline">
        ...
        <TabsContent value="professional">
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <Label htmlFor="vehicleType">{t("professional.vehicle_type")}</Label>
                  <Select 
                  ...
                  >
                    <SelectTrigger>
                      <SelectValue placeholder={t("professional.select_vehicle")} />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div>
                  <Label htmlFor="vehicleModel">{t("professional.vehicle_model")}</Label>
                  <Input
                    id="vehicleModel"
                ...
                <div>
                  <Label htmlFor="licenseNumber">{t("professional.license_number")}</Label>
                  <Input
                    id="licenseNumber"
                ...
                <div>
                  <Label htmlFor="experience">{t("professional.experience")}</Label>
                  <Input
                    id="experience"
                    type="number"
                    min="0"
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-500">
                              {t("documents.uploaded_on")}: {new Date(document.uploadedAt).toLocaleDateString()}
                            </p>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          <Badge className={getStatusColor(document.status)}>
                            ...
                            {t(`documents.statuses.${document.status.toLowerCase()}`)}
                          </Badge>
                          <Button
                            variant="outline"
                            size="sm"
                      ...
                      {document.status === 'REJECTED' && document.rejectionReason && (
                        <div className="mt-3 p-3 bg-red-50 border border-red-200 rounded">
                          <p className="text-sm text-red-800">
                            <strong>{t("documents.rejection_reason")}:</strong> {document.rejectionReason}
                          </p>
                        </div>
                      )}
                    </div>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="text-center">
              <h3 className="text-lg font-semibold mb-2">{t("submit.ready_title")}</h3>
              <p className="text-gray-600 mb-4">{t("submit.ready_description")}</p>
              <Button 
              ...
              {!canSubmit() && (
                <p className="text-sm text-red-600 mt-2">{t("submit.requirements_not_met")}</p>
              )}
            </div>
          </CardContent>
        </Card>
      ...
      <Dialog open={showDocumentDialog} onOpenChange={setShowDocumentDialog}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <div>
              <Label htmlFor="documentType">{t("upload_dialog.document_type")}</Label>
              <Select value={selectedDocumentType} onValueChange={setSelectedDocumentType}>
                <SelectTrigger>
                  <SelectValue placeholder={t("upload_dialog.select_type")} />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="IDENTITY_CARD">{getDocumentTypeName("IDENTITY_CARD")}</SelectItem>
                  <SelectItem value="DRIVING_LICENSE">{getDocumentTypeName("DRIVING_LICENSE")}</SelectItem>
                  <SelectItem value="VEHICLE_REGISTRATION">{getDocumentTypeName("VEHICLE_REGISTRATION")}</SelectItem>
                  <SelectItem value="INSURANCE">{getDocumentTypeName("INSURANCE")}</SelectItem>
                  <SelectItem value="CRIMINAL_RECORD">{getDocumentTypeName("CRIMINAL_RECORD")}</SelectItem>
                  <SelectItem value="BANK_RIB">{getDocumentTypeName("BANK_RIB")}</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <div>
