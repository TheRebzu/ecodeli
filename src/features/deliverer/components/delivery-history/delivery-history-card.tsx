...
import { CalendarIcon, PackageIcon, TruckIcon, MapPinIcon, EuroIcon, ExternalLinkIcon } from 'lucide-react'
    ...
    <Card className="hover:shadow-md transition-shadow">
      <CardHeader className="pb-3">
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <CardTitle className="text-lg line-clamp-1">{delivery.announcement.title}</CardTitle>
          ...
          <div className="flex items-center gap-2 ml-4">
            ...
            <Badge variant="outline">
              {typeLabels[delivery.announcement.type as keyof typeof typeLabels] || delivery.announcement.type}
            </Badge>
          </div>
        </div>
      </CardHeader>
      ...
      <CardContent className="pt-0">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="space-y-3">
            <div>
              <h4 className="font-medium text-sm text-gray-700 mb-1">Itinéraire</h4>
              <div className="space-y-2 text-sm">
                <div className="flex items-start gap-2">
                  <MapPinIcon className="h-4 w-4 text-green-500 mt-0.5 flex-shrink-0" />
                  <span className="line-clamp-2">{delivery.announcement.pickupAddress}</span>
                </div>
                <div className="flex items-start gap-2">
                  <MapPinIcon className="h-4 w-4 text-red-500 mt-0.5 flex-shrink-0" />
                  <span className="line-clamp-2">{delivery.announcement.deliveryAddress}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3">
            <div>
              <h4 className="font-medium text-sm text-gray-700 mb-1">Rémunération</h4>
            ...
            {delivery.actualDeliveryDate && (
              <div>
                <h4 className="font-medium text-sm text-gray-700 mb-1">Livré le</h4>
                <p className="text-sm">{formatDate(delivery.actualDeliveryDate)}</p>
              </div>
            )}

            {delivery.pickupDate && !delivery.actualDeliveryDate && (
              <div>
                <h4 className="font-medium text-sm text-gray-700 mb-1">Récupéré le</h4>
            ...
            {delivery.payment && (
              <div>
                <h4 className="font-medium text-sm text-gray-700 mb-1">Paiement</h4>
                <Badge 
                  variant={delivery.payment.status === 'COMPLETED' ? 'default' : 'secondary'}
                  className="text-xs"
                >
                  {delivery.payment.status === 'COMPLETED' ? 'Payé' : 'En attente'}
                </Badge>
        ...
        <div className="flex items-center justify-between mt-4 pt-4 border-t">
          <div className="text-sm text-gray-500">
            {delivery.announcement.description && (
              <p className="line-clamp-2">{delivery.announcement.description}</p>
            )}
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {delivery.validationCode && delivery.status === 'IN_TRANSIT' && (
              <div className="text-sm">
                ...
                <span className="font-mono font-bold">{delivery.validationCode}</span>
              </div>
            )}
          </div>
        </div>
