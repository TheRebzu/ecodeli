...
import { Loader2, MapPin, Clock, Package, Euro, AlertCircle, CheckCircle, Star, Search, Filter, Target, Truck, Calendar, User } from 'lucide-react'
...
export function DelivererOpportunitiesPage() {
  ...
  useEffect(() => {
    loadOpportunities()
  }, [maxDistance, selectedType, minPrice, maxPrice, urgentOnly, sortBy, sortOrder])
    ...
    try {
      ...
      if (result.success) {
        toast({
          ...
          description: result.message || 'Vous avez accepté cette livraison avec succès.',
        })
        ...
        toast({
          title: '❌ Erreur',
          description: result.error || 'Impossible d\'accepter cette opportunité',
          variant: 'destructive'
        })
      }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-green-50 to-blue-50 rounded-lg p-6 border">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-2">🚩 Opportunités de Livraison</h1>
            <p className="text-gray-600">
              Découvrez les annonces de livraison disponibles dans votre zone
            </p>
          </div>
          <div className="text-right">
            <div className="text-2xl font-bold text-green-600">{data?.stats.totalOpportunities || 0}</div>
            <div className="text-sm text-gray-600">opportunités disponibles</div>
          </div>
        </div>
        ...
        {data?.stats && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-white rounded-lg p-3 text-center">
              <div className="text-lg font-bold text-red-600">{data.stats.urgentCount}</div>
              <div className="text-xs text-gray-600">Urgentes</div>
            </div>
            <div className="bg-white rounded-lg p-3 text-center">
              <div className="text-lg font-bold text-green-600">{formatPrice(data.stats.averagePrice)}</div>
              <div className="text-xs text-gray-600">Prix moyen</div>
            </div>
            <div className="bg-white rounded-lg p-3 text-center">
              <div className="text-lg font-bold text-blue-600">{data.stats.averageDistance.toFixed(1)}km</div>
              <div className="text-xs text-gray-600">Distance moy.</div>
            </div>
            <div className="bg-white rounded-lg p-3 text-center">
              <div className="text-lg font-bold text-purple-600">{data.delivererInfo.searchRadius}km</div>
        ...
        {filteredOpportunities.length === 0 ? (
          ...
          <div className="grid gap-4">
            {filteredOpportunities.map((opportunity) => (
              <Card key={opportunity.id} className="hover:shadow-lg transition-all duration-200 border-l-4 border-l-green-500">
                <CardHeader>
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        {opportunity.isUrgent && (
                          <Badge className={getUrgencyColor(opportunity.isUrgent)}>
                            ⚡ URGENT
                          </Badge>
                        )}
                        <Badge className={getDistanceColor(opportunity.distance)}>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex flex-wrap items-center gap-4 text-sm">
                    <div className="flex items-center gap-2">
                      ...
                      <span>Enlèvement: {formatDate(opportunity.pickupDate)}</span>
                    </div>
                    <div className="flex items-center gap-2">
                    ...
                    {opportunity.packageDetails?.weight && (
                      <div className="flex items-center gap-2">
                        ...
                        <span>Poids: {opportunity.packageDetails.weight}kg</span>
                      </div>
                    )}
                  </div>
                  ...
                  {opportunity.packageDetails && (
                    <div className="bg-gray-50 rounded-lg p-3">
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
                        {opportunity.packageDetails.weight && (
                          <div>Poids: <strong>{opportunity.packageDetails.weight}kg</strong></div>
                        )}
                        {opportunity.packageDetails.length && (
                          <div>Dimensions: <strong>{opportunity.packageDetails.length}x{opportunity.packageDetails.width}x{opportunity.packageDetails.height}cm</strong></div>
                        )}
                        ...
                        {opportunity.packageDetails.insuredValue && (
                          <div>Assuré: <strong>{formatPrice(opportunity.packageDetails.insuredValue)}</strong></div>
                        )}
                      </div>
                    </div>
                  )}
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-gray-600">{opportunity.client.name}</span>
                      </div>
                      {opportunity.client.rating && (
                        <div className="flex items-center gap-1">
                          <Star className="h-4 w-4 text-yellow-500 fill-current" />
                          <span className="text-sm">{opportunity.client.rating}</span>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex gap-2">
                      <Button
                        variant="outline"
                        size="sm"
                        onClick={() => router.push(`/client/announcements/${opportunity.id}`)}
                      >
                        👁️ Voir détails
                      </Button>
                      <Button
