  ...
  const getFilteredRoutes = () => {
    ...
    if (searchTerm) {
      filtered = filtered.filter(r => 
        ...
        r.departureLocation.address.toLowerCase().includes(searchTerm.toLowerCase()) ||
        r.arrivalLocation.address.toLowerCase().includes(searchTerm.toLowerCase())
      )
    }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-purple-50 to-blue-50 rounded-lg p-6 border">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-2">🛣️ Mes Routes & Trajets</h1>
            <p className="text-gray-600">
              Gérez vos trajets récurrents et trouvez des correspondances automatiquement
            </p>
          </div>
          <div className="flex items-center gap-4">
            <div className="text-right">
              <div className="text-2xl font-bold text-purple-600">{data?.stats.active || 0}</div>
            ...
            <Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
              ...
              <DialogContent className="max-w-2xl">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Définissez un trajet récurrent pour recevoir des notifications d'annonces correspondantes
                  </DialogDescription>
                </DialogHeader>
                <CreateRouteForm onSuccess={() => {
        ...
        {data?.stats && (
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            ...
            <div className="bg-white rounded-lg p-3 text-center">
              <div className="text-lg font-bold text-green-600">{data.stats.active}</div>
              <div className="text-xs text-gray-600">Actives</div>
            </div>
            <div className="bg-white rounded-lg p-3 text-center">
              <div className="text-lg font-bold text-blue-600">{data.stats.recurring}</div>
              <div className="text-xs text-gray-600">Récurrentes</div>
            </div>
            <div className="bg-white rounded-lg p-3 text-center">
              <div className="text-lg font-bold text-orange-600">{data.stats.totalCapacity}</div>
              <div className="text-xs text-gray-600">Capacité totale</div>
            </div>
            <div className="bg-white rounded-lg p-3 text-center">
              <div className="text-lg font-bold text-red-600">{data.stats.totalMatches}</div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="grid w-full grid-cols-4">
          ...
          <TabsTrigger value="with-matches">
            Avec correspondances ({data?.routes.filter(r => r.announcements.length > 0).length || 0})
          </TabsTrigger>
        </TabsList>
          ...
          {filteredRoutes.length === 0 ? (
            ...
            <div className="grid gap-4">
              {filteredRoutes.map((route) => {
                const vehicleInfo = vehicleTypes[route.vehicleType as keyof typeof vehicleTypes]
                
                return (
                  <Card key={route.id} className={`hover:shadow-lg transition-all duration-200 border-l-4 ${route.isActive ? 'border-l-green-500' : 'border-l-gray-400'}`}>
                    <CardHeader>
                          ...
                          <div className="flex items-center gap-3 mb-2">
                            ...
                            {route.isRecurring && (
                              <Badge className="bg-blue-100 text-blue-800">
                                ...
                                {recurringPatterns[route.recurringPattern as keyof typeof recurringPatterns]}
                              </Badge>
                            )}
                            {route.announcements.length > 0 && (
                              <Badge className="bg-green-100 text-green-800">
                                🎯 {route.announcements.length} correspondance{route.announcements.length > 1 ? 's' : ''}
                              </Badge>
                            )}
                          </div>
                      ...
                      {route.isRecurring && (
                        <div className="bg-blue-50 rounded-lg p-3">
                          ...
                          <div className="grid grid-cols-2 gap-4 text-sm text-blue-700">
                            <div>
                              <span className="font-medium">Fréquence:</span> {recurringPatterns[route.recurringPattern as keyof typeof recurringPatterns]}
                            </div>
                            <div>
                              <span className="font-medium">Jours:</span> {getDaysOfWeekText(route.recurringDays)}
                            </div>
                          </div>
                        </div>
                      )}
                      ...
                      {route.announcements.length > 0 && (
                        <div className="space-y-2">
                          <p className="text-sm font-medium flex items-center gap-2">
                            ...
                            Annonces correspondantes ({route.announcements.length})
                          </p>
                          <div className="space-y-2">
                            {route.announcements.slice(0, 3).map((announcement) => (
                              <div key={announcement.id} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                                <div className="flex-1">
                            ...
                            {route.announcements.length > 3 && (
                              <p className="text-xs text-gray-500 text-center">
                                +{route.announcements.length - 3} autres correspondances
                              </p>
                            )}
                          </div>
                        </div>
                        ...
                        <div className="flex gap-2">
                          ...
                          {route.announcements.length > 0 && (
                            <Button
                              variant="outline"
                              size="sm"
                              onClick={() => router.push(`/deliverer/routes/${route.id}/matches`)}
                            >
