...
export default function RouteNotificationSystem({ delivererId }: RouteNotificationSystemProps) {
  ...
  const fetchData = async () => {
    ...
    try {
      const [routesRes, notificationsRes, settingsRes] = await Promise.all([
        ...
        fetch(`/api/deliverer/routes/notification-settings?delivererId=${delivererId}`)
      ]);
  ...
  const setupRealTimeNotifications = () => {
    ...
    if (typeof window !== 'undefined' && 'WebSocket' in window) {
      const ws = new WebSocket(`${process.env.NEXT_PUBLIC_WS_URL}/deliverer/${delivererId}/route-notifications`);
  ...
  const saveRoute = async () => {
    try {
      ...
      if (response.ok) {
        toast({
          title: editingRoute ? t("success.route_updated") : t("success.route_created"),
  ...
  const respondToNotification = async (notificationId: string, action: 'ACCEPTED' | 'DECLINED') => {
    try {
      const response = await fetch(`/api/deliverer/routes/notifications/${notificationId}/respond`, {
        method: 'POST',
        headers: { "Content-Type": "application/json" },
      ...
      if (response.ok) {
        setNotifications(prev => 
          prev.map(n => n.id === notificationId ? { ...n, action, isRead: true } : n)
        );
  ...
  const updateSettings = async (newSettings: Partial<NotificationSettings>) => {
    try {
      ...
      const response = await fetch('/api/deliverer/routes/notification-settings', {
        method: 'PUT',
        headers: { "Content-Type": "application/json" },
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600">{t("stats.active_routes")}</p>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600">{t("stats.new_notifications")}</p>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600">{t("stats.notifications_enabled")}</p>
                <p className="text-2xl font-bold">{settings.enabled ? t("yes") : t("no")}</p>
              </div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-gray-600">{t("stats.match_threshold")}</p>
                <p className="text-2xl font-bold">{settings.minMatchPercentage}%</p>
              </div>
      ...
      <Tabs defaultValue="routes" className="space-y-4">
        ...
        <TabsContent value="routes" className="space-y-4">
          {routes.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {routes.map((route) => (
                <Card key={route.id} className={`${route.isActive ? 'border-green-200' : 'border-gray-200'}`}>
                  <CardHeader>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-gray-600">{route.startPoint}</span>
                      </div>
                      <div className="flex items-center gap-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-medium">{t("route.schedule")}:</span>
                        <span className="text-gray-600">{route.startTime} - {route.endTime}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <Calendar className="h-4 w-4 text-purple-600" />
                        <span className="font-medium">{t("route.days")}:</span>
                        <span className="text-gray-600">
                          {route.scheduledDays.map(day => t(`days.${day.toLowerCase()}`)).join(', ')}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between pt-3 border-t">
                      <div className="text-xs text-gray-500">
                        {t("route.radius")}: {route.radiusKm}km • {t("route.capacity")}: {route.maxCapacity}
                      </div>
                      <div className="flex gap-2">
                        <Button
                          variant="outline"
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-2">
                          <h4 className="font-medium">{notification.announcementTitle}</h4>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm mb-3">
                          <div>
                            ...
                            <p className="text-gray-600 ml-6">{notification.pickupAddress}</p>
                          </div>
                          <div>
                            <div className="flex items-center gap-2">
                              <Navigation className="h-4 w-4 text-red-600" />
                            ...
                            <p className="text-gray-600 ml-6">{notification.deliveryAddress}</p>
                          </div>
                        </div>

                        <div className="flex items-center gap-4 text-xs text-gray-500">
                          <span>{t("distance")}: {notification.distance}km</span>
                          <span className="font-semibold">{t("price")}: {notification.estimatedPrice}€</span>
                          <span>{t("received")}: {formatDate(notification.createdAt)}</span>
                        </div>
                      </div>
                      ...
                      {notification.action && (
                        <Badge 
                          className={notification.action === 'ACCEPTED' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}
                        >
      ...
      <Dialog open={showRouteDialog} onOpenChange={setShowRouteDialog}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>
              {editingRoute ? t("route_dialog.edit_title") : t("route_dialog.add_title")}
            </DialogTitle>
            <DialogDescription>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <Label htmlFor="vehicleType">{t("route_dialog.vehicle_type")}</Label>
                <Select 
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="SCOOTER">{t("vehicles.scooter")}</SelectItem>
                    <SelectItem value="TRUCK">{t("vehicles.truck")}</SelectItem>
                    <SelectItem value="WALKING">{t("vehicles.walking")}</SelectItem>
                  </SelectContent>
                </Select>
              </div>
            </div>
            ...
            <div>
              <Label htmlFor="description">{t("route_dialog.description")}</Label>
              <Textarea
                id="description"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label htmlFor="startPoint">{t("route_dialog.start_point")}</Label>
                <Input
                  id="startPoint"
            ...
            <div className="grid grid-cols-3 gap-4">
              <div>
                <Label htmlFor="startTime">{t("route_dialog.start_time")}</Label>
                <Input
                  id="startTime"
                  type="time"
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setRouteForm({...routeForm, radiusKm: parseInt(e.target.value) || 3})}
                />
              </div>
            </div>
          </div>
      ...
      <Dialog open={showSettingsDialog} onOpenChange={setShowSettingsDialog}>
        <DialogContent>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <Label className="text-base font-medium">{t("settings.notifications_enabled")}</Label>
                <p className="text-sm text-gray-600">{t("settings.notifications_enabled_desc")}</p>
              </div>
              <Switch
            ...
            <div>
              <Label htmlFor="minMatch">{t("settings.min_match_percentage")}: {settings.minMatchPercentage}%</Label>
              <Input
                id="minMatch"
                type="range"
                min="50"
                ...
                onChange={(e) => updateSettings({ minMatchPercentage: parseInt(e.target.value) })}
                className="mt-2"
              />
            </div>

            <div>
              <Label htmlFor="maxDistance">{t("settings.max_distance")}: {settings.maxDistance}km</Label>
              <Input
                id="maxDistance"
                type="range"
                min="1"
            ...
            <div>
              <Label htmlFor="minPrice">{t("settings.min_price")}: {settings.minPrice}€</Label>
              <Input
                id="minPrice"
                type="range"
                min="5"
                ...
                onChange={(e) => updateSettings({ minPrice: parseInt(e.target.value) })}
                className="mt-2"
              />
            </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                <Label className="text-base font-medium">{t("settings.sound_enabled")}</Label>
                <p className="text-sm text-gray-600">{t("settings.sound_enabled_desc")}</p>
              </div>
              <Switch
