...
export class SmartMatchingService {
  ...
  static async findAnnouncementsForRoute(
    ...
    try {
      ...
      const announcements = await this.getEligibleAnnouncements(
        route,
        finalCriteria,
      );
  ...
  private static async calculateMatchScore(
    ...
    const estimatedDistance = this.calculateDeviationDistance(
      route,
      announcement,
    );
  ...
  private static calculateDistanceScore(
    ...
    return Math.max(
      0,
      100 - (maxDeviation / criteria.maxDeviationDistance) * 100,
    );
  }
  ...
  private static calculateTimeScore(
    ...
    const timeDiffHours =
      Math.abs(routeDate.getTime() - announcementDate.getTime()) /
      (1000 * 60 * 60);
  ...
  private static calculateCapacityScore(route: any, announcement: any): number {
    ...
    const packageVolume =
      (packageData.length * packageData.width * packageData.height) / 1000;

    // Vérifier si le colis peut être transporté
    if (
      packageData.weight > availableWeight ||
      packageVolume > availableVolume
    ) {
      return 0;
    }
  ...
  private static calculatePriceScore(route: any, announcement: any): number {
    ...
    const estimatedRouteCost =
      routePricePerKg * announcement.packageDetails.weight;
   ...
   */
  private static async getEligibleRoutes(
    announcement: any,
    criteria: MatchingCriteria,
  ) {
   ...
   */
  private static async getEligibleAnnouncements(
    route: any,
    criteria: MatchingCriteria,
  ) {
    const routeDate = new Date(route.departureDate);
   ...
   */
  private static calculateDeviationDistance(
    route: any,
    announcement: any,
  ): number {
    ...
    const deviationDistance =
      this.calculateDistance(
        route.startLocation.lat,
        route.startLocation.lng,
        announcement.startLocation.lat,
        announcement.startLocation.lng,
      ) +
      this.calculateDistance(
        announcement.startLocation.lat,
        announcement.startLocation.lng,
        announcement.endLocation.lat,
        announcement.endLocation.lng,
      ) +
      this.calculateDistance(
        announcement.endLocation.lat,
        announcement.endLocation.lng,
  ...
  private static generateRecommendations(
    ...
    if (recommendations.length === 0) {
      recommendations.push(
        "Excellente correspondance ! Aucune recommandation particulière.",
      );
    }
  ...
  static async saveMatch(
    ...
    try {
      ...
      ecoLogger.matching.matchCreated(
        routeId,
        announcementId,
        matchScore.score,
      );
    } catch (error) {
      ecoLogger.matching.error("Error saving match", {
        routeId,
        announcementId,
        error,
      });
  ...
  static async runAutomaticMatching(): Promise<{
    ...
    try {
      ...
      for (const announcement of announcements) {
        try {
          const matches = await this.findMatchesForAnnouncement(
            announcement.id,
          );
          processed++;
          ...
          for (const match of goodMatches.slice(0, 5)) {
            // Max 5 matches par annonce
            ...
            if (match.matchScore.score >= 85) {
              await this.notifyDelivererOfMatch(
                match.route.deliverer.userId,
                announcement,
              );
              notifications++;
            }
          ...
          ecoLogger.matching.error(
            "Error processing announcement in automatic matching",
            {
              announcementId: announcement.id,
              error: error instanceof Error ? error.message : "Unknown error",
            },
   ...
   */
  private static async notifyDelivererOfMatch(
