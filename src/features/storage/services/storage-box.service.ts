...
export class StorageBoxService {
    ...
    try {
      ...
      const availableBoxes = boxes.filter(box => {
        ...
        const hasActiveRental = box.rentals.some(rental => {
          const now = new Date()
          return (!rental.endDate || rental.endDate > now) && rental.startDate <= now
        })
  ...
  static async getStorageStats(locationId?: string) {
    try {
      ...
      const [totalBoxes, availableBoxes, occupiedBoxes, totalRentals, activeRentals] = await Promise.all([
   ...
   */
  private static async generateQRCode(rentalId: string, accessCode: string): Promise<string> {
    try {
      const qrData = {
        type: 'STORAGE_ACCESS',
        rentalId,
   ...
   */
  static async findNearbyBoxes(lat: number, lng: number, radiusKm: number = 10) {
    try {
