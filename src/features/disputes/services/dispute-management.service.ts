...
interface DisputeResolution {
  disputeId: string
  resolution: 'FAVOR_REPORTER' | 'FAVOR_REPORTED' | 'PARTIAL_REFUND' | 'NO_ACTION' | 'MEDIATION'
  adminNotes: string
...
class DisputeManagementService {
    ...
    try {
      ...
      const existingDispute = await db.dispute.findFirst({
        where: {
          ...
          status: { in: ['OPEN', 'INVESTIGATING', 'WAITING_EVIDENCE', 'MEDIATION'] }
        }
      })
      ...
      const estimatedResolutionDate = this.calculateResolutionDate(validated.priority)
   ...
   */
  async getDisputeStatistics(period: 'DAY' | 'WEEK' | 'MONTH' = 'MONTH'): Promise<{
    ...
    try {
      ...
      const averageResolutionTime = resolvedDisputesWithTime.length > 0 ?
        ...
        }, 0) / resolvedDisputesWithTime.length : 0

      // Formater les résultats
      const byCategory = disputesByCategory.reduce((acc, item) => {
        acc[item.category as DisputeCategory] = item._count
        return acc
      }, {} as Record<DisputeCategory, number>)

      const byResolution = disputesByResolution.reduce((acc, item) => {
        if (item.resolution) {
      ...
      const topReportedUsers = topReported.map(item => {
        ...
        return {
          ...
          userName: user ? `${user.profile?.firstName} ${user.profile?.lastName}` : 'Inconnu',
  ...
  private async updateTrustScores(dispute: any, resolution: string): Promise<void> {
  ...
  private async notifyStatusUpdate(disputeId: string, status: DisputeStatus): Promise<void> {
    console.log(`Dispute status update notification: ${disputeId} -> ${status}`)
  }

  private async notifyDisputeResolution(disputeId: string, resolution: string): Promise<void> {
    console.log(`Dispute resolution notification: ${disputeId} -> ${resolution}`)
  }

  private async notifyEscalation(disputeId: string, reason: string): Promise<void> {
