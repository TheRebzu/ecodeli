...
export function useProviderServiceRequests() {
  ...
  const [pagination, setPagination] = useState<ServiceRequestsResponse['pagination'] | null>(null)
  ...
  const fetchServiceRequests = async (params?: {
    ...
    try {
      ...
      const response = await fetch(`/api/provider/service-requests?${searchParams.toString()}`)
  ...
  const applyToServiceRequest = async (serviceRequestId: string, proposal: {
    price: number
    ...
    try {
      const response = await fetch(`/api/provider/service-requests/${serviceRequestId}/apply`, {
        method: 'POST',
        headers: {
