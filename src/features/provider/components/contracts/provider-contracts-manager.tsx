...
} from "@/features/provider/schemas/contract.schema";
...
export function ProviderContractsManager({
  ...
  const signContract = async (
    contractId: string,
    signatureType: "PROVIDER" | "ECODELI",
  ) => {
    try {
      const response = await execute(
        `/api/provider/contracts/${contractId}/sign`,
        {
          method: "POST",
          body: JSON.stringify({
            contractId,
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">Gestion de vos contrats avec EcoDeli</p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Liste de vos contrats avec EcoDeli</CardDescription>
        </CardHeader>
        <CardContent>
          {contracts.length === 0 ? (
                        ...
                        {contract.status === "PENDING_SIGNATURE" &&
                          !contract.signedByProvider && (
                            <Button
                              size="sm"
                              onClick={() =>
                                signContract(contract.id) || "PROVIDER"
                              }
                            >
                              Signer
                            </Button>
