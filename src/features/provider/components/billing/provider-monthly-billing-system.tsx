...
export default function ProviderMonthlyBillingSystem({
  providerId,
}: ProviderMonthlyBillingSystemProps) {
  const t = useTranslations("provider.billing");
  const [billings, setBillings] = useState<MonthlyBilling[]>([]);
  const [selectedBilling, setSelectedBilling] = useState<MonthlyBilling | null>(
    null,
  );
  ...
  const handleApproveBilling = async (billingId: string) => {
    try {
      const response = await fetch(
        `/api/provider/billing/${billingId}/approve`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const handleDisputeBilling = async (billingId: string, reason: string) => {
    try {
      const response = await fetch(
        `/api/provider/billing/${billingId}/dispute`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="container mx-auto p-6">
      ...
      {metrics && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">
                {t("metrics.current_month_revenue")}
              </CardTitle>
              <Euro className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">
                €{metrics.currentMonth.netAmount.toFixed(2)}
              </div>
              <div className="flex items-center text-xs text-muted-foreground">
                <TrendingUp className="h-3 w-3 mr-1" />
                {calculateGrowth(
                  metrics.currentMonth.netAmount,
                  metrics.previousMonth.netAmount,
                ) > 0
                  ? "+"
                  : ""}
                {calculateGrowth(
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">
                {t("metrics.services_completed")}
              </CardTitle>
              <CheckCircle className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">
                {metrics.currentMonth.totalServices}
              </div>
              <div className="flex items-center text-xs text-muted-foreground">
                <TrendingUp className="h-3 w-3 mr-1" />
                {calculateGrowth(
                  metrics.currentMonth.totalServices,
                  metrics.previousMonth.totalServices,
                ) > 0
                  ? "+"
                  : ""}
                {calculateGrowth(
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">
                {t("metrics.total_hours")}
              </CardTitle>
              <Clock className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">
                {metrics.currentMonth.totalHours}h
              </div>
              <div className="flex items-center text-xs text-muted-foreground">
                <TrendingUp className="h-3 w-3 mr-1" />
                {calculateGrowth(
                  metrics.currentMonth.totalHours,
                  metrics.previousMonth.totalHours,
                ) > 0
                  ? "+"
                  : ""}
                {calculateGrowth(
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">
                {t("metrics.average_rating")}
              </CardTitle>
              <CheckCircle className="h-4 w-4 text-muted-foreground" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold">
                {metrics.currentMonth.averageRating.toFixed(1)}/5
              </div>
              <Progress
                value={metrics.currentMonth.averageRating * 20}
                className="mt-2"
              />
            </CardContent>
          </Card>
      ...
      <div className="flex justify-between items-center mb-6">
          ...
          <Select value={filterPeriod} onValueChange={setFilterPeriod}>
            ...
            <SelectContent>
              ...
              <SelectItem value="current_year">
                {t("filters.current_year")}
              </SelectItem>
              <SelectItem value="previous_year">
                {t("filters.previous_year")}
              </SelectItem>
            </SelectContent>
          </Select>
        </div>
        ...
        <Button
          onClick={() =>
            handleGenerateMonthlyBilling(new Date().toISOString().substr(0, 7))
          }
        >
              ...
              <CardHeader>
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <CardDescription>
                      {billing.totalServices} {t("services_completed")} •{" "}
                      {billing.totalHours}h {t("worked")}
                    </CardDescription>
                  </div>
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                  <div>
                    ...
                    <div className="font-semibold">
                      €{billing.grossAmount.toFixed(2)}
                    </div>
                  </div>
                  <div>
                    <span className="text-sm text-gray-600">
                      {t("commission")} ({billing.commissionRate}%):
                    </span>
                    <div className="font-semibold text-red-600">
                      -€{billing.commission.toFixed(2)}
                    </div>
                  </div>
                  <div>
                    <span className="text-sm text-gray-600">
                      {t("taxes")} ({billing.taxRate}%):
                    </span>
                    <div className="font-semibold text-red-600">
                      -€{billing.taxes.toFixed(2)}
                    </div>
                  </div>
                  <div>
                    <span className="text-sm text-gray-600">
                ...
                <div className="flex justify-between items-center">
                  <div className="text-sm text-gray-600">
                    {t("generated_at")}:{" "}
                    {new Date(billing.generatedAt).toLocaleDateString()}
                    {billing.dueDate && (
                      <span className="ml-4">
                        {t("due_date")}:{" "}
                  ...
                  <div className="flex gap-2">
                    ...
                    {billing.status === "pending_review" && (
                      <Button
                        size="sm"
                        onClick={() => handleApproveBilling(billing.id)}
                      >
                    ...
                    {["pending_review", "approved"].includes(
                      ...
                      <Button
                        ...
                        size="sm"
                        onClick={() =>
                          handleDisputeBilling(
                            billing.id,
                            "Contestation sur les montants",
                          )
                        }
                      >
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">
                          €{metrics.currentMonth.grossAmount.toFixed(2)}
                        </span>
                      </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between">
                      ...
                      <span className="font-bold">
                        €{metrics.yearToDate.netAmount.toFixed(2)}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span>{t("total_services")}:</span>
                      <span className="font-medium">
                        {metrics.yearToDate.totalServices}
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span>{t("total_hours")}:</span>
                      <span className="font-medium">
                        {metrics.yearToDate.totalHours}h
                      </span>
                    </div>
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">
                        €
                        {metrics.yearToDate.totalServices > 0
                          ? (
                              metrics.yearToDate.grossAmount /
                              metrics.yearToDate.totalServices
                            ).toFixed(2)
                          : "0.00"}
                      </span>
                ...
                <CardContent>
                  <div className="space-y-3">
                    <div>
                      <div className="flex justify-between mb-1">
                        ...
                        <span className="text-sm font-medium">
                          {metrics.yearToDate.averageRating.toFixed(1)}/5
                        </span>
                      </div>
                    ...
                    <div>
                      ...
                      <Progress
                        value={(metrics.currentMonth.totalServices / 20) * 100}
                      />
                    </div>
                  </div>
                </CardContent>
      ...
      {showDetailDialog && selectedBilling && (
        <Dialog open={showDetailDialog} onOpenChange={setShowDetailDialog}>
          <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>
                {t("detail_dialog.title", {
                  period: formatPeriod(selectedBilling.period),
                })}
              </DialogTitle>
              <DialogDescription>
            ...
            <Tabs defaultValue="summary" className="w-full">
              <TabsList className="grid w-full grid-cols-3">
                <TabsTrigger value="summary">
                  {t("detail_tabs.summary")}
                </TabsTrigger>
                <TabsTrigger value="services">
                  {t("detail_tabs.services")}
                </TabsTrigger>
                <TabsTrigger value="detailed">
                  {t("detail_tabs.detailed")}
                </TabsTrigger>
              </TabsList>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-1 text-sm">
                      ...
                      <div className="flex justify-between text-red-600">
                        <span>
                          {t("commission")} ({selectedBilling.commissionRate}%):
                        </span>
                        <span>-€{selectedBilling.commission.toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between text-red-600">
                        <span>
                          {t("taxes")} ({selectedBilling.taxRate}%):
                        </span>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-1 text-sm">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span>
                          {selectedBilling.totalServices > 0
                            ? (
                                selectedBilling.totalHours /
                                selectedBilling.totalServices
                              ).toFixed(1)
                            : 0}
                          h
                        </span>
                      ...
                      <div className="flex justify-between">
                        ...
                        <span>
                          €
                          {selectedBilling.totalHours > 0
                            ? (
                                selectedBilling.grossAmount /
                                selectedBilling.totalHours
                              ).toFixed(2)
                            : "0.00"}
                        </span>
