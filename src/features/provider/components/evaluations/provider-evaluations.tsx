...
import { Star, TrendingUp, TrendingDown, Award, AlertCircle, MessageSquare } from "lucide-react";
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            <div className={`text-2xl font-bold ${getRatingColor(stats?.averageRating || 0)}`}>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{stats?.monthlyAverage.toFixed(1)}/5</div>
            <p className="text-xs text-muted-foreground">
              {stats?.trend === "up" ? "+" : stats?.trend === "down" ? "-" : ""}
              {Math.abs((stats?.monthlyAverage || 0) - (stats?.previousMonthAverage || 0)).toFixed(1)} vs mois dernier
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {stats ? Math.round((stats.ratingDistribution[5] + stats.ratingDistribution[4]) / stats.totalEvaluations * 100) : 0}%
            </div>
            <p className="text-xs text-muted-foreground">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {stats ? stats.ratingDistribution[1] + stats.ratingDistribution[2] : 0}
            </div>
            <p className="text-xs text-muted-foreground">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Les avis de vos clients sur vos prestations</CardDescription>
        </CardHeader>
        <CardContent>
            ...
            <TabsContent value={filter} className="space-y-4 mt-6">
              {filteredEvaluations.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-muted-foreground">Aucune évaluation dans cette catégorie</p>
                </div>
              ) : (
                  ...
                  <div key={evaluation.id} className="border rounded-lg p-4 space-y-3">
                    <div className="flex items-start justify-between">
                      <div className="flex items-start gap-3">
                        <Avatar>
                          ...
                          <AvatarFallback>{evaluation.clientName[0]}</AvatarFallback>
                        </Avatar>
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            {evaluation.serviceName} • {format(new Date(evaluation.createdAt), "d MMMM yyyy", { locale: fr })}
                          </p>
                        </div>
                      </div>
                      <Badge variant={evaluation.rating >= 4 ? "default" : evaluation.rating >= 3 ? "secondary" : "destructive"}>
                        {evaluation.rating}/5
                      </Badge>
                    </div>
                    ...
                    {evaluation.response ? (
                      <div className="bg-gray-50 rounded p-3 mt-2">
                        ...
                        <p className="text-sm text-gray-600">{evaluation.response}</p>
                      </div>
                    ) : (
      ...
      <Card className="bg-blue-50 border-blue-200">
        <CardHeader>
          <CardTitle className="text-blue-900">Impact des évaluations</CardTitle>
        </CardHeader>
        <CardContent>
          <ul className="space-y-2 text-sm text-blue-800">
            <li>• Les évaluations influencent votre visibilité sur la plateforme</li>
            <li>• Une note moyenne supérieure à 4.5/5 vous donne accès aux clients Premium</li>
            <li>• Répondez aux évaluations négatives pour montrer votre professionnalisme</li>
            <li>• Les évaluations sont prises en compte dans les négociations tarifaires avec EcoDeli</li>
          </ul>
        </CardContent>
      </Card>
    </div>
