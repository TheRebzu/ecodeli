...
export default function ProviderValidationManager({
  providerId,
}: ProviderValidationManagerProps) {
  ...
  const getStatusBadge = (status: string) => {
    const statusConfig = {
      ...
      pending_documents: {
        ...
        label: t("status.pending_documents"),
        icon: AlertTriangle,
      },
    };
  ...
  const getStepStatusBadge = (status: string) => {
    const statusConfig = {
      pending: {
        color: "bg-gray-100 text-gray-800",
        label: t("step_status.pending"),
      },
      in_progress: {
        color: "bg-blue-100 text-blue-800",
        label: t("step_status.in_progress"),
      },
      completed: {
        color: "bg-green-100 text-green-800",
        label: t("step_status.completed"),
      },
      failed: {
        color: "bg-red-100 text-red-800",
        label: t("step_status.failed"),
      },
    };
  ...
  const getDocumentStatusBadge = (status: string) => {
    const statusConfig = {
      missing: {
        color: "bg-gray-100 text-gray-800",
        label: t("document_status.missing"),
      },
      uploaded: {
        color: "bg-blue-100 text-blue-800",
        label: t("document_status.uploaded"),
      },
      approved: {
        color: "bg-green-100 text-green-800",
        label: t("document_status.approved"),
      },
      rejected: {
        color: "bg-red-100 text-red-800",
        label: t("document_status.rejected"),
      },
    };
  ...
  const calculateCompletionPercentage = () => {
    ...
    const completedSteps = application.validationSteps.filter(
      (step) => step.status === "completed",
    ).length;
    return Math.round(
      (completedSteps / application.validationSteps.length) * 100,
    );
  };
  ...
  const getNextRequiredAction = () => {
    ...
    if (pendingStep) {
      const missingDocs = pendingStep.documents.filter(
      ...
      <Card>
        <CardContent className="flex flex-col items-center justify-center py-12">
          ...
          <h3 className="text-lg font-semibold mb-2">
            {t("no_application.title")}
          </h3>
          <p className="text-gray-600 text-center mb-4">
            {t("no_application.description")}
          </p>
          <Button
            onClick={() => (window.location.href = "/provider/validation")}
          >
    ...
    <div className="container mx-auto p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-2">
              {
                application.validationSteps.filter(
                  (s) => s.status === "completed",
                ).length
              }{" "}
              / {application.validationSteps.length}{" "}
              {t("progress.steps_completed")}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {application.overallScore}/{application.maxScore}
            </div>
            <p className="text-xs text-muted-foreground">
              {t("score.percentage", {
                percentage: Math.round(
                  (application.overallScore / application.maxScore) * 100,
                ),
              })}
            </p>
          </CardContent>
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-orange-700">
                  {nextAction.type === "document" && (
                    <>
                      {t("next_action.upload_documents", {
                        step: nextAction.step.name,
                      })}
                      :
                      {nextAction.documents.map((doc) => (
                        <span key={doc.id} className="ml-1 font-medium">
                          {doc.name}
                        </span>
                      ))}
                    </>
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                      ...
                      <div className="font-medium">
                        {step.documents.filter((d) => d.required).length}
                      </div>
                    </div>
                  </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="font-medium">
                      {t("timeline.application_created")}
                    </p>
                ...
                {application.approvedAt && (
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium">
                        {t("timeline.application_approved")}
                      </p>
                      <p className="text-sm text-gray-600">
                ...
                {application.rejectedAt && (
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium">
                        {t("timeline.application_rejected")}
                      </p>
                      <p className="text-sm text-gray-600">
