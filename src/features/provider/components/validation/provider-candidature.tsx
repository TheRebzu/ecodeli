...
export function ProviderCandidature() {
  ...
  const [validationData, setValidationData] = useState<ProviderValidationData | null>(null);
  ...
  const fetchValidationData = async () => {
    ...
    try {
      setLoading(true);
      const response = await get(`/api/provider/validation/overview?providerId=${user.id}`);
      if (response) {
  ...
  const validationSteps: ValidationStep[] = [
    {
      ...
      status: validationData ? 
        validationData.profileCompletion === 100 ? 'completed' : 
        validationData.profileCompletion > 0 ? 'in_progress' : 'pending'
        : 'pending',
      route: '/provider/validation/profile',
      icon: User,
      requiredFields: ['businessName', 'siret', 'description', 'phone', 'address']
    },
    {
      id: 'services',
      ...
      status: validationData ? 
        validationData.servicesCompletion === 100 ? 'completed' : 
        validationData.servicesCompletion > 0 ? 'in_progress' : 'pending'
        : 'pending',
    ...
    },
    {
      ...
      status: validationData ? 
        validationData.certificationsCompletion === 100 ? 'completed' : 
        validationData.certificationsCompletion > 0 ? 'in_progress' : 'pending'
        : 'pending',
    ...
    },
    {
      ...
      status: validationData ? 
        validationData.ratesCompletion === 100 ? 'completed' : 
        validationData.ratesCompletion > 0 ? 'in_progress' : 'pending'
        : 'pending',
  ...
  const getTotalCompletion = () => {
    ...
    return Math.round(
      (validationData.profileCompletion + 
       ...
       validationData.ratesCompletion) / 4
    );
  };
    ...
    <div className="space-y-6">
      ...
      <div className="text-center space-y-4">
        ...
        <p className="text-muted-foreground max-w-2xl mx-auto">
          Complétez votre candidature en 4 étapes pour devenir prestataire certifié EcoDeli. 
          Chaque étape est essentielle pour valider votre profil professionnel.
        </p>
      </div>
        ...
        <CardContent>
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm font-medium">{getTotalCompletion()}%</span>
            </div>
            <Progress value={getTotalCompletion()} className="h-2" />
            
            {validationData?.validationStatus === 'REJECTED' && validationData.validationNotes && (
              <div className="bg-red-50 border border-red-200 rounded-lg p-4">
                <div className="flex items-start space-x-2">
                  <AlertCircle className="w-5 h-5 text-red-600 mt-0.5" />
                  <div>
                    <p className="font-medium text-red-800">Candidature rejetée</p>
                    <p className="text-sm text-red-600 mt-1">{validationData.validationNotes}</p>
                  </div>
                </div>
              </div>
            )}
            ...
            {validationData?.validationStatus === 'APPROVED' && (
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    <p className="font-medium text-green-800">Candidature approuvée</p>
                    <p className="text-sm text-green-600">
                      Félicitations ! Vous êtes maintenant prestataire certifié EcoDeli
                    </p>
                  </div>
                </div>
              </div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid gap-6">
          {validationSteps.map((step, index) => (
            <Card key={step.id} className={`transition-all ${step.status === 'completed' ? 'border-green-200' : ''}`}>
              <CardHeader>
                <div className="flex items-center justify-between">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-2">
                    {step.status === 'completed' ? (
                      ...
                      <span className="text-sm text-muted-foreground">
                        {validationData ? 
                          step.id === 'profile' ? validationData.profileCompletion :
                          step.id === 'services' ? validationData.servicesCompletion :
                          step.id === 'certifications' ? validationData.certificationsCompletion :
                          step.id === 'rates' ? validationData.ratesCompletion : 0
                        : 0}%
                      </span>
                    )}
                  </div>
      ...
      {canSubmitForValidation() && (
        <Card className="border-green-200 bg-green-50">
          <CardHeader>
            <CardTitle className="text-green-800">Prêt pour la validation</CardTitle>
            <CardDescription className="text-green-700">
              Toutes les étapes sont complétées. Vous pouvez maintenant soumettre votre candidature.
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Button onClick={submitForValidation} className="w-full bg-green-600 hover:bg-green-700">
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              • En cas de questions, contactez notre support à prestataires@ecodeli.fr
            </p>
          </div>
        </CardContent>
      </Card>
