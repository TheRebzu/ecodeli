...
export class BillingService {
  ...
  static async generateMonthlyInvoice(
    ...
    if (existingInvoice) {
      throw new Error(
        `Facture déjà générée pour ${billingPeriod.month}/${billingPeriod.year}`,
      );
    }
  ...
  private static async generateInvoicePDF(data: {
    ...
    doc.setFontSize(12);
    doc.text(
      `Période: ${data.billingPeriod.month}/${data.billingPeriod.year}`,
      120,
      40,
    );
    ...
    doc.setFontSize(12);
    doc.text(
      `${data.provider.user.profile?.firstName || ""} ${data.provider.user.profile?.lastName || ""}`,
      20,
      90,
    );
    ...
    doc.setFontSize(12);
    doc.text(
      `Total heures: ${data.invoice.totalHours.toFixed(1)}h`,
      100,
      yPosition,
    );
    yPosition += 10;
    doc.text(
      `Total brut: ${data.invoice.totalAmount.toFixed(2)}€`,
      100,
      yPosition,
    );
    yPosition += 10;
    doc.text(
      `Commission EcoDeli (${(data.invoice.commissionRate * 100).toFixed(0)}%): ${data.invoice.commissionAmount.toFixed(2)}€`,
      100,
      yPosition,
    );
    yPosition += 10;
    doc.setFontSize(14);
    doc.text(
      `Montant net: ${data.invoice.netAmount.toFixed(2)}€`,
      100,
      yPosition,
    );
  ...
  static async getBillingStats(providerId: string, monthsCount = 6) {
    ...
    stats.averageMonthlyRevenue =
      stats.totalRevenue / Math.max(1, stats.totalInvoices);
