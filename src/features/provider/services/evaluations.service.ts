...
export class EvaluationsService {
  ...
  static async getEvaluationStats(
    ...
    const previousAverage =
      previousReviews.length > 0
        ? previousReviews.reduce((sum, review) => sum + review.rating, 0) /
  ...
  static async getEvaluationsByServiceType(providerId: string) {
    ...
    evaluations.forEach((evaluation) => {
      ...
      const serviceName =
        evaluation.booking?.service?.name || "Service inconnu";
    ...
    Object.keys(serviceStats).forEach((type) => {
      serviceStats[type].averageRating =
        Math.round(serviceStats[type].averageRating * 10) / 10;
    });
