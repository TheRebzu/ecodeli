...
import { ShoppingCart, MapPin, Clock, Settings, TrendingUp, AlertTriangle } from 'lucide-react'
...
export function CartDropOverview() {
  ...
  const deliveryZones = [
    { postalCode: '75001', deliveryFee: 5.00 },
    { postalCode: '75002', deliveryFee: 5.00 },
    { postalCode: '75003', deliveryFee: 6.00 },
    { postalCode: '75004', deliveryFee: 6.00 },
  ]
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">{t('stats.monthly')}</p>
          </CardContent>
        </Card>

        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">{t('stats.averageOrder')}</CardTitle>
            <ShoppingCart className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{stats.averageOrderValue.toFixed(2)}€</div>
            <p className="text-xs text-muted-foreground">{t('stats.perOrder')}</p>
          </CardContent>
        </Card>
      </div>
            ...
            <div className="space-y-2">
              {deliveryZones.map((zone, index) => (
                <div key={index} className="flex items-center justify-between p-2 border rounded">
                  ...
                  <Badge variant="secondary">{zone.deliveryFee.toFixed(2)}€</Badge>
                </div>
              ))}
            </div>
          </CardContent>
