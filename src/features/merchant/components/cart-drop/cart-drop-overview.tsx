    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{config?.stats?.totalOrders || 0}</div>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">{t('stats.activeZones')}</CardTitle>
            <MapPin className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{config?.deliveryZones?.length || 0}</div>
            <p className="text-xs text-muted-foreground">{t('stats.deliveryZones')}</p>
          </CardContent>
        </Card>

        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">{t('stats.timeSlots')}</CardTitle>
            <Clock className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{config?.timeSlots?.length || 0}</div>
            <p className="text-xs text-muted-foreground">{t('stats.availableSlots')}</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">€{config?.stats?.revenue?.toFixed(2) || '0.00'}</div>
            <p className="text-xs text-muted-foreground">{t('stats.fromCartDrop')}</p>
          </CardContent>
        </Card>
      </div>
          ...
          <CardContent>
            {config?.deliveryZones && config.deliveryZones.length > 0 ? (
              <div className="space-y-2">
                {config.deliveryZones.slice(0, 3).map((zone: any, index: number) => (
              ...
              <div className="text-center py-4">
                ...
                <p className="text-sm text-muted-foreground mt-2">{t('noZonesConfigured')}</p>
        ...
        <Card>
          ...
          <CardContent>
            {config?.timeSlots && config.timeSlots.length > 0 ? (
              <div className="space-y-2">
                {config.timeSlots.slice(0, 3).map((slot: any, index: number) => (
                  <div key={index} className="flex items-center justify-between p-2 border rounded">
                    <div>
              ...
              <div className="text-center py-4">
                ...
                <p className="text-sm text-muted-foreground mt-2">{t('noSlotsConfigured')}</p>
                <Button variant="outline" size="sm" className="mt-2" asChild>
                  <Link href="/merchant/cart-drop/schedules">
                    {t('configureSlots')}
                  </Link>
