...
export function ProductForm({ product, mode = 'add' }: ProductFormProps) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{mode === 'add' ? t('add.title') : t('edit.title')}</CardTitle>
        <CardDescription>{mode === 'add' ? t('add.description') : t('edit.description')}</CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <div className="border-2 border-dashed border-muted-foreground/25 rounded-lg p-6 text-center">
              ...
              <p className="text-sm text-muted-foreground mb-2">{t('form.uploadImages')}</p>
              <Input
                type="file"
                multiple
                accept="image/*"
