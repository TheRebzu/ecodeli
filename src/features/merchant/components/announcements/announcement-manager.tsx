...
export default function AnnouncementManager({
  merchantId,
}: MerchantAnnouncementManagerProps) {
  ...
  const fetchAnnouncements = async () => {
    try {
      const response = await fetch(
        `/api/merchant/announcements?merchantId=${merchantId}`,
      );
      if (response.ok) {
  ...
  const handleUpdateAnnouncement = async () => {
    ...
    try {
      const response = await fetch(
        `/api/merchant/announcements/${editingAnnouncement.id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  const handleDeleteAnnouncement = async (announcementId: string) => {
    ...
    try {
      const response = await fetch(
        `/api/merchant/announcements/${announcementId}`,
        {
          method: "DELETE",
        },
      );
  ...
  const handleStatusChange = async (
    ...
    try {
      const response = await fetch(
        `/api/merchant/announcements/${announcementId}/status`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
                ...
                <div className="space-y-4">
                  <div>
                    <Label htmlFor="template">
                      {t("bulk_dialog.template")}
                    </Label>
                    <Select
                      value={bulkImport.template}
                      onValueChange={(value) =>
                        setBulkImport({ ...bulkImport, template: value as any })
                      }
                    >
                      <SelectTrigger>
                        <SelectValue />
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <Label htmlFor="pickupAddress">
                        {t("create_dialog.pickup_address")}
                      </Label>
                      <Input
                        id="pickupAddress"
                    ...
                    <div>
                      <Label htmlFor="deliveryAddress">
                        {t("create_dialog.delivery_address")}
                      </Label>
                      <Input
                        id="deliveryAddress"
      ...
      <Tabs defaultValue="active" className="w-full">
        ...
        <TabsContent value="active" className="space-y-4">
          {filteredAnnouncements
            .filter((a) => ["published", "paused"].includes(a.status))
            .map((announcement) => (
                ...
                <CardContent>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm">
                        {t("accepted_bids")}: {announcement.acceptedBids}
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Eye className="h-4 w-4" />
                      <span className="text-sm">
                        {t("total_views")}: {announcement.totalViews}
                      </span>
                    </div>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Select
                        onValueChange={(value) =>
                          handleStatusChange(announcement.id, value)
                        }
                      >
                        <SelectTrigger className="w-32">
                          <SelectValue
                            placeholder={t("actions.change_status")}
                          />
                        </SelectTrigger>
                        <SelectContent>
