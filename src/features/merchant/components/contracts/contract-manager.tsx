...
export default function ContractManager({
  ...
  const fetchContracts = async () => {
    try {
      const response = await fetch(
        `/api/merchant/contracts?merchantId=${merchantId}`,
      );
      if (response.ok) {
  ...
  const handleSignContract = async (contractId: string) => {
    try {
      const response = await fetch(
        `/api/merchant/contracts/${contractId}/sign`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const handleTerminateContract = async (contractId: string) => {
    try {
      const response = await fetch(
        `/api/merchant/contracts/${contractId}/terminate`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
  ...
  const getContractTypeInfo = (type: string) => {
    const typeConfig = {
      standard: {
        ...
        commissionRate: 15,
        features: [
          t("features.basic_support"),
          t("features.standard_analytics"),
        ],
      },
      premium: {
  ...
  const getBillingStatusBadge = (status: string) => {
    const statusConfig = {
      paid: {
        color: "bg-green-100 text-green-800",
        label: t("billing_status.paid"),
      },
      pending: {
        color: "bg-yellow-100 text-yellow-800",
        label: t("billing_status.pending"),
      },
      overdue: {
        color: "bg-red-100 text-red-800",
        label: t("billing_status.overdue"),
      },
      cancelled: {
        color: "bg-gray-100 text-gray-800",
        label: t("billing_status.cancelled"),
      },
    };
    ...
    <div className="container mx-auto p-6">
        ...
        <div className="flex justify-between items-center">
          ...
          >
            <DialogTrigger asChild>
              <Button>{t("actions.new_contract")}</Button>
            </DialogTrigger>
            <DialogContent>
              <DialogHeader>
              ...
              <div className="space-y-4">
                <div>
                  <Label htmlFor="contractType">
                    {t("new_contract_dialog.type")}
                  </Label>
                  <Select
                    value={newContractData.type}
                    onValueChange={(value) =>
                      setNewContractData({ ...newContractData, type: value })
                    }
                  >
                    <SelectTrigger>
                      <SelectValue />
                ...
                <div>
                  <Label htmlFor="contractTitle">
                    {t("new_contract_dialog.title_field")}
                  </Label>
                  <Input
                    id="contractTitle"
                ...
                <div>
                  ...
                  <Textarea
                    ...
                    placeholder={t(
                      "new_contract_dialog.description_placeholder",
                    )}
                  />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label htmlFor="monthlyVolume">
                      {t("new_contract_dialog.monthly_volume")}
                    </Label>
                    <Input
                      id="monthlyVolume"
                      type="number"
                      ...
                      onChange={(e) =>
                        setNewContractData({
                          ...newContractData,
                          minimumMonthlyVolume: parseInt(e.target.value),
                        })
                      }
                    />
                  </div>
                  <div>
                    <Label htmlFor="maxDeliveries">
                      {t("new_contract_dialog.max_deliveries")}
                    </Label>
                    <Input
                      id="maxDeliveries"
                      type="number"
      ...
      <Tabs defaultValue="overview" className="w-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {["standard", "premium", "enterprise"].map((type) => {
              ...
              return (
                <Card
                  key={type}
                  className={hasActiveContract ? "border-green-500" : ""}
                >
                  <CardHeader>
        ...
        <TabsContent value="active" className="space-y-4">
          {contracts
            .filter((c) => ["active", "pending_review"].includes(c.status))
            .map((contract) => (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm">
                        €{contract.monthlyFee}/mois
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-sm">
                        {t("commission")}: {contract.commissionRate}%
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
            ...
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-12">
                ...
                <h3 className="text-lg font-semibold mb-2">
                  {t("empty_active.title")}
                </h3>
                <p className="text-gray-600 text-center mb-4">
                  {t("empty_active.description")}
                </p>
          ...
          {billingHistory.length === 0 && (
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-12">
                ...
                <h3 className="text-lg font-semibold mb-2">
                  {t("empty_billing.title")}
                </h3>
                <p className="text-gray-600 text-center">
                  {t("empty_billing.description")}
                </p>
              </CardContent>
            </Card>
          )}
          ...
          {contractDocuments.length === 0 && (
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-12">
                <FileText className="h-12 w-12 text-gray-400 mb-4" />
                <h3 className="text-lg font-semibold mb-2">
                  {t("empty_documents.title")}
                </h3>
                <p className="text-gray-600 text-center">
                  {t("empty_documents.description")}
                </p>
              </CardContent>
            </Card>
          )}
        ...
        >
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                {t("terminate_dialog.description", {
