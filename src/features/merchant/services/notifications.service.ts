...
export class MerchantNotificationsService {
  ...
  static async createNotification(
    ...
    notificationData: Omit<
      ...
      "id" | "merchantId" | "read" | "createdAt" | "readAt"
    >,
  ...
  static async getNotificationStats(
    ...
    try {
      const [total, unread, categoryStats, priorityStats, recentActivity] =
        await Promise.all([
          // Total notifications
          prisma.notification.count({
            where: { merchantId },
      ...
      const updatedPreferences = await prisma.notificationPreferences.upsert({
        ...
        create: {
          ...
          categories: JSON.stringify(
            preferences.categories || this.getDefaultCategories(),
          ),
          quietHours: JSON.stringify(
            preferences.quietHours || this.getDefaultQuietHours(),
          ),
          frequency: JSON.stringify(
            preferences.frequency || this.getDefaultFrequency(),
          ),
        },
      });
  ...
  static async createBusinessAlert(
    ...
    alertData: Omit<
      BusinessAlert,
      "id" | "merchantId" | "resolved" | "createdAt" | "resolvedAt"
    >,
   ...
   */
  static async resolveBusinessAlert(
    alertId: string,
    merchantId: string,
  ): Promise<void> {
    try {
  ...
  private static async sendEmailNotification(
    ...
    console.log(
      `Email notification sent to merchant ${merchantId}:`,
      notification.title,
    );
  }
  ...
  private static async sendSMSNotification(
    ...
    console.log(
      `SMS notification sent to merchant ${merchantId}:`,
      notification.title,
    );
  }
  ...
  private static async sendPushNotification(
    ...
    console.log(
      `Push notification sent to merchant ${merchantId}:`,
      notification.title,
    );
  }
