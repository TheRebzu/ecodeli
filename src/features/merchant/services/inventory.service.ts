...
export class MerchantInventoryService {
   ...
   */
  static async getProductById(productId: string, merchantId: string): Promise<Product | null> {
    try {
   ...
   * Supprime un produit
   */
  static async deleteProduct(productId: string, merchantId: string): Promise<void> {
    try {
      // Vérifier que le produit appartient au commerçant
      const product = await prisma.product.findFirst({
        where: {
      ...
      if (activeOrders > 0) {
        throw new Error('Impossible de supprimer un produit avec des commandes en cours')
      }
        ...
        await tx.product.update({
          ...
          data: {
            ...
            status: newQuantity === 0 ? 'OUT_OF_STOCK' : 
                   newQuantity <= product.minStockLevel ? 'ACTIVE' : 'ACTIVE'
          }
        })

        // Créer des alertes si nécessaire
        await this.checkStockAlerts(productId, newQuantity, product.minStockLevel, product.maxStockLevel)
  ...
  static async getInventoryStats(merchantId: string): Promise<InventoryStats> {
    try {
      ...
      // Calculs des valeurs
      const totalValue = productValues.reduce((sum, p) => sum + (p.price * p.stockQuantity), 0)
      const totalCostValue = productValues.reduce((sum, p) => sum + (p.costPrice * p.stockQuantity), 0)
      ...
      // Répartition par catégories
      const totalCategoryProducts = categories.reduce((sum, cat) => sum + cat._count, 0)
      const categoryBreakdown = categories.map(cat => ({
        category: cat.category,
        productCount: cat._count,
        totalValue: cat._sum.stockQuantity || 0,
        percentage: totalCategoryProducts > 0 ? (cat._count / totalCategoryProducts) * 100 : 0
      }))
   ...
   */
  static async getInventoryAlerts(merchantId: string): Promise<InventoryAlert[]> {
    try {
...
export const productSchema = z.object({
  ...
  minStockLevel: z.number().int().min(0, 'Le niveau minimum ne peut pas être négatif'),
  maxStockLevel: z.number().int().min(0, 'Le niveau maximum ne peut pas être négatif'),
