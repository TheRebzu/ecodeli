...
export class CartDropService {
   ...
   */
  static async getConfiguration(userId: string): Promise<CartDropConfiguration | null> {
    ...
    return {
      ...
      deliveryZones: Array.isArray(config.deliveryZones) ? config.deliveryZones as DeliveryZone[] : [],
      timeSlots: Array.isArray(config.timeSlots) ? config.timeSlots as TimeSlot[] : this.getDefaultTimeSlots(),
      maxOrdersPerSlot: config.maxOrdersPerSlot,
      minOrderAmount: 20.00, // Valeur par défaut
   ...
   */
  static async updateConfiguration(userId: string, config: CartDropConfiguration): Promise<void> {
    const merchant = await prisma.merchant.findUnique({
      where: { userId }
    })
   ...
   */
  static async getOrders(userId: string, filters: {
    status?: string
    dateFrom?: string
    dateTo?: string
    page?: number
  ...
  static async createOrder(userId: string, orderData: {
    ...
    const deliveryFee = this.calculateDeliveryFee(orderData.deliveryAddress, config.deliveryZones)
   ...
   */
  static async updateOrderStatus(userId: string, orderId: string, status: string): Promise<void> {
    const merchant = await prisma.merchant.findUnique({
      where: { userId }
    })
  ...
  static async getStats(userId: string): Promise<CartDropStats> {
    ...
    const activeOrders = orders.filter(o => ['PENDING', 'CONFIRMED', 'PREPARING', 'READY_FOR_PICKUP', 'IN_TRANSIT'].includes(o.status)).length
   ...
   */
  private static calculateDeliveryFee(address: string, zones: DeliveryZone[]): number {
   ...
   */
  static async checkSlotAvailability(userId: string, scheduledDate: Date): Promise<boolean> {
    const merchant = await prisma.merchant.findUnique({
      where: { userId },
...
export const cartDropConfigSchema = z.object({
  ...
  preparationTime: z.number().min(15, 'Temps de préparation minimum 15 minutes')
})
...
export const updateOrderStatusSchema = z.object({
  status: z.enum(['PENDING', 'CONFIRMED', 'PREPARING', 'READY_FOR_PICKUP', 'IN_TRANSIT', 'DELIVERED', 'CANCELLED'])
})