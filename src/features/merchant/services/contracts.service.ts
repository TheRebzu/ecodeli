...
export class ContractsService {
   ...
   */
  static async getCurrentContract(userId: string): Promise<ContractSummary | null> {
    ...
    const daysUntilExpiry = contract.validUntil 
      ? Math.ceil((contract.validUntil.getTime() - now.getTime()) / (1000 * 60 * 60 * 24))
      : undefined
    ...
    return {
      ...
      deliveryZones: Array.isArray(contract.deliveryZones) ? contract.deliveryZones : [],
   ...
   */
  static async getContractAmendments(userId: string): Promise<ContractAmendment[]> {
    const merchant = await prisma.merchant.findUnique({
      where: { userId },
      include: {
        contract: {
   ...
   */
  static async getBillingCycles(userId: string, filters: {
    status?: string
    year?: number
    page?: number
    limit?: number
   ...
   */
  static async signContract(userId: string, contractId: string, signature: string): Promise<void> {
    const merchant = await prisma.merchant.findUnique({
      where: { userId },
    ...
    const signatureHash = Buffer.from(`${userId}-${contractId}-${Date.now()}`).toString('base64')
   ...
   */
  static async requestRenewal(userId: string, renewalData: {
  ...
  static async getContractStats(userId: string): Promise<ContractStats> {
    ...
    const monthlyRevenue = merchant.payments.reduce((sum, payment) => 
    ...
    return {
      ...
      pendingSignature: merchant.contract && !merchant.contract.merchantSignedAt ? 1 : 0,
      expiringContracts: merchant.contract?.validUntil && 
        merchant.contract.validUntil.getTime() - Date.now() < 30 * 24 * 60 * 60 * 1000 ? 1 : 0,
      totalCommissions: monthlyCommissions,
      averageCommissionRate: merchant.contract?.commissionRate || 0,
      monthlyRevenue,
      contractTypes: merchant.contract ? [{ type: merchant.contract.type, count: 1 }] : []
    }
  }
    ...
    return [
      {
        ...
        description: 'Contrat de base pour commerçants avec conditions standards',
        commissionRate: 15.0,
        setupFee: 0,
        monthlyFee: 29.99,
        features: [
   ...
   */
  static async generateContractPDF(userId: string, contractId: string): Promise<string> {
    const merchant = await prisma.merchant.findUnique({
      where: { userId },
      include: {
        contract: true,
