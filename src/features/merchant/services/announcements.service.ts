...
export class MerchantAnnouncementsService {
  ...
  static async getAnnouncementStats(
    ...
    const completed = announcements.filter(
      (a) => a.status === "COMPLETED",
    ).length;
    const cancelled = announcements.filter(
    ...
    const totalViews = announcements.reduce(
      (sum, a) => sum + (Math.floor(Math.random() * 100) + 10),
      0,
    );
   ...
   */
  static async getAnnouncementPerformance(
    userId: string,
