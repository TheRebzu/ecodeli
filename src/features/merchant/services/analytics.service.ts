...
export class MerchantAnalyticsService {
  ...
  static async getRevenueMetrics(
    ...
    try {
      ...
      const previousStartDate = new Date(
        timeRange.startDate.getTime() - periodDuration,
      );
      const previousEndDate = new Date(
        timeRange.endDate.getTime() - periodDuration,
      );
      ...
      const previousPeriodRevenue = previousOrders.reduce(
        (sum, order) => sum + order.totalAmount,
        0,
      );
      const growthRate =
      ...
      // Revenus par jour
      const revenueByDay = this.groupRevenueByPeriod(
        currentOrders,
        timeRange.period,
      );
      ...
      currentOrders.forEach((order) => {
        order.items.forEach((item) => {
          const current = productRevenue.get(item.name) || {
            revenue: 0,
            quantity: 0,
            name: item.name,
          };
  ...
  static async getCustomerMetrics(
    ...
    try {
      ...
      const customerLifetimeValues = await this.calculateCustomerLifetimeValues(
        merchantId,
        Array.from(uniqueCustomers),
      );
      const averageCustomerLifetimeValue =
        customerLifetimeValues.length > 0
          ? customerLifetimeValues.reduce((sum, clv) => sum + clv, 0) /
            customerLifetimeValues.length
          : 0;

      // Segmentation clients
      const customersBySegment = await this.segmentCustomers(
        merchantId,
        timeRange,
      );
      ...
      const customerStats = new Map<
        string,
        { orders: number; revenue: number; lastOrder: Date; name: string }
      >();
  ...
  static async getDeliveryMetrics(
    ...
    try {
      ...
      const completedDeliveries = deliveries.filter(
        (d) => d.actualDeliveryDate && d.createdAt,
      );
      const averageDeliveryTime =
        completedDeliveries.length > 0
          ? completedDeliveries.reduce((sum, d) => {
              const deliveryTime =
                d.actualDeliveryDate!.getTime() - d.createdAt.getTime();
      ...
      deliveries.forEach((delivery) => {
        const zone = this.extractZoneFromAddress(
          delivery.announcement.deliveryAddress || "Paris",
        );
        const current = zoneStats.get(zone) || {
          deliveries: 0,
          totalTime: 0,
          successful: 0,
          revenue: 0,
        ...
        if (delivery.status === "DELIVERED") {
          ...
          if (delivery.actualDeliveryDate && delivery.createdAt) {
            current.totalTime +=
              (delivery.actualDeliveryDate.getTime() -
      ...
      const deliveryZonePerformance = Array.from(zoneStats.entries())
        .map(([zone, stats]) => ({
          ...
          successRate:
            stats.deliveries > 0
              ? (stats.successful / stats.deliveries) * 100
              : 0,
      ...
      const deliveryMethodBreakdown = Array.from(methodStats.entries()).map(
        ([method, stats]) => ({
          method,
          count: stats.count,
          percentage:
            totalDeliveries > 0 ? (stats.count / totalDeliveries) * 100 : 0,
  ...
  static async getCartDropMetrics(
    ...
    try {
      ...
      const cartDropRevenue = cartDropOrders.reduce(
        (sum, order) => sum + order.totalAmount,
        0,
      );
      const averageCartValue =
        totalCartDropOrders > 0 ? cartDropRevenue / totalCartDropOrders : 0;
      ...
      const zoneStats = new Map<
        string,
        { orders: number; revenue: number; totalFees: number }
      >();

      cartDropOrders.forEach((order) => {
        const postalCode = this.extractPostalCodeFromAddress(
          order.deliveryAddress,
        );
        const current = zoneStats.get(postalCode) || {
          orders: 0,
          revenue: 0,
          totalFees: 0,
        };
      ...
      const zonePerformance = Array.from(zoneStats.entries())
        .map(([postalCode, stats]) => ({
          ...
          revenue: stats.revenue,
          averageDeliveryFee:
            stats.orders > 0 ? stats.totalFees / stats.orders : 0,
        }))
        .sort((a, b) => b.orders - a.orders);
  ...
  private static async calculateCustomerLifetimeValues(
    merchantId: string,
    customerIds: string[],
  ): Promise<number[]> {
  ...
  private static async segmentCustomers(
    merchantId: string,
    timeRange: AnalyticsTimeRange,
  ) {
    ...
    const totalCustomers = segmentResults.reduce(
