...
export function useMerchantPromotions(merchantId: string): UsePromotionsReturn {
  ...
  const setFilters = useCallback(
    (newFilters: Partial<UsePromotionsState["filters"]>) => {
      setState((prev) => ({
        ...prev,
  ...
  const createPromotion = useCallback(
    async (promotionData: any): Promise<Promotion> => {
      try {
        const response = await fetch("/api/merchant/promotions", {
          method: "POST",
          headers: {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorData.error || "Erreur lors de la création de la promotion",
          );
        }
    ...
    async (
      ...
      try {
        ...
        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(
            errorData.error || "Erreur lors de la mise à jour de la promotion",
          );
        }
    ...
    async (promotionId: string): Promise<void> => {
      try {
        ...
        if (!response.ok) {
          const errorData = await response.json();
          throw new Error(
            errorData.error || "Erreur lors de la suppression de la promotion",
          );
        }
  ...
  const togglePromotionStatus = useCallback(
    async (
      ...
      try {
        const response = await fetch(
          `/api/merchant/promotions/${promotionId}/toggle`,
          {
            method: "PATCH",
            headers: {
              "Content-Type": "application/json",
  ...
  const validatePromotionCode = useCallback(
    async (code: string, orderId: string): Promise<any> => {
      try {
  ...
  const duplicatePromotion = useCallback(
    async (promotionId: string): Promise<Promotion> => {
      try {
        const response = await fetch(
          `/api/merchant/promotions/${promotionId}/duplicate`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
...
export function usePromotion(promotionId: string, merchantId: string) {
  ...
  const fetchPromotion = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `/api/merchant/promotions/${promotionId}?merchantId=${merchantId}`,
      );
...
export function usePromotionStats(merchantId: string, period?: string) {
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `/api/merchant/promotions/stats?merchantId=${merchantId}&period=${period || "30d"}`,
      );
...
export function useCampaignTemplates() {
  ...
  const createFromTemplate = useCallback(
    async (templateId: string, customData: any) => {
      try {
...
export function usePromotionUsage(promotionId: string) {
  ...
  const fetchUsages = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `/api/merchant/promotions/${promotionId}/usage?page=${pagination.page}&limit=${pagination.limit}`,
      );
