 ...
 */
export function useMerchantNotifications(merchantId: string): UseNotificationsReturn {
   ...
   */
  const setFilters = useCallback((newFilters: Partial<UseNotificationsState['filters']>) => {
    setState(prev => ({
      ...prev,
   ...
   */
  const markAllAsRead = useCallback(async (category?: NotificationCategory): Promise<void> => {
    try {
  ...
  const refreshStats = useCallback(async (): Promise<void> => {
    try {
      const response = await fetch(`/api/merchant/notifications/stats?merchantId=${merchantId}`)
...
export function useNotificationPreferences(merchantId: string) {
  ...
  const fetchPreferences = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/merchant/notifications/preferences?merchantId=${merchantId}`)
  ...
  const updatePreferences = useCallback(async (updates: Partial<NotificationPreferences>): Promise<void> => {
    try {
      const response = await fetch('/api/merchant/notifications/preferences', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json'
        },
...
export function useBusinessAlerts(merchantId: string) {
  ...
  const resolveAlert = useCallback(async (alertId: string): Promise<void> => {
    try {
      const response = await fetch(`/api/merchant/notifications/alerts/${alertId}/resolve`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json'
        },
...
export function useRealTimeNotifications(merchantId: string) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const response = await fetch(`/api/merchant/notifications/latest?merchantId=${merchantId}`)
        
        if (response.ok) {
          const data = await response.json()
          if (data.notification && data.notification.id !== lastNotification?.id) {
...
export function useNotificationBadge(merchantId: string) {
  ...
  const updateUnreadCount = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/merchant/notifications/unread-count?merchantId=${merchantId}`)
