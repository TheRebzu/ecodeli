...
export function useMerchantCartDrop(): UseMerchantCartDropReturn {
  ...
  const updateConfiguration = async (
    newConfig: CartDropConfiguration,
  ): Promise<boolean> => {
    try {
      const response = await fetch("/api/merchant/cart-drop/config", {
        method: "PUT",
  ...
  const updateOrderStatus = async (
    ...
    try {
      const response = await fetch(
        `/api/merchant/cart-drop/orders/${orderId}/status`,
        {
          method: "PATCH",
          headers: {
            "Content-Type": "application/json",
  ...
  const checkSlotAvailability = async (
    ...
    try {
      const response = await fetch(
        `/api/merchant/cart-drop/availability?date=${scheduledDate.toISOString()}`,
      );
      if (!response.ok) {
        return false;
      }
  ...
  const calculateDeliveryFee = (postalCode: string): number => {
    ...
    const zone = config.deliveryZones.find(
      (z) => z.postalCode === postalCode && z.isActive,
    );
...
export function useDeliveryZones(initialZones: DeliveryZone[] = []) {
  ...
  const updateZone = (index: number, updatedZone: DeliveryZone) => {
    setZones((prev) =>
      prev.map((zone, i) => (i === index ? updatedZone : zone)),
    );
  };
...
export function useTimeSlots(initialSlots: TimeSlot[] = []) {
  ...
  const updateSlot = (index: number, updatedSlot: TimeSlot) => {
    setSlots((prev) =>
      prev.map((slot, i) => (i === index ? updatedSlot : slot)),
    );
  };
