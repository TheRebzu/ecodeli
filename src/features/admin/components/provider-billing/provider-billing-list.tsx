...
export function ProviderBillingList({ statusFilter, searchTerm, monthFilter }: BillingListProps) {
  ...
  const handleDownloadInvoice = async (invoiceId: string) => {
    try {
      const response = await fetch(`/api/admin/provider-billing/${invoiceId}/download`)
      if (response.ok) {
              ...
              <TableRow key={invoice.id}>
                ...
                <TableCell>
                  <div className="flex items-center gap-2">
                    ...
                    {new Date(parseInt(invoice.year), parseInt(invoice.month) - 1).toLocaleDateString('fr-FR', {
                      month: 'long',
                      year: 'numeric'
                    })}
                  </div>
                ...
                <TableCell>
                  <Badge variant="outline">{invoice.servicesCount} {t('invoices.services')}</Badge>
                </TableCell>
                <TableCell>
                ...
                <TableCell>
                  <div className="flex items-center gap-2">
                    <Button
                      ...
                      disabled={invoice.status === 'GENERATED' || invoice.status === 'PAID'}
                    >
