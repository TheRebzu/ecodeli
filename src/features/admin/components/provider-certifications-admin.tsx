...
export function ProviderCertificationsAdmin({
  providerId,
}: ProviderCertificationsAdminProps) {
  ...
  const [selectedCertification, setSelectedCertification] =
    useState<ProviderCertification | null>(null);
  ...
  const fetchCertifications = async () => {
    try {
      ...
      const response = await execute(
        `/api/admin/provider-certifications?${params.toString()}`,
      );
      if (response) {
  ...
  const openValidationDialog = (certification: ProviderCertification) => {
    ...
    setValidationStatus(
      certification.status === "APPROVED" ? "APPROVED" : "REJECTED",
    );
          ...
          <DialogHeader>
            <DialogTitle>
              {selectedCertification
                ? `Validation: ${selectedCertification.name}`
                : "Validation"}
            </DialogTitle>
            <DialogDescription>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) =>
                    setValidationStatus(value as "APPROVED" | "REJECTED")
                  }
                >
                  <SelectTrigger>
                    <SelectValue />
