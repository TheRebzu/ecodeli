...
export class DocumentValidationService {
  ...
  static async validateDocument(data: DocumentValidationData) {
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      if (document.user.deliverer && status === "APPROVED") {
        ...
        const allApproved = allDelivererDocs.every(
          (doc) => doc.id === documentId || doc.validationStatus === "APPROVED",
        );
        ...
        if (allApproved) {
          ...
          await tx.notification.create({
            data: {
              ...
              message:
                "Félicitations ! Votre compte livreur a été validé et activé. Vous pouvez maintenant accepter des livraisons.",
      ...
      if (document.user.provider && status === "APPROVED") {
        ...
        const allApproved = allProviderDocs.every(
          (doc) => doc.id === documentId || doc.validationStatus === "APPROVED",
        );
        ...
        if (allApproved) {
          ...
          await tx.notification.create({
            data: {
              ...
              message:
                "Félicitations ! Votre compte prestataire a été validé et activé. Vous pouvez maintenant proposer vos services.",
              data: { activatedAt: new Date() },
            },
          });
        }
  ...
  static async getValidationStats(dateFrom?: Date, dateTo?: Date) {
    ...
    const byTypeFormatted = byType.reduce(
      (acc, item) => {
        if (!acc[item.type]) {
        ...
        acc[item.type][
          item.validationStatus.toLowerCase() as keyof (typeof acc)[typeof item.type]
        ] = item._count;
        return acc;
      },
      {} as Record<
   ...
   */
  static async forceActivateDeliverer(
    delivererId: string,
    adminId: string,
    reason: string,
  ) {
