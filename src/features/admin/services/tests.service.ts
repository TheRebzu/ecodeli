...
export class TestsService {
    ...
    try {
      ...
      switch (type) {
        case 'welcome':
          ...
          result = await EmailService.sendVerificationEmail(email, 'https://ecodeli.com/verify?token=test', 'fr')
          break
        ...
        case 'password-reset':
          ...
          emailContent = 'Cliquez sur le lien pour réinitialiser votre mot de passe.'
          html = `<h2>Réinitialisation du mot de passe</h2><p>Cliquez sur le lien ci-dessous pour réinitialiser votre mot de passe :</p>`
          result = await EmailService.sendPasswordResetEmail(email, 'https://ecodeli.com/reset-password?token=test', 'fr')
          break
        ...
        default:
          ...
          result = await EmailService.sendGenericEmail(email, emailSubject, html)
          break
      }
      ...
      console.error(`❌ Erreur envoi email de test: ${error instanceof Error ? error.message : 'Erreur inconnue'}`)
  ...
  static async sendTestNotification(data: NotificationTestRequest) {
    try {
      const { title, message, targetType, targetValue, includeImage, imageUrl } = data
      ...
      switch (targetType) {
        ...
        case 'role':
          ...
          if (users.length === 0) {
            throw new Error(`Aucun utilisateur trouvé avec le rôle ${targetValue}`)
          }
        ...
        case 'user':
          if (!targetValue) {
            throw new Error('Email utilisateur requis pour le ciblage utilisateur')
          }
      ...
      console.log('📱 Simulation envoi notification OneSignal:', notificationData)
      ...
      // Log de l'erreur (console.log au lieu de prisma.systemLog)
      console.error(`❌ Erreur envoi notification de test: ${error instanceof Error ? error.message : 'Erreur inconnue'}`)

      throw new Error(`Erreur lors de l'envoi de la notification: ${error instanceof Error ? error.message : 'Erreur inconnue'}`)
    }
  }
  ...
  static async checkServicesStatus() {
    ...
    try {
      ...
      status.notifications = { status: 'operational', message: 'Service disponible' }
    } catch (error) {
      status.notifications = { status: 'error', message: 'Service indisponible' }
    }
