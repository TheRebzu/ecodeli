...
export function DocumentGenerator({ 
  ...
  const handleGenerate = async () => {
    if (!selectedType || !entityId) {
      toast({
        title: "Erreur",
        description: "Veuillez sélectionner un type de document et saisir un ID d'entité",
        variant: "destructive"
      })
      return
    }
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            {selectedType && (
              <p className="text-sm text-muted-foreground mt-1">
                {selectedType === 'delivery-slip' && 'ID de l\'annonce à documenter'}
                {selectedType === 'contract' && 'ID du contrat à générer'}
                {selectedType === 'invoice' && 'ID du paiement à facturer'}
                {selectedType === 'certificate' && 'ID de la livraison terminée'}
              </p>
            )}
          </div>
          ...
          {selectedDocType && (
            <div className={`p-4 rounded-lg border-2 border-dashed ${selectedDocType.color.replace('bg-', 'border-')} bg-opacity-10`}>
              <div className="flex items-center gap-3">
                <div className={`p-2 rounded-lg ${selectedDocType.color} text-white`}>
              ...
              {generatedDocs.map((doc, index) => {
                ...
                return (
                  <div key={index} className="flex items-center justify-between p-3 bg-muted rounded-lg">
                    <div className="flex items-center gap-3">
                      <div className={`p-2 rounded-lg ${docType?.color || 'bg-gray-500'} text-white`}>
                        <Icon className="h-4 w-4" />
                      </div>
                      <div>
                        <div className="font-medium">{docType?.label}</div>
                        <div className="text-sm text-muted-foreground">
                          {doc.fileName} • {new Date(doc.generatedAt).toLocaleString('fr-FR')}
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
      ...
      <Card>
        ...
        <CardContent className="text-sm text-muted-foreground space-y-2">
          <p><strong>Bordereau de livraison:</strong> Utilisez l'ID de l'annonce pour générer le document officiel</p>
          <p><strong>Contrat de service:</strong> Utilisez l'ID du contrat entre un prestataire et un client</p>
          <p><strong>Facture:</strong> Utilisez l'ID du paiement pour générer la facture correspondante</p>
          <p><strong>Certificat:</strong> Utilisez l'ID d'une livraison terminée pour certifier la réussite</p>
        </CardContent>
      </Card>
    </div>
  )
