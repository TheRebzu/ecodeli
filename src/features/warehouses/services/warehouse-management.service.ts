...
class WarehouseManagementService {
  ...
  async findOptimalWarehouse(
    ...
    try {
      ...
      for (const warehouse of warehouses) {
        ...
        const capacityScore =
          capacity.availableSlots / (capacity.totalSlots || 1);
  ...
  async createWarehouseTransfer(
    transferData: z.infer<typeof warehouseTransferSchema>,
  ): Promise<any> {
    try {
      ...
      const warehouse = await db.warehouse.findUnique({
        ...
        include: {
          packages: { where: { status: { in: ["STORED", "PREPARING"] } } },
        },
      });
      ...
      const transfer = await db.$transaction(async (tx) => {
        ...
        const position = await this.assignStoragePosition(
    ...
    try {
      ...
      return {
        ...
        fees: {
          ...
          totalFees: tracking.storageFeePaid
            ? 0
            : this.calculateStorageFee(tracking.arrivalDate),
   ...
   */
  private async assignStoragePosition(
    warehouseId: string,
  ): Promise<{ zone: string; shelf: string; position: string }> {
   ...
   */
  private async scheduleWarehouseNotifications(
    transferId: string,
  ): Promise<void> {
