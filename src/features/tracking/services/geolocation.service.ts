...
export class GeolocationService {
   ...
   */
  static async getCurrentDeliveryPosition(
    deliveryId: string,
  ...
  static async startDeliveryTracking(
    ...
    try {
      ...
      if (existingSession) {
        throw new Error(
          "Une session de tracking est déjà active pour cette livraison",
        );
      }
      ...
      if (delivery.client) {
        await NotificationService.createNotification({
          ...
          title: "📍 Suivi activé",
          message:
            "Vous pouvez maintenant suivre votre livraison en temps réel.",
          data: {
            deliveryId,
  ...
  private static async updateSessionStats(
    ...
    if (lastPosition) {
      ...
      const timeDiff =
        newPosition.timestamp.getTime() - lastPosition.timestamp.getTime();
  ...
  private static async checkProximityAlerts(
    ...
    if (deliveryCoords.lat && deliveryCoords.lng) {
      ...
      if (
        distanceToDelivery <= alertDistance &&
  ...
  private static async calculateFinalStats(sessionId: string): Promise<void> {
    ...
    const averageSpeed =
      totalTime > 0 ? totalDistance / 1000 / (totalTime / (1000 * 60 * 60)) : 0;
