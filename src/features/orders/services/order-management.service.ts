...
export class OrderManagementService {
  ...
  static async createDeliveryOrder(orderData: {
    ...
    try {
      ...
      await NotificationService.createNotification({
        ...
        message: 'Votre demande de livraison a été enregistrée et recherche un livreur.',
        data: {
  ...
  static async createServiceOrder(orderData: {
    ...
    try {
      ...
      const booking = await prisma.$transaction(async (tx) => {
        ...
        // Réserver le créneau
        const { AvailabilityService } = await import('@/features/calendar/services/availability.service')
      ...
      await NotificationService.notifyNewBooking(
        ...
        booking.id,
        service.name,
        booking.client?.user.profile?.firstName + ' ' + booking.client?.user.profile?.lastName || 'Client',
  ...
  private static async updateServiceStatus(
    ...
    // Envoyer les notifications appropriées
    const message = this.getServiceStatusMessage(newStatus, booking.service.name)
   ...
   */
  private static async triggerDeliveryMatching(deliveryId: string): Promise<void> {
    try {
      // Importer le service de matching
      const { SmartMatchingService } = await import('@/features/matching/services/smart-matching.service')
   ...
   */
  private static getServiceStatusMessage(status: string, serviceName: string): string {
    ...
    try {
      ...
      return {
        deliveries: await prisma.delivery.findMany({
          where: { clientId },
          include: { announcement: true, deliverer: { include: { user: { include: { profile: true } } } } },
          orderBy: { createdAt: 'desc' },
          take: Math.floor(limit / 2)
        }),
        services: await prisma.booking.findMany({
          where: { clientId },
          include: { service: { include: { provider: { include: { user: { include: { profile: true } } } } } } },
          orderBy: { createdAt: 'desc' },
          take: Math.floor(limit / 2)
        })
      }
