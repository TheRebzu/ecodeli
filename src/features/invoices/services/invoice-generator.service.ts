...
export class InvoiceGeneratorService {
   ...
   */
  static async generateAnnouncementInvoice(
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          "La facture ne peut être générée que pour une livraison terminée",
        );
      }
   ...
   */
  static async generateProviderMonthlyInvoice(
    providerId: string,
    invoiceId: string,
  ): Promise<string> {
    try {
  ...
  private static addInvoiceInfo(pdf: jsPDF, invoice: any) {
    ...
    pdf.text(
      `Période: ${invoice.billingPeriodStart?.toLocaleDateString("fr-FR")} - ${invoice.billingPeriodEnd?.toLocaleDateString("fr-FR")}`,
      20,
      85,
    );
    pdf.text(
  ...
  private static addDeliveryInfo(pdf: jsPDF, delivery: any) {
    ...
    pdf.text(
      `Date: ${delivery.scheduledAt.toLocaleDateString("fr-FR")}`,
      20,
      85,
    );
    if (delivery.deliveredAt) {
      pdf.text(
        `Livré le: ${delivery.deliveredAt.toLocaleDateString("fr-FR")}`,
        20,
        90,
      );
    }
  ...
  private static addDeliveryParties(pdf: jsPDF, delivery: any) {
    ...
    if (deliverer) {
      pdf.text(
        `${deliverer.firstName || ""} ${deliverer.lastName || ""}`.trim(),
        120,
        120,
      );
    } else {
  ...
  private static addDeliveryAmounts(pdf: jsPDF, delivery: any) {
    ...
    if (delivery.payment?.tip && delivery.payment.tip > 0) {
      ...
      pdf.text("TOTAL:", 130, startY + 16);
      pdf.text(
        `${(delivery.announcement.price + delivery.payment.tip).toFixed(2)}€`,
        170,
        startY + 16,
      );
    }
  ...
  private static addFooter(pdf: jsPDF) {
    ...
    pdf.text(
      "EcoDeli - Plateforme de crowdshipping éco-responsable",
      20,
      pageHeight - 20,
    );
    pdf.text(
      "Conditions générales disponibles sur www.ecodeli.fr",
      20,
      pageHeight - 15,
    );
   ...
   */
  static async generateMerchantContract(
    merchantId: string,
    contractData: any,
  ): Promise<string> {
    try {
  ...
  private static addContractParties(
    pdf: jsPDF,
    merchant: any,
    contractData: any,
  ) {
    ...
    const clauses = [
      {
        ...
        content:
          "Le présent contrat a pour objet de définir les conditions dans lesquelles EcoDeli met à disposition du Commerçant sa plateforme de crowdshipping.",
      },
      {
      ...
      },
      {
        ...
        content:
          "Le Commerçant s'engage à respecter les conditions générales d'utilisation d'EcoDeli et à maintenir un service de qualité.",
      },
    ];
  ...
  private static addAnnouncementInvoiceInfo(pdf: jsPDF, announcement: any) {
    ...
    if (announcement.delivery?.trackingNumber) {
      pdf.text(
        `Numéro de suivi: ${announcement.delivery.trackingNumber}`,
        20,
        95,
      );
    }
  ...
  private static addAnnouncementDeliveryDetails(pdf: jsPDF, announcement: any) {
    ...
    pdf.setFont("helvetica", "normal");
    const deliveryLines = pdf.splitTextToSize(
    ...
    if (announcement.delivery?.deliveredAt) {
      ...
      pdf.setFont("helvetica", "normal");
      pdf.text(
        new Date(announcement.delivery.deliveredAt).toLocaleDateString("fr-FR"),
        80,
        currentY,
      );
      currentY += 10;
    ...
    const typeText =
      announcement.type === "PACKAGE"
        ...
        : announcement.type === "DOCUMENT"
          ...
          : announcement.type;
    pdf.text(
      typeText + (announcement.isUrgent ? " (Urgent)" : ""),
      80,
      currentY,
    );
  }
  ...
  private static addAnnouncementAmounts(pdf: jsPDF, announcement: any) {
    ...
    pdf.text(
      "Paiement sécurisé - Transaction référence: " +
        (announcement.payment?.stripePaymentId?.slice(-12) || "N/A"),
      20,
      currentY + 5,
    );
    ...
    pdf.text(
      "• Service de mise en relation entre particuliers",
      20,
      currentY + 10,
    );
  }
...
export async function generateProviderInvoicePDF(
  providerId: string,
  invoiceId: string,
) {
  // Utilise la méthode statique de la classe
  return InvoiceGeneratorService.generateProviderMonthlyInvoice(
    providerId,
    invoiceId,
  );
}
