...
class AdvancedAnalyticsService {
   ...
   */
  async getAnnouncementMetrics(timeframe: AnalyticsTimeframe): Promise<AnnouncementMetrics> {
    try {
      ...
      // Métriques par type
      const byType = announcements.reduce((acc, ann) => {
        acc[ann.type] = (acc[ann.type] || 0) + 1
        return acc
      }, {} as Record<AnnouncementType, number>)

      // Métriques par statut
      const byStatus = announcements.reduce((acc, ann) => {
        acc[ann.status] = (acc[ann.status] || 0) + 1
        return acc
      }, {} as Record<AnnouncementStatus, number>)

      // Prix moyen et revenus
      const totalRevenue = announcements.reduce((sum, ann) => sum + (ann.finalPrice || ann.basePrice), 0)
      const averagePrice = totalRevenue / totalAnnouncements

      // Taux de complétion
      const completedCount = announcements.filter(ann => ann.status === 'COMPLETED').length
      const completionRate = completedCount / totalAnnouncements * 100

      // Taux de matching
      const matchedCount = announcements.filter(ann => ann.matches.length > 0).length
      const matchingRate = matchedCount / totalAnnouncements * 100

      // Temps de livraison moyen
      const deliveredAnnouncements = announcements.filter(ann => ann.delivery?.deliveredAt)
      const averageDeliveryTime = deliveredAnnouncements.length > 0 ?
        ...
        }, 0) / deliveredAnnouncements.length : 0
    ...
    try {
      ...
      const averageMatchTime = matches.length > 0 ?
        matches.reduce((sum, match) => {
          const timeDiff = match.createdAt.getTime() - match.announcement.createdAt.getTime()
      ...
      const averageMatchScore = matches.length > 0 ?
        matches.reduce((sum, m) => sum + m.globalScore, 0) / matches.length * 100 : 0
      ...
      const onTimeDeliveryRate = onTimeDeliveries.length / deliveries.length * 100

      const allReviews = deliveries.flatMap(d => d.reviews)
      const averageDeliveryRating = allReviews.length > 0 ?
        allReviews.reduce((sum, r) => sum + r.rating, 0) / allReviews.length : 0

      const deliverySuccessRate = deliveries.length / matches.filter(m => m.status === 'ACCEPTED').length * 100
   ...
   */
  async getGeographicMetrics(timeframe: AnalyticsTimeframe): Promise<GeographicMetrics> {
    try {
      const { startDate, endDate } = timeframe

      // Top villes par nombre d'annonces
      const cityStats = await db.$queryRaw<Array<{city: string, count: bigint, revenue: number}>>`
        SELECT 
      ...
      const averageDistance = announcements.length > 0 ?
        announcements.reduce((sum, ann) => sum + (ann.distance || 0), 0) / announcements.length : 0

      // Top routes
      const routeStats = await db.$queryRaw<Array<{
        pickup_city: string, 
      ...
      const warehouseUtilization = warehouses.map(warehouse => ({
        ...
        name: warehouse.name,
        utilizationRate: Math.round((warehouse.packages.length / (warehouse.totalSlots || 1000)) * 100),
   ...
   */
  async getUserBehaviorMetrics(timeframe: AnalyticsTimeframe): Promise<UserBehaviorMetrics> {
    try {
      const { startDate, endDate } = timeframe
      ...
      const registrationConversionRate = newUsersWithAnnouncements / newUsers * 100
      ...
      const averageAnnouncementsPerUser = activeUsers > 0 ? totalAnnouncements / activeUsers : 0
      ...
      const userRetentionRate = currentPeriodUsers.length > 0 ? retainedUsers / currentPeriodUsers.length * 100 : 0
      ...
      const averageRevenuePerUser = activeUsers > 0 ? (totalRevenue._sum.amount || 0) / activeUsers : 0
      ...
      const subscriptionConversionRate = newUsers > 0 ? subscriptionUsers / newUsers * 100 : 0
      ...
      return {
        ...
        registrationConversionRate: Math.round(registrationConversionRate * 100) / 100,
        sourceBreakdown,
        averageSessionDuration: avgSessionDuration,
        averageAnnouncementsPerUser: Math.round(averageAnnouncementsPerUser * 100) / 100,
      ...
      throw new Error('Erreur lors du calcul des métriques de comportement utilisateur')
    }
  }
    ...
    try {
      ...
      const demandForecast = Array.from({ length: 7 }, (_, i) => {
        ...
        const baseLoad = historicalData.length > 0 ? 
          historicalData.reduce((sum, d) => sum + d._count, 0) / historicalData.length : 10
        
        const weekdayMultiplier = [0.8, 1.2, 1.1, 1.0, 1.3, 0.9, 0.7][date.getDay()]
      ...
      const avgDailyLoad = historicalData.length > 0 ?
        historicalData.reduce((sum, d) => sum + d._count, 0) / 7 : 10
  ...
  async getComprehensiveDashboard(timeframe: AnalyticsTimeframe): Promise<{
    ...
    try {
      const [announcements, performance, geographic, userBehavior, predictive] = await Promise.all([
      ...
      throw new Error('Erreur lors de la génération du tableau de bord complet')
    }
  }
}
