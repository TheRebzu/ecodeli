...
export class AnalyticsService {
   ...
   */
  static async getDashboardStats(timeframe: AnalyticsTimeframe): Promise<DashboardStats> {
    const [
      totalUsers,
      activeDeliverers,
      deliveriesStats,
    ...
    const growthMetrics = {
      ...
      deliveriesGrowth: this.calculateGrowth(deliveriesStats._count.id, previousPeriodStats.deliveries),
      revenueGrowth: this.calculateGrowth(revenueStats._sum.amount || 0, previousPeriodStats.revenue)
    }
   ...
   */
  static async getPerformanceMetrics(timeframe: AnalyticsTimeframe): Promise<PerformanceMetrics> {
    const [
      deliveryStats,
      avgDeliveryTime,
      avgResponseTime,
    ...
    const successfulDeliveries = deliveryStats.find(stat => stat.status === 'DELIVERED')?._count.id || 0
   ...
   */
  static async getRevenueBreakdown(timeframe: AnalyticsTimeframe): Promise<RevenueBreakdown> {
   ...
   */
  static async getGeographicStats(timeframe: AnalyticsTimeframe): Promise<GeographicStats[]> {
   ...
   */
  private static async getPreviousPeriodStats(timeframe: AnalyticsTimeframe): Promise<any> {
   ...
   */
  private static async getRepeatCustomerRate(timeframe: AnalyticsTimeframe): Promise<number> {
   ...
   */
  private static async generateAnalyticsPDF(data: any, filename: string): Promise<string> {
