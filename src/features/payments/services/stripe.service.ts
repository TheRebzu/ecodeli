...
export class StripeService {
    ...
    try {
      ...
      await prisma.$transaction(async (tx) => {
        ...
        if (payment.type === "DELIVERY" && payment.delivery) {
          ...
          await this.processStorageRentalPayment(
            tx,
            payment.storageRental,
            payment.amount,
          );
        }
      });
   ...
   */
  private static async processDeliveryPayment(
    tx: any,
    delivery: any,
    amount: number,
  ) {
   ...
   */
  private static async processBookingPayment(
    tx: any,
    booking: any,
    amount: number,
  ) {
    ...
    if (booking.service.provider.wallet) {
      ...
      await tx.walletTransaction.create({
        data: {
          ...
          balanceAfter:
            booking.service.provider.wallet.balance + providerAmount,
        },
      });
    }
   ...
   */
  private static async processStorageRentalPayment(
    tx: any,
    rental: any,
    amount: number,
  ) {
