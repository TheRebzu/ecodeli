      ...
      const calculateStats = (deliveries: any[]): SubscriptionUsage => {
        return {
          ...
          savings: deliveries.reduce((total, delivery) => {
            const basePrice =
              delivery.announcement?.basePrice || delivery.price || 0;
            const finalPrice =
              delivery.announcement?.finalPrice || delivery.price || basePrice;
            return total + Math.max(0, basePrice - finalPrice);
          }, 0),
          priorityShipments: deliveries.filter(
            (d) =>
              d.announcement?.isUrgent || d.announcement?.priority === "HIGH",
          ).length,
          insuranceUsed: deliveries.reduce((total, delivery) => {
            return (
              total +
              (delivery.payment?.insuranceClaim || delivery.insuranceFee || 0)
            );
          }, 0),
        };
      };
