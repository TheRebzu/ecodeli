...
export class ClientDashboardService {
  ...
  async getDashboardData(userId: string): Promise<ClientDashboardData> {
    ...
    const [announcements, bookings, storageBoxes, deliveries] =
      await Promise.all([
    ...
    try {
      ...
      return announcements.map((announcement) => ({
        ...
        deliverer: announcement.delivery?.deliverer
          ...
          : null,
        trackingCode:
          announcement.delivery?.trackingNumber ||
          `TRK${announcement.id.slice(-6).toUpperCase()}`,
        estimatedDelivery:
          announcement.delivery?.deliveryDate ||
          new Date(Date.now() + 24 * 60 * 60 * 1000),
      }));
    } catch (error) {
      console.log(
        "⚠️ [DashboardService] Erreur récupération annonces:",
        error.message,
      );
      return [];
    }
  ...
  async getRecentBookings(
    ...
    try {
      ...
      console.log(
        "⚠️ [DashboardService] Erreur récupération bookings:",
        error.message,
      );
      return [];
    }
  ...
  async getActiveStorageBoxes(
    ...
    try {
      ...
      console.log(
        "⚠️ [DashboardService] Erreur récupération storage boxes:",
        error.message,
      );
      return [];
    }
  ...
  async getActiveDeliveries(
    ...
    try {
      ...
      console.log(
        "⚠️ [DashboardService] Erreur récupération deliveries:",
        error.message,
      );
      return [];
    }
  ...
  async getNotifications(
    ...
    return notifications.map((notif) => ({
      ...
      actionUrl: notif.data
        ? (notif.data as any).actionUrl || undefined
        : undefined,
   ...
   * Calculer les économies liées à l'abonnement
   */
  private calculateSubscriptionSavings(
    subscriptionPlan: string,
    totalSpent: number,
  ): number {
    const discountRates = {
    ...
    const rate =
      discountRates[subscriptionPlan as keyof typeof discountRates] || 0;
  ...
  private categorizeNotification(
    type: string,
  ): "delivery" | "booking" | "payment" | "storage" | "system" {
