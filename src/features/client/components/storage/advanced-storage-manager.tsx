...
export default function AdvancedStorageManager({
  clientId,
}: AdvancedStorageManagerProps) {
  ...
  const confirmPayment = async (rentalId: string) => {
    try {
      const response = await fetch(
        `/api/client/storage-boxes/rentals/${rentalId}/confirm-payment`,
        {
          method: "POST",
    ...
    try {
      ...
      if (boxesRes.ok) {
        ...
        const mappedBoxes = (boxesData.boxes || []).map((box: any) => {
          ...
          const config =
            sizeConfig[box.size as keyof typeof sizeConfig] ||
            sizeConfig.MEDIUM;
  ...
  const handleRentBox = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(
          "Erreur: " +
            (errorData.error || "Erreur lors de la création de la location"),
        );
      }
    } catch (error) {
  ...
  const handleEndRental = async (rentalId: string) => {
    try {
      const response = await fetch(
        `/api/client/storage-boxes/rentals/${rentalId}/end`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const getAvailabilityBadge = (availability: string) => {
    const config = {
      ...
      occupied: {
        ...
        label: t("availability.occupied"),
      },
      maintenance: {
        color: "bg-orange-100 text-orange-800",
        label: t("availability.maintenance"),
      },
      reserved: {
  ...
  const getStatusBadge = (status: string) => {
    const config = {
      active: {
        color: "bg-green-100 text-green-800",
        label: t("rental_status.active"),
      },
      completed: {
        color: "bg-gray-100 text-gray-800",
        label: t("rental_status.completed"),
      },
      cancelled: {
        color: "bg-red-100 text-red-800",
        label: t("rental_status.cancelled"),
      },
      overdue: {
        color: "bg-orange-100 text-orange-800",
        label: t("rental_status.overdue"),
      },
    };
  ...
  const getTypeBadge = (type: string) => {
    const config = {
      secure: {
        color: "bg-green-100 text-green-800",
        label: t("types.secure"),
      },
      refrigerated: {
        color: "bg-blue-100 text-blue-800",
        label: t("types.refrigerated"),
      },
      standard: {
        color: "bg-gray-100 text-gray-800",
        label: t("types.standard"),
      },
      fragile: {
        color: "bg-yellow-100 text-yellow-800",
        label: t("types.fragile"),
      },
    };
  ...
  const filteredBoxes = storageBoxes.filter((box) => {
    ...
    if (filters.maxPrice && box.dailyRate > parseFloat(filters.maxPrice))
      return false;
    if (
      filters.maxDistance &&
      box.distance &&
      box.distance > parseFloat(filters.maxDistance)
    )
      return false;
    return true;
  });
              ...
              <Card key={box.id} className="hover:shadow-lg transition-shadow">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span>
                      {t("access_hours")}: {box.accessHours?.start || "00:00"} -{" "}
                      {box.accessHours?.end || "23:59"}
                    </span>
                  </div>
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm">
                        {t("access_code")}: {rental.accessCode}
                      </span>
                    </div>
          ...
          {rentals.filter((r) => r.status === "active").length === 0 && (
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-12">
                <Package className="h-12 w-12 text-gray-400 mb-4" />
                <h3 className="text-lg font-semibold mb-2">
                  {t("empty_rentals.title")}
                </h3>
                <p className="text-gray-600 text-center">
                  {t("empty_rentals.description")}
                </p>
              </CardContent>
            </Card>
          )}
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="hours">
                        {t("duration_types.hours")}
                      </SelectItem>
                      <SelectItem value="days">
                        {t("duration_types.days")}
                      </SelectItem>
                      <SelectItem value="weeks">
                        {t("duration_types.weeks")}
                      </SelectItem>
                      <SelectItem value="months">
                        {t("duration_types.months")}
                      </SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              ...
              <div className="flex items-center space-x-2">
                <Switch
                  ...
                  onCheckedChange={(checked) =>
                    setRentalForm({ ...rentalForm, autoExtend: checked })
                  }
                />
