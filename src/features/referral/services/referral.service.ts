...
export class ReferralService {
  ...
  static async createReferralProgram(data: CreateReferralProgramData) {
    try {
      ...
      console.error('Erreur lors de la création du programme de parrainage:', error)
      throw error
    }
  }
   ...
   */
  static async generateReferralCode(programId: string, referrerId: string, customCode?: string) {
    try {
   ...
   */
  static async useReferralCode(code: string, refereeId: string, metadata?: any) {
    try {
      ...
      // Notifications
      await this.notifyReferralCreated(referral, referralCode.referrer, referee)
   ...
   */
  static async validateReferralConditions(referralId: string, activityType: string, value?: number) {
    try {
  ...
  static async createInfluencerApplication(data: InfluencerApplicationData) {
    try {
      ...
      console.error('Erreur lors de la création de la candidature influenceur:', error)
      throw error
    }
  }
   ...
   */
  static async approveInfluencerApplication(programId: string, approvedBy: string) {
    try {
   ...
   */
  private static async checkReferralConditions(referralId: string): Promise<boolean> {
    try {
      ...
      const hasRegistration = activities.some(a => a.activityType === 'REGISTRATION')
      ...
      if (program.referrerReward) {
        await prisma.referralReward.create({
          data: {
            ...
            expiresAt: new Date(Date.now() + program.rewardValidityDays * 24 * 60 * 60 * 1000)
          }
        })
      }
  ...
  private static async createInfluencerLinks(programId: string) {
    try {
      const baseUrls = [
        { url: `${process.env.NEXT_PUBLIC_APP_URL}/register`, description: 'Inscription' },
        { url: `${process.env.NEXT_PUBLIC_APP_URL}/services`, description: 'Services' },
        { url: `${process.env.NEXT_PUBLIC_APP_URL}/pricing`, description: 'Tarifs' }
      ]
   ...
   */
  private static async notifyReferralCreated(referral: any, referrer: any, referee: any) {
    try {
  ...
  private static async notifyInfluencerApplication(application: any) {
    try {
      ...
      console.error('Erreur lors de la notification candidature influenceur:', error)
    }
  }
}