...
class AnnouncementService {
  ...
  async createAnnouncement(
    ...
    try {
      ...
      if (!pickupCoords.lat || !pickupCoords.lng) {
        ...
        const pickupGeocode = await geocodingService.geocodeAddressWithCache(
          data.pickupAddress,
        );
        if (pickupGeocode) {
      ...
      if (!deliveryCoords.lat || !deliveryCoords.lng) {
        ...
        const deliveryGeocode = await geocodingService.geocodeAddressWithCache(
      ...
      if (
        ...
        if (routeResult) {
          ...
          finalPrice = geocodingService.calculateDistanceBasedPrice(
            distance,
            data.basePrice,
          );
        }
      ...
      const announcement = await prisma.$transaction(async (tx) => {
        ...
        const newAnnouncement = await tx.announcement.create({
          data: {
            ...
            urgencyFee: data.isUrgent
              ? this.calculateUrgencyFee(
                  finalPrice,
                  subscription?.plan || "FREE",
                )
              : null,
            insuranceFee: data.requiresInsurance
      ...
      logger.info(
        `Annonce créée: ${announcement.id}, prix final: ${finalPrice}€, distance: ${distance}km`,
      );
  ...
  async updateAnnouncement(
    ...
    if (userRole !== "ADMIN") {
      const isOwner =
        (userRole === "CLIENT" && announcement.clientId === userId) ||
  ...
  async deleteAnnouncement(
    ...
    if (announcement.status === "IN_PROGRESS") {
      throw new Error(
        "Cette annonce ne peut pas être supprimée car elle est en cours",
      );
    }

    await prisma.$transaction(async (tx) => {
      // Supprimer les détails spécifiques
      await tx.packageAnnouncement.deleteMany({
        where: { announcementId: id },
      });
      await tx.serviceAnnouncement.deleteMany({
        where: { announcementId: id },
      });
      await tx.routeAnnouncementMatch.deleteMany({
        where: { announcementId: id },
      });
      ...
      const compatibleRoutes = await prisma.route.findMany({
        where: {
          ...
          departureTime: {
            gte: new Date(),
            lte: new Date(
              announcement.desiredDate.getTime() + 24 * 60 * 60 * 1000,
            ), // +24h
          },
        },
        include: {
  ...
  private getRequiredCapacity(announcement: any): number {
    ...
    if (
      announcement.type === "PERSON_TRANSPORT" &&
  ...
  private async checkSubscriptionLimits(userId: string): Promise<void> {
    ...
    if (limits.maxAnnouncementsPerMonth !== -1) {
      ...
      if (monthlyCount >= limits.maxAnnouncementsPerMonth) {
        throw new Error(
          `Limite mensuelle atteinte (${limits.maxAnnouncementsPerMonth} annonces)`,
        );
      }
    }
    ...
    if (limits.maxActiveAnnouncements !== -1) {
      ...
      if (activeCount >= limits.maxActiveAnnouncements) {
        throw new Error(
          `Limite d'annonces actives atteinte (${limits.maxActiveAnnouncements})`,
        );
      }
    }
  }
   ...
   */
  async activateAnnouncementAfterPayment(
  ...
  async cancelAnnouncement(
    ...
    try {
      ...
      if (announcement.status === "IN_PROGRESS") {
        throw new Error(
          "Impossible d'annuler une annonce en cours de livraison",
        );
      }
  ...
  async validateDeliveryWithCode(
    ...
    try {
      ...
      try {
        const { InvoiceGeneratorService } = await import(
          "@/features/invoices/services/invoice-generator.service"
        );
        const invoiceUrl =
   ...
   */
  private async getUserAnnouncementStats(
    userId: string,
    userRole?: string,
  ): Promise<any> {
    const where =
    ...
    return {
      monthly: {
        ...
        remaining:
          limits.maxAnnouncementsPerMonth === -1
            ? -1
