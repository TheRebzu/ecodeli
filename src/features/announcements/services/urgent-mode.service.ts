...
class UrgentModeService {
  ...
  async activateUrgentMode(
    ...
    try {
      ...
      if (
        maxPriceAcceptable &&
        dynamicPricing.finalPrice > maxPriceAcceptable
      ) {
        throw new Error(
          `Prix urgence (${dynamicPricing.finalPrice}€) dépasse le maximum accepté (${maxPriceAcceptable}€)`,
        );
      }
  ...
  private async calculateUrgentPricing(
    ...
    const hoursUntilDeadline =
      (deliveryDeadline.getTime() - now.getTime()) / (1000 * 60 * 60);
    ...
    const urgencyMultiplier =
      timeMultiplier * demandMultiplier * distanceMultiplier;
  ...
  private async calculateDemandMultiplier(announcement: any): Promise<number> {
    ...
    // Ratio demande/offre
    const demandRatio =
      activeDeliverersCount > 0 ? recentUrgentCount / activeDeliverersCount : 1;

    // Multiplicateur entre 1.0 et maxDemandMultiplier
    return Math.min(
      this.urgentPricingConfig.demandBasedMultiplier,
      1 + demandRatio * 0.5,
    );
  }
      ...
      await db.announcement.update({
        ...
        data: {
          ...
          minMatchScore: Math.max(
            0.3,
            (announcement.minMatchScore || 0.6) - 0.2,
          ),
        },
      });
  ...
  private async sendUrgentReminders(announcementId: string): Promise<void> {
    ...
    const acceptedMatches = announcement.matches.filter(
      (m) => m.status === "ACCEPTED",
    );
        ...
        await tx.announcement.update({
          ...
          data: {
            ...
            priorityScore: Math.min(
              100,
              (announcement.priorityScore || 0) + 10,
            ),
          },
        });
  ...
  async getUrgentModeStats(
    ...
    const avgPrice =
      urgentAnnouncements.reduce((sum, a) => sum + (a.finalPrice || 0), 0) /
      totalUrgent;
    const avgUrgencyFee =
      urgentAnnouncements.reduce((sum, a) => sum + (a.urgencyFee || 0), 0) /
      totalUrgent;
    const successRate =
      (urgentAnnouncements.filter((a) => a.status === "COMPLETED").length /
        totalUrgent) *
      100;
    ...
    return {
      ...
      revenueFromUrgency: urgentAnnouncements.reduce(
        (sum, a) => sum + (a.urgencyFee || 0),
        0,
      ),
    };
  }
