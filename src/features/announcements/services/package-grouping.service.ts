...
const groupingCriteriaSchema = z.object({
  ...
  compatibleTypes: z.array(z.string()).default(['PACKAGE_DELIVERY', 'SHOPPING'])
})
...
class PackageGroupingService {
    ...
    try {
      ...
      const eligibleAnnouncements = await db.announcement.findMany({
        where: {
          ...
          isGrouped: false,
          createdAt: {
            gte: new Date(Date.now() - validatedCriteria.maxTimeWindow * 60 * 60 * 1000)
          }
        },
        include: {
          author: {
      ...
      for (let i = 0; i < eligibleAnnouncements.length; i++) {
        ...
        if (potentialGroup.length >= 2) {
          const opportunity = await this.createGroupingOpportunity(potentialGroup, validatedCriteria)
          if (opportunity) {
      ...
      return opportunities.sort((a, b) => b.savings.percentage - a.savings.percentage)
  ...
  async proposeGrouping(
    ...
    try {
      if (announcementIds.length < 2) {
        throw new Error('Au moins 2 annonces sont nécessaires pour un groupage')
      }
      ...
      await this.notifyGroupingProposal(deliveryGroup.id, proposals.map(p => p.id))
  ...
  async respondToGroupingProposal(
    ...
    try {
      ...
      const pendingCount = allProposals.filter(p => p.status === 'PENDING').length - 1 // -1 car on vient de mettre à jour
      ...
      if (pendingCount === 0) {
        ...
        if (acceptedCount === allProposals.length) {
          ...
          await this.cancelGrouping(proposal.deliveryGroup.id, 'Refus de l\'un des clients')
   ...
   */
  async getGroupingStatistics(period: 'WEEK' | 'MONTH' | 'YEAR' = 'MONTH'): Promise<{
    ...
    try {
      ...
      const [totalGroups, successfulGroups, savingsData, topDeliverers] = await Promise.all([
      ...
      const successRate = totalGroups > 0 ? (successfulGroups / totalGroups) * 100 : 0
      ...
      return {
        ...
        averageSavingsPerGroup: Math.round((savingsData._avg.savingsAmount || 0) * 100) / 100,
        averagePackagesPerGroup: Math.round((savingsData._avg.totalAnnouncements || 0) * 10) / 10,
  ...
  private async calculateGroupPricing(announcements: any[]): Promise<{
    ...
    const originalTotal = announcements.reduce((sum, ann) => sum + ann.basePrice, 0)
  ...
  private async notifyGroupingProposal(groupId: string, proposalIds: string[]): Promise<void> {
    console.log(`Notifying grouping proposal for group ${groupId}, proposals: ${proposalIds.join(', ')}`)
  }
}
