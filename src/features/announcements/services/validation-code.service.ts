...
class ValidationCodeService {
  ...
  async validateCode(
    ...
    try {
      ...
      await this.triggerPostValidationActions(
        validationRecord.announcementId,
        userId,
      );
      ...
      const averageValidationTime =
        usedCodes.length > 0
          ? usedCodes.reduce((sum, code) => {
              const timeDiff =
                code.usedAt!.getTime() - code.createdAt.getTime();
              return sum + timeDiff / (1000 * 60); // en minutes
            }, 0) / usedCodes.length
          : 0;

      const usageRate =
        totalGenerated > 0 ? (totalUsed / totalGenerated) * 100 : 0;
   ...
   */
  private async triggerPostValidationActions(
    announcementId: string,
    validatedBy: string,
  ): Promise<void> {
    try {
      ...
      console.log(
        `Post-validation actions triggered for announcement ${announcementId} by user ${validatedBy}`,
      );
    } catch (error) {
