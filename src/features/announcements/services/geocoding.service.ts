...
export class GeocodingService {
  ...
  static async geocodeAddress(
    ...
    try {
      ...
      logger.info(
        `Géocodage réussi: ${geocodingResult.city} (${geocodingResult.lat}, ${geocodingResult.lng})`,
      );
  ...
  static async calculateRoute(
    ...
    try {
      ...
      logger.info(
        `Calcul de route: (${startLat}, ${startLng}) → (${endLat}, ${endLng})`,
      );
      ...
      logger.info(
        `Route calculée: ${routeResult.distance.toFixed(2)}km, ${Math.round(routeResult.duration / 60)}min`,
      );
  ...
  static async validateServiceArea(address: string): Promise<boolean> {
    try {
      ...
      const isInServiceArea = serviceCities.some((city) =>
   ...
   */
  static calculateDistanceBasedPrice(
    distance: number,
    basePrice: number = 5,
  ): number {
  ...
  static async geocodeAddressWithCache(
    address: string,
  ): Promise<GeocodingResult | null> {
