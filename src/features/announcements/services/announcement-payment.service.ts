...
export class AnnouncementPaymentService {
  ...
  static async createPaymentIntent(input: CreatePaymentIntentInput): Promise<PaymentIntentResult> {
    ...
    try {
      logger.info(`Création PaymentIntent pour annonce ${announcementId}, montant: ${amount}€`)
      ...
      const finalAmount = this.calculateFinalAmount(amount, subscription?.plan || 'FREE', announcement.isUrgent || false)
      ...
      logger.info(`PaymentIntent créé: ${paymentIntent.id}, montant final: ${finalAmount}€`)
  ...
  static async confirmPaymentIntent(
    ...
    clientId: string
  ): Promise<{ success: boolean; requiresAction?: boolean; clientSecret?: string }> {
    try {
   ...
   */
  static async capturePayment(paymentId: string, deliveryId?: string): Promise<void> {
    try {
      ...
      if (payment.status !== 'CONFIRMED') {
        throw new Error(`Impossible de capturer un paiement en statut: ${payment.status}`)
      }
      ...
      if (payment.delivery) {
        await this.transferToDelivererWallet(payment.delivery.delivererId, payment)
      }
      ...
      logger.info(`Paiement capturé avec succès: ${paymentId}, montant: ${payment.amount}€`)
  ...
  static async refundPayment(
    ...
    try {
      ...
      logger.info(`Remboursement effectué: ${refund.id}, montant: ${refundAmount}€`)
   ...
   */
  private static calculateFinalAmount(baseAmount: number, plan: string, isUrgent: boolean): number {
   ...
   */
  private static async handlePaymentConfirmed(paymentId: string, paymentIntent: any): Promise<void> {
    try {
      await prisma.$transaction(async (tx) => {
        ...
        logger.info(`Annonce ${payment.announcementId} activée après paiement confirmé`)
      })
   ...
   */
  private static async transferToDelivererWallet(delivererId: string, payment: any): Promise<void> {
    try {
      ...
      logger.info(`Transfert wallet effectué: ${delivererFee}€ vers livreur ${delivererId}`)
  ...
  private static calculateDelivererFee(totalAmount: number): number {
    ...
    const delivererRate = 0.80   // 80% pour le livreur
