...
const bidSubmissionSchema = z.object({
  ...
  estimatedDeliveryTime: z
    .number()
    .min(15, "Délai minimum 15 minutes")
    .optional(),
  additionalNotes: z
    .string()
...
class ReverseAuctionService {
  ...
  async submitBid(bidData: BidSubmission): Promise<{
    ...
    try {
      ...
      if (bidData.proposedPrice < auction.minimumPrice) {
        throw new Error(
          `Le prix proposé doit être supérieur à ${auction.minimumPrice}€`,
        );
      }

      if (bidData.proposedPrice >= auction.currentBestPrice) {
        throw new Error(
          "Votre prix doit être inférieur au meilleur prix actuel",
        );
      }

      // Vérifier que le livreur n'a pas déjà enchéri
      const existingBid = auction.bids.find(
        (bid) => bid.bidderId === bidData.bidderId,
      );
      if (existingBid) {
      ...
      const bid = await db.auctionBid.create({
        data: {
          ...
          validUntil:
            bidData.validUntil || new Date(Date.now() + 24 * 60 * 60 * 1000),
          delivererRating,
  ...
  async acceptBid(
    ...
    try {
      ...
      if (bid.auction.announcement.authorId !== clientId && !isAutomatic) {
        throw new Error(
          "Seul le créateur de l'annonce peut accepter une enchère",
        );
      }
      ...
      const averagePrice =
        prices.reduce((sum, price) => sum + price, 0) / totalBids;
      const savingsAmount = bid.auction.initialPrice - bid.proposedPrice;
      const savingsPercentage =
        (savingsAmount / bid.auction.initialPrice) * 100;
  ...
  async getAuctionResults(announcementId: string): Promise<AuctionResult> {
    try {
      ...
      const savingsAmount = auction.winningBid
        ? auction.initialPrice - auction.winningBid.proposedPrice
        : 0;
      const savingsPercentage = auction.winningBid
        ? (savingsAmount / auction.initialPrice) * 100
        : 0;
  ...
  async cancelAuction(
    ...
    try {
      ...
      const canCancel =
        user?.role === "ADMIN" || auction.announcement.authorId === userId;
  ...
  private async notifyMatchedDeliverers(
    announcementId: string,
    auctionId: string,
  ): Promise<void> {
  ...
  private async notifyBidUpdate(
    ...
    console.log(
      `Bid update notification for auction ${auctionId}, bid ${bidId}, winning: ${isWinning}`,
    );
  }

  private async notifyAuctionResult(
    auctionId: string,
    winningBidId: string,
  ): Promise<void> {
    // Implémenter notification résultat final
    console.log(
      `Auction result notification for auction ${auctionId}, winner: ${winningBidId}`,
    );
  }

  private async notifyAuctionCancellation(
    auctionId: string,
    reason: string,
  ): Promise<void> {
    // Implémenter notification annulation
    console.log(
      `Auction cancellation notification for auction ${auctionId}, reason: ${reason}`,
    );
  }
}
