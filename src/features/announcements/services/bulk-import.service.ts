...
class BulkImportService {
  ...
  generateCSVTemplate(announcementType?: AnnouncementType): string {
    ...
    const serviceHeaders = ["serviceDuration", "serviceType", "numberOfPeople"];
    ...
    if (
      !announcementType ||
      ["HOME_SERVICE", "PET_SITTING"].includes(announcementType)
    ) {
    ...
    const exampleRow = {
      ...
      description:
        "Livraison d'un colis fragile de 2kg entre Paris et Versailles",
      ...
      for (let i = 0; i < records.length; i++) {
        ...
        try {
          ...
          const announcementData = {
            ...
            packageDetails:
              validatedData.type === "PACKAGE_DELIVERY"
                ? {
            ...
            serviceDetails: ["HOME_SERVICE", "PET_SITTING"].includes(
              validatedData.type,
            )
              ? {
          ...
          if (options.autoPublish && options.sendNotifications) {
            matchingService
              .triggerRouteMatching(announcement.id)
              .catch(console.error);
          }
        } catch (error) {
          result.failed++;
  ...
  async exportToCSV(
    ...
    try {
      ...
      const csvData = announcements.map((announcement) => ({
        ...
        // Évaluations
        averageRating:
          announcement.reviews.length > 0
            ? announcement.reviews.reduce((sum, r) => sum + r.rating, 0) /
              announcement.reviews.length
            : "",
