...
export const petSittingDetailsSchema = z.object({
  // Informations animal
  petType: z.enum(['DOG', 'CAT', 'BIRD', 'RABBIT', 'HAMSTER', 'FISH', 'REPTILE', 'OTHER']),
  ...
  temperament: z.array(z.enum(['CALM', 'PLAYFUL', 'AGGRESSIVE', 'SHY', 'SOCIAL', 'ANXIOUS'])),
  ...
  // Services demandés
  servicesRequested: z.array(z.enum([
  ...
  // Équipements fournis
  equipmentProvided: z.array(z.enum([
    'FOOD', 'TOYS', 'LEASH', 'CARRIER', 'BED', 'LITTER_BOX', 
    'MEDICATION', 'CLEANING_SUPPLIES', 'GROOMING_TOOLS'
  ])).default([]),
  
  equipmentNeeded: z.array(z.string()).default([]),
  
  // Préférences gardien
  preferredSitterGender: z.enum(['MALE', 'FEMALE', 'NO_PREFERENCE']).default('NO_PREFERENCE'),
  preferredSitterAge: z.object({
    min: z.number().min(18).max(100),
    max: z.number().min(18).max(100)
  }).optional(),
  
  requiresExperience: z.boolean().default(false),
  requiredExperienceLevel: z.enum(['BEGINNER', 'INTERMEDIATE', 'EXPERT']).optional(),
...
export const petSittingAnnouncementSchema = z.object({
  ...
  preferredContactMethod: z.enum(['PHONE', 'EMAIL', 'APP', 'SMS']).default('APP'),
  responseTimeExpected: z.enum(['IMMEDIATE', 'WITHIN_HOUR', 'WITHIN_DAY']).default('WITHIN_HOUR')
})

export type PetSittingDetails = z.infer<typeof petSittingDetailsSchema>
export type PetSittingAnnouncement = z.infer<typeof petSittingAnnouncementSchema>