...
import { GoogleMap, LoadScript, Marker, DirectionsRenderer, Polygon, InfoWindow } from '@react-google-maps/api'
...
export function AnnouncementMap({
      ...
      directionsService.route(
        {
          ...
          waypoints: warehouseLocations.slice(0, 8).map(w => ({ // Max 8 waypoints
  ...
  const getMarkerIcon = (type: string, status?: string) => {
    ...
    switch (type) {
      ...
      case 'deliverer':
        return status === 'available' ? `${baseUrl}deliverer-available.png` : 
               status === 'busy' ? `${baseUrl}deliverer-busy.png` : 
        ...
        <div style={{ height }} className="relative">
          ...
          >
            ...
            >
              ...
              {/* Marqueurs entrepôts */}
              {showWarehouses && warehouseLocations.map((warehouse) => (
                <Marker
                  key={warehouse.id}
              ...
              {/* Marqueurs livreurs */}
              {showDeliverers && delivererLocations.map((deliverer) => (
                <Marker
                  key={deliverer.id}
              ...
              {selectedMarker && (
                ...
                >
                  <div className="p-2 max-w-xs">
                    ...
                    {selectedMarker.type === 'deliverer' && selectedMarker.info && (
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          <Truck className="h-4 w-4" />
                          <span className="text-sm">{selectedMarker.info.vehicleType}</span>
                        </div>
                        <div className="flex items-center gap-2">
                          <span className="text-sm">⭐ {selectedMarker.info.rating}/5</span>
                          <Badge 
                            variant={selectedMarker.info.status === 'available' ? 'default' : 'secondary'}
                            className="text-xs"
                          >
                    ...
                    {selectedMarker.type === 'warehouse' && selectedMarker.info && (
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          <span className="text-sm">
                            Capacité: {selectedMarker.info.currentLoad}/{selectedMarker.info.capacity}
                          </span>
                        </div>
