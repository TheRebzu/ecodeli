  ...
  useEffect(() => {
    if (bounds) {
      ...
      const routeBounds = new LatLngBounds(
        route.map((point) => new LatLng(point[0], point[1])),
      );
...
export const TrackingMap: React.FC<TrackingMapProps> = ({
  ...
  const getMapBounds = () => {
    ...
    if (delivery?.currentPosition) {
      points.push(
        new LatLng(delivery.currentPosition.lat, delivery.currentPosition.lng),
      );
    }
            ...
            {realTime?.estimatedArrival && (
              <div className="text-right">
                ...
                <p className="text-sm font-medium text-gray-900">
                  {new Date(realTime.estimatedArrival).toLocaleTimeString(
                    "fr-FR",
                    {
                      hour: "2-digit",
                      minute: "2-digit",
        ...
        >
          ...
          {route?.polyline && route.polyline.length > 0 && (
            <Polyline
              ...
              opacity={0.8}
              dashArray={
                delivery?.status === "DELIVERED" ? "10, 10" : undefined
              }
            />
          )}
