...
export function AnnouncementTracking({ 
  ...
  const getStatusSteps = () => {
    return [
      ...
      { key: 'MATCHED', label: 'Livreur trouvé', icon: '🤝', completed: tracking.some(t => t.status === 'MATCHED') },
      { key: 'PICKUP_PENDING', label: 'En route vers collecte', icon: '🚚', completed: tracking.some(t => t.status === 'PICKUP_PENDING') },
      { key: 'PICKED_UP', label: 'Colis récupéré', icon: '📦', completed: tracking.some(t => t.status === 'PICKED_UP') },
      { key: 'IN_TRANSIT', label: 'En livraison', icon: '🛣️', completed: tracking.some(t => t.status === 'IN_TRANSIT') },
      { key: 'DELIVERED', label: 'Livré', icon: '✅', completed: tracking.some(t => t.status === 'DELIVERED') }
    ]
  }
    ...
    return (
      <div className="bg-white rounded-lg shadow-sm border p-6">
        <div className="text-center">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-2">Erreur de suivi</h3>
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-3">
            ...
            >
              ...
              <span>{isRealTimeActive ? 'Temps réel actif' : 'Temps réel inactif'}</span>
            </button>
            <button
              onClick={refresh}
              disabled={loading}
      ...
      {currentLocation && (
        <div className="p-6 bg-blue-50 border-b">
          <div className="flex items-start space-x-4">
            ...
            <div>
              <h3 className="font-medium text-blue-900 mb-1">Position actuelle</h3>
              <p className="text-blue-700">{formatLocation(currentLocation)}</p>
              {estimatedArrival && (
                <p className="text-sm text-blue-600 mt-2">
                  ⏰ Arrivée estimée: {new Date(estimatedArrival).toLocaleString('fr-FR')}
                </p>
              )}
            </div>
          </div>
              ...
              tracking.map((update, index) => (
                <div key={update.id} className="flex items-start space-x-4 pb-4 border-b border-gray-100 last:border-b-0">
                  <div className="flex-shrink-0 w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center">
                    <span className="text-sm">{getTimelineIcon(update.status)}</span>
                  </div>
        ...
        <div className="p-6 border-t">
          ...
          <div className="bg-gray-100 rounded-lg h-64 flex items-center justify-center">
            <div className="text-center text-gray-500">
              ...
              <p className="text-sm">Position: {formatLocation(currentLocation)}</p>
            </div>
          </div>
        </div>
      )}
