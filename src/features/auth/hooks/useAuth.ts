...
export function useAuth(): AuthState {
  ...
  const canAccess = (resource: string): boolean => {
    ...
    switch (userRole) {
      ...
      case "CLIENT":
        return ["announcements", "bookings", "payments"].some((r) =>
          resource.includes(r),
        );
      case "DELIVERER":
        return ["deliveries", "routes", "earnings"].some((r) =>
          resource.includes(r),
        );
      case "MERCHANT":
        return ["products", "orders", "contracts"].some((r) =>
          resource.includes(r),
        );
      case "PROVIDER":
        return ["services", "bookings", "invoices"].some((r) =>
          resource.includes(r),
        );
      default:
        return false;
  ...
  useEffect(() => {
    if (session?.user) {
      ...
      if (
        ...
        session.user.profile?.tutorialCompleted === false
      ) {
