...
export function ValidationGuard({ 
  ...
  const isFullyValidated = () => {
    ...
    return status.emailVerified && status.profileVerified && 
           (status.documentsRequired === 0 || status.documentsApproved === status.documentsRequired)
  }
  ...
  const getValidationStep = () => {
    ...
    if (status.documentsRequired > 0 && status.documentsApproved < status.documentsRequired) {
      return {
        ...
        description: 'Soumettez les documents requis pour finaliser votre validation.',
        icon: FileText,
        ...
        <CardContent className="space-y-4">
          <Alert className="border-blue-200 bg-blue-50">
            ...
            <AlertDescription className="text-blue-800">
              Cette étape est nécessaire pour sécuriser votre compte et vous donner accès à tous les services EcoDeli.
            </AlertDescription>
          </Alert>

          <div className="space-y-3">
            <Button asChild className="w-full">
              <Link href={validationStep.href}>
                {validationStep.action}
              </Link>
            </Button>
