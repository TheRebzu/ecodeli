...
export class AuthService {
  ...
  static async registerDeliverer(data: DelivererRegisterData) {
    try {
      ...
      return {
        user,
        message:
          "Inscription réussie. Votre compte sera activé après validation de vos documents.",
      };
    } catch (error) {
  ...
  static async registerMerchant(data: MerchantRegisterData) {
    try {
      ...
      return {
        user,
        message:
          "Inscription réussie. Un contrat vous sera proposé prochainement.",
      };
    } catch (error) {
  ...
  static async registerProvider(data: ProviderRegisterData) {
    try {
      ...
      return {
        user,
        message:
          "Inscription réussie. Votre compte sera activé après validation de vos certifications.",
      };
    } catch (error) {
      ...
      if (user.role === "DELIVERER" && user.deliverer) {
        ...
        await prisma.notification.create({
          data: {
            ...
            title: "Compte validé",
            message:
              "Votre compte livreur a été validé. Votre carte NFC sera expédiée sous 7 jours.",
          },
        });
      ...
      if (user.role === "PROVIDER" && user.provider) {
        ...
        await prisma.notification.create({
          data: {
            ...
            title: "Compte validé",
            message:
              "Votre compte prestataire a été validé. Vous pouvez maintenant créer vos services.",
          },
        });
      }
  ...
  static async checkPermissions(
    ...
    try {
      ...
      if (user.role === "DELIVERER" && user.deliverer) {
        if (
          user.deliverer.validationStatus !== "APPROVED" ||
          !user.deliverer.isActive
        ) {
          return false;
        }
      } else if (user.role === "PROVIDER" && user.provider) {
        if (
          user.provider.validationStatus !== "APPROVED" ||
