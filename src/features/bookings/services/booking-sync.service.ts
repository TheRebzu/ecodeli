...
export class BookingSyncService {
      ...
      if (newBookingStatus) {
        await prisma.$transaction(async (tx) => {
          ...
          if (newBookingStatus === 'CONFIRMED') {
            await this.createConfirmationNotifications(tx, booking, payment.amount);
          } else if (newBookingStatus === 'CANCELLED') {
            await this.createCancellationNotifications(tx, booking);
          }
        });

        console.log(`✅ Booking ${booking.id} status synced: ${booking.status} → ${newBookingStatus}`);
      }
    } catch (error) {
      ...
      if (newPaymentStatus) {
        await prisma.payment.update({
          ...
          data: {
            ...
            paidAt: newPaymentStatus === 'COMPLETED' ? new Date() : booking.payment.paidAt,
            failedAt: newPaymentStatus === 'FAILED' ? new Date() : booking.payment.failedAt,
            metadata: {
        ...
        console.log(`✅ Payment ${booking.payment.id} status synced: ${booking.payment.status} → ${newPaymentStatus}`);
      }
    } catch (error) {
  ...
  static async forceSyncBooking(bookingId: string): Promise<void> {
    try {
      ...
      if (booking.payment) {
        await this.syncBookingOnPaymentChange(booking.payment.id, booking.payment.status);
      }
  ...
  static async auditAndFixInconsistencies(): Promise<{
    ...
    try {
      ...
      for (const booking of bookingsWithPayments) {
        ...
        try {
          const needsSync = this.detectInconsistency(booking.status, booking.payment.status);
      ...
      console.log(`🔍 Audit completed: ${results.checked} checked, ${results.fixed} fixed, ${results.errors.length} errors`);
      return results;
    } catch (error) {
  ...
  private static async createMissingPayment(booking: any): Promise<void> {
    ...
    console.log(`✅ Created missing payment for completed booking ${booking.id}`);
  }
