...
export function ServiceRequestsManager() {
  ...
  const { serviceRequests, isLoading, deleteServiceRequest } =
    useClientServices();
  ...
  useEffect(() => {
    ...
    if (activeTab !== "all") {
      filtered = filtered.filter((s) => {
        switch (activeTab) {
          case "active":
            return ["ACTIVE", "BOOKED"].includes(s.status);
          case "progress":
  ...
  const ServiceRequestCard = ({
    ...
    const statusInfo =
      statusConfig[serviceRequest.status as keyof typeof statusConfig];
    const urgencyInfo =
      urgencyConfig[serviceRequest.urgency as keyof typeof urgencyConfig] ||
      ...
      <Card className="hover:shadow-md transition-shadow cursor-pointer">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 mt-1">
                  ...
                  {serviceRequest.estimatedDuration} min
                </p>
              </div>
            </div>
        ...
        <CardContent>
          ...
          <div className="flex items-center justify-between text-sm text-gray-600 mb-4">
            <span>
              {t("budget")}: <strong>{serviceRequest.budget}€</strong>
            </span>
            <span>
              {t("scheduledFor")}:{" "}
          ...
          <div className="flex gap-2 pt-3 border-t">
            <Button
              ...
              size="sm"
              onClick={() =>
                router.push(`/client/service-requests/${serviceRequest.id}`)
              }
              className="flex-1"
            >
              👁️ {t("viewDetails")}
            ...
            {serviceRequest.status === "DRAFT" && (
              <Button
                ...
                onClick={() =>
                  router.push(
                    `/client/service-requests/${serviceRequest.id}/edit`,
                  )
                }
              >
                ✏️ {t("edit")}
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button
          onClick={() => router.push("/client/service-requests/create")}
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="grid w-full grid-cols-5">
          <TabsTrigger value="all">
            {t("all")} ({serviceRequests.length})
          </TabsTrigger>
          <TabsTrigger value="active">
            {t("active")} (
            {
              serviceRequests.filter((s) =>
                ["ACTIVE", "BOOKED"].includes(s.status),
              ).length
            }
            )
          </TabsTrigger>
          ...
          <TabsTrigger value="drafts">
            {t("drafts")} (
            {serviceRequests.filter((s) => s.status === "DRAFT").length})
          </TabsTrigger>
        </TabsList>
