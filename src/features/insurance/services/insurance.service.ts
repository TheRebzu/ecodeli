...
export class InsuranceService {
   ...
   */
  static async createDeliveryCoverage(
    deliveryId: string,
    ...
    try {
      ...
      console.error(
        "Erreur lors de la création de la couverture livraison:",
        error,
      );
      throw error;
    }
  ...
  static async createServiceCoverage(serviceId: string, serviceData: any) {
    try {
      ...
      console.error(
        "Erreur lors de la création de la couverture service:",
        error,
      );
      throw error;
    }
  ...
  static async verifyCoverage(entityType: string, entityId: string) {
    try {
      ...
      const verification = {
        ...
        totalCoverage: coverages.reduce(
          (sum, c) => sum + (c.maxCoverage - c.currentUsage),
          0,
        ),
      };
  ...
  static async createClaim(data: CreateClaimData) {
    try {
      ...
      if (data.amount > availableCoverage) {
        throw new Error(
          `Montant réclamé (${data.amount}€) supérieur à la couverture disponible (${availableCoverage}€)`,
        );
      }
  ...
  static async approveClaim(
    ...
    try {
      ...
      const availableCoverage =
        claim.coverage.maxCoverage - claim.coverage.currentUsage;
      if (approvedAmount > availableCoverage) {
        throw new Error(
          "Montant approuvé supérieur à la couverture disponible",
        );
      }
  ...
  static async createServiceWarranty(
    ...
    try {
      ...
      console.error(
        "Erreur lors de la création de la garantie service:",
        error,
      );
      throw error;
    }
  ...
  static async createDeliveryWarranty(
    ...
    try {
      ...
      console.error(
        "Erreur lors de la création de la garantie livraison:",
        error,
      );
      throw error;
    }
  ...
  static async createWarrantyClaim(data: WarrantyClaimData) {
    try {
      ...
      if (data.requestedAmount > warranty.maxClaimAmount) {
        throw new Error(
          `Montant réclamé supérieur au maximum autorisé (${warranty.maxClaimAmount}€)`,
        );
      }
      ...
      console.error(
        "Erreur lors de la création de la réclamation garantie:",
        error,
      );
      throw error;
    }
      ...
      if (entityType === "user") {
        ...
        if (user) {
          ...
          const totalDeliveries =
            user.clientDeliveries.length + user.delivererDeliveries.length;
          const claimRatio =
            totalDeliveries > 0
              ? user._count.claimsAsClaimant / totalDeliveries
              : 0;
