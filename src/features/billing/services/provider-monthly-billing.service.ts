...
export class ProviderMonthlyBillingService {
  static async processMonthlyBilling(): Promise<void> {
    try {
      ...
      if (currentDay !== 30) {
        logger.info('Facturation mensuelle prestataires: pas le bon jour (30 du mois)')
        return
      }
      ...
      logger.info(`Démarrage facturation mensuelle pour ${providers.length} prestataires`)
  ...
  private static async generateMonthlyInvoice(provider: any): Promise<void> {
    try {
      ...
      if (interventions.length === 0) {
        logger.info(`Aucune intervention pour ${provider.user.email} ce mois-ci`)
        return
      }
      ...
      for (const intervention of interventions) {
        ...
        invoiceItems.push({
          ...
          quantity: intervention.actualDuration ? intervention.actualDuration / 60 : intervention.booking.duration / 60,
      ...
      const tax = (subtotal - platformFee) * 0.20
      ...
      const invoice = await prisma.invoice.create({
        data: {
          ...
          metadata: {
            ...
            totalHours: invoiceItems.reduce((acc, item) => acc + item.quantity, 0)
          },
          items: {
            create: invoiceItems
          }
      ...
      const pdfUrl = await this.generateInvoicePDF(invoice, provider, invoiceItems)
      ...
      logger.info(`Facture générée pour ${provider.user.email}: ${invoiceNumber} - ${total.toFixed(2)}€`)

    } catch (error) {
      logger.error(`Erreur génération facture pour prestataire ${provider.id}:`, error)
    }
  }
  ...
  private static async generateInvoicePDF(invoice: any, provider: any, items: any[]): Promise<string> {
    try {
      const doc = new jsPDF()
      ...
      items.forEach(item => {
        ...
        doc.text(`${item.quantity}h x ${item.unitPrice}€ = ${item.total.toFixed(2)}€`, 20, yPos + 10)
        yPos += 25
      })

      yPos += 10
      doc.text(`Sous-total: ${invoice.subtotal.toFixed(2)}€`, 20, yPos)
      doc.text(`Commission EcoDeli (10%): -${invoice.metadata.platformFee.toFixed(2)}€`, 20, yPos + 10)
  ...
  private static async simulateBankTransfer(provider: any, amount: number): Promise<void> {
    try {
      ...
      logger.info(`Virement simulé pour ${provider.user.email}: ${amount.toFixed(2)}€`)
