...
export class ProviderBillingService {
      ...
      for (const provider of activeProviders) {
        try {
          ...
          if (provider.bookings.length === 0) {
            console.log(
              `Aucune intervention à facturer pour le prestataire ${provider.id}`,
            );
            continue;
          }
          ...
          if (existingInvoice) {
            console.log(
              `Facture déjà générée pour le prestataire ${provider.id}`,
            );
            continue;
          }

          // Générer la facture
          const invoice = await this.createProviderInvoice(
            provider,
            billingPeriod,
          );
          results.success++;
          ...
          console.log(
            `Facture générée avec succès pour ${provider.user.profile?.firstName} ${provider.user.profile?.lastName} - ${invoice.invoiceNumber}`,
          );
        } catch (error) {
          console.error(
          ...
          results.errors.push(
            `Prestataire ${provider.id}: ${error instanceof Error ? error.message : "Erreur inconnue"}`,
          );
        }
      }

      console.log(
        `Génération terminée: ${results.success} succès, ${results.errors.length} erreurs`,
      );
      return results;
    } catch (error) {
  ...
  static async createProviderInvoice(
    ...
    try {
      ...
      const invoice = await prisma.$transaction(async (tx) => {
        ...
        for (const intervention of earnings.interventions) {
          await tx.invoiceItem.create({
            data: {
              ...
              quantity:
                intervention.intervention?.actualDuration ||
                intervention.service.duration ||
                60,
        ...
        // 3. Générer le PDF
        const pdfContent = this.generateInvoicePDF(
          newInvoice,
          provider,
          earnings,
        );
  ...
  static calculateProviderEarnings(
    ...
    const interventions = provider.bookings.filter(
      (booking: any) =>
  ...
  static async getBillingStats(month?: number, year?: number): Promise<any> {
    ...
    const [totalInvoices, totalAmount, activeProviders, pendingInvoices] =
      await Promise.all([
