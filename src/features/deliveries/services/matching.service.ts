...
import { DeliveryStatus, AnnouncementStatus, AnnouncementType } from '@prisma/client'
...
export class MatchingService {
  ...
  static async findAllMatches(): Promise<MatchScore[]> {
    ...
    const availableAnnouncements = await prisma.announcement.findMany({
        ...
        status: AnnouncementStatus.ACTIVE,
          type: {
          in: [AnnouncementType.PACKAGE_DELIVERY, AnnouncementType.SHOPPING, AnnouncementType.INTERNATIONAL_PURCHASE]
          },
        deliveries: {
    ...
    for (const deliverer of activeDeliverers) {
      for (const announcement of availableAnnouncements) {
        const match = await this.calculateDelivererAnnouncementMatch(deliverer, announcement)
   ...
   */
  static async findMatchesForAnnouncement(announcementId: string): Promise<MatchScore[]> {
   ...
   */
  private static async calculateDelivererAnnouncementMatch(deliverer: any, announcement: any): Promise<MatchScore> {
    const locationScore = this.calculateLocationScore(
      { address: deliverer.profile?.address || deliverer.profile?.city || 'Paris' },
    ...
    const reasons = this.generateMatchingReasons(locationScore, timeScore, capacityScore, priceScore)
    ...
    return {
      ...
      compatibility,
      reasons,
      distance: this.estimateDistance(deliverer.profile?.city || 'Paris', announcement.pickupAddress),
      estimatedTime: this.estimateDeliveryTime(announcement.pickupAddress, announcement.deliveryAddress)
    }
  }
  ...
  private static calculateLocationScore(
    ...
    const delivererCity = this.extractCityFromAddress(delivererLocation.address)
    ...
    if (delivererCity.toLowerCase() === pickupCity.toLowerCase()) {
      ...
      const distance = this.estimateDistance(delivererLocation.address, pickupLocation.address)
    ...
    const deliveryDistance = this.estimateDistance(pickupLocation.address, deliveryLocation.address)
   ...
   */
  private static calculateTimeScore(delivererAvailable: Date, announcementDate: Date): number {
    const availableTime = new Date(delivererAvailable)
    const announcementTime = new Date(announcementDate)

    // Calculer la différence en heures
    const diffHours = Math.abs(announcementTime.getTime() - availableTime.getTime()) / (1000 * 60 * 60)
   ...
   */
  private static calculateCapacityScore(routeCapacity: number, announcementWeight: number, announcementVolume: number): number {
   ...
   */
  private static calculatePriceScore(announcementPrice: number, delivererPriceRange: { min: number; max: number }): number {
    if (announcementPrice >= delivererPriceRange.min && announcementPrice <= delivererPriceRange.max) {
    ...
    if (announcementPrice > delivererPriceRange.max) {
      ...
      const excess = (announcementPrice - delivererPriceRange.max) / delivererPriceRange.max
    ...
    if (announcementPrice < delivererPriceRange.min) {
      ...
      const deficit = (delivererPriceRange.min - announcementPrice) / delivererPriceRange.min
  ...
  private static isSameRegion(city1: string, city2: string): boolean {
    ...
    const regions: { [key: string]: string[] } = {
      'ile-de-france': ['paris', 'boulogne', 'neuilly', 'levallois', 'issy', 'versailles'],
  ...
  private static isNearbyCity(city1: string, city2: string): boolean {
    ...
    const nearbyCities: { [key: string]: string[] } = {
      'paris': ['boulogne', 'neuilly', 'levallois', 'issy', 'versailles', 'creteil'],
    ...
    return nearbyCities[city1Lower]?.includes(city2Lower) || 
  ...
  private static extractCityFromAddress(address: string): string {
    ...
    const cityKeywords = ['Paris', 'Lyon', 'Marseille', 'Lille', 'Toulouse', 'Nice', 'Nantes', 'Montpellier', 'Strasbourg', 'Bordeaux']
  ...
  private static estimateDistance(address1: string, address2: string): number {
    ...
    const distances: { [key: string]: { [key: string]: number } } = {
      'Paris': { 'Lyon': 465, 'Marseille': 775, 'Lille': 225, 'Toulouse': 679, 'Nice': 933 },
      'Lyon': { 'Paris': 465, 'Marseille': 315, 'Lille': 690, 'Toulouse': 537, 'Nice': 473 },
      'Marseille': { 'Paris': 775, 'Lyon': 315, 'Lille': 1000, 'Toulouse': 405, 'Nice': 204 },
      'Lille': { 'Paris': 225, 'Lyon': 690, 'Marseille': 1000, 'Toulouse': 904, 'Nice': 1158 },
      'Toulouse': { 'Paris': 679, 'Lyon': 537, 'Marseille': 405, 'Lille': 904, 'Nice': 539 },
      'Nice': { 'Paris': 933, 'Lyon': 473, 'Marseille': 204, 'Lille': 1158, 'Toulouse': 539 }
    }
   ...
   */
  private static estimateDeliveryTime(pickupAddress: string, deliveryAddress: string): number {
   ...
   */
  static async findMatchesForDeliverer(delivererId: string, limit: number = 10): Promise<MatchScore[]> {
   ...
   */
  static async executeAutoMatching(): Promise<{ processed: number; matches: number; notifications: number }> {
    console.log('Début du matching automatique...')
    
    const matches = await this.findAllMatches()
    const highQualityMatches = matches.filter(m => m.compatibility === 'HIGH')
    ...
    for (const match of highQualityMatches) {
      try {
        ...
        if (opportunity) {
          notificationsSent++
          console.log(`Opportunité créée: ${match.delivererId} -> ${match.announcementId} (${match.totalScore}/100)`)
        }
    ...
    console.log(`Matching terminé: ${matches.length} matchs traités, ${highQualityMatches.length} haute qualité, ${notificationsSent} notifications`)
  ...
  static async acceptOpportunity(
    ...
    try {
      ...
      if (!announcement || announcement.status !== 'ACTIVE' || announcement.deliveries.length > 0) {
        return {
          success: false,
   ...
   */
  static async getDelivererOpportunities(delivererId: string): Promise<DelivererOpportunity[]> {
    try {
