...
export class ValidationCodeService {
  static async generateValidationCode(deliveryId: string): Promise<string> {
    try {
      ...
      logger.info(
        `Code de validation généré pour la livraison ${deliveryId}: ${code}`,
      );
      return code;
    } catch (error) {
  ...
  static async validateCode(
    ...
    try {
      ...
      if (!validation) {
        logger.warn(
          `Tentative de validation avec code invalide: ${code} pour livraison ${deliveryId}`,
        );
        return false;
      }
      ...
      logger.info(
        `Code de validation utilisé avec succès pour la livraison ${deliveryId}`,
      );
      return true;
    } catch (error) {
