...
export class DeliveryTrackingService {
   ...
   */
  static async addTrackingUpdate(input: TrackingUpdateInput): Promise<TrackingUpdate> {
    const { deliveryId, status, message, location, estimatedArrival, delay, isAutomatic, metadata } = input
   ...
   */
  static async updateDeliveryStatus(input: UpdateDeliveryStatusInput): Promise<Delivery> {
    const { deliveryId, status, location, notes, proofPhotos, timestamp } = input
    ...
    try {
      return await prisma.$transaction(async (tx) => {
        ...
        this.validateStatusTransition(existingDelivery.status as DeliveryStatus, status)
        ...
        await tx.trackingUpdate.create({
          data: {
            ...
            metadata: notes ? JSON.stringify({ notes, photos: proofPhotos }) : null
          }
        })
   ...
   */
  static async getTrackingHistory(deliveryId: string): Promise<TrackingUpdate[]> {
    try {
    ...
    try {
      ...
      return {
        ...
        lastUpdate: lastTracking ? {
          ...
          location: lastTracking.location ? JSON.parse(lastTracking.location) : null
        } : null,
        estimatedDelivery: delivery.estimatedDeliveryAt,
        isCompleted: ['DELIVERED', 'CANCELLED', 'RETURNED'].includes(delivery.status)
      }
    } catch (error) {
  ...
  static async getActiveDeliveriesForDeliverer(delivererId: string) {
    try {
      ...
      return deliveries.map(delivery => ({
        ...
        timeRemaining: this.calculateTimeRemaining(delivery.estimatedDeliveryAt),
        nextAction: this.getNextActionForDeliverer(delivery.status as DeliveryStatus)
      }))
    } catch (error) {
   ...
   */
  private static validateStatusTransition(currentStatus: DeliveryStatus, newStatus: DeliveryStatus): void {
   ...
   */
  static async updateLocation(deliveryId: string, delivererId: string, location: { 
    latitude: number, 
    longitude: number, 
    accuracy?: number 
  }) {
  ...
  static async calculateETA(deliveryId: string) {
    try {
      ...
      if (!delivery || !delivery.currentLocation) {
        return { eta: delivery?.estimatedDeliveryAt || null, confidence: 'LOW' }
      }
