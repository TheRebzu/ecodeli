...
export function ValidationCodeDisplay({
  deliveryId,
  onCodeGenerated,
}: ValidationCodeDisplayProps) {
  ...
  const fetchValidationCode = async () => {
    try {
      const response = await fetch(
        `/api/deliveries/${deliveryId}/validation-code`,
      );
      if (response.ok) {
  ...
  const isExpired =
    validationData && new Date(validationData.expiresAt) <= new Date();
