...
export class TicketService {
  ...
  static async replyToTicket(
    ...
    try {
      ...
      if (!isInternal) {
        const recipientId =
          authorId === ticket.authorId ? ticket.assignedToId : ticket.authorId;
        if (recipientId) {
  ...
  private static async autoAssignTicket(ticketId: string, category: string) {
    try {
      ...
      const availableAgent = agents.reduce((prev, current) =>
        prev._count.assignedTickets < current._count.assignedTickets
          ? prev
          : current,
      );
  ...
  static async getTicketStats(
    ...
    try {
      ...
      const [total, open, inProgress, resolved, avgSatisfaction] =
        await Promise.all([
          prisma.supportTicket.count({ where }),
