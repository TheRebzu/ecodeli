...
export const calculatePlanPrice = (planId: string, basePrice: number): number => {
...
export const calculateInsuranceFee = (planId: string, itemValue: number): number => {
  const plan = getSubscriptionPlan(planId)
  if (!plan) return 0
...
export const calculatePriorityShippingFee = (
  ...
  if (plan.id === 'premium' && usedPriorityShipments < plan.limits.freeShipments!) {
    return 0
  }
