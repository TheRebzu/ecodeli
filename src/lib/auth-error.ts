...
export function getAuthErrorMessage(error?: string | null): string {
  ...
  const errors: Record<AuthErrorCode, string> = {
    ...
    OAuthCreateAccount:
      "Erreur lors de la création du compte avec le fournisseur d'authentification.",
    EmailCreateAccount:
    ...
    OAuthAccountNotLinked:
      "Cet email est déjà associé à un compte utilisant un autre fournisseur d'authentification.",
