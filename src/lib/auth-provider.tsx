  ...
  const canPerform = (action: string): boolean => {
    ...
    switch (action) {
      ...
      case "accept_delivery":
        return (
          auth.role === "DELIVERER" && auth.validationStatus === "VALIDATED"
        );
      case "manage_inventory":
        return (
          auth.role === "MERCHANT" && auth.validationStatus === "VALIDATED"
        );
      case "provide_service":
        return (
          auth.role === "PROVIDER" && auth.validationStatus === "VALIDATED"
        );
      case "admin_access":
...
export function ProtectedRoute({
  ...
  const { isLoading, isAuthenticated, role, isActive, canAccess } =
    useAuthContext();
