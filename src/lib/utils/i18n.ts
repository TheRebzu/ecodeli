...
export function useI18n() {
  ...
  return {
    ...
    format,
    // Formatage des dates
    formatDate: (date: Date | string, style: 'short' | 'long' | 'time' = 'short') => {
...
export function useErrorTranslations() {
  ...
  return {
    getErrorMessage: (errorKey: string, fallback: string = 'Une erreur est survenue') => {
      try {
        return t(errorKey)
      } catch {
        return fallback
...
export const localeUtils = {
  ...
  getLanguageName: (locale: Locale, targetLocale: Locale = defaultLocale): string => {
...
export function getLocalizedMetadata(locale: Locale) {
  const metadata = {
    fr: {
      ...
      description: 'Plateforme de crowdshipping écologique connectant expéditeurs, livreurs et prestataires de services.',
      keywords: 'livraison, écologique, crowdshipping, services, transport, France'
    },
    en: {
      title: 'EcoDeli - Ecological Delivery and Personal Services',
      description: 'Ecological crowdshipping platform connecting senders, deliverers and service providers.',
      keywords: 'delivery, ecological, crowdshipping, services, transport, France'
    }
  }
