...
export class LabelReplacer {
  ...
  private loadExtractedLabels(): ExtractedLabel[] {
    try {
      ...
      throw new Error(`Impossible de charger les labels depuis ${this.config.labelsFile}: ${error}`)
    }
  }
   ...
   */
  private async replaceInFile(filePath: string, labels: ExtractedLabel[]): Promise<void> {
      ...
      if (line && line.includes(label.text)) {
        ...
        for (const pattern of patterns) {
          if (line.includes(pattern)) {
            const newLine = line.replace(pattern, 
  ...
  private ensureTranslationImport(content: string, filePath: string): string {
    ...
    if (content.includes('useTranslations') || content.includes('import.*next-intl')) {
      return content
    }
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.startsWith('import ') && !importInserted) {
        ...
        let j = i
        while (j < lines.length && (lines[j].startsWith('import ') || lines[j].trim() === '')) {
          j++
        }
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.includes('export default function') || line.includes('export function') || 
        ...
        if (j < lines.length) {
          ...
          lines.splice(j + 1, 0, `  const ${this.config.translationFunction} = useTranslations()`)
          hookInserted = true
          break
        }
      }
   ...
   */
  private async updateTranslationFiles(labels: ExtractedLabel[]): Promise<void> {
    const translationsByNamespace = labels.reduce((acc, label) => {
      const namespace = label.namespace || this.config.defaultNamespace
    ...
    for (const locale of ['fr', 'en']) {
      ...
      try {
        ...
        for (const [namespace, namespaceLabels] of Object.entries(translationsByNamespace)) {
  ...
  generateReport(): string {
    ...
    const byFile = this.replacements.reduce((acc, replacement) => {
 ...
 */
export async function replaceLabels(config?: Partial<ReplaceConfig>): Promise<Replacement[]> {
