...
export const ecoLogger = {
  ...
  auth: {
    ...
    failed: (email: string, reason: string, ip?: string) => 
      logger.warn('Login failed', { email, reason, ip, action: 'login_failed' }),
  ...
  delivery: {
    created: (deliveryId: string, clientId: string, announceId: string) =>
      logger.info('Delivery created', { deliveryId, clientId, announceId, action: 'delivery_created' }),
    
    accepted: (deliveryId: string, delivererId: string) =>
      logger.info('Delivery accepted', { deliveryId, delivererId, action: 'delivery_accepted' }),
    
    completed: (deliveryId: string, validationCode: string) =>
      logger.info('Delivery completed', { deliveryId, validationCode, action: 'delivery_completed' }),
    
    cancelled: (deliveryId: string, reason: string) =>
      logger.warn('Delivery cancelled', { deliveryId, reason, action: 'delivery_cancelled' })
  },
  
  // Logs de paiements
  payment: {
    intent: (paymentId: string, amount: number, currency: string, userId: string) =>
      logger.info('Payment intent created', { paymentId, amount, currency, userId, action: 'payment_intent' }),
    
    success: (paymentId: string, amount: number, userId: string) =>
      logger.info('Payment successful', { paymentId, amount, userId, action: 'payment_success' }),
    
    failed: (paymentId: string, error: string, userId: string) =>
      logger.error('Payment failed', { paymentId, error, userId, action: 'payment_failed' }),
    
    refund: (paymentId: string, amount: number, reason: string) =>
      logger.info('Payment refunded', { paymentId, amount, reason, action: 'payment_refund' })
  },
  
  // Logs administratifs
  admin: {
    documentValidation: (userId: string, documentType: string, status: string, adminId: string) =>
      logger.info('Document validation', { userId, documentType, status, adminId, action: 'document_validation' }),
    
    userActivation: (userId: string, status: string, adminId: string) =>
      logger.info('User activation status changed', { userId, status, adminId, action: 'user_activation' }),
  ...
  system: {
    matchingRun: (announceId: string, matchesFound: number, topScore: number) =>
      logger.info('Matching algorithm executed', { announceId, matchesFound, topScore, action: 'matching_run' }),
    
    monthlyBilling: (providerId: string, invoiceId: string, amount: number) =>
      logger.info('Monthly billing generated', { providerId, invoiceId, amount, action: 'monthly_billing' }),
    
    notificationSent: (userId: string, type: string, channel: string) =>
      logger.info('Notification sent', { userId, type, channel, action: 'notification_sent' }),
...
export const createContextLogger = (userId?: string, role?: string, sessionId?: string) => {
