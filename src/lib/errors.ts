...
const errorMessages = {
  fr: {
    ...
    [ErrorCode.ACCOUNT_INACTIVE]: 'Votre compte est inactif. Contactez le support.',
    [ErrorCode.ACCOUNT_SUSPENDED]: 'Votre compte a été suspendu. Contactez le support.',
    [ErrorCode.EMAIL_NOT_VERIFIED]: 'Veuillez vérifier votre email avant de continuer',
    ...
    [ErrorCode.DELIVERY_ALREADY_ACCEPTED]: 'Cette livraison a déjà été acceptée',
    ...
    [ErrorCode.INSUFFICIENT_WALLET_BALANCE]: 'Solde insuffisant dans le portefeuille',
  ...
  en: {
    ...
    [ErrorCode.EMAIL_NOT_VERIFIED]: 'Please verify your email before continuing',
    [ErrorCode.INSUFFICIENT_PERMISSIONS]: 'You don\'t have the necessary permissions',
    ...
    [ErrorCode.ANNOUNCEMENT_ALREADY_TAKEN]: 'This announcement has already been taken',
...
export const createError = {
  auth: {
    invalidCredentials: (context?: Record<string, any>) => 
      new EcoDeliError(ErrorCode.INVALID_CREDENTIALS, undefined, 401, true, context),
    ...
    insufficientPermissions: (context?: Record<string, any>) => 
      new EcoDeliError(ErrorCode.INSUFFICIENT_PERMISSIONS, undefined, 403, true, context),
    
    accountInactive: (context?: Record<string, any>) => 
      new EcoDeliError(ErrorCode.ACCOUNT_INACTIVE, undefined, 403, true, context),
  },
  
  validation: {
    required: (field: string) => 
      new EcoDeliError(ErrorCode.REQUIRED_FIELD, `${field} is required`, 422, true, { field }),
    
    invalidEmail: (email: string) => 
      new EcoDeliError(ErrorCode.INVALID_EMAIL, undefined, 422, true, { email }),
    
    invalidPhone: (phone: string) => 
      new EcoDeliError(ErrorCode.INVALID_PHONE, undefined, 422, true, { phone }),
  },
  
  delivery: {
    notFound: (deliveryId: string) => 
      new EcoDeliError(ErrorCode.DELIVERY_NOT_FOUND, undefined, 404, true, { deliveryId }),
    
    alreadyAccepted: (deliveryId: string) => 
      new EcoDeliError(ErrorCode.DELIVERY_ALREADY_ACCEPTED, undefined, 409, true, { deliveryId }),
    
    invalidCode: (code: string) => 
      new EcoDeliError(ErrorCode.INVALID_VALIDATION_CODE, undefined, 400, true, { code }),
  },
  ...
  system: {
    database: (originalError: Error) => 
      new EcoDeliError(ErrorCode.DATABASE_ERROR, originalError.message, 500, false, { 
        originalError: originalError.message,
        stack: originalError.stack 
      }),
    
    external: (service: string, error: any) => 
      new EcoDeliError(ErrorCode.EXTERNAL_API_ERROR, `${service} error`, 502, true, { 
        service, 
        error: error.message 
      }),
  }
...
export function getErrorMessage(code: ErrorCode, locale: 'fr' | 'en' = 'fr'): string {
  return errorMessages[locale][code] || errorMessages.fr[code] || code
}

// Handler d'erreur pour les API routes Next.js
export function handleApiError(error: unknown, locale: 'fr' | 'en' = 'fr'): NextResponse {
  let response: {
    success: boolean
    message: string
    error: string
