...
import { type MobileMenuProps, type MenuItem } from '@/components/layout/types/layout.types'
...
export function MobileMenu({
  ...
  const renderMenuItem = (item: MenuItem, level = 0) => {
    ...
    if (hasChildren) {
      return (
        <div key={item.key} className={cn("border-b border-border last:border-b-0")}>
          <button
          ...
          >
            ...
            <div className="flex items-center justify-between p-4 border-b border-border">
              <div className="flex items-center space-x-3">
                ...
                />
                <span className="font-bold text-lg text-foreground">EcoDeli</span>
              </div>
  ...
  const baseItems: MenuItem[] = [
    {
      ...
      icon: ({ className }) => (
        <svg className={className} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 5a2 2 0 012-2h4a2 2 0 012 2v0M8 11h.01M12 11h.01M16 11h.01M8 15h.01M12 15h.01M16 15h.01" />
        </svg>
      )
    }
  ]
    ...
    case 'CLIENT':
      return [
        ...
        },
        {
          ...
          label: 'Services',
          children: [
            { key: 'book-service', label: 'Réserver', href: '/client/services' },
            { key: 'my-bookings', label: 'Mes réservations', href: '/client/bookings' }
          ]
        }
      ]
