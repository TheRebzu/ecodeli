    ...
    <div className="space-y-6">
        ...
        <Dialog open={contractTemplateDialogOpen} onOpenChange={setContractTemplateDialogOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>{t('selectTemplateDescription')}</DialogDescription>
            </DialogHeader>
            ...
            <DialogFooter>
              <Button variant="outline" onClick={() => setContractTemplateDialogOpen(false)}>
                {t('cancel')}
              </Button>
            </DialogFooter>
          </DialogContent>
      ...
      <Tabs defaultValue="ALL" value={activeTab} onValueChange={handleTabChange}>
        <TabsList className="grid w-full grid-cols-2 md:grid-cols-6">
          ...
          <TabsTrigger value={ContractStatus.DRAFT}>{t('status.draft')}</TabsTrigger>
          <TabsTrigger value={ContractStatus.PENDING_SIGNATURE}>
            {t('status.pendingSignature')}
          </TabsTrigger>
          <TabsTrigger value={ContractStatus.ACTIVE}>{t('status.active')}</TabsTrigger>
          <TabsTrigger value={ContractStatus.TERMINATED}>{t('status.terminated')}</TabsTrigger>
          <TabsTrigger value={ContractStatus.EXPIRED}>{t('status.expired')}</TabsTrigger>
        </TabsList>
                  ...
                  <tbody>
                    {filteredContracts.map(contract => (
                      <tr key={contract.id} className="border-t">
                        <td className="py-3 px-2 font-medium">{contract.title}</td>
                        <td className="py-3 px-2">{contract.merchant}</td>
                        <td className="py-3 px-2">{getStatusBadge(contract.status)}</td>
                        <td className="py-3 px-2">{formatDate(contract.createdAt)}</td>
                        <td className="py-3 px-2">{formatDate(contract.expiresAt)}</td>
