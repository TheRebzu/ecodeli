    ...
    <Card className="w-full">
      <CardHeader>
        <div className="flex justify-between items-start">
          <div>
            <CardTitle className="text-lg">{getDocumentTypeName(document.type)}</CardTitle>
            <CardDescription>
            ...
            document.rejectionReason && (
              <div className="mt-2 p-3 bg-red-50 border border-red-200 rounded-md">
                <div className="flex gap-2 items-start">
                  ...
                  <div>
                    <div className="text-sm font-medium text-red-700">{t('rejection_reason')}:</div>
                    <div className="text-sm text-red-600">{document.rejectionReason}</div>
                  </div>
                </div>
              </div>
            )}

          {document.verificationStatus === VerificationStatus.APPROVED && document.notes && (
            <div className="mt-2 p-3 bg-green-50 border border-green-200 rounded-md">
              <div className="flex gap-2 items-start">
                <Check className="h-5 w-5 text-green-500 mt-0.5" />
                <div>
                  <div className="text-sm font-medium text-green-700">{t('approval_notes')}:</div>
                  <div className="text-sm text-green-600">{document.notes}</div>
                </div>
              </div>
            </div>
          )}
      ...
      <Dialog open={isRejectDialogOpen} onOpenChange={setIsRejectDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>{t('reject_document_description')}</DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={isApproveDialogOpen} onOpenChange={setIsApproveDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>{t('approve_document_description')}</DialogDescription>
          </DialogHeader>
          ...
          <div className="py-4">
            ...
            />
            <p className="text-sm text-muted-foreground mt-2">{t('notes_optional')}</p>
          </div>
          ...
          <div className="mt-2">
            {canShowPreview() ? (
              ...
              <div className="flex flex-col items-center justify-center p-8 border border-dashed rounded-md">
                ...
                <p className="text-muted-foreground">{t('preview_not_available')}</p>
                <Button
                  variant="outline"
                  className="mt-4"
