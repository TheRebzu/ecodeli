...
const ExtendReservationDialog = dynamic<ExtendReservationDialogProps>(
  () => import('./extend-reservation-dialog').then(mod => mod.ExtendReservationDialog),
  { ssr: false }
);
...
export function BoxReservations() {
  ...
  const [dialogOpen, setDialogOpen] = useState<'extend' | 'access' | null>(null);
    ...
    return (
      <Card className="w-full">
        <CardHeader>
          ...
          <CardDescription>{t('reservations.noReservationsFound')}</CardDescription>
        </CardHeader>
        <CardContent className="flex justify-center py-8">
          <div className="text-center space-y-3">
            <PackageCheck className="mx-auto h-12 w-12 text-muted-foreground" />
            <h3 className="font-medium text-lg">{t('reservations.noReservations')}</h3>
            <p className="text-sm text-muted-foreground">{t('reservations.startSearching')}</p>
            <Button className="mt-4" onClick={() => router.push('/client/storage/search')}>
  ...
  const renderReservationCard = (reservation: ReservationWithBoxAndWarehouse) => {
    const { box, startDate, endDate, status, totalPrice } = reservation;
    const today = new Date();
    const isActive = ['ACTIVE', 'EXTENDED'].includes(status);
    const isExpiringSoon =
      isActive && endDate && (endDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24) <= 3;

    return (
      <Card
        key={reservation.id}
        className={cn('transition-all duration-200', isExpiringSoon && 'border-orange-400')}
      >
        <CardHeader>
          <div className="flex justify-between items-start">
            ...
            <div className="text-right">
              ...
              <p className="text-xs text-muted-foreground">{t('reservations.totalPrice')}</p>
            </div>
          </div>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 gap-4 mt-2">
            <div className="space-y-1">
              <div className="text-xs text-muted-foreground">{t('reservations.startDate')}</div>
            ...
            <div className="space-y-1">
              <div className="text-xs text-muted-foreground">{t('reservations.endDate')}</div>
              <div className="flex items-center gap-1">
                <Calendar className="h-3.5 w-3.5" />
          ...
          <div className="mt-4 space-y-1">
            <div className="text-xs text-muted-foreground">{t('reservations.boxDetails')}</div>
  ...
  return (
    <div className="space-y-6 w-full">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <h2 className="text-2xl font-bold tracking-tight">{t('reservations.title')}</h2>
      ...
      <Tabs defaultValue="ACTIVE" className="w-full" onValueChange={setActiveTab}>
        <TabsList className="grid grid-cols-3 mb-4">
          ...
          <TabsTrigger value="COMPLETED">{t('reservations.completed')}</TabsTrigger>
