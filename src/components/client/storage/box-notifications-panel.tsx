...
export function BoxNotificationsPanel() {
  ...
  const createSubscriptionMutation = api.storage.subscribeToAvailability.useMutation({
    onSuccess: () => {
      subscriptionsQuery.refetch();
    },
  });
  const deleteSubscriptionMutation = api.storage.deactivateSubscription.useMutation({
    onSuccess: () => {
      subscriptionsQuery.refetch();
    },
  });
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="active" onValueChange={setActiveTab}>
        <TabsList className="grid grid-cols-2 mb-4">
          <TabsTrigger value="active">{t('notifications.activeTab')}</TabsTrigger>
          <TabsTrigger value="create">{t('notifications.createTab')}</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="active">
          {Array.isArray(subscriptions) && subscriptions.length > 0 ? (
                    ...
                    <CardHeader>
                      ...
                      <CardDescription>
                        {t('notifications.createdAt', {
                          date: format(new Date(sub.createdAt), 'PPP', { locale: fr }),
                        })}
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                      <div className="space-y-3">
                        <div className="flex items-center gap-2">
                          ...
                          <div className="text-sm">
                            <span className="font-medium">{t('notifications.dateRange')}:</span>{' '}
                            {format(new Date(sub.startDate), 'P', { locale: fr })} -{' '}
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div className="text-sm">
                            <span className="font-medium">{t('notifications.location')}:</span>{' '}
                            {sub.location || t('notifications.anyLocation')},{' '}
                            {sub.radius ? t('notifications.radius', { radius: sub.radius }) : ''}
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div className="text-sm">
                            <span className="font-medium">{t('notifications.boxDetails')}:</span>{' '}
                            {sub.boxType
            ...
            <Alert>
              ...
              <AlertDescription>{t('notifications.noNotificationsDesc')}</AlertDescription>
            </Alert>
          )}
        </TabsContent>
        ...
        <TabsContent value="create">
          <Card>
            <CardHeader>
              ...
              <CardDescription>{t('notifications.createDescription')}</CardDescription>
            </CardHeader>
            <CardContent>
              <BoxSearchForm
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>{t('notifications.deleteDescription')}</AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
