...
export default function DeliveryRatingForm({
  ...
  const [photos, setPhotos] = useState<string[]>(existingRating?.photoUrls || []);
  ...
  const RatingStar = ({ value }: { value: number }) => (
    <button
      ...
      className={cn(
        ...
        currentRating >= value ? 'text-yellow-500 scale-110' : 'text-gray-300 hover:text-yellow-200'
      )}
      disabled={isSubmitting}
    >
      <Star className="w-8 h-8" fill={currentRating >= value ? 'currentColor' : 'none'} />
    </button>
  );
    ...
    <Card className={className}>
      ...
      <CardContent className="space-y-4">
        ...
        <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-4">
          ...
          <div className="space-y-2 pt-4">
            ...
            />
            {errors.comment && (
              <p className="text-sm text-destructive">{errors.comment.message as string}</p>
            )}
              ...
              {photos.length < 3 && (
                <button
                  ...
                  className={cn(
                    ...
                    (isUploading || isSubmitting || !isOnline) && 'opacity-50 cursor-not-allowed'
                  )}
                >
                  {isUploading ? (
      ...
      <CardFooter className="flex justify-between">
        ...
        <ButtonWithLoading
          ...
          disabled={
            (!canRate && !existingRating) || currentRating === 0 || isSubmitting || !isOnline
          }
