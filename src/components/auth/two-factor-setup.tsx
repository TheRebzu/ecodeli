  ...
  if (isSetupError) {
    return (
      <Card>
        ...
        <CardFooter>
          <Button onClick={() => refetchSetup()}>{t('setup.error.retryButton')}</Button>
        </CardFooter>
      </Card>
    );
  }
      ...
      <Card>
        ...
        <CardContent>
          <Alert variant="default" className="bg-success/10 border-success">
            ...
            <AlertDescription>{t('alreadyEnabled.alert.description')}</AlertDescription>
          </Alert>
        </CardContent>
        <CardFooter className="flex flex-col space-y-2 sm:flex-row sm:justify-between sm:space-x-2 sm:space-y-0">
          ...
          >
            {disableTwoFactor.isLoading ? t('disable.loading') : t('disable.button')}
          </Button>
        </CardFooter>
      </Card>
    );
  ...
  if (showBackupCodes && setupData?.backupCodes) {
    return (
      <Card>
        ...
        <CardContent>
          <Alert variant="default" className="mb-4 bg-warning/10 border-warning">
            <AlertCircle className="h-4 w-4 text-warning" />
            <AlertTitle>{t('backupCodes.warning.title')}</AlertTitle>
            <AlertDescription>{t('backupCodes.warning.description')}</AlertDescription>
          </Alert>
        ...
        <div className="space-y-2">
          ...
          <div className="flex space-x-2">
            ...
            <Button
              ...
              disabled={verificationCode.length !== 6 || verifyTwoFactor.isLoading}
            >
              {verifyTwoFactor.isLoading ? t('setup.verifying') : t('setup.verify')}
            </Button>
          </div>
          <p className="text-xs text-muted-foreground">{t('setup.verificationHelp')}</p>
        </div>
      </CardContent>
    </Card>
  );
