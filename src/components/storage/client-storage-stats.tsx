...
export function ClientStorageStats({
  ...
  const { data: stats, isLoading, error } = api.storage.getMyStorageStats.useQuery(
    undefined,
    {
  ...
  const getExperienceLevel = (totalReservations: number) => {
    if (totalReservations >= 50) return { level: 'Expert', color: 'text-purple-600', badge: 'purple' };
    if (totalReservations >= 20) return { level: 'Avancé', color: 'text-blue-600', badge: 'blue' };
    if (totalReservations >= 10) return { level: 'Régulier', color: 'text-green-600', badge: 'green' };
    if (totalReservations >= 5) return { level: 'Intermédiaire', color: 'text-yellow-600', badge: 'yellow' };
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="text-center p-4 bg-green-50 rounded-lg">
                  ...
                  <div className="text-sm text-green-700 mb-2">CO₂ économisé</div>
                  <div className="text-xs text-muted-foreground">
                    Équivalent à {Math.round(stats.sustainability.co2Saved / 2.3)} km en voiture
                  </div>
                </div>
                ...
                <div className="text-center p-4 bg-blue-50 rounded-lg">
                  ...
                  <div className="text-sm text-blue-700 mb-2">Déchets évités</div>
                  <div className="text-xs text-muted-foreground">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-sm text-muted-foreground">
                      Box actuellement réservée{stats.activeReservations > 1 ? 's' : ''}
                    </div>
                  </div>
              ...
              <CardContent>
                <div className="space-y-3">
                  {stats.favoriteBoxes.slice(0, 3).map((favorite, index) => (
                    <div key={favorite.box?.id} className="flex items-center justify-between p-3 border rounded-lg">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 bg-primary/10 rounded-full flex items-center justify-center text-sm font-bold">
                          #{index + 1}
                        </div>
                        <div>
                          <div className="font-medium">{favorite.box?.name}</div>
                      ...
                      <div className="text-right">
                        <div className="font-medium">{favorite.usageCount} fois</div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="p-4 border rounded-lg">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-medium">Valeur moyenne par réservation</span>
                ...
                <div className="p-4 border rounded-lg">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-sm font-medium">Réservations terminées</span>
