...
export function OptimalPricingCalculator({
  ...
  const { data: pricing, isLoading, error } = api.storage.calculateOptimalPricing.useQuery(
    {
      boxId,
      startDate,
      endDate,
        ...
        <div className="bg-muted/50 p-4 rounded-lg">
          <div className="flex items-start gap-2">
            ...
            <div className="space-y-1 text-sm">
              <div>
                <span className="font-medium">Prix par jour :</span> {pricing.pricePerDay.toFixed(2)}€
              </div>
              <div>
                <span className="font-medium">Durée :</span> {pricing.days} jour{pricing.days > 1 ? 's' : ''}
              </div>
              {hasDiscounts && (
