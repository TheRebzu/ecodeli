...
import { KeyRound, QrCode, MapPin, Copy, Phone, Shield, Building2 } from 'lucide-react';
...
export function AccessBoxDialog({ reservation, open, onClose }: AccessBoxDialogProps) {
      ...
      <DialogContent className="sm:max-w-[425px]">
        ...
        <div className="space-y-4 py-4">
          <div className="rounded-md border p-4 space-y-3">
            ...
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium">{t('access.warehouse')}</span>
            ...
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium">{t('access.warehouseAddress')}</span>
              <div className="flex items-center gap-2">
                <span className="text-sm">{reservation.box.warehouse.address}</span>
                <Button
                  variant="ghost"
                  size="icon"
                  className="h-6 w-6"
            ...
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium">{t('access.warehousePhone')}</span>
              <div className="flex items-center gap-2">
                <span className="text-sm">
                  {reservation.box.warehouse.contactPhone || t('access.phoneNotAvailable')}
                </span>
                <Button
                  variant="ghost"
                  size="icon"
                  className="h-6 w-6"
                  onClick={() =>
                    window.open(`tel:${reservation.box.warehouse.contactPhone}`, '_blank')
                  }
          ...
          {accessCode ? (
            <div className="space-y-3">
              <Alert>
                ...
                <AlertDescription>{t('access.codeDescription')}</AlertDescription>
              </Alert>

              <div className="flex items-center justify-center p-4 border rounded-md bg-muted">
                <div className="text-3xl font-mono tracking-widest select-all">{accessCode}</div>
                <Button
                  variant="ghost"
                  size="icon"
                  className="ml-2"
            ...
            <div className="flex flex-col items-center gap-4 p-6 border rounded-md">
              ...
              <p className="text-center text-sm">{t('access.getCodeDescription')}</p>
              <Button className="w-full" onClick={handleGetAccessCode} disabled={isLoading}>
