...
export function DocumentVerification({ document, isAdmin = false }: DocumentVerificationProps) {
  ...
  const getVerificationStatusBadge = (status: string) => {
    switch (status) {
      case "APPROVED":
        return <Badge className="bg-green-600"><CheckCircle2 className="w-3 h-3 mr-1" /> {t("verification.approved")}</Badge>;
      case "REJECTED":
        return <Badge variant="destructive"><XCircle className="w-3 h-3 mr-1" /> {t("verification.rejected")}</Badge>;
      case "PENDING":
      default:
        return <Badge variant="outline" className="text-orange-600 border-orange-600"><Clock className="w-3 h-3 mr-1" /> {t("verification.pending")}</Badge>;
    }
  };
    ...
    <Card className="w-full">
      ...
      <CardContent className="space-y-4">
        <div className="flex items-center justify-between">
          <span className="text-sm text-muted-foreground">{t("filename")}: {document.filename}</span>
          <a 
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {document.verifications.length > 0 ? (
              ...
              <span className="text-sm text-muted-foreground">{t("verification.noVerifications")}</span>
            )}
          </div>
        </div>
