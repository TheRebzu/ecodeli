...
export function MerchantInvoiceList({ className }: MerchantInvoiceListProps) {
  ...
  const [statusFilter, setStatusFilter] = useState<'ALL' | 'DRAFT' | 'PENDING' | 'PAID' | 'CANCELLED' | 'REFUNDED'>('ALL');
  const [dateRange, setDateRange] = useState<{ from?: Date; to?: Date }>({});
  const [invoiceTypeFilter, setInvoiceTypeFilter] = useState<'MERCHANT_FEE' | 'SERVICE' | 'OTHER'>('MERCHANT_FEE');

  // Récupérer les factures du merchant
  const { data: invoicesData, isLoading: invoicesLoading } = api.invoice.getMerchantInvoices.useQuery({
    page: currentPage,
    limit: 10,
    ...
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {loading ? <Skeleton className="h-8 w-20" /> : formatCurrency(stats?.totalAmount || 0)}
            </div>
            <p className="text-xs text-muted-foreground">
              Facturé ce mois
            </p>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              {loading ? <Skeleton className="h-4 w-16" /> : `${stats?.paymentRate.toFixed(1)}% payées`}
            </p>
          </CardContent>
        </Card>
      ...
      {!loading && stats && stats.pendingInvoices > 0 && (
        <Alert>
          ...
          <AlertDescription>
            Vous avez {stats.pendingInvoices} facture(s) en attente de paiement pour un montant total de {formatCurrency((stats.totalAmount || 0) - (stats.paidAmount || 0))}.
          </AlertDescription>
        </Alert>
      )}
                  ...
                  <TableRow key={invoice.id}>
                    ...
                    <TableCell>
                      <Badge variant="outline">
                        {invoice.invoiceType === 'MERCHANT_FEE' ? 'Frais merchant' :
                         invoice.invoiceType === 'SERVICE' ? 'Service' : 'Autre'}
                      </Badge>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <div className="font-medium">{formatCurrency(parseFloat(invoice.totalAmount.toString()))}</div>
      ...
      {pagination && pagination.pages > 1 && (
        <div className="flex items-center justify-between">
          <div className="text-sm text-muted-foreground">
            Affichage de {(currentPage - 1) * 10 + 1} à {Math.min(currentPage * 10, pagination.total)} sur {pagination.total} factures
          </div>
