...
export function NavBar({ locale, role = 'client', className, items }: NavBarProps) {
  ...
  const defaultNavItems: Record<string, NavItem[]> = {
    client: [
      ...
      { label: 'Livraisons', icon: Package, href: `/${locale}/client/deliveries` },
      { label: 'Services', icon: ShoppingBag, href: `/${locale}/client/services` },
      { label: 'Messages', icon: MessageSquare, href: `/${locale}/client/messages`, badge: 3 },
      { label: 'Profil', icon: User, href: `/${locale}/client/profile` },
    ],
    deliverer: [
      { label: 'Accueil', icon: Home, href: `/${locale}/deliverer` },
      { label: 'Livraisons', icon: Package, href: `/${locale}/deliverer/deliveries` },
      { label: 'Messages', icon: MessageSquare, href: `/${locale}/deliverer/messages` },
      { label: 'Profil', icon: User, href: `/${locale}/deliverer/profile` },
    ],
    merchant: [
      { label: 'Accueil', icon: Home, href: `/${locale}/merchant` },
      { label: 'Annonces', icon: ShoppingBag, href: `/${locale}/merchant/announcements` },
      { label: 'Messages', icon: MessageSquare, href: `/${locale}/merchant/messages` },
      { label: 'Profil', icon: User, href: `/${locale}/merchant/profile` },
    ],
    provider: [
      { label: 'Accueil', icon: Home, href: `/${locale}/provider` },
      { label: 'Services', icon: ShoppingBag, href: `/${locale}/provider/services` },
      { label: 'Messages', icon: MessageSquare, href: `/${locale}/provider/messages` },
...
export function MobileNavBar({ locale, role }: { locale: string; role?: string }) {
  return <NavBar locale={locale} role={role as any} />;
}

// Barre de navigation latérale pour les tablettes et ordinateurs
export function DesktopSideNav({ locale, role }: { locale: string; role?: string }) {
  return (
