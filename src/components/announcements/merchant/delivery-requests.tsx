...
export function DeliveryRequests({ announcementId, requests = [], isLoading = false }: DeliveryRequestsProps) {
              ...
              <div className="p-6">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                  <div className="flex items-center gap-3">
                    <Avatar>
                      <AvatarImage src={request.deliverer?.image || ''} alt={request.deliverer?.name || ''} />
                      <AvatarFallback>
                        <User className="h-5 w-5" />
                      </AvatarFallback>
                    </Avatar>
                    <div>
                      <div className="font-medium">{request.deliverer?.name || t('unknownDeliverer')}</div>
                      {request.deliverer?.rating !== undefined && (
                        <div className="flex items-center text-sm text-muted-foreground">
                          ...
                          {request.deliverer.totalDeliveries && (
                            <span className="ml-2">({request.deliverer.totalDeliveries} livraisons)</span>
                          )}
                        </div>
                      )}
                    </div>
                  ...
                  <div className="flex items-center gap-2">
                    <Badge variant={request.status === 'PENDING' ? 'outline' : request.status === 'ACCEPTED' ? 'success' : 'destructive'}>
                      {t(`status.${request.status.toLowerCase()}`)}
                    </Badge>
                    <div className="font-semibold">
                      {request.proposedPrice !== undefined ? formatCurrency(request.proposedPrice) : ''}
                    </div>
                  </div>
                </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-4">
                <Avatar className="h-14 w-14">
                  <AvatarImage src={selectedRequest.deliverer.image || ''} alt={selectedRequest.deliverer.name} />
                  <AvatarFallback>
