...
export function AnnouncementDetail() {
  ...
  const acceptApplicationMutation =
    api.announcement.acceptApplication.useMutation({
      onSuccess: () => {
  ...
  const { data: messagesData, refetch: refetchMessages } =
    api.announcement.getMessages.useQuery(
      { announcementId: id },
      {
              ...
              <div className="mt-6 space-y-4">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-gray-700">
                      {formatExactDate(announcement.deadline)}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">
                      {announcement.packageValue.toFixed(2)}€
                    </span>
                  </div>
              ...
              <div className="mt-4 space-y-3">
                ...
                {isAssignedDeliverer && isAssigned && (
                  <Button
                    className="w-full"
                    onClick={() =>
                      handleUpdateStatus(AnnouncementStatus.IN_TRANSIT)
                    }
                ...
                {isAssignedDeliverer && isInTransit && (
                  <Button
                    className="w-full"
                    onClick={() =>
                      handleUpdateStatus(AnnouncementStatus.DELIVERED)
                    }
                    disabled={updateStatusMutation.isLoading}
                  >
                    {updateStatusMutation.isLoading && (
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      ...
                      <AlertDialogFooter>
                        ...
                        <AlertDialogAction
                          onClick={() =>
                            handleUpdateStatus(AnnouncementStatus.CANCELLED)
                          }
                          disabled={updateStatusMutation.isLoading}
                        >
                          {updateStatusMutation.isLoading && (
            ...
            <CardContent>
              <div className="space-y-8">
                <div className="flex items-center gap-4">
                  <div
                    className={`rounded-full p-2 ${announcement.status === AnnouncementStatus.OPEN || announcement.status === AnnouncementStatus.ASSIGNED || announcement.status === AnnouncementStatus.IN_TRANSIT || announcement.status === AnnouncementStatus.DELIVERED ? "bg-green-100 text-green-600" : "bg-gray-100 text-gray-400"}`}
                  >
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      {formatExactDate(announcement.createdAt)}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-4">
                  <div
                    className={`rounded-full p-2 ${announcement.status === AnnouncementStatus.ASSIGNED || announcement.status === AnnouncementStatus.IN_TRANSIT || announcement.status === AnnouncementStatus.DELIVERED ? "bg-green-100 text-green-600" : "bg-gray-100 text-gray-400"}`}
                  >
                    ...
                    {announcement.deliverer ? (
                      <div className="flex items-center gap-2 mt-1">
                        <Avatar className="h-6 w-6">
                          <AvatarImage
                            src={announcement.deliverer.image || ""}
                          />
                          <AvatarFallback>
                            {announcement.deliverer.name?.charAt(0) || "D"}
                          </AvatarFallback>
                        </Avatar>
                ...
                <div className="flex items-center gap-4">
                  <div
                    className={`rounded-full p-2 ${announcement.status === AnnouncementStatus.IN_TRANSIT || announcement.status === AnnouncementStatus.DELIVERED ? "bg-green-100 text-green-600" : "bg-gray-100 text-gray-400"}`}
                  >
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      {announcement.status === AnnouncementStatus.IN_TRANSIT ||
                ...
                <div className="flex items-center gap-4">
                  <div
                    className={`rounded-full p-2 ${announcement.status === AnnouncementStatus.DELIVERED ? "bg-green-100 text-green-600" : "bg-gray-100 text-gray-400"}`}
                  >
          ...
          <TabsContent value="messages" className="mt-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  {isClient
                    ? t("messagesWithDeliverer", {
                        name: announcement.deliverer?.name || t("deliverer"),
                      })
                    : t("messagesWithClient", {
