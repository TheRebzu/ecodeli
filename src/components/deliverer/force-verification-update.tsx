...
export default function ForceVerificationUpdate() {
  ...
  const forceUpdateMutation = api.verification.forceUpdateDelivererVerification.useMutation({
    onSuccess: data => {
      setIsUpdating(false);
      toast({
  ...
  return (
    <div className="bg-yellow-50 dark:bg-yellow-900/30 border border-yellow-200 dark:border-yellow-800 p-4 rounded-md my-6">
      <h3 className="text-lg font-semibold mb-2">{t('verification.forceUpdate.title')}</h3>
      <p className="text-sm text-muted-foreground mb-4">
        {t('verification.forceUpdate.description')}
      </p>
      <Button onClick={handleForceUpdate} variant="secondary" disabled={isUpdating}>
        {isUpdating ? (
          <>
