...
export default function DelivererPlanning() {
  ...
  // Queries
  const { data: delivererSchedule, refetch: refetchSchedule } = api.deliverer.schedule.getWeeklySchedule.useQuery({
  ...
  const calendarEvents = useMemo(() => {
    ...
    if (delivererSchedule?.deliveries) {
      delivererSchedule.deliveries.forEach(delivery => {
        ...
        const colors = statusColors[delivery.status as keyof typeof statusColors] || statusColors.SCHEDULED;
        ...
        <Alert>
          ...
          <AlertDescription>
            <div className="flex items-center justify-between">
              <span>
                {routeOptimizations.length} suggestions d'optimisation disponibles pour augmenter vos gains
              </span>
              <Button 
                variant="outline" 
                size="sm"
          ...
          {selectedEvent && (
            <div className="space-y-4">
              ...
              {selectedEvent.extendedProps.type === 'route' && (
                <Button 
                  className="w-full" 
                  onClick={() => handleOptimizeRoute(format(new Date(selectedEvent.start), 'yyyy-MM-dd'))}
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center space-x-2">
              <Switch
                ...
                onCheckedChange={(checked) => setAvailabilityForm({...availabilityForm, isActive: checked})}
              />
