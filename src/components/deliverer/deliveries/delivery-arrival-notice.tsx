    ...
    >
      ...
      <CardHeader className="pb-2">
        <div className="flex justify-between items-center">
          <Badge variant={getUrgencyColor() as any} className="uppercase text-xs font-bold">
            {urgencyLevel === 'critical'
              ? t('arriving')
              : urgencyLevel === 'urgent'
                ? t('almostThere')
      ...
      <CardContent className="pb-3">
        <div className="grid grid-cols-2 gap-4 mb-4">
          ...
          <div className="flex flex-col">
            ...
            <span className="font-semibold flex items-center">
              ...
              {arrivalInfo.currentLocation ? arrivalInfo.currentLocation : t('unknown')}
            </span>
          </div>
        </div>
        ...
        {arrivalInfo.delivererName && (
          <div className="flex items-center space-x-3 mb-2">
            ...
            <div className="text-sm">
              ...
              <p className="text-muted-foreground text-xs">{t('yourDeliverer')}</p>
            </div>
          </div>
        )}
        ...
        {arrivalInfo.special && arrivalInfo.special.trim() && (
          <Alert className="mt-2 p-2">
            <AlertCircle className="h-4 w-4" />
            <AlertDescription className="text-xs">{arrivalInfo.special}</AlertDescription>
          </Alert>
        )}
      </CardContent>
