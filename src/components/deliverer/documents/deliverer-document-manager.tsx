...
export default function DelivererDocumentManager() {
  ...
  const { data: verificationStatus, refetch } = api.delivery.verification.getDelivererStatus.useQuery();

  // Upload d'un document
  const uploadMutation = api.delivery.deliverer.uploadDocument.useMutation({
    onSuccess: () => {
      toast({
  ...
  const getDocumentStatus = (documentType: string): DocumentStatus | null => {
    if (!verificationStatus?.applications?.[0]?.requiredDocuments) return null;
    
    return verificationStatus.applications[0].requiredDocuments.find(
  ...
  const handleUpload = (documentType: string, data: any) => {
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
              <div>
                <div className="text-2xl font-bold text-green-600">
                  {REQUIRED_DOCUMENTS.filter(doc => getDocumentStatus(doc.type)?.status === 'APPROVED').length}
                </div>
                <div className="text-sm text-muted-foreground">Approuvés</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-yellow-600">
                  {REQUIRED_DOCUMENTS.filter(doc => getDocumentStatus(doc.type)?.status === 'PENDING').length}
                </div>
                <div className="text-sm text-muted-foreground">En attente</div>
              </div>
              <div>
                <div className="text-2xl font-bold text-red-600">
                  {REQUIRED_DOCUMENTS.filter(doc => getDocumentStatus(doc.type)?.status === 'REJECTED').length}
                </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      >
                        ...
                        {status?.status === 'REJECTED' ? 'Re-upload' : 'Upload'}
                      </Button>
                    </div>
                  </div>
                </CardContent>
