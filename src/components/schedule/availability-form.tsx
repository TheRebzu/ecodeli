...
import { createAvailabilitySchema } from '@/schemas/service.schema';
                            ...
                            {weekDays.map(day => (
                              <SelectItem key={day.value} value={day.value.toString()}>
                                <div className="flex items-center justify-between w-full">
                                  <span>{day.label}</span>
                                  {hasExistingAvailability(day.value) && (
                                    <Badge variant="secondary" className="ml-2">Existe</Badge>
                                  )}
                                </div>
                              </SelectItem>
                            ))}
                  ...
                  <div className="p-3 bg-muted rounded-lg">
                    ...
                    <div className="mt-1 text-sm">
                      {isBulkMode && selectedDays.length > 0 ? (
                        <span>
                          {selectedDays.map(day => weekDays.find(wd => wd.value === day)?.short).join(', ')} 
