      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div className="space-y-1">
            <CardTitle className="text-2xl">{t('invoice')} {invoice.number}</CardTitle>
            <div className="flex items-center gap-2">
              {statusDetails.icon}
              ...
              <table className="min-w-full divide-y divide-border">
                <thead>
                  <tr className="bg-muted/50">
                    <th className="px-4 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">{t('description')}</th>
                    <th className="px-4 py-3 text-center text-xs font-medium text-muted-foreground uppercase tracking-wider">{t('quantity')}</th>
                    <th className="px-4 py-3 text-right text-xs font-medium text-muted-foreground uppercase tracking-wider">{t('unitPrice')}</th>
                    <th className="px-4 py-3 text-center text-xs font-medium text-muted-foreground uppercase tracking-wider">{t('taxRate')}</th>
                    <th className="px-4 py-3 text-right text-xs font-medium text-muted-foreground uppercase tracking-wider">{t('amount')}</th>
                  </tr>
                </thead>
                <tbody className="bg-background divide-y divide-border">
                  {invoice.items.map((item) => (
                    <tr key={item.id}>
                      ...
                      <td className="px-4 py-3 text-sm text-right">{formatCurrency(item.unitPrice, invoice.currency)}</td>
                      <td className="px-4 py-3 text-sm text-center">{item.taxRate}%</td>
                      <td className="px-4 py-3 text-sm font-medium text-right">{formatCurrency(item.quantity * item.unitPrice, invoice.currency)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
