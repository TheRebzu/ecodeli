...
export default function DocumentUploader({
  ...
  const uploadDocumentMutation = api.auth.verification.uploadDocument.useMutation({
    onSuccess: (data, variables) => {
      setFiles(prev => prev.map(file => 
        file.id === variables.fileId 
          ? { ...file, status: "success", progress: 100, url: data.document.fileUrl }
          : file
      ));
    ...
    onError: (error, variables) => {
      setFiles(prev => prev.map(file => 
        file.id === variables.fileId 
          ? { ...file, status: "error", progress: 0, errorMessage: error.message }
          : file
      ));
  ...
  const uploadAllFiles = () => {
    files.filter(file => file.status === "uploading" && file.progress === 0)
         .forEach(uploadFile);
  };
  ...
  return (
    <Card className={cn("w-full", className)}>
      <CardHeader>
        ...
        <CardDescription>
          Téléchargez vos documents de vérification. Formats acceptés : JPG, PNG, PDF (max {maxSize}MB)
        </CardDescription>
      </CardHeader>
        ...
        {files.length > 0 && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h4 className="font-medium">Documents sélectionnés ({files.length})</h4>
                ...
                <div key={documentFile.id} className="border rounded-lg p-4">
                  ...
                  <div className="mt-3">
                    ...
                    <Select
                      ...
                      onValueChange={(value) => updateFileType(documentFile.id, value as DocumentType)}
                    >
                      <SelectTrigger>
                  ...
                  {documentFile.status === "uploading" && documentFile.progress > 0 && (
                    <div className="mt-3">
                      <Progress value={documentFile.progress} className="h-2" />
                    </div>
                  )}
                  ...
                  {documentFile.status === "error" && (
                    <Alert className="mt-3" variant="destructive">
                      ...
                      <AlertDescription>{documentFile.errorMessage}</AlertDescription>
                    </Alert>
                  )}
