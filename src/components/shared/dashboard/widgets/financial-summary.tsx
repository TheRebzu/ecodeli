        ...
        {points.map((point, index) => {
          ...
          return (
            <circle
              ...
              cy={y}
              r="1.5"
              className={index === data.length - 1 ? 'fill-primary' : 'fill-primary/50'}
            />
          );
        })}
      </svg>
...
export function FinancialSummary({ financials, isLoading = false }: FinancialSummaryProps) {
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-6">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <p className="text-sm font-medium text-muted-foreground">{t('currentMonthExpenses')}</p>
            <h3 className="text-3xl font-bold tracking-tight">{currentExpenses.toFixed(2)}€</h3>
          ...
          <div>
            <p className="text-sm font-medium text-muted-foreground">{t('estimatedSavings')}</p>
            <h3 className="text-3xl font-bold tracking-tight text-green-600">
              {(financials?.estimatedSavings || 0).toFixed(2)}€
            </h3>
            <p className="text-xs text-muted-foreground mt-1">{t('withEcoDeli')}</p>
          </div>
        </div>
