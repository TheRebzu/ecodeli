...
export const userRoleSchema = z.enum(['CLIENT', 'DELIVERER', 'MERCHANT', 'PROVIDER', 'ADMIN']);

export const userStatusSchema = z.enum(['PENDING_VERIFICATION', 'ACTIVE', 'SUSPENDED', 'INACTIVE']);
...
const baseUserSchemaObj = z.object({
  email: z.string().email({ message: 'Adresse email invalide' }),
...
export const providerRegisterSchema = z
  .object({
    ...
    address: z.string().min(5, { message: 'Adresse requise' }),
    phone: z.string().min(10, { message: 'Numéro de téléphone requis' }),
    services: z.array(z.string()).min(1, { message: 'Au moins un service requis' }),
  })
  .refine(data => data.password === data.confirmPassword, {
    message: 'Les mots de passe ne correspondent pas',
    path: ['confirmPassword'],
