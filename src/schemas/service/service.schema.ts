...
export const serviceSchema = z.object({ type: z.enum([
    "TRANSPORT",
    "AIRPORT_TRANSFER",
    "PET_SITTING",
    "HOUSEKEEPING",
...
export const createServiceSchema = z.object({ name: z
    .string()
...
export const updateServiceSchema = createServiceSchema.partial().extend({ id: z.string().cuid({ message: "ID de service invalide"  }),
  isActive: z.boolean().optional()});

// Schéma pour la création d'une catégorie de service
export const createServiceCategorySchema = z.object({ name: z
    .string()
...
export const updateServiceCategorySchema = createServiceCategorySchema
  .partial()
  .extend({ id: z.string().cuid({ message: "ID de catégorie invalide"  })});
...
export const createBookingSchema = z.object({ serviceId: z.string().cuid({ message: "ID de service invalide"  }),
...
export const updateBookingSchema = z.object({ id: z.string().cuid({ message: "ID de réservation invalide"  }),
  status: z
...
export const cancelBookingSchema = z.object({ bookingId: z.string().cuid({ message: "ID de réservation invalide"  }),
  reason: z
    .string()
...
export const serviceSearchSchema = z.object({ type: z
    .enum([
      "TRANSPORT",
      "AIRPORT_TRANSFER",
      "PET_SITTING",
  ...
  dateRange: z
    ...
    .optional(),
  priceRange: z
    .object({ min: z.number().min(0).optional(),
