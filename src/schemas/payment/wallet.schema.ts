 ...
 */
export const WalletSchema = z.object({ id: z.string().cuid().optional(),
  userId: z.string(),
...
export const CreateWalletSchema = WalletSchema.omit({ id: true,
  ...
  stripeAccountId: true }).extend({ userId: z.string() });

/**
 * Schéma pour mettre à jour un portefeuille
 */
export const UpdateWalletSchema = WalletSchema.partial().omit({ id: true,
  userId: true,
  balance: true });
 ...
 */
export const WalletBalanceSchema = z.object({ balance: z.number().nonnegative(),
  currency: z.string().default("EUR"),
 ...
 */
export const WalletTransactionSchema = z.object({ id: z.string().cuid().optional(),
  walletId: z.string(),
  amount: z.number(),
  currency: z.string().default("EUR"),
  type: z.enum([
 ...
 */
export const CreateTransactionSchema = z.object({ walletId: z.string(),
 ...
 */
export const WithdrawalRequestSchema = z.object({ id: z.string().cuid().optional(),
  walletId: z.string(),
 ...
 */
export const CreateWithdrawalRequestSchema = z.object({ amount: z.number().positive(),
  currency: z.string().default("EUR") });

/**
 * Schéma pour mettre à jour une demande de retrait
 */
export const UpdateWithdrawalRequestSchema = z.object({ status: z.enum(["PENDING", "PROCESSING", "COMPLETED", "FAILED", "CANCELLED"]),
  processedAt: z.date().optional(),
  rejectionReason: z.string().optional(),
  stripePayoutId: z.string().optional() });
 ...
 */
export const WalletStatsSchema = z.object({ totalEarnings: z.number().nonnegative(),
 ...
 */
export const WalletConfigSchema = z.object({ minimumWithdrawalAmount: z.number().min(1).default(10),
  automaticWithdrawal: z.boolean().default(false),
 ...
 */
export const BankInfoSchema = z.object({ iban: z
    .string()
    .min(15)
    .max(34)
    .regex(/^[A-Z0-9]+$/),
 ...
 */
export const TransactionSearchSchema = z.object({ walletId: z.string(),
  type: z
    .enum([
      "EARNING",
      "WITHDRAWAL",
...
export const walletBaseSchema = z.object({ userId: z.string().cuid(),
  currency: z.string().default("EUR") });

// Schéma pour créer un portefeuille
export const createWalletSchema = z.object({ userId: z.string().cuid("ID utilisateur invalide"),
  currency: z.string().default("EUR"),
...
export const addFundsSchema = z.object({ walletId: z.string().cuid(),
  amount: z
    .number()
    .positive()
...
export const deductFundsSchema = z.object({ walletId: z.string().cuid(),
  amount: z
    .number()
    .positive()
    .min(0.01, { message: "Le montant minimum est de 0,01 €"  }),
...
export const confirmTransactionSchema = z.object({ transactionId: z.string().cuid(),
  stripeTransferId: z.string().optional() });

// Schéma pour l'historique des transactions
export const transactionHistorySchema = z.object({ walletId: z.string().cuid(),
...
export const transferToConnectSchema = z.object({ amount: z.number().positive().min(0.01),
  destinationAccountId: z.string(),
  description: z.string().optional(),
  metadata: z.record(z.string()).optional() });

// Schéma pour la création d'un compte Stripe Connect
export const createConnectAccountSchema = z.object({ walletId: z.string().cuid(),
...
export const verifyConnectAccountSchema = z.object({ walletId: z.string().cuid() });
...
export const requestWithdrawalSchema = z.object({ walletId: z.string().cuid(),
  amount: z.number().positive(),
  preferredMethod: z
...
export const processWithdrawalSchema = z
  .object({ withdrawalId: z.string().cuid("ID retrait invalide"),
...
export const updateAutomaticWithdrawalSettingsSchema = z.object({ walletId: z.string().cuid(),
...
export const generateEarningsReportSchema = z.object({ userId: z.string().cuid(),
  startDate: z.date(),
  endDate: z.date(),
  format: z.enum(["PDF", "CSV", "JSON"]).default("PDF") });

// Schéma pour l'obtention du solde et des statistiques d'un portefeuille
export const getWalletStatsSchema = z.object({ walletId: z.string().cuid(),
...
export const walletTransactionSchema = z.object({ walletId: z.string().cuid("ID portefeuille invalide"),
...
export const walletTransactionFilterSchema = z.object({ walletId: z.string().cuid("ID portefeuille invalide"),
...
export const getWalletBalanceSchema = z.object({ walletId: z.string().cuid("ID portefeuille invalide") });

// Schéma pour mettre à jour les préférences de portefeuille
export const updateWalletPreferencesSchema = z.object({ walletId: z.string().cuid("ID portefeuille invalide"),
  minimumWithdrawalAmount: z.number().positive().optional(),
