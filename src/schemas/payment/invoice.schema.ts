...
export const invoiceBaseSchema = z.object({ amount: z.number().positive("Le montant doit être positif"),
  currency: z.string().default("EUR"),
  dueDate: z.date().min(new Date(), "La date d'échéance doit être future"),
  description: z.string().optional() });

// Schéma pour la création d'une facture
export const createInvoiceSchema = invoiceBaseSchema.extend({ userId: z.string().cuid("ID utilisateur invalide"),
  invoiceType: z
    .enum([
      "SERVICE",
      "DELIVERY",
...
export const invoiceFilterSchema = z.object({ userId: z.string().cuid("ID utilisateur invalide").optional(),
  status: z
    .enum([
      InvoiceStatus.DRAFT,
...
export const updateInvoiceStatusSchema = z.object({ invoiceId: z.string().cuid("ID facture invalide"),
  status: z.enum([
    InvoiceStatus.DRAFT,
    InvoiceStatus.ISSUED,
    InvoiceStatus.PAID,
...
export const createCreditNoteSchema = z.object({ originalInvoiceId: z.string().cuid("ID facture originale invalide"),
  amount: z.number().positive("Le montant doit être positif"),
  reason: z.string(),
  refundPaymentId: z
    .string()
    .cuid("ID paiement de remboursement invalide")
    .optional(),
  notes: z.string().optional() });

// Schéma pour envoyer une facture par email
export const sendInvoiceEmailSchema = z.object({ invoiceId: z.string().cuid("ID facture invalide"),
  recipientEmail: z.string().email("Email invalide").optional(),
  customMessage: z.string().optional(),
  includePDF: z.boolean().default(true) });

// Schéma pour générer un PDF de facture
export const generateInvoicePdfSchema = z.object({ invoiceId: z.string().cuid("ID facture invalide"),
  template: z.enum(["DEFAULT", "SIMPLE", "DETAILED"]).default("DEFAULT") });

// Schéma pour obtenir des statistiques de facturation
export const invoiceStatsSchema = z.object({ userId: z.string().cuid("ID utilisateur invalide").optional(),
  startDate: z.date(),
  endDate: z.date(),
  groupBy: z.enum(["DAY", "WEEK", "MONTH", "QUARTER", "YEAR"]).default("MONTH") });

// Schéma pour la facturation automatique mensuelle des marchands
export const monthlyMerchantBillingSchema = z.object({ merchantId: z.string().cuid("ID commerçant invalide").optional(),
  periodStart: z.date().optional(),
  periodEnd: z.date().optional(),
...
export const monthlyProviderBillingSchema = z.object({ providerId: z.string().cuid("ID prestataire invalide").optional(),
  periodStart: z.date().optional(),
  periodEnd: z.date().optional(),
  forceGenerate: z.boolean().default(false),
  autoProcess: z.boolean().default(true),
...
export const billingStatsSchema = z.object({ period: z.enum(["MONTH", "QUARTER", "YEAR"]).default("MONTH"),
...
export type MonthlyMerchantBillingInput = z.infer<
  typeof monthlyMerchantBillingSchema
>;
export type MonthlyProviderBillingInput = z.infer<
  typeof monthlyProviderBillingSchema
>;
