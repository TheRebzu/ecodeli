 ...
 */
export const CommissionSchema = z.object({ id: z.string().cuid().optional(),
  paymentId: z.string(),
 ...
 */
export const CalculateCommissionSchema = z.object({ amount: z.number().positive(),
 ...
 */
export const ApplyCommissionSchema = z.object({ paymentId: z.string(),
  serviceType: CommissionServiceTypeEnum,
  customRate: z.number().min(0).max(1).optional() });
 ...
 */
export const CreatePromotionSchema = z.object({ serviceType: z.enum(["DELIVERY", "SERVICE"]),
 ...
 */
export const CommissionSearchSchema = z.object({ serviceType: CommissionServiceTypeEnum.optional(),
...
export const commissionBaseSchema = z.object({ serviceType: z.string(),
  rate: z.number().min(0).max(100),
  isActive: z.boolean().default(true) });

// Schéma pour la création d'une commission
export const createCommissionSchema = commissionBaseSchema.extend({ description: z.string().optional(),
...
export const updateCommissionSchema = z.object({ commissionId: z.string().cuid(),
...
// Schéma pour la recherche de commissions
export const searchCommissionsSchema = z.object({ serviceType: z.string().optional(),
  isActive: z.boolean().optional(),
...
export const getCommissionSchema = z.object({ commissionId: z.string().cuid() });

// Schéma pour la création d'une promotion temporaire
export const createPromotionSchema = z.object({ serviceType: z.string(),
  rate: z.number().min(0).max(100),
  startDate: z.date(),
  endDate: z.date(),
  description: z.string().optional(),
...
export const commissionReportSchema = z.object({ startDate: z.date(),
  endDate: z.date(),
...
export const commissionTierSchema = z.object({ commissionId: z.string().cuid(),
  tiers: z.array(
    z.object({
