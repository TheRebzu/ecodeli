...
export const deliverySchema = z.object({ pickupAddress: z.string().min(5, "L'adresse de ramassage est requise"),
...
export const deliveryCoordinatesUpdateSchema = z.object({ deliveryId: z.string().min(1),
  latitude: z.number().min(-90).max(90),
  longitude: z.number().min(-180).max(180) });

// Schéma pour la mise à jour du statut de livraison
export const deliveryStatusUpdateSchema = z.object({ deliveryId: z.string().min(1),
...
export const deliveryConfirmationSchema = z.object({ deliveryId: z.string().min(1),
...
export const deliveryRatingSchema = z.object({ deliveryId: z.string().min(1),
  rating: z.number().int().min(1).max(5),
  comment: z.string().max(500).optional() });

// Schéma pour le filtrage des livraisons
export const deliveryFilterSchema = z.object({ status: extendedDeliveryStatusSchema.optional(),
...
export const createDeliveryTrackingSchema = z.object({ pickupAddress: z.string().min(5),
...
export const generateConfirmationCodeSchema = z.object({ deliveryId: z.string().min(1) });

// Schéma pour le signalement d'un problème
export const deliveryIssueReportSchema = z.object({ deliveryId: z.string().min(1),
  issueType: z.enum(["DELAY", "DAMAGE", "LOSS", "OTHER"]),
  description: z.string().min(10).max(500),
  attachmentUrl: z.string().url().optional() });

// Schéma pour l'assignment automatique de livreur
export const autoAssignDelivererSchema = z.object({ announcementId: z.string().min(1),
  preferences: z
    .object({
...
export const delivererSearchCriteriaSchema = z.object({ latitude: z.number().min(-90).max(90),
  longitude: z.number().min(-180).max(180),
...
export const delivererAvailabilitySchema = z.object({ isAvailable: z.boolean(),
  location: z
    .object({
      latitude: z.number().min(-90).max(90),
      longitude: z.number().min(-180).max(180) })
    .optional(),
  workingHours: z
    .object({ start: z.string().regex(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/),
      end: z.string().regex(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/) })
    .optional()});

// Schéma pour l'optimisation de route
export const routeOptimizationSchema = z.object({ delivererId: z.string().min(1),
...
export const deliveryProofSchema = z.object({ deliveryId: z.string().min(1),
  type: z.enum(["PHOTO", "SIGNATURE", "DOCUMENT"]),
  fileUrl: z.string().url(),
  notes: z.string().max(200).optional() });

// Schéma pour les notifications de livraison
export const deliveryNotificationSchema = z.object({ deliveryId: z.string().min(1),
  type: z.enum([
    "STATUS_UPDATE",
    "DELAY_ALERT",
    "ARRIVAL_NOTICE",
...
export const deliveryHistoryFilterSchema = z.object({ userId: z.string().min(1),
