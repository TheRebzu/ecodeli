...
export const geoCoordinatesSchema = z.object({ latitude: z.number().min(-90).max(90),
...
export const geoBoundsSchema = z.object({ southWest: geoCoordinatesSchema,
  northEast: geoCoordinatesSchema });

// Schéma pour les métadonnées de position
export const positionMetadataSchema = z
  .object({ batteryLevel: z.number().min(0).max(100).optional(),
...
export const updateLocationSchema = z.object({ deliveryId: z
    .string()
...
export const updateDeliveryStatusSchema = z.object({ deliveryId: z.string().min(1),
...
export const createCheckpointSchema = z.object({ deliveryId: z.string().min(1),
...
export const updateCheckpointSchema = z.object({ id: z
    .string()
...
export const updateETASchema = z.object({ deliveryId: z.string().min(1),
...
export const deliveryIssueCreateSchema = z.object({ deliveryId: z.string().min(1),
  type: z.enum([
    "DELAY",
    "DAMAGE",
    "LOSS",
...
export const trackingQuerySchema = z.object({ status: deliveryStatusEnumSchema.optional(),
...
export const deliveryPositionFilterSchema = trackingQuerySchema.pick({ deliveryId: true,
  startDate: true,
  endDate: true,
  limit: true,
  page: true });
