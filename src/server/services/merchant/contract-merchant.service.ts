...
import {
  contractService,
  ContractService} from "@/server/services/shared/contract.service";
...
export class MerchantContractService extends ContractService {
  ...
  async calculateContractPerformance(
    ...
    const { bonusEarned: bonusEarned, penaltiesApplied: penaltiesApplied } =
  ...
  async scheduleContractRenewals() {
    ...
    for (const contract of expiringContracts) {
      try {
        ...
        renewalResults.push({ contractId: contract.id,
          status: "success",
          renewed });
      } catch (error) {
        renewalResults.push({ contractId: contract.id,
