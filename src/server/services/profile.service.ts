...
class ProfileService {
  ...
  async getRoleSpecificProfile(userId: string, role: UserRole) {
    const profileMap = {
      CLIENT: async () => await db.client.findUnique({
        where: { userId },
        include: {
          deliveryAddresses: true
        }
      }),
      DELIVERER: async () => await db.deliverer.findUnique({
        where: { userId }
      }),
      MERCHANT: async () => await db.merchant.findUnique({
        where: { userId }
      }),
      PROVIDER: async () => await db.provider.findUnique({
        where: { userId },
        include: {
          skills: true
        }
      }),
      ADMIN: async () => await db.admin.findUnique({
        where: { userId }
      }),
    };
   ...
   */
  async updateUserBaseInfo(userId: string, data: { name?: string, email?: string, phoneNumber?: string, image?: string }) {
