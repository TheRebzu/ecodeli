  ...
  async generateReportPdf(data: ReportPdfData): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      try {
        ...
        // Récupérer les données
        const { title, subtitle, dateRange, summary, tables, charts, notes } = data;
      ...
      summary: [
        {
          ...
          value: this.formatCurrency(salesData.totals?.totalRevenue || 0, 'EUR'),
        ...
        },
        {
          ...
          value: this.formatCurrency(salesData.totals?.avgOrderValue || 0, 'EUR'),
        },
      ],
      tables: [
        {
...
${filters.categoryFilter ? `Catégorie: ${filters.categoryFilter}` : ''}`,
   ...
   */
  async generateDeliveryReportPdf(deliveryData: any, filters: any): Promise<Buffer> {
    const reportData: ReportPdfData = {
