...
export const subscriptionService = {
  ...
  async cancelSubscription(userId: string, options = { cancelAtPeriodEnd: true, reason: '' }) {
    ...
    if (subscription.stripeSubscriptionId) {
      await stripeService.cancelSubscription(subscription.stripeSubscriptionId, options.cancelAtPeriodEnd);
    }
    ...
    return await db.subscription.update({
      where: { id: subscription.id },
      data: {
        status: options.cancelAtPeriodEnd ? SubscriptionStatus.ACTIVE : SubscriptionStatus.CANCELLED,
        cancelledAt: new Date(),
        cancelAtPeriodEnd: options.cancelAtPeriodEnd,
        metadata: {
          ...(subscription.metadata || {}),
   ...
   */
  async applyDiscounts(userId: string, amount: number, options: {
