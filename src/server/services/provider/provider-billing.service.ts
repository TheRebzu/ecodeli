...
export class ProviderBillingService {
  /**
   * R�cup�re les factures du prestataire avec filtres
   */
  ...
  /**
   * G�n�re une facture automatique pour les interventions termin�es
   */
  static async generateAutomaticInvoice(
    ...
    try {
      // R�cup�rer les interventions termin�es
      ...
      if (interventions.length === 0) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "Aucune intervention termin�e trouv�e",
        });
      }

      // V�rifier que toutes les interventions sont du m�me client
      const clientId = interventions[0].clientId;
      if (!interventions.every((intervention) => intervention.clientId === clientId)) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "Toutes les interventions doivent �tre du m�me client",
        });
      }
      ...
      throw new TRPCError({
        code: "INTERNAL_SERVER_ERROR",
        message: "Erreur lors de la g�n�ration de la facture automatique",
      });
    }
  }
    ...
    try {
      ...
      await prisma.notification.create({
        data: {
          ...
          content: `Vous avez re�u une facture n�${invoice.invoiceNumber}`,
          data: {
  ...
  /**
   * R�cup�re les statistiques de facturation
   */
  static async getBillingStats(providerId: string) {
    try {
      ...
      throw new TRPCError({
        code: "INTERNAL_SERVER_ERROR",
        message: "Erreur lors de la r�cup�ration des statistiques de facturation",
      });
    }
  }
  ...
  static async generateMonthlyReport(
    ...
    try {
      ...
      const completionRate = 100; // D�j� filtr� sur COMPLETED

      // R�cup�rer les noms des services top
  ...
  /**
   * R�cup�re les revenus par p�riode
   */
