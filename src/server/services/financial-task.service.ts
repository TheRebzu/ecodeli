...
export class FinancialTaskService {
  ...
  async createFinancialTask(userId: string, data: CreateFinancialTaskInput) {
    try {
      return await this.prisma.financialTask.create({
        data: {
          ...data,
          userId,
        },
  ...
  async getFinancialTaskById(id: string, userId?: string) {
    try {
      const task = await this.prisma.financialTask.findUnique({
        where: { id },
      });
  ...
  async getFinancialTasks(
    ...
    try {
      ...
      const totalTasks = await this.prisma.financialTask.count({ where });

      // Récupération des tâches avec pagination
      const tasks = await this.prisma.financialTask.findMany({
        where,
        orderBy,
        skip,
        take: limit,
  ...
  async updateFinancialTask(data: UpdateFinancialTaskInput, userId: string) {
    try {
      ...
      const updatedTask = await this.prisma.financialTask.update({
        where: { id: data.id },
        data: {
          title: data.title,
  ...
  async toggleFinancialTaskStatus(id: string, completed: boolean, userId: string) {
    try {
      ...
      return await this.prisma.financialTask.update({
        where: { id },
        data: {
          completed,
  ...
  async deleteFinancialTask(id: string, userId: string) {
    try {
      ...
      // Supprimer la tâche
      await this.prisma.financialTask.delete({
        where: { id },
      });
  ...
  async getFinancialTaskStats(userId: string) {
    try {
      ...
      const totalTasks = await this.prisma.financialTask.count({
        where: { userId },
      });

      // Nombre de tâches terminées
      const completedTasks = await this.prisma.financialTask.count({
        where: { userId, completed: true },
      });

      // Nombre de tâches par priorité
      const tasksByPriority = await this.prisma.financialTask.groupBy({
        by: ['priority'],
        where: { userId },
        _count: true,
      });
      ...
      const tasksByCategory = await this.prisma.financialTask.groupBy({
        by: ['category'],
        where: { userId },
        _count: true,
      });
      ...
      const overdueTasks = await this.prisma.financialTask.count({
        where: {
          userId,
          dueDate: { lt: new Date() },
          completed: false,
