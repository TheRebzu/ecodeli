...
export const dashboardRouter = createTRPCRouter({
  ...
  getDelivererDashboard: protectedProcedure.query(async ({ ctx }) => {
    const userId = ctx.session.user.id;
    const userRole = ctx.session.user.role;
    ...
    return {
      ...
      documents,
      stats: {
        totalDeliveries: await ctx.prisma.delivery.count({
        ...
        averageRating: await ctx.prisma.deliveryRating
          .aggregate({
  ...
  getMerchantDashboard: protectedProcedure.query(async ({ ctx }) => {
    const userId = ctx.session.user.id;
    const userRole = ctx.session.user.role;
    ...
    return {
      ...
      stats: {
        ...
        totalOrders: stores.reduce(
          (acc, store) => acc + store._count.orders,
          0,
        ),
        pendingOrders: await ctx.prisma.order.count({
          where: {
        ...
        revenue: await ctx.prisma.order
          .aggregate({
            where: {
              storeId: { in: storeIds },
              status: "DELIVERED",
  ...
  getProviderDashboard: protectedProcedure.query(async ({ ctx }) => {
    const userId = ctx.session.user.id;
    const userRole = ctx.session.user.role;
    ...
    return {
      ...
      stats: {
        ...
        totalBookings: services.reduce(
          (acc, service) => acc + service._count.bookings,
          0,
        ),
        ...
        averageRating: await ctx.prisma.serviceRating
          .aggregate({
            where: { booking: { service: { providerId: userId } } },
            _avg: { rating: true },
          })
          .then((result) => result._avg.rating || 0),
        revenue: await ctx.prisma.serviceBooking
          .aggregate({
            where: {
              service: { providerId: userId },
              status: "COMPLETED",
  ...
  getAdminDashboard: protectedProcedure.query(async ({ ctx }) => {
    ...
    const orderStats = {
      ...
      processingOrders: await ctx.prisma.order.count({
        where: { status: "PROCESSING" },
      }),
      readyOrders: await ctx.prisma.order.count({
        where: { status: "READY_FOR_PICKUP" },
      }),
      inTransitOrders: await ctx.prisma.order.count({
        where: { status: "IN_TRANSIT" },
      }),
      deliveredOrders: await ctx.prisma.order.count({
        where: { status: "DELIVERED" },
      }),
      cancelledOrders: await ctx.prisma.order.count({
    ...
    const storeStats = {
      ...
      verifiedStores: await ctx.prisma.store.count({
        where: { isVerified: true },
      }),
      unverifiedStores: await ctx.prisma.store.count({
    ...
    const serviceStats = {
      ...
      confirmedBookings: await ctx.prisma.serviceBooking.count({
        where: { status: "CONFIRMED" },
      }),
      completedBookings: await ctx.prisma.serviceBooking.count({
        where: { status: "COMPLETED" },
      }),
      cancelledBookings: await ctx.prisma.serviceBooking.count({
        where: { status: "CANCELLED" },
      }),
    };

    // Récupérer les documents en attente de vérification
    const pendingDocuments = await ctx.prisma.document.count({
      where: { status: "PENDING" },
    });
