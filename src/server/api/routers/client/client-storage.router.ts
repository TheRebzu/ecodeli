...
import {
  ...
  boxAccessSchema,
  boxDetailsSchema} from "@/schemas/storage/storage.schema";
...
export const storageRouter = router({ // Recherche de box disponibles
  ...
  calculateOptimalPricing: protectedProcedure
    ...
    .query(({ input, ctx  }) => {
      return storageService.calculateOptimalPricing({ ...input,
