...
export const deliveryRouter = router({
  ...
  deliverer: router({
    ...
    updateProfile: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const delivererId = input.delivererId || ctx.session.user.id;
        return await DeliveryService.updateDelivererProfile(delivererId, input, ctx.session.user.id);
      }),
    ...
    uploadDocument: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const delivererId = input.delivererId || ctx.session.user.id;
        return await DeliveryService.uploadDelivererDocument(delivererId, input, ctx.session.user.id);
      }),
    ...
    respondToProposal: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        return await DeliveryService.respondToDeliveryProposal(input.matchingId, input.response, ctx.session.user.id);
      }),
  ...
  schedule: router({
    ...
    update: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const delivererId = input.delivererId || ctx.session.user.id;
        return await DeliveryService.updateDelivererSchedule(delivererId, input.schedules, ctx.session.user.id);
      }),
    ...
    updateAvailability: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        return await DeliveryService.updateDelivererAvailability(ctx.session.user.id, input, ctx.session.user.id);
      }),
    ...
    getAvailableInArea: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        return await DeliveryService.getAvailableDeliverersInArea(input.latitude, input.longitude, input.radiusKm);
      }),
    ...
    deleteException: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (exception.schedule.delivererId !== ctx.session.user.id && ctx.session.user.role !== 'ADMIN') {
          throw new TRPCError({ code: 'FORBIDDEN', message: 'Accès refusé' });
        }
  ...
  matching: router({
    ...
    calculateScore: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        return await DeliveryService.calculateMatchingScore(input.announcementId, input.delivererId);
      }),
  ...
  proof: router({
    ...
    getByDelivery: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        if (delivery.clientId !== ctx.session.user.id && 
  ...
  verification: router({
    ...
    validateDocument: adminProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        return await DeliveryService.validateDelivererDocument(input.documentId, input, ctx.session.user.id);
      }),
    ...
    getDocumentHistory: protectedProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        // Vérifier les permissions
        if (document.application.delivererId !== ctx.session.user.id && ctx.session.user.role !== 'ADMIN') {
          throw new TRPCError({ code: 'FORBIDDEN', message: 'Accès refusé' });
        }
    ...
    getVerificationStats: adminProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        const [totalDocuments, pendingDocuments, approvedDocuments, rejectedDocuments, autoApproved] = await Promise.all([
