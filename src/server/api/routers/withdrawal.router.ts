...
import { isRoleAllowed, hasDocumentAccess } from '@/lib/auth-helpers';
import { db } from '@/server/db';
import { withdrawalBaseSchema, requestWithdrawalSchema } from '@/schemas/withdrawal.schema';
  ...
  requestWithdrawal: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        if (
          !isRoleAllowed(ctx.session.user.role as UserRole, ['DELIVERER', 'PROVIDER', 'MERCHANT'])
        ) {
          throw new TRPCError({
            code: 'FORBIDDEN',
      ...
      try {
        ...
        const daysUntilArrival = withdrawal.estimatedArrival
          ? Math.max(
              0,
              Math.floor(
                (withdrawal.estimatedArrival.getTime() - now.getTime()) / (1000 * 60 * 60 * 24)
              )
            )
          : null;
