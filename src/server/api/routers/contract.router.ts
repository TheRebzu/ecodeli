...
export const contractRouter = router({
   ...
   */
  getActiveContract: protectedProcedure
    .query(async ({ ctx }) => {
      const userId = ctx.session.user.id;
  ...
  getMerchantStats: protectedProcedure
    .query(async ({ ctx }) => {
      ...
      const totalMonthlyFees = activeContracts.reduce((sum, c) => 
        sum + (c.monthlyFee ? parseFloat(c.monthlyFee.toString()) : 0), 0
      );

      const averageCommissionRate = activeContracts.length > 0 
        ? activeContracts.reduce((sum, c) => 
            sum + (c.commissionRate ? parseFloat(c.commissionRate.toString()) : 0), 0
