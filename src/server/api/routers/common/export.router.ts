...
const exportDataSchema = z.object({
  reportType: z.enum(['sales', 'user-activity', 'delivery', 'payments', 'transactions']),
...
function generateSalesCsv(data: any): string {
  const headers = ['Date', 'Montant', 'Catégorie', 'Statut', 'Client', 'Commission'];
  const rows = [];
...
function generateUserActivityCsv(data: any): string {
  const headers = ['Date', 'Nouvelles inscriptions', 'Connexions', 'Utilisateurs actifs', 'Rôle'];
  const rows = [];
...
function generateDeliveryCsv(data: any): string {
  const headers = ['Date', 'Livraisons totales', 'Livraisons à temps', 'Temps moyen (min)', 'Taux de réussite (%)'];
  const rows = [];
...
function generatePaymentsCsv(data: any): string {
  const headers = ['Date', 'Type', 'Montant', 'Statut', 'Référence', 'Utilisateur'];
  const rows = [];
...
function generateTransactionsCsv(data: any): string {
  const headers = ['Date', 'Type', 'Description', 'Montant', 'Statut', 'Devise'];
  const rows = [];
