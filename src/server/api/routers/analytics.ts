...
export const analyticsRouter = router({
  ...
  merchantStatistics: merchantProcedure.query(async ({ ctx }) => {
    ...
    const [totalAnnouncements, completedDeliveries, totalSpent, storeStats] =
      await Promise.all([
        // Total announcements
        prisma.announcement.count({
          where: {
    ...
    .mutation(async ({ input }) => {
      ...
      if (includeFinancialStats) {
        ...
        reportData.financialStats = {
          ...
          revenueByType: revenueByType.reduce(
            (acc, item) => {
      ...
      if (includeDeliveryStats) {
        const [totalAnnouncements, announcementsByStatus, deliveriesByStatus] =
          await Promise.all([
        ...
        reportData.deliveryStats = {
          totalAnnouncements,
          announcementsByStatus: announcementsByStatus.reduce(
            (acc, item) => {
          ...
          deliveriesByStatus: deliveriesByStatus.reduce(
            (acc, item) => {
              acc[item.status] = item._count;
              return acc;
            },
