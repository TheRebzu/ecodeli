...
const createReviewSchema = z.object({ bookingId: z.string().cuid(),
...
const providerResponseSchema = z.object({ reviewId: z.string().cuid(),
  response: z.string().min(10).max(500),
  isPublic: z.boolean().default(true) });

const reviewFiltersSchema = z.object({ rating: z.number().int().min(1).max(5).optional(),
...
const evaluationAnalysisSchema = z.object({ period: z.enum(["WEEK", "MONTH", "QUARTER", "YEAR"]).default("MONTH"),
  serviceId: z.string().cuid().optional(),
...
export const providerEvaluationsRouter = router({ /**
  ...
  getPublicReviews: publicProcedure
    .input(
      z.object({ providerId: z.string().cuid(),
        serviceId: z.string().cuid().optional(),
    ...
    .query(async ({ ctx, input: input  }) => {
      ...
      try {
        ...
        const [
          ...
          topHighlights,
          commonImprovements,
          responseRate] = await Promise.all([
        ...
        if (input.compareWithPrevious) {
          ...
          comparison = {
            ...
            averageRating: calculateGrowthRate(
              currentStats.avg.rating || 0,
              previousStats.avg.rating || 0,
            )};
        }
        ...
        const timeline = await ctx.db.$queryRaw`
          SELECT 
            ...
            COUNT(CASE WHEN wouldrecommend = true THEN 1 END)::int as recommendations
          FROM "ServiceReview"
          WHERE provider_id = ${provider.id}
            AND created_at >= ${startDate}
            AND created_at <= ${endDate}
        ...
        return {
          ...
          data: {
            ...
            overview: {
              ...
              averageRating: currentStats.avg.rating || 0,
              averageCriteriaRating:
                currentStats.avg.averageCriteriaRating || 0,
              recommendationRate: nps,
              responseRate:
                responseRate.count.all > 0
                  ? (responseRate.count.providerResponse /
                      responseRate.count.all) *
                    100
                  : 0},
  ...
  markReviewHelpful: protectedProcedure
    .input(
      z.object({ reviewId: z.string().cuid(),
        helpful: z.boolean() }),
    )
    .mutation(async ({ ctx, input: input  }) => {
      const { user } = ctx.session;
