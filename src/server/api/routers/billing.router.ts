...
import { billingSettingsSchema } from '@/schemas/billing.schema';
...
export const billingRouter = createTRPCRouter({
   ...
   */
  getBillingSettings: adminProcedure
    .query(async ({ ctx }) => {
      try {
   ...
   */
  getCommissionRates: adminProcedure
    .query(async ({ ctx }) => {
      try {
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        return {
          success: true,
          commissionRate,
          message: id ? 'Taux de commission mis à jour avec succès' : 'Taux de commission créé avec succès'
        };
      } catch (error: any) {
        throw new TRPCError({
          code: 'INTERNAL_SERVER_ERROR',
      ...
      try {
        ...
        const updatedCycle = await ctx.db.billingCycle.update({
          ...
          data: {
            ...
            notes: notes ? (billingCycle.notes ? `${billingCycle.notes}\n\n${notes}` : notes) : billingCycle.notes,
            statistics: {
              totalInvoiced,
              totalPaid,
  ...
  getFinancialStats: adminProcedure
    ...
    .query(async ({ ctx, input }) => {
      try {
        ...
        const currentPeriod = {
          start: startDate || (period === 'month' ? startOfMonth(new Date()) : new Date(new Date().getFullYear(), 0, 1)),
          end: endDate || (period === 'month' ? endOfMonth(new Date()) : new Date(new Date().getFullYear(), 11, 31))
        };
        ...
        const previousPeriod = {
          start: period === 'month' 
            ...
            : new Date(currentPeriod.start.getFullYear() - 1, currentPeriod.start.getMonth(), currentPeriod.start.getDate()),
          end: period === 'month'
            ? subMonths(currentPeriod.end, 1)
            : new Date(currentPeriod.end.getFullYear() - 1, currentPeriod.end.getMonth(), currentPeriod.end.getDate())
        };
        ...
        const [
          ...
          compareWithPrevious ? ctx.db.invoice.groupBy({
            by: ['status'],
            where: {
              issuedDate: {
          ...
          compareWithPrevious ? ctx.db.payment.groupBy({
            by: ['status'],
            where: {
              createdAt: {
                gte: previousPeriod.start,
        ...
        const previousInvoiceTotals = compareWithPrevious ? calculateTotals(previousInvoiceStats) : { count: 0, amount: 0 };
        
        const currentPaymentTotals = calculateTotals(
          currentPaymentStats.filter(stat => stat.status === 'COMPLETED')
        );
        const previousPaymentTotals = compareWithPrevious ? calculateTotals(
          previousPaymentStats.filter(stat => stat.status === 'COMPLETED')
        ) : { count: 0, amount: 0 };
        ...
        const variations = {
          ...
          invoiceAmount: calculateVariation(currentInvoiceTotals.amount, previousInvoiceTotals.amount),
          paymentCount: calculateVariation(currentPaymentTotals.count, previousPaymentTotals.count),
          paymentAmount: calculateVariation(currentPaymentTotals.amount, previousPaymentTotals.amount)
        };
