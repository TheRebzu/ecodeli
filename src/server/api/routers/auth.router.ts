...
export const authRouter = router({
  ...
  registerClient: publicProcedure.input(clientRegisterSchema).mutation(async ({ input }) => {
    return await authService.registerClient(input);
  }),

  // Inscription d'un livreur
  registerDeliverer: publicProcedure.input(delivererRegisterSchema).mutation(async ({ input }) => {
  ...
  registerMerchant: publicProcedure.input(merchantRegisterSchema).mutation(async ({ input }) => {
    return await authService.registerMerchant(input);
  }),

  // Inscription d'un prestataire
  registerProvider: publicProcedure.input(providerRegisterSchema).mutation(async ({ input }) => {
  ...
  uploadDocument: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      return await documentService.uploadDocument(userId, input.type, mockFile, input.expiryDate);
    }),
