...
import {
  ...
  billingCycleSchema,
  billingStatsSchema} from "@/schemas/payment/invoice.schema";
...
export const invoiceRouter = router({ /**
    ...
    .query(async ({ ctx, input: input  }) => {
      try {
        ...
        const [
          ...
          currentPaidInvoices,
          previousInvoices,
          previousPaidInvoices] = await Promise.all([
          ctx.db.invoice.findMany({
            where: {
              userId,
              issueDate: {
        ...
        const currentTotal = currentInvoices.reduce(
        ...
        const stats = {
          ...
          pendingInvoices: currentInvoices.filter(
  ...
  sendInvoiceByEmail: protectedProcedure
    ...
    .mutation(async ({ ctx, input: input  }) => {
      try {
        ...
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR",
          message:
            error.message || "Erreur lors de l'envoi de la facture par email",
          cause: error });
      }
    }),
  ...
  getInvoiceSummary: protectedProcedure.query(async ({ ctx  }) => {
    try {
      ...
      return {
        recentInvoices,
        paidAmount: paidAmount.sum.amount ? Number(paidAmount.sum.amount) : 0,
        pendingAmount: pendingAmount.sum.amount
          ? Number(pendingAmount.sum.amount)
          : 0,
        invoicesByStatus: statusSummary,
        period: {
          start: format(threeMonthsAgo, "PPP", { locale }),
          end: format(new Date(), "PPP", { locale })}};
    } catch (error: any) {
      throw new TRPCError({ code: "INTERNAL_SERVER_ERROR",
        message:
          error.message ||
          "Erreur lors de la récupération du résumé des factures",
        cause: error });
    }
  }),
  ...
  getAllInvoices: adminProcedure
    .input(
      z.object({ page: z.number().int().positive().default(1),
        limit: z.number().int().positive().max(100).default(10),
        status: z
          .enum(["ALL", "DRAFT", "PENDING", "PAID", "CANCELLED", "REFUNDED"])
          .optional(),
        startDate: z.date().optional(),
        endDate: z.date().optional(),
        sortOrder: z.enum(["asc", "desc"]).default("desc"),
        ...
        invoiceType: z
          .enum(["ALL", "SUBSCRIPTION", "SERVICE", "COMMISSION"])
          .optional() }),
    )
    .query(async ({ ctx, input: input  }) => {
      try {
        ...
        const stats = {
          byStatus: statsByStatus.reduce(
            (acc: Record<string, any>, stat) => {
              acc[stat.status] = {
                count: stat.count,
                amount: stat.sum.amount ? Number(stat.sum.amount) : 0};
              return acc;
            },
            {
          ...
          total: {
            ...
            amount: statsByStatus.reduce((sum, stat) => {
              return sum + (stat.sum.amount ? Number(stat.sum.amount) : 0);
            }, 0)}};
    ...
    .mutation(async ({ ctx, input: input  }) => {
      try {
        ...
        if (input.sendEmail && input.status === "PENDING") {
          await invoiceService.sendInvoiceByEmail({ invoiceId: invoice.id,
            to: user.email });
        }
  ...
  updateInvoice: adminProcedure
    ...
    .mutation(async ({ ctx, input: input  }) => {
      try {
        const adminId = ctx.session.user.id;
        const { invoiceId: invoiceId, ...updateData } = input;
  ...
  cancelInvoice: adminProcedure
    ...
    .mutation(async ({ ctx, input: input  }) => {
      try {
        const adminId = ctx.session.user.id;
        const { invoiceId: invoiceId, reason: reason } = input;
  ...
  getInvoiceStats: adminProcedure
    ...
    .query(async ({ ctx, input: input  }) => {
      try {
        ...
        const stats = await invoiceService.generateInvoiceStats({ period,
          compareWithPrevious,
          startDate,
          endDate });
        ...
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR",
          message:
            error.message || "Erreur lors de la génération des statistiques",
          cause: error });
      }
    }),
  ...
  generateInvoiceReport: adminProcedure
    .input(
      z.object({ reportType: z.enum(["MONTHLY", "QUARTERLY", "ANNUAL", "CUSTOM"]),
        startDate: z.date().optional(),
        endDate: z.date().optional(),
    ...
    .mutation(async ({ ctx, input: input  }) => {
      try {
        ...
        // Générer le rapport
        const report = await invoiceService.generateInvoiceReport({ ...input,
    ...
    .mutation(async ({ ctx, input: input  }) => {
      try {
        ...
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR",
          message:
            error.message ||
            "Erreur lors de la génération des factures marchandes",
          cause: error });
      }
    }),
    ...
    .mutation(async ({ ctx, input: input  }) => {
      try {
        ...
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR",
          message:
            error.message ||
            "Erreur lors de la génération des factures prestataires",
          cause: error });
      }
    }),
  ...
  createBillingCycle: adminProcedure
    ...
    .mutation(async ({ ctx, input: input  }) => {
      try {
        ...
        const cycle = await billingService.createBillingCycle({ merchantId: input.merchantId,
          providerId: input.providerId,
  ...
  executeBillingCycle: adminProcedure
    .input(
      z.object({ billingCycleId: z.string().cuid("ID cycle invalide") }),
    )
    .mutation(async ({ ctx, input: input  }) => {
      try {
        const adminId = ctx.session.user.id;
  ...
  runMonthlyBilling: adminProcedure
    .input(
      z.object({ forceRun: z.boolean().default(false) }),
    )
    .mutation(async ({ ctx, input: input  }) => {
      try {
        const adminId = ctx.session.user.id;
  ...
  scheduleMonthlyCycles: adminProcedure
    .input(
      z.object({ scheduledDate: z.date().default(() => new Date()) }),
    )
    .mutation(async ({ ctx, input: input  }) => {
      try {
        const adminId = ctx.session.user.id;
        ...
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR",
          message:
            error.message || "Erreur lors de la programmation des cycles",
          cause: error });
      }
    }),
  ...
  executeScheduledCycles: adminProcedure.mutation(async ({ ctx  }) => {
    try {
      ...
      throw new TRPCError({ code: "INTERNAL_SERVER_ERROR",
        message:
          error.message || "Erreur lors de l'exécution des cycles programmés",
        cause: error });
    }
  }),
  ...
  runMonthlyMerchantBilling: adminProcedure
    .input(
      z.object({ date: z.date().optional(),
        forceRun: z.boolean().default(false) }),
    )
    .mutation(async ({ ctx, input: input  }) => {
      try {
        const { merchantBillingService } =
  ...
  getMerchantBillingStats: protectedProcedure
    ...
    .query(async ({ ctx, input: input  }) => {
      ...
      const [
        ...
        paidInvoices,
        paidAmount,
        pendingInvoices] = await Promise.all([
      ...
      return {
        ...
        totalAmount: parseFloat(
          totalAmount.sum.totalAmount?.toString() || "0",
        ),
        paidInvoices,
        paidAmount: parseFloat(paidAmount.sum.totalAmount?.toString() || "0"),
        pendingInvoices,
        paymentRate:
