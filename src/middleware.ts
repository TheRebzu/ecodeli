    ...
    try {
      ...
      if (user.role === "DELIVERER" || user.role === "PROVIDER") {
        ...
        if (user.validationStatus !== "APPROVED") {
          ...
          if (
            pathname.includes("/provider/validation") ||
            pathname.includes("/api/provider/validation")
          ) {
            console.log(
              `✅ Middleware: Accès autorisé à la page de validation - ${user.role} (ID: ${user.id})`,
            );
            return NextResponse.next();
          }

          console.log(
            `🚨 Middleware: Validation en attente - ${user.role} (ID: ${user.id})`,
          );
          const locale = pathname.split("/")[1] || "fr";
          if (user.role === "PROVIDER") {
            return NextResponse.redirect(
              new URL(`/${locale}/provider/validation`, request.url),
            );
          } else {
            return NextResponse.redirect(
              new URL(`/${locale}/deliverer/validation`, request.url),
            );
          }
        }
      }
