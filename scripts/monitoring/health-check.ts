...
class EcoDeliHealthMonitor {
  ...
  async runHealthCheck(): Promise<HealthCheckReport> {
    ...
    // API endpoints
    const apiEndpoints = ["/api/health", "/api/trpc/public.health"];
    ...
    const totalDegraded = allResults.filter(
      (r) => r.status === "degraded",
    ).length;
    ...
    if (avgResponseTime > 3000) {
      recommendations.push(
        "⚠️ Temps de réponse moyen élevé (>3s). Vérifier les performances du serveur.",
      );
    }
    if (totalErrors > 0) {
      recommendations.push(
        "❌ Des endpoints sont en erreur. Vérifier les logs serveur.",
      );
    }
    const slowPages = allResults.filter((r) => r.responseTime > 5000);
    if (slowPages.length > 0) {
      recommendations.push(
        `🐌 ${slowPages.length} pages avec temps de réponse > 5s. Optimiser ces pages.`,
      );
    }
  ...
  private printReport(report: HealthCheckReport): void {
    ...
    console.log(
      `Statut global : ${this.getStatusEmoji(report.globalStatus)} ${report.globalStatus}`,
    );
    console.log("\n📈 RÉSUMÉ EXÉCUTIF");
    console.log("─".repeat(40));
    console.log(
      `Pages testées : ${report.summary.testedPages}/${report.summary.totalPages}`,
    );
      ...
      const timeColor =
        result.responseTime > 3000
          ...
          : result.responseTime > 1000
            ...
            : "🟢";
      console.log(
        `  ${statusIcon} ${result.endpoint.padEnd(30)} ${timeColor} ${result.responseTime}ms ${result.httpCode ? `(HTTP ${result.httpCode})` : ""} ${result.error ? `- ${result.error}` : ""}`,
      );
    });
  }
...
async function main() {
  ...
  const apiToken =
    process.env.API_MONITORING_TOKEN || process.env.TOKEN_API_GLOBAL;
