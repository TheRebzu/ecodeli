...
async function testFileDetection() {
  ...
  for (const directory of directories) {
    ...
    try {
      ...
      console.log(chalk.yellow(`⚠️ Le répertoire ${fullDirPath} n'existe pas ou n'est pas accessible`));
      continue;
    }
    ...
    for (const ext of extensions) {
      ...
      for (const [index, pattern] of patterns.entries()) {
        try {
          ...
          console.log(chalk.green(`  ✅ ${files.length} fichiers trouvés avec la méthode ${index + 1}`));
