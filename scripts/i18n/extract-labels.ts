  ...
  for (const directory of config.extraction.directories) {
    ...
    for (const ext of config.extraction.extensions) {
      ...
      try {
        ...
        console.error(chalk.red(`❌ Erreur lors de la recherche avec le pattern ${pattern}: ${error}`));
      }
    }
  }
...
async function findUseTranslationsNamespaces(file: string): Promise<string[]> {
  try {
    ...
    if (namespaces.length > 0) {
      console.log(chalk.green(`✅ Trouvé ${namespaces.length} namespace(s) dans ${file}: ${namespaces.join(', ')}`));
    }
...
async function extractAllTranslations(): Promise<TranslationMap> {
  ...
  console.log(chalk.green(`✅ Extraction terminée: ${Object.keys(translationMap).length} clés trouvées`));
...
async function generateTranslationFiles(translationMap: TranslationMap): Promise<void> {
  try {
    ...
    try {
      ...
      console.warn(chalk.yellow(`⚠️ Impossible de lire fr.json: ${error}. Création d'un nouveau fichier.`));
    }
    ...
    try {
      ...
      console.warn(chalk.yellow(`⚠️ Impossible de lire en.json: ${error}. Création d'un nouveau fichier.`));
    }
    ...
    console.error(chalk.red(`❌ Erreur lors de la génération des fichiers de traduction: ${error}`));
  }
}
...
async function main() {
  ...
  try {
    ...
    console.log(chalk.blue('ℹ️ Vous pouvez maintenant générer les traductions avec: pnpm i18n:generate'));
  } catch (error) {
