...
async function runExtraction() {
  console.log(
    chalk.blue("🚀 Démarrage de l'extraction des clés de traduction..."),
  );
  ...
  try {
    ...
    if (options.check) {
      console.log(
        chalk.blue(
          "ℹ️ Mode vérification activé, aucune modification ne sera effectuée",
        ),
      );
    }
    ...
    if (Object.keys(translationMap).length === 0) {
      ...
      if (!options.force) {
        console.error(
          chalk.red(
            "❌ Arrêt de l'extraction. Utilisez --force pour continuer même sans clés",
          ),
        );
        process.exit(1);
      }
    ...
    if (!options.check) {
      ...
      if (options.verbose && totalKeys > 0) {
        ...
        keyExamples.forEach((key) => {
          ...
          console.log(
            `   - ${key} (${occurrences} occurrence${occurrences > 1 ? "s" : ""})`,
          );
        });
      }
    }
    ...
    if (!options.check && options.generate) {
      console.log(
        chalk.blue("🔄 Démarrage de la génération des traductions..."),
      );
      try {
        execSync("npx tsx scripts/i18n/run-generation.ts", {
          stdio: "inherit",
        });
        console.log(
        ...
        console.error(
          chalk.red(
            `❌ Erreur lors de la génération des traductions: ${error}`,
          ),
        );
    ...
    if (options.validate) {
      console.log(
        chalk.blue("🔍 Démarrage de la validation des traductions..."),
      );
      try {
        execSync("npx tsx scripts/i18n/run-validation.ts", {
          stdio: "inherit",
        });
        console.log(
          chalk.green("✅ Validation des traductions terminée avec succès"),
        );
      } catch (error) {
        console.error(
          chalk.red(
            `❌ Erreur lors de la validation des traductions: ${error}`,
          ),
        );
