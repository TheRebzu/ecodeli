...
async function translateWithDeepL(
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Erreur DeepL: ${response.status} ${response.statusText}`,
      );
    }
...
async function translateWithGoogle(
  ...
  try {
    ...
    const response = await fetch(
      `${GOOGLE_TRANSLATE_API_URL}?key=${GOOGLE_TRANSLATE_API_KEY}`,
      {
        ...
        body: JSON.stringify({
          q: text,
          source:
            LANGUAGE_CODES[sourceLang as keyof typeof LANGUAGE_CODES]?.google ||
            sourceLang,
          target:
            LANGUAGE_CODES[targetLang as keyof typeof LANGUAGE_CODES]?.google ||
            targetLang,
          format: "text",
        }),
      },
    ...
    if (!response.ok) {
      throw new Error(
        `Erreur Google Translate: ${response.status} ${response.statusText}`,
      );
    }
...
function translateWithDictionary(
  ...
  if (!dict) {
    console.warn(
      chalk.yellow(`⚠️ Dictionnaire non disponible pour ${dictKey}`),
    );
    return text;
  }
...
export async function getTranslationFromAPI(
  ...
  try {
    ...
    console.warn(
      chalk.yellow(
        "⚠️ Aucune API de traduction configurée, utilisation du dictionnaire simple",
      ),
    );
...
export async function translateToAllLanguages(
  ...
  for (const targetLang of targetLanguages) {
    ...
    try {
      ...
      const translatedChunks = await Promise.all(
        chunks.map((chunk) =>
          getTranslationFromAPI(chunk, sourceLanguage, targetLang),
        ),
      );
      ...
      console.error(
        chalk.red(
          `❌ Erreur lors de la traduction vers ${targetLang}: ${error}`,
        ),
      );
