...
async function translateWithDeepL(
  ...
  try {
    ...
    const response = await fetch(DEEPL_API_URL, {
      ...
      body: JSON.stringify({
        text: [text],
        source_lang: LANGUAGE_CODES[sourceLang as keyof typeof LANGUAGE_CODES]?.deepl || sourceLang.toUpperCase(),
        target_lang: LANGUAGE_CODES[targetLang as keyof typeof LANGUAGE_CODES]?.deepl || targetLang.toUpperCase(),
      }),
    });
...
export async function getTranslationFromAPI(
  ...
  console.log(chalk.blue(`🔄 Traduction de "${text.substring(0, 50)}${text.length > 50 ? '...' : ''}" de ${sourceLanguage} vers ${targetLanguage}`));
  ...
  try {
    ...
    console.warn(chalk.yellow('⚠️ Aucune API de traduction configurée, utilisation du dictionnaire simple'));
...
export function checkTranslationAPIAvailability(): { 
  ...
  if (deepl) {
    ...
    message = '⚠️ Aucune API de traduction configurée, utilisation du dictionnaire simple uniquement';
  }
