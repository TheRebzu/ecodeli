        ...
        if (error.type === 'missing') {
          ...
          if (!availableKeys.has(error.key)) {
            ...
            if (locale === 'fr') {
              ...
              translationValue = commonTranslations[lastPart] || `[À TRADUIRE] ${lastPart}`;
            } else {
            ...
            modified = true;
            console.log(chalk.green(`➕ Ajout de la clé manquante: ${error.key} = "${translationValue}"`));
          }
        }
      }
  ...
  try {
    ...
    for (const locale of locales) {
      ...
      if (missingErrors.length === 0) {
        ...
        console.log(chalk.yellow(`⚠️ ${missingErrors.length} clé(s) manquante(s) pour ${locale}`));
        ...
        if (options.verbose) {
          ...
          if (missingErrors.length > 5) {
            console.log(`   ... et ${missingErrors.length - 5} autres clés manquantes`);
          }
        }
      }
      
      if (options.verbose && unusedErrors.length > 0) {
        console.log(chalk.blue(`ℹ️ ${unusedErrors.length} clé(s) inutilisée(s) pour ${locale}`));
      }
    }
    ...
    if (missingErrors.length > 0) {
      ...
      for (const error of missingErrors) {
        if (error.locale) {
          errorsByLocale.set(error.locale, (errorsByLocale.get(error.locale) || 0) + 1);
        }
      }
