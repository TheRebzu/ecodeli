...
program
  ...
  .option(
    "-a, --all",
    "Exécuter tout le workflow (extraction + validation + correction)",
  )
  ...
  try {
    ...
    for (const locale of locales) {
      try {
        ...
        console.log(
          chalk.green(`✅ ${locale.toUpperCase()}: Traductions valides`),
        );
      } catch (error) {
        console.log(
          chalk.yellow(`⚠️ ${locale.toUpperCase()}: Problèmes détectés`),
        );
      }
    }
  } catch (error) {
...
async function interactiveMenu(): Promise<void> {
  ...
  console.log(
    chalk.cyan("1. 🚀 Workflow complet (extraction + validation + correction)"),
  );
...
async function fullWorkflow(): Promise<void> {
  ...
  if (success) {
    ...
    console.log(
      chalk.white("Les traductions ont été mises à jour et validées."),
    );
    console.log(
      chalk.blue(
    ...
    console.log(
      chalk.white(
        "Certaines étapes ont rencontré des problèmes. Vérifiez les logs ci-dessus.",
      ),
    );
  }
}
