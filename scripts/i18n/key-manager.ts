...
function validateObjectStructure(obj: Record<string, any>, path: string = ''): void {
  for (const [key, value] of Object.entries(obj)) {
    ...
    if (typeof value === 'object' && value !== null) {
      ...
      throw new Error(`Type invalide: "${typeof value}" pour la clé "${currentPath}". Seuls les objets et les chaînes sont autorisés.`);
    }
  }
}
      ...
      for (const [key, value] of Object.entries(values as Record<string, any>)) {
        if (typeof value === 'object' && value !== null) {
          ...
          if (existingPath && existingPath !== currentPath) {
            console.warn(chalk.yellow(`⚠️ Valeur en double détectée: "${valueStr}" aux chemins "${existingPath}" et "${currentPath}". Conservé à "${existingPath}".`));
          } else {
