...
function runCommand(command: string, description: string): boolean {
  try {
    ...
    console.error(
      chalk.red(`❌ Erreur lors de ${description.toLowerCase()}: ${error}`),
    );
    return false;
  }
}
  ...
  try {
    ...
    if (!options.fixOnly) {
      ...
      if (!success) {
        console.warn(
          chalk.yellow(
            "⚠️ L'extraction des clés a échoué mais le workflow continue",
          ),
        );
      }
    }
    ...
    if (!options.extractOnly) {
      ...
      if (!success) {
        console.warn(
          chalk.yellow(
            "⚠️ La génération des traductions a échoué mais le workflow continue",
          ),
        );
      }
    }
    ...
    if (!options.skipValidation && !options.extractOnly) {
      success = runCommand(
        "npx tsx scripts/i18n/run-validation.ts",
        "Validation des traductions",
      );
      if (!success) {
        console.warn(
          chalk.yellow(
            "⚠️ La validation des traductions a échoué, des problèmes peuvent subsister",
          ),
        );
      }
    }
    ...
    console.error(
      chalk.red(`❌ Erreur lors de l'exécution du workflow i18n: ${error}`),
    );
    return 1;
  }
}
...
runWorkflow()
  ...
  .catch((error) => {
    console.error(
      chalk.red(`❌ Erreur non gérée dans le workflow i18n: ${error}`),
    );
    process.exit(1);
  });
