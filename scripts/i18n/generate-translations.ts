...
async function findTranslationFiles(): Promise<string[]> {
  ...
  try {
    ...
    console.error(
      chalk.red(
        `❌ Erreur lors de la recherche des fichiers de traduction: ${error}`,
      ),
    );
    return [];
  }
...
async function loadTranslationFile(filePath: string): Promise<any> {
  try {
    ...
    console.error(
      chalk.red(
        `❌ Erreur lors du chargement du fichier ${filePath}: ${error}`,
      ),
    );
    return {};
  }
...
async function saveTranslationFile(
  ...
  try {
    ...
    console.log(
      chalk.green(`✅ Fichier de traduction sauvegardé: ${filePath}`),
    );
  } catch (error) {
    console.error(
      chalk.red(
        `❌ Erreur lors de la sauvegarde du fichier ${filePath}: ${error}`,
      ),
    );
  }
}
...
async function generateAllTranslations(): Promise<void> {
  console.log(
    chalk.blue("🚀 Génération des traductions pour toutes les langues..."),
  );
  ...
  try {
    ...
    if (translationFiles.length === 0) {
      console.error(
        chalk.red("❌ Aucun fichier de traduction trouvé dans src/messages/"),
      );
      return;
    }
    ...
    if (!primaryFile) {
      console.error(
        chalk.red(
          `❌ Fichier de langue principale ${PRIMARY_LANGUAGE}.json non trouvé`,
        ),
      );
      return;
    }
    ...
    for (const language of SUPPORTED_LANGUAGES) {
      ...
      console.log(
        chalk.blue(`🔄 Mise à jour des traductions pour ${language}...`),
      );
      ...
      try {
        ...
        console.log(
          chalk.yellow(
            `⚠️ Aucun fichier existant pour ${language}, création d'un nouveau fichier`,
          ),
        );
      }
      ...
      console.log(
        chalk.blue(
          `🔄 Traduction de ${totalEntries} entrées pour ${language}...`,
        ),
      );
      ...
      for (let i = 0; i < entriesToTranslate.length; i += batchSize) {
        ...
        console.log(
          chalk.blue(
            `🔄 Traduction du lot ${i}-${i + batch.length} (${progressPercent}%)...`,
          ),
        );
        ...
        for (const entry of batch) {
          ...
          let existingValue = getNestedValue(
          ...
          if (
            ...
            try {
              ...
              const translatedValue = await generateEnglishTranslation(
                entry.value,
              );
              ...
              if (updated % 10 === 0) {
                console.log(
                  chalk.green(
                    `✅ ${updated}/${totalEntries} entrées traduites pour ${language}`,
                  ),
                );
              }
            } catch (error) {
              console.error(
                chalk.red(
                  `❌ Erreur lors de la traduction de "${fullKey}": ${error}`,
                ),
              );
              setNestedValue(
                existingTranslations,
      ...
      console.log(
        chalk.green(`✅ ${updated} entrées mises à jour pour ${language}`),
      );
    }

    console.log(
      chalk.green("✅ Génération des traductions terminée avec succès"),
    );
  } catch (error) {
    console.error(
