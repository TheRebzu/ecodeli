...
async function askForLanguageCode(): Promise<string> {
  ...
  return new Promise(resolve => {
    rl.question(
      chalk.blue('🌐 Entrez le code ISO de la langue à ajouter (ex: "es" pour espagnol): '),
      answer => {
        rl.close();
...
async function updateRoutingFile(langCode: string): Promise<void> {
  ...
  try {
    ...
    if (content.includes(`['${langCode}'`) || content.includes(`["${langCode}"`)) {
      console.log(
        chalk.yellow(`⚠️ La langue '${langCode}' est déjà présente dans le fichier de routage.`)
      );
      return;
    }
  ...
  try {
    ...
    try {
      ...
      if (!options.force) {
        const shouldOverwrite = await confirm(
          chalk.yellow(`⚠️ Le fichier pour ${langCode} existe déjà. Voulez-vous l'écraser?`)
        );
    ...
    if (translate) {
      ...
      for (let i = 0; i < keys.length; i += batchSize) {
        ...
        console.log(
          chalk.blue(`🔄 Traduction du lot ${i}-${i + batchKeys.length} (${progressPercent}%)...`)
        );
        ...
        for (const key of batchKeys) {
          ...
          try {
            // Traduire la valeur
            const translations = await translateToAllLanguages(sourceValue, PRIMARY_LANGUAGE, [
              langCode,
            ]);
  ...
  try {
    ...
    if (isLanguageSupported(langCode) && !options.force) {
      const shouldContinue = await confirm(
        chalk.yellow(`⚠️ La langue '${langCode}' est déjà supportée. Voulez-vous continuer?`)
      );
    ...
    try {
      ...
      console.warn(
        chalk.yellow("⚠️ La validation a échoué mais le processus d'ajout est terminé.")
      );
    }

    console.log(
      chalk.green(`✅ Langue ${langCode} (${getLanguageName(langCode)}) ajoutée avec succès!`)
    );
