...
program
  .argument(
    "[langCode]",
    'Code ISO de la langue à ajouter (ex: "es" pour espagnol)',
  )
  .option("-f, --force", "Forcer l'ajout même si la langue existe déjà")
  .option(
    "-t, --translate",
    "Traduire automatiquement les chaînes depuis la langue principale",
  )
...
async function askForLanguageCode(): Promise<string> {
  ...
  return new Promise((resolve) => {
    rl.question(
      chalk.blue(
        '🌐 Entrez le code ISO de la langue à ajouter (ex: "es" pour espagnol): ',
      ),
      (answer) => {
        rl.close();
...
async function updateRoutingFile(langCode: string): Promise<void> {
  ...
  try {
    ...
    if (
      content.includes(`['${langCode}'`) ||
      content.includes(`["${langCode}"`)
    ) {
      console.log(
        chalk.yellow(
          `⚠️ La langue '${langCode}' est déjà présente dans le fichier de routage.`,
        ),
      );
      return;
    }
    ...
    console.log(
      chalk.green(`✅ Fichier de routage mis à jour: ${routingFilePath}`),
    );
  } catch (error) {
    console.error(
      chalk.red(
        `❌ Erreur lors de la mise à jour du fichier de routage: ${error}`,
      ),
    );
    throw error;
  }
 ...
 */
async function createLanguageFile(
  langCode: string,
  ...
  try {
    ...
    try {
      ...
      console.error(
        chalk.red(`❌ Fichier source non trouvé: ${sourceFilePath}`),
      );
    ...
    try {
      ...
      if (!options.force) {
        const shouldOverwrite = await confirm(
          chalk.yellow(
            `⚠️ Le fichier pour ${langCode} existe déjà. Voulez-vous l'écraser?`,
          ),
        );
    ...
    if (translate) {
      console.log(
        chalk.blue(`🔄 Traduction automatique du contenu vers ${langCode}...`),
      );
      ...
      for (let i = 0; i < keys.length; i += batchSize) {
        ...
        console.log(
          chalk.blue(
            `🔄 Traduction du lot ${i}-${i + batchKeys.length} (${progressPercent}%)...`,
          ),
        );
        ...
        for (const key of batchKeys) {
          ...
          try {
            ...
            translatedFlat[key] =
              translations[langCode] || `[TO_TRANSLATE] ${sourceValue}`;
          } catch (error) {
            console.error(
              chalk.red(
                `❌ Erreur lors de la traduction de "${key}": ${error}`,
              ),
            );
    ...
    if (targetTranslations.common && targetTranslations.common.languages) {
      ...
      for (const code in LANGUAGE_NAMES) {
        if (langCode === code) {
          ...
          targetTranslations.common.languages[code] =
            targetTranslations.common.languages[code];
        }
      }
    }
    ...
    console.log(
      chalk.green(
        `✅ Fichier de traduction créé pour ${langCode}: ${targetFilePath}`,
      ),
    );
  } catch (error) {
    console.error(
      chalk.red(
        `❌ Erreur lors de la création du fichier de traduction: ${error}`,
      ),
    );
    throw error;
  }
...
async function main() {
  try {
    console.log(
      chalk.blue("🌐 Ajout d'une nouvelle langue au système de traduction..."),
    );
    ...
    if (!langCode || langCode.length < 2) {
      console.error(
        chalk.red(
          '❌ Code de langue invalide. Utilisez un code ISO valide (ex: "es" pour espagnol).',
        ),
      );
      process.exit(1);
    }
    ...
    if (isLanguageSupported(langCode) && !options.force) {
      const shouldContinue = await confirm(
        chalk.yellow(
          `⚠️ La langue '${langCode}' est déjà supportée. Voulez-vous continuer?`,
        ),
      );
    ...
    try {
      ...
      console.warn(
        chalk.yellow(
          "⚠️ La validation a échoué mais le processus d'ajout est terminé.",
        ),
      );
    }
    ...
    console.log(
      chalk.green(
        `✅ Langue ${langCode} (${getLanguageName(langCode)}) ajoutée avec succès!`,
      ),
    );
    console.log(chalk.blue("\nProchaines étapes:"));
    console.log(
      `1. Vérifiez le fichier src/messages/${langCode}.json pour compléter les traductions manquantes.`,
    );
    console.log(
      "2. Exécutez npm run i18n:validate pour vérifier la qualité des traductions.",
    );
    console.log("3. Relancez l'application pour voir les changements.");
  } catch (error) {
    console.error(
      chalk.red(`❌ Erreur lors de l'ajout de la langue: ${error}`),
    );
    process.exit(1);
  }
}
