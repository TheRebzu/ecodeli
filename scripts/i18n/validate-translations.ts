...
async function validateAllTranslations(): Promise<ValidationReport> {
  ...
  try {
    ...
    if (!referenceFile) {
      console.error(
        chalk.red(
          `❌ Fichier de référence ${PRIMARY_LANGUAGE}.json non trouvé.`,
        ),
      );
      return report;
    }
    ...
    console.log(
      chalk.blue(`📚 Chargement du fichier de référence: ${referenceFile}`),
    );
...
async function generateValidationReport(
  ...
  try {
    ...
    console.error(
      chalk.red(`❌ Erreur lors de la génération du rapport: ${error}`),
    );
  }
}
      ...
      for (const error of localeErrors) {
        if (error.type === "missing") {
          ...
          if (!current[finalKey]) {
            ...
            modified = true;
            console.log(
              chalk.green(`➕ Ajout de la clé manquante: ${error.key}`),
            );
          }
        }
      }
  ...
  try {
    ...
    for (const locale of locales) {
      ...
      if (errors.length === 0) {
        ...
        console.log(
          chalk.yellow(
            `⚠️ ${errors.length} erreur(s) trouvée(s) pour ${locale}`,
          ),
        );
