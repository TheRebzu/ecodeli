...
async function cleanTranslations(baseLocale: string = 'fr'): Promise<void> {
  ...
  try {
    ...
    if (report.summary.orphanedKeys === 0) {
      ...
      console.log(
        chalk.green(`✅ ${report.summary.orphanedKeys} clés orphelines ont été supprimées.`)
      );
    }
    ...
    if (hasOtherIssues) {
      ...
      if (report.summary.missingTranslations > 0) {
        console.log(
          chalk.yellow(`  - ${report.summary.missingTranslations} traductions manquantes`)
        );
        console.log(
          chalk.blue('    Utiliser "pnpm i18n:generate --fix" pour les ajouter automatiquement.')
        );
      }
