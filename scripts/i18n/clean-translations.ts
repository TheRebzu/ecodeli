...
async function cleanTranslations(baseLocale: string = 'fr'): Promise<void> {
  ...
  try {
    ...
    const options = {
      ...
      clean: true,
      strict: false,
      ignorePrefixes: ['[NEEDS_TRANSLATION]', 'TO_TRANSLATE:', '(à traduire)', 'TRANSLATE:']
    };
    ...
    if (report.summary.orphanedKeys === 0) {
      ...
      console.log(chalk.green(`✅ ${report.summary.orphanedKeys} clés orphelines ont été supprimées.`));
    }
    ...
    if (hasOtherIssues) {
      ...
      if (report.summary.missingTranslations > 0) {
        console.log(chalk.yellow(`  - ${report.summary.missingTranslations} traductions manquantes`));
        console.log(chalk.blue('    Utiliser "pnpm i18n:generate --fix" pour les ajouter automatiquement.'));
      }
      
      if (report.summary.malformedKeys > 0) {
        console.log(chalk.yellow(`  - ${report.summary.malformedKeys} clés malformées`));
        console.log(chalk.blue('    Vérification manuelle requise.'));
      }
      
      if (report.summary.emptyValues > 0) {
        console.log(chalk.yellow(`  - ${report.summary.emptyValues} valeurs vides`));
      }
    }
    
    console.log(chalk.green.bold('✅ Nettoyage terminé avec succès!'));
  } catch (error) {
    console.error(chalk.red(`❌ Erreur lors du nettoyage des traductions: ${error}`));
...
export async function runCleaning(args: string[]): Promise<void> {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if ((arg === '--base-locale' || arg === '--locale') && i + 1 < args.length) {
      baseLocale = args[++i];
    }
  }
