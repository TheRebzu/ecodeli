...
class ImportFixer {
  ...
  private indexProjectFiles(): void {
    ...
    console.log(chalk.green(`✅ ${totalFiles} fichiers indexés avec ${this.fileIndex.size} clés d'index`));
  }
  ...
  private generateFileNameVariations(fileName: string): string[] {
    ...
    const suffixes = ['-hook', '-service', '-router', '-component', '-form', '-modal', '-dialog', '-card', '-list', '-item', '-details', '-dashboard', '-page'];
  ...
  private findByPartialPath(brokenPath: string): string[] {
    ...
    for (let i = 0; i < parts.length; i++) {
      const partialPath = parts.slice(i).join('/').replace(/\.(ts|tsx|js|jsx)$/, '');
  ...
  private findByContentAnalysis(brokenPath: string, fromFile: string): string[] {
    ...
    const fromDirParts = fromDir.replace(this.projectRoot, '').split(path.sep).filter(p => p);
    ...
    const contextualDirs = ['admin', 'client', 'deliverer', 'merchant', 'provider', 'shared', 'common'];
  ...
  private selectBestCandidate(candidates: string[], brokenPath: string, fromFile: string): string | null {
  ...
  public async fixImports(): Promise<void> {
    ...
    if (this.fixes.length > 0) {
      ...
      fixesByFile.forEach((fixes, file) => {
        ...
        fixes.forEach(fix => {
          console.log(chalk.gray(`   L${fix.line}: `) + 
...
async function main() {
  ...
  console.log(chalk.gray(`🎯 Mode optimisation: ${aggressiveOptimization ? 'Agressif' : 'Standard'}\n`));
