...
function generateOpenAPIDoc(procedure: RouterDocumentation['procedures'][0], routerPath: string): string {
...
async function addDocumentationToRouter(routerDoc: RouterDocumentation): Promise<void> {
  ...
  try {
    ...
    // Find router export
    const routerExportRegex = /export\s+const\s+\w+Router\s*=\s*router\s*\(\s*{/;
...
async function main(): Promise<void> {
  ...
  for (const [filePath, doc] of Object.entries(routerDocs)) {
    const fullPath = path.join(process.cwd(), 'src/server/api/routers', filePath);
  ...
  console.log('\n📋 Next steps:');
  console.log('1. Check the generated documentation at: http://localhost:3000/fr/developers/api-docs');
