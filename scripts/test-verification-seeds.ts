...
async function main() {
  logger.info('TEST_VERIFICATION_SEEDS', '🚀 Test des seeds de vérification documentaire');
  ...
  try {
    ...
    logger.success('TEST', `✅ ${delivererDocs.length} documents livreurs trouvés`);
    ...
    logger.success('TEST', `✅ ${providerDocs.length} documents prestataires trouvés`);
    ...
    logger.success('TEST', `✅ ${merchantDocs.length} documents commerçants trouvés`);
    ...
    if (history.length > 0) {
      logger.success('TEST', `✅ ${history.length} entrées d'historique trouvées`);
    } else {
      logger.warning('TEST', '⚠️ Aucune entrée d\'historique trouvée (erreurs TypeScript à corriger)');
    }
    ...
    docsByRole.forEach(group => {
      logger.info('STATS', `👥 ${group.userRole}: ${group._count.id} documents`);
    });
    ...
    docsByStatus.forEach(group => {
      logger.info('STATS', `📊 ${group.verificationStatus}: ${group._count.id} documents`);
    });
    ...
    verificationsByStatus.forEach(group => {
      logger.info('STATS', `🔍 Vérifications ${group.status}: ${group._count.id}`);
    });

    // Calcul des taux
    const totalDocs = delivererDocs.length + providerDocs.length + merchantDocs.length;
    ...
    logger.info('RATES', `✅ Approbation: ${approvalRate}% (${approvedDocs}/${totalDocs})`);
    logger.info('RATES', `❌ Rejet: ${rejectionRate}% (${rejectedDocs}/${totalDocs})`);
    logger.info('RATES', `⏳ En attente: ${pendingRate}% (${pendingDocs}/${totalDocs})`);
    ...
    if (docsWithoutUsers.length === 0) {
      logger.success('INTEGRITY', '✅ Tous les documents ont des utilisateurs associés');
    } else {
      logger.error('INTEGRITY', `❌ ${docsWithoutUsers.length} documents sans utilisateur`);
    }
    ...
    if (verificationsWithoutDocs.length === 0) {
      logger.success('INTEGRITY', '✅ Toutes les vérifications ont des documents associés');
    } else {
      logger.error('INTEGRITY', `❌ ${verificationsWithoutDocs.length} vérifications sans document`);
    }
    ...
    if (docsWithInvalidDates.length === 0) {
      ...
      logger.warning('INTEGRITY', `⚠️ ${docsWithInvalidDates.length} documents approuvés mais expirés`);
    }

    logger.success('TEST_VERIFICATION_SEEDS', '✅ Tous les tests de vérification sont terminés');

  } catch (error: any) {
    logger.error('TEST_VERIFICATION_SEEDS', `❌ Erreur lors des tests: ${error.message}`);
    process.exit(1);
  } finally {
