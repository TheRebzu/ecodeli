...
async function addMerchantProducts() {
  ...
  try {
    ...
    console.log(`   - Active products: ${await prisma.product.count({ where: { isActive: true } })}`)
    console.log(`   - Low stock products: ${await prisma.product.count({ where: { stockQuantity: { lte: 10 } } })}`)
