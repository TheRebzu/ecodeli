...
class ImportFixer {
  ...
  async run(): Promise<void> {
    console.log(
      chalk.bold.cyan("🔧 EcoDeli - Correcteur automatique d'imports\n"),
    );
    ...
    try {
      ...
      console.log(
        chalk.blue(`📁 ${files.length} fichier(s) TypeScript trouvé(s)\n`),
      );
      ...
      console.error(
        chalk.red("❌ Erreur lors de la correction des imports :"),
        error,
      );
      process.exit(1);
    }
  ...
  private parseImportLine(
    line: string,
  ): { module: string; importNames: string[] } | null {
  ...
  private async identifyFixes(analyses: ImportAnalysis[]): Promise<void> {
    ...
    console.log(
      chalk.yellow(`📋 ${this.fixes.length} correction(s) identifiée(s)\n`),
    );
  }

  private async checkImport(
    filePath: string,
    importItem: ImportAnalysis["imports"][0],
  ): Promise<void> {
    ...
    if (importItem.isRelative || importItem.isAlias) {
      const withExtension = await this.addMissingExtension(
        importItem.module,
        filePath,
      );
    ...
    const barrelOptimized = this.optimizeBarrelImport(
      importItem.original,
      importItem.module,
    );
    ...
    if (fixes.length > 0) {
      ...
      const priorityOrder = [
        "Conversion en alias @/",
        "Ajout extension manquante",
        "Optimisation import barrel",
        "Tri des imports destructurés",
      ];
      const bestFix = fixes.sort(
        (a, b) =>
  ...
  private async addMissingExtension(
    module: string,
    filePath: string,
  ): Promise<string | null> {
  ...
  private async applyFixes(): Promise<void> {
    ...
    for (const [filePath, fixes] of fixesByFile) {
      try {
        ...
        console.log(
          chalk.green(
            `✅ ${path.relative(this.projectRoot, filePath)}: ${fixes.length} correction(s)`,
          ),
        );
        ...
        console.log(
          chalk.red(
            `❌ ${path.relative(this.projectRoot, filePath)}: ${(error as Error).message}`,
          ),
        );
      }
    }
  ...
  private displaySummary(): void {
    ...
    console.log(
      chalk.cyan(`🔧 ${this.fixes.length} correction(s) appliquée(s)`),
    );
