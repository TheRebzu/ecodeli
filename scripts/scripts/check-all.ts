...
class ProjectChecker {
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      const hasCriticalFailures = this.results.some(r => 
        r.status === 'fail' && this.checks.find(c => c.name === r.name)?.critical
      );
    ...
    try {
      ...
      if (this.isFixMode && check.fixCommand && !this.isDryRun) {
        ...
        try {
          ...
          console.log(chalk.red(`❌ Correction échouée: ${fixError.message.slice(0, 100)}...`));
        }
      }
  ...
  private async generateReportFile(): Promise<void> {
    const report = {
      ...
      summary: {
        ...
        criticalFailures: this.results.filter(r => 
          r.status === 'fail' && this.checks.find(c => c.name === r.name)?.critical
        ).length
      },
  ...
  private displaySummary(totalTime: number): void {
    ...
    if (failed > 0 && !this.isFixMode) {
      console.log(chalk.yellow('🔧 Utilisez --fix pour corriger automatiquement'));
    }
    ...
    if (this.results.some(r => r.status === 'fail' && r.name === 'TypeScript')) {
...
export async function fixCommonIssues(): Promise<void> {
  console.log(chalk.blue('🔧 Correction automatique des problèmes courants...\n'));
