...
class AnnouncementFeaturesTest {
  ...
  async runAllTests(): Promise<void> {
    console.log(
      "🧪 Démarrage des tests des fonctionnalités d'annonces EcoDeli\n",
    );
  ...
  private async testTypeScriptValidation(): Promise<void> {
    ...
    await this.runTest(
      suite,
      "Import AnnouncementMatchingService",
      async () => {
    ...
    await this.runTest(
      suite,
      "Import AnnouncementLifecycleWorkflow",
      async () => {
  ...
  private async testMatchingServices(): Promise<void> {
    ...
    await this.runTest(
      suite,
      "Création AnnouncementMatchingService",
      async () => {
    ...
    await this.runTest(
      suite,
      "Validation des types MatchingCriteria",
      async () => {
        const mockCriteria = {
          routeId: "route-123",
  ...
  private async testEscrowServices(): Promise<void> {
    ...
    await this.runTest(
      suite,
      "Validation des types EscrowTransaction",
      async () => {
  ...
  private async testBusinessWorkflows(): Promise<void> {
    ...
    await this.runTest(
      suite,
      "Création AnnouncementLifecycleWorkflow",
      async () => {
        const mockPrisma = {} as any;
        const mockConfig = {
        ...
        const workflow = new AnnouncementLifecycleWorkflow(
          mockPrisma,
          mockConfig,
        );
        if (!workflow) {
    ...
    await this.runTest(
      suite,
      "Validation logique de transition d'état",
      async () => {
  ...
  private async testSeedCompatibility(): Promise<void> {
    ...
    await this.runTest(suite, "Statuts d'annonces supportés", async () => {
      ...
      if (
        !supportedStatuses.includes("DRAFT") ||
