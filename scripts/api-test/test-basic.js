...
async function testBasicConnectivity() {
  ...
  try {
    ...
    console.log(
      `Headers: ${JSON.stringify(homeResponse.headers["content-type"])}`,
    );
    ...
    try {
      ...
      if (csrfResponse.data) {
        console.log(
          `CSRF Token: ${csrfResponse.data.csrfToken ? "Present" : "Missing"}`,
        );
      }
    ...
    try {
      const trpcResponse = await axios.post(
        `${BASE_URL}/api/trpc/public.health`,
        {
          input: {},
        },
        {
