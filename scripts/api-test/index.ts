...
program
  .name('api-test')
  .description('EcoDeli API Test Suite')
  .version('1.0.0');
...
program
  ...
  .option('-u, --user <user>', 'User to test as (client, deliverer, merchant, etc.)', 'client')
  .option('-a, --action <action>', 'Specific action to test (create, list, get, update, delete, all)', 'all')
...
program
  ...
  .option('-s, --scenario <name>', 'Scenario to run (full-cycle, client-flow, deliverer-flow)', 'full-cycle')
  .action(async (options) => {
    ...
    switch (options.scenario) {
      case 'full-cycle':
        ...
        break;
      case 'client-flow':
        logger.info('Client flow scenario not yet implemented');
        break;
      case 'deliverer-flow':
        logger.info('Deliverer flow scenario not yet implemented');
        break;
      default:
...
program
  ...
  .option('-a, --action <action>', 'Auth action (login, logout, session)', 'login')
  .action(async (options) => {
