...
export async function runFullDeliveryCycle(): Promise<void> {
  ...
  scenarioLogger.info('Simulating complete delivery flow from announcement to completion');
  ...
  try {
    ...
    const announcementData = {
      ...
      description: 'Important legal documents need to be delivered today. Handle with care.',
      pickupAddress: {
    ...
    scenarioLogger.info(`Deliverer 1 found ${searchResults1.total} announcements`);
    ...
    const application1 = await RequestHelper.trpc(
      ...
      'deliverer.announcements.apply',
      {
        ...
        proposedPrice: 32.00,
        estimatedDeliveryTime: new Date(Date.now() + 3 * 60 * 60 * 1000).toISOString(),
    ...
    const application2 = await RequestHelper.trpc(
      ...
      'deliverer.announcements.apply',
      {
        announcementId: announcement.id,
        proposedPrice: 30.00,
        estimatedDeliveryTime: new Date(Date.now() + 2.5 * 60 * 60 * 1000).toISOString(),
        message: 'Express delivery available. Verified deliverer with 5-star rating.'
      }
    );
    ...
    const pickupConfirmation = await RequestHelper.trpc(
      ...
      'deliverer.delivery.confirmPickup',
      {
        ...
        notes: 'Package picked up successfully. Documents in perfect condition.'
      }
    );
    ...
    await RequestHelper.trpc(
      deliverer2,
      'deliverer.delivery.updateLocation',
      {
        deliveryId: selection.deliveryId,
        location: {
          latitude: 48.8373,
