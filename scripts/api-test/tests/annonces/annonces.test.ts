...
export class AnnouncementTests {
  ...
  async testListAnnouncements(
    ...
    try {
      ...
      const response = await RequestHelper.trpc<
        typeof input,
        { items: Announcement[]; total: number }
      >(user, "client.announcements.list", input);

      this.logger.success(
        `Found ${response.items.length} announcements (Total: ${response.total})`,
      );
  ...
  async testGetAnnouncement(
    ...
    try {
      const announcement = await RequestHelper.trpc<
        { id: string },
        Announcement
      >(user, "client.announcements.get", { id: announcementId });
  ...
  async testDeleteAnnouncement(
    ...
    try {
      await RequestHelper.trpc(user, "client.announcements.delete", {
        id: announcementId,
      });
   ...
   */
  async testSearchAnnouncementsAsDeliverer(
    ...
    try {
      ...
      const response = await RequestHelper.trpc<
        typeof input,
        { items: Announcement[]; total: number }
      >(user, "deliverer.announcements.search", input);

      this.logger.success(
        `Found ${response.items.length} announcements for deliverer`,
      );
      ...
      if (response.items.length > 0) {
        ...
        response.items.forEach((a) => {
          this.logger.info(
            `- ${a.title} (${a.pickupAddress.city} → ${a.deliveryAddress.city}) - €${a.price}`,
          );
        });
      }
  ...
  async testApplyToAnnouncement(
    ...
    try {
      const input = {
        ...
        message:
          "I can deliver this package tomorrow. I have experience with fragile items.",
      };
