...
export class AuthTests {
  ...
  async testAllUserLogins(): Promise<void> {
    ...
    const userTypes = Object.keys(defaultUsers) as Array<
      keyof typeof defaultUsers
    >;
  ...
  async testInvalidCredentials(): Promise<void> {
    ...
    for (const test of invalidTests) {
      try {
        ...
        if (result.success) {
          this.logger.warning(
            `⚠️  ${test.name}: Expected failure but got success`,
          );
        } else {
          this.logger.success(
            `✅ ${test.name}: Correctly failed with: ${result.error}`,
          );
        }
      } catch (error) {
