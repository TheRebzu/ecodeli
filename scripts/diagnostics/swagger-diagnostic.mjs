      ...
      if (statusOk) {
        ...
        if (test.url.includes('/api/openapi')) {
          if (response.data && typeof response.data === 'object') {
            ...
            console.log(chalk.gray(`   Title: ${response.data.info?.title || 'N/A'}`));
            console.log(chalk.gray(`   Version: ${response.data.info?.version || 'N/A'}`));
            console.log(chalk.gray(`   Paths: ${Object.keys(response.data.paths || {}).length} endpoints`));
          } else {
        ...
        console.log(chalk.red(`❌ Status: ${response.status} (expected: ${test.expected})`));
        
        if (response.data) {
          console.log(chalk.red(`   Error: ${JSON.stringify(response.data, null, 2)}`));
        }
      }
...
async function testTrpcEndpoints() {
  ...
  for (const test of trpcTests) {
    ...
    try {
      const response = await axios.post(`${BASE_URL}/api/trpc/${test.endpoint}`, {
        input: test.input
      }, {
        headers: {
      ...
      console.log(chalk.red(`❌ tRPC ${test.endpoint} error: ${error.message}`));
    }
...
async function generateDiagnosticReport() {
  ...
  console.log(chalk.gray('   - Make sure Next.js dev server is running: pnpm dev'));
  console.log(chalk.gray('   - Check if port 3000 is available'));
  console.log('');
  
  console.log(chalk.white('2. OpenAPI spec returns 500 error:'));
  console.log(chalk.gray('   - Check swagger-jsdoc configuration in src/lib/openapi.ts'));
