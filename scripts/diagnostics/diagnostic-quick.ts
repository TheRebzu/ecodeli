...
class QuickDiagnostic {
  ...
  private async checkImports(): Promise<void> {
    ...
    try {
      ...
      const output = error.stdout?.toString() || error.stderr?.toString() || "";
      const hasIssues =
        output.includes("Fix") || output.includes("corrections");
  ...
  private async applyAutoFixes(): Promise<void> {
    ...
    const fixableResults = this.results.filter(
      (r) => r.canAutoFix && r.status !== "pass",
    );
  ...
  private displaySummary(totalTime: number): void {
    ...
    if (warnings > 0)
      console.log(chalk.yellow(`⚠️  Avertissements: ${warnings}`));
    ...
    if (issues.length > 0) {
      ...
      for (const issue of issues) {
        ...
        console.log(
          chalk.gray(
            `  ${icon} ${issue.name}: ${issue.message || "Problème détecté"}`,
          ),
        );
    ...
    if (issues.length > 0 && !this.isFixMode) {
      ...
      console.log(
        chalk.yellow("🔧 Exécutez `pnpm fix` pour corriger automatiquement"),
      );
      console.log(
        chalk.blue("📊 Exécutez `pnpm check:full` pour un diagnostic complet"),
      );
    }
  }
}
