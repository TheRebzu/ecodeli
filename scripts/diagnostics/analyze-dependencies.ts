...
const MOVEMENT_PATTERNS = [
  // lib -> lib/[category]
  {
    from: /^lib\/([^\/]+)$/,
    to: "lib/$2/$1",
    examples: ["lib/auth-error → lib/auth/errors"],
  },
...
function detectFileMovements(): void {
  console.log(
    chalk.blue("🔍 Détection automatique des déplacements de fichiers..."),
  );
  ...
  console.log(
    chalk.yellow(
      `  ⚠️  ${missingPaths.size} chemins d'import potentiellement cassés détectés`,
    ),
  );
  ...
  console.log(
    chalk.green(
      `✓ ${detectedMappings.size} déplacements automatiques détectés`,
    ),
  );
}
...
function findMovementCandidates(missingPath: string): MovedFile[] {
  ...
  for (const [pathKey, fullPath] of filesByPath.entries()) {
    ...
    const commonSegments = segments.filter((seg) =>
    ...
    if (commonSegments.length > 0) {
      const confidence =
        ...
        Math.max(segments.length, candidateSegments.length);
...
function calculatePathSimilarity(path1: string, path2: string): number {
  ...
  const segmentSimilarity =
    commonSegments.length / Math.max(segments1.length, segments2.length);
...
function buildFileMap(): void {
  console.log(
    chalk.blue("🔍 Construction de la carte des fichiers et exports..."),
  );
  ...
  console.log(
    chalk.green(`✓ ${fileExports.size} fichiers avec exports détectés`),
  );
...
function createMissingFile(
  ...
  try {
    ...
    console.log(
      chalk.yellow(
        `  📝 Fichier stub créé: ${path.relative(process.cwd(), filePath)}`,
      ),
    );
    return true;
  } catch (error) {
...
function fixFileImports(filePath: string): {
  fixed: number;
  notFound: string[];
  created: number;
} {
  ...
  for (const importInfo of imports) {
    ...
    if (importExists(importInfo.importPath, filePath)) {
      if (verbose) {
        console.log(
          chalk.gray(`  ✓ Import déjà valide: ${importInfo.importPath}`),
        );
      }
      continue;
    }
    ...
    if (newPath) {
      ...
      const newImportLine = importInfo.fullLine.replace(
        importInfo.importPath,
        newPath,
      );
...
async function main(): Promise<void> {
  ...
  if (isDryRun) {
    console.log(
      chalk.yellow("🔍 Mode DRY-RUN: aucun fichier ne sera modifié\n"),
    );
  }

  if (createMissing) {
    console.log(
      chalk.blue("📝 Mode création automatique de fichiers activé\n"),
    );
  }
  ...
  for (const file of allFiles) {
    ...
    const brokenImports = relativeImports.filter(
      (imp) => !importExists(imp.importPath, file),
    );
  ...
  console.log(
    chalk.white(
      `  Fichiers ${isDryRun ? "à modifier" : "modifiés"}: ${filesModified}`,
    ),
  );
  console.log(
    chalk.green(
      `  Imports ${isDryRun ? "à corriger" : "corrigés"}: ${totalImportsFixed}`,
    ),
  );
  ...
  if (createMissing) {
    console.log(
      chalk.yellow(
        `  Fichiers ${isDryRun ? "à créer" : "créés"}: ${totalFilesCreated}`,
      ),
    );
  }
  ...
  if (isDryRun && (totalImportsFixed > 0 || totalFilesCreated > 0)) {
    ...
    console.log(
      chalk.yellow(
        "   Utilisez --create-missing pour créer des fichiers stub.",
      ),
    );
    ...
    if (totalFilesCreated > 0) {
      console.log(
        chalk.yellow(
          "⚠️  Vérifiez les fichiers créés et implémentez-les correctement.",
        ),
      );
    }
  } else {
...
if (args.includes("--help") || args.includes("-h")) {
  ...
  console.log("\nOptions:");
  console.log(
    "  -d, --dry-run     Afficher les changements sans modifier les fichiers",
  );
  console.log("  -v, --verbose     Afficher plus d'informations");
  console.log(
    "  -i, --interactive Mode interactif (affiche les candidats multiples)",
  );
  console.log("  -c, --create-missing Créer des fichiers stub manquants");
  console.log("  -h, --help        Afficher cette aide");
  console.log("\nExemples:");
  console.log(
    "  pnpm fix:imports                    # Corriger tous les imports",
  );
  console.log(
    "  pnpm fix:imports --dry-run          # Prévisualiser les changements",
  );
  console.log("  pnpm fix:imports --verbose          # Mode détaillé");
  console.log(
    "  pnpm fix:imports --create-missing   # Créer des fichiers manquants",
  );
  console.log(
    "  pnpm fix:imports --dry-run --verbose # Mode complet de prévisualisation",
  );
  process.exit(0);
}
