...
async function readSchemaFiles(): Promise<SchemaFile[]> {
  ...
  for (const domain of domains) {
    ...
    for (const file of files) {
      ...
      console.log(chalk.gray(`  - Analysé: ${path.relative(process.cwd(), filePath)}`));
    }
  }
  
  console.log(chalk.green(`  ✅ ${schemaFiles.length} fichiers de schéma analysés`));
  return schemaFiles;
}
...
function extractSchemaComponents(files: SchemaFile[]) {
  ...
  console.log(chalk.green(`  ✅ ${models.size} modèles et ${enums.size} enums extraits`));
  return { models, enums };
}

// Validation du schéma
function validateSchema(files: SchemaFile[], models: Map<string, string>, enums: Map<string, string>): ValidationResult {
...
function displayValidationResults(result: ValidationResult) {
  ...
  if (errors.length === 0 && warnings.length === 0) {
    console.log(chalk.green('  ✅ Aucun problème détecté dans les schémas fragmentés'));
    return;
  }
...
async function validatePrismaGeneration() {
  ...
  try {
    ...
    execSync('npx prisma validate --schema=prisma/schema.temp.prisma', { stdio: 'inherit' });
      ...
      if (line.includes('@relation')) {
        ...
        if (fieldMatch) {
          ...
          if (fieldType.match(/^[A-Z]/)) {
            const relationMatch = line.match(/@relation\(\s*(?:name\s*:\s*"([^"]+)")?\s*,?\s*fields\s*:\s*\[([^\]]+)\]\s*,\s*references\s*:\s*\[([^\]]+)\]/);
