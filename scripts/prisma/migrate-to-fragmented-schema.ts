...
async function main() {
  ...
  try {
    ...
    console.log(
      chalk.gray('Pour annuler cette migration, exécutez : pnpm prisma:rollback-fragmentation')
    );
  } catch (error) {
...
async function backupCurrentSchema() {
  ...
  console.log(
    chalk.green(`  ✅ Schéma sauvegardé dans ${path.relative(ROOT_DIR, SCHEMA_BACKUP_PATH)}`)
  );
}
...
async function runTests() {
  ...
  try {
    ...
    return new Promise<void>((resolve, reject) => {
      readline.question(
        chalk.yellow('  ⚠️ Voulez-vous continuer malgré les erreurs de test ? (y/N) '),
        (answer: string) => {
          readline.close();
...
async function performPostMigrationChecks() {
  ...
  const requiredScripts = ['db:schema:build', 'db:schema:validate', 'db:generate'];
