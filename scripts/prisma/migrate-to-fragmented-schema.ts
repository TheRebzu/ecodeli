...
const SCHEMA_BACKUP_PATH = path.join(BACKUP_DIR, `schema_backup_${getTimestamp()}.prisma`);
...
async function main() {
  ...
  try {
    ...
    console.log(chalk.gray('Pour annuler cette migration, exécutez : pnpm prisma:rollback-fragmentation'));
...
async function backupCurrentSchema() {
  ...
  if (!fs.existsSync(SCHEMA_PATH)) {
    throw new Error(`Le fichier schema.prisma est introuvable à ${SCHEMA_PATH}`);
  }
  
  fs.copyFileSync(SCHEMA_PATH, SCHEMA_BACKUP_PATH);
  console.log(chalk.green(`  ✅ Schéma sauvegardé dans ${path.relative(ROOT_DIR, SCHEMA_BACKUP_PATH)}`));
}
  ...
  try {
    ...
    if (isWindows) {
      ...
      try {
        ...
        throw new Error('Docker n\'est pas installé. Impossible de créer un backup de la base de données sur Windows');
      }
    ...
    console.log(chalk.green(`  ✅ Base de données sauvegardée dans ${path.relative(ROOT_DIR, DB_BACKUP_PATH)}`));
  } catch (error) {
    console.error(chalk.yellow('  ⚠️ Impossible de sauvegarder la base de données:'), error);
    console.log(chalk.yellow('  ⚠️ Continuation sans sauvegarde de la base. Assurez-vous d\'avoir un backup récent.'));
  }
}
...
async function validateSchemaFragments() {
  ...
  try {
    ...
    throw new Error('La validation des fragments a échoué. Corrigez les erreurs avant de continuer.');
  }
}
...
async function runTests() {
  ...
  try {
    ...
    execSync('node -r tsx/cjs tests/schemas/fragmentation.test.ts', { stdio: 'inherit' });
    console.log(chalk.green('  ✅ Tests réussis'));
  } catch (error) {
    console.log(chalk.yellow('  ⚠️ Certains tests ont échoué. Vérifiez les résultats.'));
    
    // Demander confirmation pour continuer
    const readline = await import('readline').then(module => module.default.createInterface({
      input: process.stdin,
      output: process.stdout
    }));
    
    return new Promise<void>((resolve, reject) => {
      readline.question(chalk.yellow('  ⚠️ Voulez-vous continuer malgré les erreurs de test ? (y/N) '), (answer: string) => {
        readline.close();
        
        if (answer.toLowerCase() === 'y') {
          console.log(chalk.yellow('  ⚠️ Continuation avec les erreurs de test...'));
          resolve();
        } else {
          reject(new Error('Migration interrompue en raison d\'échecs de test.'));
        }
      });
    });
  }
...
async function performPostMigrationChecks() {
  ...
  if (!schemaContent.includes('generator client') || !schemaContent.includes('datasource db')) {
    throw new Error('Le schéma généré est incomplet. Vérifiez le contenu du fichier.');
  }
  ...
  for (const script of requiredScripts) {
    if (!packageJson.scripts[script]) {
      console.log(chalk.yellow(`  ⚠️ Le script '${script}' est manquant dans package.json`));
    }
  }
...
async function rollbackMigration() {
  ...
  if (fs.existsSync(SCHEMA_BACKUP_PATH)) {
    ...
    console.log(chalk.green(`  ✅ Schéma d'origine restauré depuis ${path.relative(ROOT_DIR, SCHEMA_BACKUP_PATH)}`));
    
    // Régénérer le client Prisma
    execSync('prisma generate', { stdio: 'inherit' });
  } else {
    throw new Error('Impossible de trouver le backup du schéma pour le rollback');
  }
}
