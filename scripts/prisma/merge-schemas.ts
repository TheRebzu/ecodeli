...
async function main() {
  ...
  try {
    ...
    console.log(chalk.green(`✅ Schéma fusionné avec succès dans ${path.relative(process.cwd(), OUTPUT_SCHEMA)}`));
    console.log(chalk.gray(`   Taille du fichier: ${(outputContent.length / 1024).toFixed(2)} Ko`));
...
async function buildSchemaContent() {
  ...
  for (const domain of DOMAIN_ORDER) {
    ...
    if (schemaFiles.length === 0) {
      console.warn(chalk.yellow(`⚠️ Aucun fichier schema trouvé dans ${domain}`));
      continue;
    }
    
    console.log(chalk.gray(`  📄 Trouvé ${schemaFiles.length} fichiers de schéma`));
