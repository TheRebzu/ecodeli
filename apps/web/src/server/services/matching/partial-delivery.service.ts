...
export class PartialDeliveryService {
  ...
  private async findOptimalRelayPoints(
    ...
    return filteredPoints.sort((a, b) => {
      const totalDistanceA = 
        getDistance(announcement.pickupLatitude, announcement.pickupLongitude, a.latitude, a.longitude) +
        getDistance(a.latitude, a.longitude, announcement.deliveryLatitude, announcement.deliveryLongitude);
      
      const totalDistanceB = 
        getDistance(announcement.pickupLatitude, announcement.pickupLongitude, b.latitude, b.longitude) +
        getDistance(b.latitude, b.longitude, announcement.deliveryLatitude, announcement.deliveryLongitude);
  ...
  private async notifyDelivererAssignment(delivererId: string, segment: PartialDeliverySegment): Promise<void> {
    // Simulation de notification
    logger.info(`Notification envoyée au livreur ${delivererId} pour le segment ${segment.segmentNumber}`);
  }

  private async notifySegmentReady(delivererId: string, segment: PartialDeliverySegment): Promise<void> {
    // Simulation de notification
