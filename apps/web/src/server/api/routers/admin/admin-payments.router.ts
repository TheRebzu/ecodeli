...
const paymentFiltersSchema = z.object({
  ...
  sortBy: z.enum(['amount', 'createdAt', 'status', 'method']).default('createdAt'),
...
export const adminPaymentsRouter = router({
   ...
   */
  getAllPayments: protectedProcedure.input(paymentFiltersSchema).query(async ({ ctx, input }) => {
    ...
    try {
      ...
      if (input.search) {
        where.OR = [
          ...
          { stripePaymentIntentId: { contains: input.search, mode: 'insensitive' } },
          {
            user: {
              OR: [
      ...
      const enrichedPayments = payments.map(payment => ({
        ...
        totalRefunded: payment.refunds.reduce(
          (sum, refund) => (refund.status === 'COMPLETED' ? sum + refund.amount : sum),
          0
        ),
        hasDispute: !!payment.disputeStatus,
        daysSinceCreation: Math.floor(
          (new Date().getTime() - payment.createdAt.getTime()) / (1000 * 60 * 60 * 24)
        ),
  ...
  getPaymentStats: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      try {
        ...
        const refundRate =
          successfulPayments > 0 ? ((refundStats._count || 0) / successfulPayments) * 100 : 0;
            ...
            breakdown: {
              byMethod: paymentsByMethod.map(method => ({
                ...
                percentage: totalVolume._sum.amount
                  ? ((method._sum.amount || 0) / (totalVolume._sum.amount || 1)) * 100
                  : 0,
              })),
            },
            risks: {
   ...
   */
  initiateRefund: protectedProcedure.input(refundRequestSchema).mutation(async ({ ctx, input }) => {
    const { user } = ctx.session;

    if (user.role !== 'ADMIN') {
      throw new TRPCError({
        code: 'FORBIDDEN',
        message: 'Seuls les administrateurs peuvent initier des remboursements',
      });
    }
  ...
  bulkActions: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (user.role !== 'ADMIN') {
        throw new TRPCError({
          code: 'FORBIDDEN',
          message: 'Seuls les administrateurs peuvent effectuer des actions en lot',
        });
      }
