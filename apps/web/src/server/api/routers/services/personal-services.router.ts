...
const homeServiceSchema = baseServiceSchema.extend({
  ...
  serviceCategories: z.array(z.enum(['CLEANING', 'MAINTENANCE', 'GARDENING', 'REPAIR', 'ASSEMBLY'])),
  ...
  bookService: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      try {
        ...
        endDateTime.setMinutes(endDateTime.getMinutes() + (input.estimatedDuration || service.duration));
...
function extractSpecificData(input: any) {
  const { type, title, description, basePrice, duration, isActive, photos, ...specificData } = input;
  return specificData;
}
