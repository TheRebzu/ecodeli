...
export const cartDropRouter = router({
   ...
   */
  getMyCashRegisters: protectedProcedure
    .query(async ({ ctx }) => {
      const { user } = ctx.session;
      ...
      try {
        ...
        const cartDrop = await ctx.db.cartDrop.create({
          data: {
            ...
            status: 'PENDING',
            items,
            estimatedDeliveryTime: calculateEstimatedDeliveryTime(input.requestedTime, input.isUrgent)
          },
          include: {
            cashRegister: {
              select: {
  ...
  createCartDropGroup: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      try {
        ...
        const deliveryZones = [...new Set(cartDrops.map(cd => cd.deliveryPostalCode.substring(0, 2)))];
...
function getValidStatusTransitions(currentStatus: CartDropStatus, userRole: string): CartDropStatus[] {
