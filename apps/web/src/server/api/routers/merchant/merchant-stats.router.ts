...
export const merchantStatsRouter = router({
   ...
   */
  getDashboardStats: protectedProcedure.input(statsFiltersSchema).query(async ({ ctx, input }) => {
    ...
    try {
      ...
      const [currentStats, previousStats, topProducts, recentOrders, pendingOrdersCount] =
        await Promise.all([
          ...
          input.compareWithPrevious
            ? getBasicStats(ctx.db, merchant.id, previousStartDate!, previousEndDate!)
            : null,
      ...
      const comparisons =
        input.compareWithPrevious && previousStats
          ? {
              ...
              orders: calculateGrowthRate(currentStats.totalOrders, previousStats.totalOrders),
      ...
      return {
        ...
        data: {
          ...
          overview: {
            ...
            returnRate:
              currentStats.totalOrders > 0
                ? (currentStats.returnedOrders / currentStats.totalOrders) * 100
                : 0,
          },
          comparisons,
   ...
   */
  getSalesStats: protectedProcedure.input(statsFiltersSchema).query(async ({ ctx, input }) => {
    const { user } = ctx.session;
    ...
    try {
      ...
      const totalRevenue = salesTimeline.reduce((sum, item) => sum + item.revenue, 0);
      const totalOrders = salesTimeline.reduce((sum, item) => sum + item.orders_count, 0);
      ...
      return {
        ...
        data: {
          ...
          byCategory: Object.entries(categoryTotals).map(([category, stats]) => ({
            category,
            ...stats,
   ...
   */
  getProductStats: protectedProcedure.input(productStatsSchema).query(async ({ ctx, input }) => {
    const { user } = ctx.session;
    ...
    try {
      ...
      const { startDate, endDate } = calculatePeriodDates({ period: input.period });
      ...
      const enrichedStats = productStats.map(product => {
        ...
        return {
          ...
          // Indicateurs
          isLowStock: product.trackInventory && product.stockQuantity <= product.lowStockAlert,
          isTopSeller: sales > 0,
      ...
      const totalRevenue = enrichedStats.reduce((sum, product) => sum + product.revenue, 0);
      ...
      const catalogStats = {
        totalProducts: await ctx.db.product.count({ where: { merchantId: merchant.id } }),
   ...
   */
  getCustomerStats: protectedProcedure.input(customerStatsSchema).query(async ({ ctx, input }) => {
    const { user } = ctx.session;
    ...
    try {
      ...
      const [totalCustomers, newCustomers, returningCustomers, customerOrders, topCustomers] =
        await Promise.all([
      ...
      const newCustomersInPeriod = Array.from(firstOrderDates.entries()).filter(
        ([, firstOrder]) => firstOrder >= startDate
      );
      ...
      const enrichedTopCustomers = await Promise.all(
        topCustomers.map(async customer => {
          ...
          return {
            ...
            averageOrderValue:
              customer._count > 0 ? (customer._sum.totalAmount || 0) / customer._count : 0,
            isNew: newCustomersInPeriod.some(([id]) => id === customer.clientId),
          };
        })
      );

      // Calculs des métriques
      const totalRevenue = customerOrders.reduce((sum, c) => sum + (c._sum.totalAmount || 0), 0);
      const averageOrderValue =
        customerOrders.length > 0
          ? totalRevenue / customerOrders.reduce((sum, c) => sum + c._count, 0)
          : 0;
      ...
      return {
        ...
        data: {
          ...
          segmentation: {
            ...
            vip: enrichedTopCustomers.filter(c => c.totalSpent > averageOrderValue * 3).length,
          },
          period: {
            type: input.period,
            startDate,
...
// Helper functions
function calculatePeriodDates(input: { period: string; startDate?: Date; endDate?: Date }) {
  const now = new Date();
...
async function getBasicStats(db: any, merchantId: string, startDate: Date, endDate: Date) {
