...
interface CartDropTerminalInterfaceProps {
  ...
  onCreateOrder: (order: Omit<CartDropOrder, 'id' | 'status'>) => Promise<string>;
...
export const CartDropTerminalInterface: React.FC<CartDropTerminalInterfaceProps> = ({
  merchantId,
  availableProducts,
  availableTimeSlots,
  onCreateOrder,
  ...
  const handleClientIdentification = async (data: string, type: 'QR' | 'NFC') => {
  ...
  const handleManualClientEntry = () => {
    if (!manualClientData.name || !manualClientData.phone || !manualClientData.address) {
            ...
            {!isManualClient ? (
                  ...
                  {onScanQR && (
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div className="text-sm opacity-75">{t('qrDescription')}</div>
                      </div>
                    </Button>
                  )}
                  ...
                  {onScanNFC && (
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div className="text-sm opacity-75">{t('nfcDescription')}</div>
                      </div>
                    </Button>
                  )}
                </div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <Label htmlFor="client-address">{t('deliveryAddress')}</Label>
                    <Input
                      id="client-address"
      ...
      {currentStep === 'delivery' && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>{t('selectDeliverySlotDescription')}</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
                ...
                {availableSlots.map(slot => (
                  <Card
                    ...
                    className={cn(
                      ...
                      selectedTimeSlot?.id === slot.id && 'ring-2 ring-primary border-primary'
                    )}
          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {[
                  { value: 'CARD', label: t('paymentMethods.card'), icon: CreditCard },
                  { value: 'CASH', label: t('paymentMethods.cash'), icon: Euro },
                  { value: 'ACCOUNT', label: t('paymentMethods.account'), icon: User },
                ].map(({ value, label, icon: Icon }) => (
                  <Card
                    key={value}
                    className={cn(
                      'cursor-pointer transition-all hover:shadow-md',
                      paymentMethod === value && 'ring-2 ring-primary border-primary'
                    )}
