...
export function ReportFilters({ filters, onFilterChange }: ReportFilterProps) {
  ...
  const handleSetPredefinedRange = (
    range: 'this-week' | 'this-month' | 'last-month' | 'last-3-months' | 'year-to-date'
  ) => {
    const now = new Date();
    let start: Date;
    let end: Date = now;
    ...
    <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Select onValueChange={handleSetPredefinedRange}>
            ...
            <SelectContent>
              ...
              <SelectItem value="last-month">{t('filters.lastMonth')}</SelectItem>
              <SelectItem value="last-3-months">{t('filters.last3Months')}</SelectItem>
              <SelectItem value="year-to-date">{t('filters.yearToDate')}</SelectItem>
            </SelectContent>
          </Select>
        </div>
      ...
      <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
        <div className="space-y-2">
          ...
          <Select value={filters.categoryFilter} onValueChange={handleCategoryFilterChange}>
            <SelectTrigger>
              <SelectValue placeholder={t('filters.allCategories')} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="">{t('filters.allCategories')}</SelectItem>
              <SelectItem value="express">{t('filters.categories.express')}</SelectItem>
              <SelectItem value="standard">{t('filters.categories.standard')}</SelectItem>
              <SelectItem value="eco">{t('filters.categories.eco')}</SelectItem>
              <SelectItem value="international">{t('filters.categories.international')}</SelectItem>
            </SelectContent>
          </Select>
        </div>
        ...
        <div className="space-y-2">
          ...
          <Select value={filters.userRoleFilter} onValueChange={handleUserRoleFilterChange}>
            <SelectTrigger>
              <SelectValue placeholder={t('filters.allUserRoles')} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="">{t('filters.allUserRoles')}</SelectItem>
              <SelectItem value="CLIENT">{t('filters.roles.client')}</SelectItem>
              <SelectItem value="DELIVERER">{t('filters.roles.deliverer')}</SelectItem>
              <SelectItem value="MERCHANT">{t('filters.roles.merchant')}</SelectItem>
              <SelectItem value="PROVIDER">{t('filters.roles.provider')}</SelectItem>
