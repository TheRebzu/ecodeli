...
import { CheckCircle, XCircle, Clock, AlertTriangle, FileCheck, Shield } from 'lucide-react';
...
export default function DocumentVerificationStatus() {
  ...
  const approvedDocuments =
    verificationStatus.applications?.[0]?.requiredDocuments?.filter(
    ...
    <div className="space-y-4">
            ...
            <div className="space-y-3">
              {verificationStatus.applications[0].requiredDocuments.map((doc: any) => (
                ...
                >
                  ...
                  <div>
                    {doc.status === 'APPROVED' && <Badge variant="success">Approuvé</Badge>}
                    {doc.status === 'REJECTED' && <Badge variant="destructive">Rejeté</Badge>}
                    {doc.status === 'PENDING' && <Badge variant="secondary">En attente</Badge>}
                  </div>
                </div>
              ))}
            </div>
      ...
      {verificationStatus.overallStatus === 'REJECTED' && (
        <Alert variant="destructive">
          ...
          <AlertDescription>
            Des documents ont été rejetés. Veuillez consulter les raisons ci-dessus et soumettre de
            nouveaux documents.
          </AlertDescription>
        </Alert>
      )}
      ...
      {verificationStatus.overallStatus === 'APPROVED' && (
        <Alert variant="success">
          ...
          <AlertDescription>
            Félicitations ! Votre compte est maintenant vérifié et vous pouvez commencer à accepter
            des livraisons.
          </AlertDescription>
        </Alert>
      )}
