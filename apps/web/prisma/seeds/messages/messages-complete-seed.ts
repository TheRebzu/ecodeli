...
export async function seedMessagingSystemComplete(
  ...
  try {
    ...
    if (result.results.conversations.created > 0) {
      ...
      logger.warning('MESSAGING_SYSTEM', 'Aucune conversation créée - skip des messages');
    ...
    if (!options.skipValidation) {
      ...
      if (!validationResult) {
        ...
        logger.success('MESSAGING_SYSTEM', '✅ Toutes les validations sont passées');
      }
    } else {
...
async function checkPrerequisites(
  ...
  try {
    ...
    logger.success('MESSAGING_SYSTEM', `✅ ${userCount} utilisateurs disponibles`);
...
async function generateFinalReport(
  ...
  if (result.success) {
    ...
    logger.info('MESSAGING_SYSTEM', '  ⚙️ Préférences configurées pour tous les utilisateurs');
