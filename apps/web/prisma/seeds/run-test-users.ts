...
async function main() {
  ...
  try {
    ...
    if (isValid && result.errors === 0) {
      ...
      logger.info('TEST_USERS', '   📧 jean.dupont@test-ecodeli.fr (CLIENT ACTIF)');
      logger.info('TEST_USERS', '   📧 marie.martin@test-ecodeli.fr (CLIENT EN ATTENTE)');
      logger.info('TEST_USERS', '   📧 pierre.durand@test-ecodeli.fr (CLIENT SUSPENDU)');
      logger.info('TEST_USERS', '   📧 antoine.livreur@test-ecodeli.fr (LIVREUR ACTIF)');
      logger.info('TEST_USERS', '   📧 sophia.velo@test-ecodeli.fr (LIVREUR EN ATTENTE)');
      logger.info('TEST_USERS', '   📧 boulangerie.martin@test-ecodeli.fr (MERCHANT ACTIF)');
