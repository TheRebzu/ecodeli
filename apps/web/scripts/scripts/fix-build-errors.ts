...
class BuildErrorFixer {
  ...
  async run(): Promise<void> {
    console.log(chalk.bold.cyan('🔧 EcoDeli - Correcteur automatique d\'erreurs de build\n'));
  ...
  private async analyzeBuildErrors(): Promise<void> {
    ...
    console.log(chalk.yellow(`📋 ${this.errors.length} erreur(s) détectée(s)\n`));
  }
  ...
  private categorizeError(message: string, code?: string): BuildError['category'] {
    const msg = message.toLowerCase();
    
    if (msg.includes('cannot find module') || msg.includes('module not found') || code === '2307') {
      return 'import';
    }
    
    if (msg.includes('type') || msg.includes('property') || msg.includes('argument') || 
  ...
  private async fixMissingImports(): Promise<void> {
    ...
    console.log(chalk.yellow(`🔗 Correction de ${importErrors.length} erreur(s) d'import...`));
    ...
    for (const error of importErrors) {
      ...
      try {
        ...
        if (error.message.includes('React') && !content.includes('import React')) {
  ...
  private async installMissingDependencies(): Promise<void> {
    ...
    if (missingDeps.length > 0) {
      console.log(chalk.blue(`📦 Installation de ${missingDeps.length} dépendance(s)...`));
      try {
        execSync(`pnpm add ${missingDeps.join(' ')}`, { stdio: 'inherit', cwd: this.projectRoot });
        this.fixResults.push({
          file: 'package.json',
        ...
        console.log(chalk.red(`❌ Erreur installation dépendances: ${(err as Error).message}`));
      }
    }

    // Installer les types manquants
    if (missingTypes.length > 0) {
      console.log(chalk.blue(`📦 Installation de ${missingTypes.length} type(s)...`));
      try {
        execSync(`pnpm add -D ${missingTypes.join(' ')}`, { stdio: 'inherit', cwd: this.projectRoot });
        this.fixResults.push({
          file: 'package.json',
        ...
        console.log(chalk.red(`❌ Erreur installation types: ${(err as Error).message}`));
      }
    }
  }

  private async fixSimpleTypeErrors(): Promise<void> {
    const typeErrors = this.errors.filter(e => e.category === 'type' && e.file);
    if (typeErrors.length === 0) return;

    console.log(chalk.yellow(`📝 Correction de ${typeErrors.length} erreur(s) de type simples...`));
      ...
      try {
        ...
        if (error.line && error.line <= lines.length) {
          ...
          if (fixes.length > 0) {
            ...
            console.log(chalk.green(`  ✅ ${error.file}:${error.line}: ${fixes.join(', ')}`));
          }
        }
  ...
  private async cleanUnusedImports(): Promise<void> {
    ...
    try {
      ...
      console.log(chalk.yellow('  ⚠️ Impossible de nettoyer automatiquement les imports'));
    }
  }

  private getCommonNextImports(message: string): string | null {
    if (message.includes('next/link')) return "import Link from 'next/link';";
    if (message.includes('next/image')) return "import Image from 'next/image';";
    if (message.includes('next/router')) return "import { useRouter } from 'next/router';";
    if (message.includes('next/navigation')) return "import { useRouter } from 'next/navigation';";
    return null;
  }
  ...
  private needsTypes(moduleName: string): boolean {
    const popularLibsWithTypes = [
      'lodash', 'moment', 'uuid', 'bcrypt', 'jsonwebtoken',
      'multer', 'cors', 'express', 'node', 'jest'
    ];
  ...
  private async generateReport(): Promise<void> {
    ...
    console.log(chalk.green(`✅ Corrections réussies: ${report.successfulFixes}`));
    console.log(chalk.red(`❌ Corrections échouées: ${report.failedFixes}`));
    console.log(chalk.yellow(`⚠️ Erreurs restantes: ${this.errors.length - report.successfulFixes}`));
  }
