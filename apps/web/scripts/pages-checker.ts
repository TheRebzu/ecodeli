...
class PagesChecker {
  ...
  extractPagePaths(): PagePath[] {
    ...
    const scanDirectory = (currentPath: string, routePrefix: string = ''): void => {
        ...
        if (item.isDirectory()) {
          ...
          if (item.name.startsWith('(') && item.name.endsWith(')')) {
            ...
            if (item.name === '[locale]') {
              ...
              newRoutePrefix = routePrefix + '/[' + item.name.slice(1, -1) + ']';
            }
          } else if (item.name.startsWith('[...') && item.name.endsWith(']')) {
            // Catch-all route
            newRoutePrefix = routePrefix + '/[...' + item.name.slice(4, -1) + ']';
          } else {
          ...
          if (relativePath.includes('(protected)')) {
            ...
            else if (relativePath.includes('/client/')) requiredRole = UserRole.CLIENT;
            else if (relativePath.includes('/deliverer/')) requiredRole = UserRole.DELIVERER;
            else if (relativePath.includes('/merchant/')) requiredRole = UserRole.MERCHANT;
            else if (relativePath.includes('/provider/')) requiredRole = UserRole.PROVIDER;
  ...
  generateTestUrls(page: PagePath): string[] {
    ...
    const dynamicParams = [
      { pattern: /\[id\]/g, values: ['1', '2', '3', 'test-id', '123', 'abc-123'] },
    ...
    for (const page of pages) {
      ...
      console.log(`   URLs à tester: ${testUrls.length}`);
        ...
        if (page.isPublic || page.isAuth) {
          ...
          testCount++;
          const errorInfo = this.formatErrorInfo(result);
          console.log(`     ✓ ${result.status} (${result.responseTime}ms) - défaut${errorInfo}`);
        }
      ...
      result.errorDetails.forEach(detail => {
        ...
        if (detail.details) {
          ...
          if (detail.details.split('\n').length > 6) {
            console.log(`          ... (détails complets dans pages-test-results.json)`);
          }
        }
      });
    }
  ...
  private generateStatsForType(type: string, results: TestResult[]): any {
    ...
    return {
      ...
      errors_count: Object.values(errorTypes).reduce((sum, count) => sum + count, 0),
    };
  }
   ...
   */
  private createSummaryFile(resultsByType: Record<string, any[]>, resultsDir: string): void {
    const summary = {
      timestamp: new Date().toISOString(),
      total_types: Object.keys(resultsByType).filter(type => resultsByType[type].length > 0).length,
    ...
    Object.entries(resultsByType).forEach(([type, results]) => {
      if (results.length > 0) {
        ...
        summary.types_overview[type] = {
          ...
          success_rate: Math.round(((results.length - errorsCount) / results.length) * 100),
        };
  ...
  private analyzeResults(results: TestResult[]): void {
    ...
    if (slowPages.length > 0) {
      ...
      slowPages.forEach(sp => {
        console.log(`   ${sp.route}: ${sp.responseTime}ms (${sp.role || 'no-auth'})`);
      });
    }
...
async function main() {
  ...
  try {
    ...
    if (args.includes('--help') || args.includes('-h')) {
      ...
      console.log('   --merchant           Tester uniquement les pages commerçant');
      console.log('   --provider           Tester uniquement les pages prestataire');
